...
validators[Types.Resource] = function(resource)
	local actualType = type(resource)
	if actualType ~= 'string' then
		return { { message = "Unexpected parameter type, expected string, got " .. actualType } }
	end

	if not parameterTypeEnums[Types.Resource][resource] then
		return { { message = "Invalid resource: " .. resource .. ". Must be one of 'metal', 'energy'." } }
	end
end
...
local triggerDefinitions = GG['MissionAPI'].ConditionDefinitions
...
local function validate(schemaParameters, actionOrTriggerType, actionOrTriggerParameters, actionOrTrigger, actionOrTriggerID)
	...
	elseif not schemaParameters[actionOrTriggerType] then
		...
		-- Reject parameters the schema does not declare, so that a typo fails
		-- loudly instead of being silently ignored at runtime:
		local declaredParameters = {}
		for _, parameter in ipairs(schemaParameters[actionOrTriggerType]) do
			declaredParameters[parameter.name] = true
		end
		local unknownParameters = {}
		for parameterName in pairs(actionOrTriggerParameters) do
			if type(parameterName) == 'string' and not declaredParameters[parameterName] then
				unknownParameters[#unknownParameters + 1] = parameterName
			end
		end
		if not table.isEmpty(unknownParameters) then
			table.sort(unknownParameters)
			for _, parameterName in ipairs(unknownParameters) do
				logError("Unknown parameter. " .. actionOrTrigger .. ": " .. actionOrTriggerID .. ", Parameter: " .. parameterName)
			end
		end
	end
end

--- Reports any key not in `allowedKeys`, so typos in top-level fields fail loudly
--- instead of being silently ignored.
local function validateNoUnknownKeys(tbl, allowedKeys, label, id)
	local unknownKeys = {}
	for key in pairs(tbl) do
		if type(key) == 'string' and not allowedKeys[key] then
			unknownKeys[#unknownKeys + 1] = key
		end
	end
	if table.isEmpty(unknownKeys) then
		return
	end
	table.sort(unknownKeys)
	for _, key in ipairs(unknownKeys) do
		logError("Unknown field. " .. label .. ": " .. id .. ", Field: " .. key)
	end
end

-- `triggered`, `repeatCount`, `occurrences` and `metricSatisfied` are added by
-- ProcessRawTriggers before validation runs. `onActivate` / `onProgress` mark a
-- condition record contributed by an objective rather than authored as a trigger.
local allowedTriggerKeys = {
	type = true, parameters = true, settings = true, actions = true,
	count = true, atLeast = true, atMost = true,
	onActivate = true, onProgress = true,
	triggered = true, repeatCount = true, occurrences = true, metricSatisfied = true,
	lastValue = true,
}

--- Validates the threshold fields against the condition's kind, so that picking
--- a type from the wrong family is an error rather than a silently ignored field.
---   event  - an occurrence tallied upwards, thresholded with `count`
---   metric - a level sampled now, thresholded with `atLeast` / `atMost`
local function validateConditionThreshold(condition, kind, label, id)
	if kind == 'metric' then
		if condition.count ~= nil then
			logError("Metric conditions are thresholded with atLeast/atMost, not count. " .. label .. ": " .. id)
		end
		if condition.atLeast == nil and condition.atMost == nil then
			logError("Metric conditions require atLeast and/or atMost. " .. label .. ": " .. id)
		end
		for _, bound in ipairs({ 'atLeast', 'atMost' }) do
			local value = condition[bound]
			if value ~= nil then
				local luaTypeResult = validateLuaType(value, 'number')
				if luaTypeResult then
					logError(luaTypeResult .. ". " .. label .. ": " .. id .. ", Field: " .. bound)
				end
			end
		end
		if type(condition.atLeast) == 'number' and type(condition.atMost) == 'number'
			and condition.atLeast > condition.atMost then
			logError("atLeast must not be greater than atMost. " .. label .. ": " .. id)
		end
	elseif kind == 'event' then
		if condition.atLeast ~= nil or condition.atMost ~= nil then
			logError("Event conditions are thresholded with count, not atLeast/atMost. " .. label .. ": " .. id)
		end
		if condition.count ~= nil then
			local luaTypeResult = validateLuaType(condition.count, 'number')
			if luaTypeResult then
				logError(luaTypeResult .. ". " .. label .. ": " .. id .. ", Field: count")
			elseif condition.count < 1 then
				logError("count must be at least 1. " .. label .. ": " .. id)
			end
		end
	end
end
...
local function validateTriggerSettings(trigger, triggerID, triggers)
	validateNoUnknownKeys(trigger.settings, triggersSchemaSettings, 'Trigger setting', triggerID)
...
-- Objectives declare their condition inline, in the same flat shape as a trigger:
-- `type` + `parameters` + a threshold. They carry no `settings` or `actions`;
-- completion effects live under `onComplete`.
local allowedObjectiveKeys = {
	textKey = true, type = true, parameters = true,
	count = true, atLeast = true, atMost = true,
	onComplete = true, coop = true,
	-- runtime bookkeeping, added by the loader before validation runs
	completed = true, progress = true, target = true,
}

local allowedOnCompleteKeys = { nextStage = true, actions = true }

local function validateObjectiveCondition(objective, objectiveIDText)
	local kinds = GG['MissionAPI'].ConditionDefinitions.Kinds

	validate(triggersSchemaParameters, objective.type, objective.parameters or {}, 'Objective', objectiveIDText)

	if objective.type and kinds[objective.type] then
		validateConditionThreshold(objective, kinds[objective.type], 'Objective', objectiveIDText)
	end
end

local function validateObjectiveOnComplete(objective, objectiveIDText, rawActions)
	local onComplete = objective.onComplete
	if onComplete == nil then
		return
	end

	local onCompleteTypeResult = validateLuaType(onComplete, 'table')
	if onCompleteTypeResult then
		logError(onCompleteTypeResult .. ". Objective: " .. objectiveIDText .. ", Field: onComplete")
		return
	end

	validateNoUnknownKeys(onComplete, allowedOnCompleteKeys, 'Objective onComplete', objectiveIDText)

	if onComplete.nextStage ~= nil then
		if type(onComplete.nextStage) ~= 'string' then
			logError("Unexpected parameter type, expected string, got " .. type(onComplete.nextStage)
				.. ". Objective: " .. objectiveIDText .. ", Field: onComplete.nextStage")
		elseif GG['MissionAPI'].Stages[onComplete.nextStage] == nil then
			logError("Objective references non-existent nextStage. Objective: " .. objectiveIDText
				.. ", Stage: " .. onComplete.nextStage)
		end
	end

	if onComplete.actions ~= nil then
		local actionsTypeResult = validateLuaType(onComplete.actions, 'table')
		if actionsTypeResult then
			logError(actionsTypeResult .. ". Objective: " .. objectiveIDText .. ", Field: onComplete.actions")
			return
		end
		for _, actionID in ipairs(onComplete.actions) do
			if not (rawActions or {})[actionID] then
				logError("Objective has invalid action ID: " .. objectiveIDText .. ", Action: " .. tostring(actionID))
			end
		end
	end
end

local function validateObjective(objectiveID, objective, rawActions)
	...
	validateNoUnknownKeys(objective, allowedObjectiveKeys, 'Objective', objectiveIDText)
	validateObjectiveSchemaFields(objective, objectiveIDText)
	validateObjectiveCondition(objective, objectiveIDText)
	validateObjectiveOnComplete(objective, objectiveIDText, rawActions)
end

local function validateObjectives(objectives, rawActions)
	for objectiveID, objective in pairs(objectives) do
		validateObjective(objectiveID, objective, rawActions)
	end
end
...
local function validateTriggers(triggers, rawActions)
	for triggerID, trigger in pairs(triggers) do
		-- Condition records contributed by objectives complete their objective
		-- instead of invoking actions, so they are exempt from the action checks.
		if trigger.onActivate == nil then
		...
		validateNoUnknownKeys(trigger, allowedTriggerKeys, 'Trigger', triggerID)
		validateConditionThreshold(trigger, GG['MissionAPI'].ConditionDefinitions.Kinds[trigger.type], 'Trigger', triggerID)
