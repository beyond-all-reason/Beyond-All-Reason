...
local function validateConditionThreshold(condition, kind, label, id)
	if kind == "metric" then
		...
		if
			type(condition.atLeast) == "number"
			and type(condition.atMost) == "number"
			and condition.atLeast > condition.atMost
		then
		...
		if type(onComplete.nextStage) ~= "string" then
			logError(
				"Unexpected parameter type, expected string, got "
					.. type(onComplete.nextStage)
					.. ". Objective: "
					.. objectiveIDText
					.. ", Field: onComplete.nextStage"
			)
...
local function validateTriggers(triggers, rawActions)
	for triggerID, trigger in pairs(triggers) do
		...
		validateConditionThreshold(
			trigger,
			GG["MissionAPI"].ConditionDefinitions.Kinds[trigger.type],
			"Trigger",
			triggerID
		)
