...
local function prevalidateTriggers()
	for triggerId, trigger in pairs(triggers) do
		if not trigger.type then
			Engine.Shared.Log("triggers_loader.lua", LOG.ERROR, "[Mission API] Trigger missing type: " .. triggerId)
		end

		if not trigger.actions or next(trigger.actions) == nil then
			Engine.Shared.Log("triggers_loader.lua", LOG.ERROR, "[Mission API] Trigger has no actions: " .. triggerId)
		end
		...
		for _, parameter in pairs(parameters[trigger.type]) do
			...
			if value == nil and parameter.required then
				Engine.Shared.Log("triggers_loader.lua", LOG.ERROR, "[Mission API] Trigger missing required parameter. Trigger: " .. triggerId .. ", Parameter: " .. parameter.name)
			end

			if value ~= nil and type ~= parameter.type then
				Engine.Shared.Log("triggers_loader.lua", LOG.ERROR, "[Mission API] Unexpected parameter type, expected " .. parameter.type .. ", got " .. type .. ". Trigger: " .. triggerId .. ", Parameter: " .. parameter.name)
			end
		end
	end
end
	...
	for triggerId, trigger in pairs(triggers) do
		for _, actionId in pairs(trigger.actions) do
			if not actions[actionId] then
				Engine.Shared.Log("triggers_loader.lua", LOG.ERROR, "[Mission API] Trigger has action that does not exist. Trigger: " .. triggerId .. ", Action: " .. actionId)
			end
		end
	end
end
