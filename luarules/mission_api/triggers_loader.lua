		...
		for _, parameter in pairs(parameters[trigger.type]) do
			...
			if value == nil and parameter.required then
				Spring.Log(
					"triggers_loader.lua",
					LOG.ERROR,
					"[Mission API] Trigger missing required parameter. Trigger: "
						.. triggerId
						.. ", Parameter: "
						.. parameter.name
				)
			...
			if value ~= nil and type ~= parameter.type then
				Spring.Log(
					"triggers_loader.lua",
					LOG.ERROR,
					"[Mission API] Unexpected parameter type, expected "
						.. parameter.type
						.. ", got "
						.. type
						.. ". Trigger: "
		...
		for _, actionId in pairs(trigger.actions) do
			if not actions[actionId] then
				Spring.Log(
					"triggers_loader.lua",
					LOG.ERROR,
					"[Mission API] Trigger has action that does not exist. Trigger: "
						.. triggerId
						.. ", Action: "
						.. actionId
				)
