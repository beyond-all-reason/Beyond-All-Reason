...
local function prevalidateActions()
	for actionId, action in pairs(actions) do
		if not action.type then
			Engine.Shared.Log("actions_loader.lua", LOG.ERROR, "[Mission API] Action missing type: " .. actionId)
		end
		...
		for _, parameter in pairs(parameters[action.type]) do
			...
			if value == nil and parameter.required then
				Engine.Shared.Log("actions_loader.lua", LOG.ERROR, "[Mission API] Action missing required parameter. Action: " .. actionId .. ", Parameter: " .. parameter.name)
			end

			if value ~= nil and type ~= parameter.type then
				Engine.Shared.Log("actions_loader.lua", LOG.ERROR, "[Mission API] Unexpected parameter type, expected " .. parameter.type .. ", got " .. type .. ". Action: " .. actionId .. ", Parameter: " .. parameter.name)
			end
		end
	end
end
