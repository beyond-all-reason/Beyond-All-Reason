...
if Spring.GetModOptions().emprework == true then
	...
	for name, data in pairs(WeaponDefNames) do
		...
		if cp.timeslow_damagefactor or cp.timeslow_damage or cp.timeslow_onlyslow then
			...
			custom.slowDamage = cp.timeslow_damage
				or ((cp.timeslow_damagefactor or (cp.timeslow_onlyslow and 1)) * cp.raw_damage)
			...
			custom.smartRetargetHealth = cp.timeslow_smartretargethealth and tonumber(cp.timeslow_smartretargethealth)
				or nil
	...
	return weaponArray,
		MAX_SLOW_FACTOR,
		DEGRADE_TIMER * 30 / UPDATE_PERIOD,
		DEGRADE_FACTOR * UPDATE_PERIOD / 30,
		UPDATE_PERIOD
end
