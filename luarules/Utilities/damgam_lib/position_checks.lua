...
local function OccupancyCheck(posx, posy, posz, posradius) -- Returns true if there are no units in the spawn area
	...
	local unitcount =
		#Spring.GetUnitsInRectangle(posx - posradius, posz - posradius, posx + posradius, posz + posradius)
	if unitcount > 0 then
		return false
	else
		return true
...
local function VisibilityCheck(posx, posy, posz, posradius, allyTeamID, checkLoS, checkAirLos, checkRadar) -- Return True when position is not in sensor ranges of specified allyTeam.
	...
	if
		checkLoS
		and (
			Spring.IsPosInLos(posx, posy, posz, allyTeamID) == true
			or Spring.IsPosInLos(xPlus, posy, zPlus, allyTeamID) == true
			or Spring.IsPosInLos(xPlus, posy, zMinus, allyTeamID) == true
			or Spring.IsPosInLos(xMinus, posy, zPlus, allyTeamID) == true
			or Spring.IsPosInLos(xMinus, posy, zMinus, allyTeamID) == true
		)
	then
		return false
	end
	...
	if
		checkRadar
		and (
			Spring.IsPosInRadar(posx, posy, posz, allyTeamID) == true
			or Spring.IsPosInRadar(xPlus, posy, zPlus, allyTeamID) == true
			or Spring.IsPosInRadar(xPlus, posy, zMinus, allyTeamID) == true
			or Spring.IsPosInRadar(xMinus, posy, zPlus, allyTeamID) == true
			or Spring.IsPosInRadar(xMinus, posy, zMinus, allyTeamID) == true
		)
	then
		return false
	end
	...
	if
		checkAirLos
		and (
			Spring.IsPosInAirLos(posx, posy, posz, allyTeamID) == true
			or Spring.IsPosInAirLos(xPlus, posy, zPlus, allyTeamID) == true
			or Spring.IsPosInAirLos(xPlus, posy, zMinus, allyTeamID) == true
			or Spring.IsPosInAirLos(xMinus, posy, zPlus, allyTeamID) == true
			or Spring.IsPosInAirLos(xMinus, posy, zMinus, allyTeamID) == true
		)
	then
		return false
	end
