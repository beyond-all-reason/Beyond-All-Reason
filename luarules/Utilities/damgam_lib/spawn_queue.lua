...
local function AddToSpawnQueue(unitName, posx, posy, posz, facing, team, frame, blocking, resurrected)
	if blocking == nil then
		blocking = UnitDefNames[unitName].blocking
	end
	...
	if UnitDefNames[unitName] then
		...
		Engine.Shared.Echo("[Spawn Queue] Failed to queue " .. unitName .. ", invalid unitDefName")
	end
end
	...
	if QueuedSpawnsNumber > 0 then
		...
		for i = 1, QueuedSpawnsNumber do
			...
			if item and n >= item.frame then
				local unitID = Engine.Synced.CreateUnit(item.unitName, item.posx, item.posy, item.posz, item.facing, item.team)
				if unitID and item.blocking == false then
					Engine.Synced.SetUnitBlocking(unitID, false, false, true)
				end
				if unitID and item.resurrected == true then
					Engine.Synced.SetUnitRulesParam(unitID, "resurrected", 1, { inlos = true })
					Engine.Synced.SetUnitHealth(unitID, 10)
				elseif unitID then
...
local function DestroyUnitsFromQueue(n) -- Call this every frame in your gadget.
	...
	if QueuedDestroyNumber > 0 then
		for i = 1, QueuedDestroyNumber do
			...
			if item and n >= item.frame then
				Engine.Synced.DestroyUnit(item.unitID, item.selfd, item.reclaimed)
