...
if gadgetHandler:IsSyncedCode() then
	...
	layout (location = 1) in vec4 worldpos_teamid;
	...
	#line 11000
	void main() {
		vec4 newWorldPos = vec4(localpos_progress.xyz* 200.0 + worldpos_teamid.xyz,1.0);
		blendedcolor = vec4(1.0,1.0,0.0,localpos_progress.a);
	...
	local function goodbye(reason)
	  Spring.Echo("MO COOp II GL4 widget exiting with reason: "..reason)
	  gadgetHandler:RemoveGadget(self)
	end
	...
	local function CoopStartPoint(epicwtf, playerID, x, y, z) --this epicwtf param is used because it seem that when a registered function is locaal, then the registration name is  passed too. if the function is part of gadget: then it is not passed.
		...
		if areSpec or spArePlayersAllied(myPlayerID, playerID) then
			if sx > 0 or sz > 0 then
				pushElementInstance(coneInstanceVBO,{x,y,z,playerTeams[playerID]},playerID,true)
			end
		end
	end
	...
	function gadget:Initialize()
		...
		-- GL4 init:
		coneInstanceVBO = makeInstanceVBOTable({{id = 1, name = 'worldposscale', size = 4}},16, "GeoInstanceVBO")
		local coneVBO, numVertices = makeConeVBO(coneSegments)
		...
		-- add testing cone: 
		pushElementInstance(coneInstanceVBO,{0,200,0,0},"test",true)
	...
	function gadget:DrawWorld()
		...
		if coneInstanceVBO.numElements > 0 then
			coneShader:Activate()
			coneInstanceVBO.VAO:DrawArrays(GL_LINES,coneInstanceVBO.numVertices,0,coneInstanceVBO.usedElements,0)
