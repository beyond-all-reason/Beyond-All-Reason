...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:GameStart()
		...
		local allyTeamIDs = Engine.Shared.GetAllyTeamList()
		for i = 1, #allyTeamIDs do
			local teamIDs = Engine.Shared.GetTeamList(allyTeamIDs[i])
			for j = 1, #teamIDs do
				local isLuaAI = (Engine.Shared.GetTeamLuaAI(teamIDs[j]) ~= nil)
				local isAiTeam = select(4, Engine.Shared.GetTeamInfo(teamIDs[j]))
				if not (isLuaAI or isAiTeam or teamIDs[j] == gaiaTeamID) then
					local playerIDs = Engine.Shared.GetPlayerList(teamIDs[j])
					local numPlayers = 0
	...
	function gadget:UnitDestroyed(unitID, unitDefID, teamID, attackerID, attackerDefID, attackerTeamID)
		...
		local curTime = Engine.Shared.GetGameSeconds()
	...
	local function calculateEfficiency(teamID)
		...
		for tID, _ in pairs(teamInfo) do
			local cur_max = Engine.Shared.GetTeamStatsHistory(tID)
			local stats = Engine.Shared.GetTeamStatsHistory(tID, cur_max, cur_max)
		...
		local cur_max = Engine.Shared.GetTeamStatsHistory(teamID)
		local stats = Engine.Shared.GetTeamStatsHistory(teamID, cur_max, cur_max)
	...
	local function ProcessAwards(_)
		...
		Engine.Unsynced.SendLuaRulesMsg(awardsMsg)
