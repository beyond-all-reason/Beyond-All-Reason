...
if gadgetHandler:IsSyncedCode() then
	...
	local function CobDebug(unitID, unitDefID, team, cmdid, line, p1, p2, p3, p4, p5, p6, p7, p8)
		...
		if lastgf == Engine.Shared.GetGameFrame() then
			count = count + 1
		else
			lastgf = Engine.Shared.GetGameFrame()
			count = 0
		end
		...
		local msg = string.format("f:<%05d.%03d> u:%05d %s.%s:%d ", lastgf, count, unitID or -1, UnitDefs[unitDefID].name or "?", funcname, line or -1)
		if p1 then
			msg = msg .. " p1=" .. tostring(p1)
		end
		if p2 then
			msg = msg .. " p2=" .. tostring(p2)
		end
		if p3 then
			msg = msg .. " p3=" .. tostring(p3)
		end
		if p4 then
			msg = msg .. " p4=" .. tostring(p4)
		end
		if p5 then
			msg = msg .. " p5=" .. tostring(p5)
		end
		if p6 then
			msg = msg .. " p6=" .. tostring(p6)
		end
		if p7 then
			msg = msg .. " p7=" .. tostring(p7)
		end
		if p8 then
