	...
	if frame > lastTurretFrame + 150 then
		if frame % 30 == 9 then
			for unitID, data in pairs(aliveBuilders) do
				if
					(Spring.GetUnitNearestEnemy(unitID, data.range * 5, true) and math.random(0, 15) == 0)
					or data.isFactory
				then
					...
					if
						(data.isFactory and #Spring.GetFullBuildQueue(unitID) < 5)
						or (
							not data.isFactory
							and (
								not unitCommands[1]
								or (unitCommands[1] and unitCommands[1].id > 0 and unitCommands[1].id ~= CMD.REPAIR)
							)
						)
					then
						...
						for buildOptionIndex, buildOptionID in pairs(data.buildOptions) do
							...
							if
								buildOptionID
								and (
									((not UnitDefs[buildOptionID].canAssist) and not UnitDefs[buildOptionID].isFactory)
									or (math.random(1, 20) == 1 and UnitDefs[buildOptionID].isFactory)
								)
							then
