...
if gadgetHandler:IsSyncedCode() then
		...
		if unitDef.customParams then
			...
			if techlevel and techlevel > 1 then
				if
					...
					or unitDef.customParams.energyconv_capacity
				then
					isProtected = true
				end
			end
		...
		for oldUnitID, params in pairs(teamSelfdUnits[teamID]) do
			if params[1] > oldestGameFrame then
				...
				local unitID = Spring.CreateUnit(
					params[2],
					unitX,
					Spring.GetGroundHeight(unitX, unitZ),
					unitZ,
					params[7],
					restoreTeamID
				)
		...
		if safeguardedUnits[unitDefID] and attackerTeam and Spring.AreTeamsAllied(unitTeam, attackerTeam) then
			...
			if isDgun or isSelfD or hasNoEnemy then
				if enableAlertEchos then
					...
					local atPosition = (x and z)
							and ("   (pos: " .. mathFloor(mathFloor(x / 100) * 100) .. ", " .. mathFloor(
								mathFloor(z / 100) * 100
							) .. ")")
						or ""
					...
					if isDgun then
						if victimName == attackerName then
							notify(
								"\255\255\100\100 -- ALERT --   "
									.. attackerName
									.. " tried to DGUN their own "
									.. unitName
									.. atPosition
							)
						else
	...
	function gadget:UnitDestroyed(unitID, unitDefID, teamID, attackerID, attackerDefID, attackerTeamID, weaponDefID)
		...
		if attackerID ~= nil then
			attackerWasSelfd = (selfdBlastUnits[attackerID] ~= nil)
				or (
					(selfdCmdUnits[attackerID] == true)
					and (weaponDefID ~= nil and selfDWeaponToUnit[weaponDefID] ~= nil)
				)
		end
