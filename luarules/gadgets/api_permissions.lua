...
if trustedNames then
	for _, playerID in ipairs(Engine.Shared.GetPlayerList()) do
		...
		local name = Engine.Shared.GetPlayerInfo(playerID)
...
if numPlayers <= 1 then
	for _, playerID in ipairs(Engine.Shared.GetPlayerList()) do
		local accountID = Spring.Utilities.GetAccountID(playerID)
		local _, _, spec = Engine.Shared.GetPlayerInfo(playerID)
...
local function ResolveTrustedName(playerID)
	...
	Engine.Shared.Log("Permissions", LOG.INFO, "Granted permissions to '" .. name .. "' (accountID: " .. accountID .. ")")
	return true
end
