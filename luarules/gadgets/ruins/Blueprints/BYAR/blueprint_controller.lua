...
local blueprintsConfig = {
	[1] = {
		table = constructorBlueprints,
		tiered = true,
		directory = "luarules/gadgets/ruins/Blueprints/" .. Game.gameShortName .. "/Blueprints/",
	},
}
		...
		if not success then
			...
			for _, blueprintFunction in ipairs(fileContents) do
				...
				if not blueprintSuccess then
					Spring.Echo(
						"[Ruins Blueprints] Failed to execute blueprint function in file: "
							.. blueprintFile
							.. " - "
							.. blueprint
					)
