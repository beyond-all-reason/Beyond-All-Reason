...
local gaiaTeamID = BAR.Utilities.GetScavTeamID() or Spring.GetGaiaTeamID()
...
function BPWallOrPopup(faction, tier, surface)
	...
	if wallRandom <= 0.1 and wallUnitDefs[faction][tier][surface].armed[1] then -- armed
		return UDN[wallUnitDefs[faction][tier][surface].armed[math.random(
			1,
			#wallUnitDefs[faction][tier][surface].armed
		)]].id
	elseif wallUnitDefs[faction][tier][surface].unarmed[1] then
		return UDN[wallUnitDefs[faction][tier][surface].unarmed[math.random(
			1,
			#wallUnitDefs[faction][tier][surface].unarmed
		)]].id
	else
