...
function gadget:GetInfo()
	return {
		...
		enabled = Engine.Shared.GetModOptions().disable_assist_ally_construction, -- or Spring.GetModOptions().easytax,  -- disabled for easytax and replaced with tax in game_tax_resource_sharing.lua
	}
end
...
local gaiaTeam = Engine.Shared.GetGaiaTeamID()
...
local function removeRoamMoveState(unitID)
	local index = Engine.Shared.FindUnitCmdDesc(unitID, CMD_MOVESTATE)
	if index then
		local moveState = select(2, Engine.Shared.GetUnitStates(unitID, false))
		local params = builderMoveStateCmdDesc.params
		params[1] = math.min(moveState or 1, MOVESTATE_ROAM - 1)
		Engine.Synced.EditUnitCmdDesc(unitID, index, builderMoveStateCmdDesc)
	end
end
...
local function validateCommands(unitID, unitTeam)
	...
	for index = 1, Engine.Shared.GetUnitCommandCount(unitID) do
	...
	if count > 0 then
		Engine.Shared.GiveOrderToUnit(unitID, CMD.REMOVE, tags)
	end
end
