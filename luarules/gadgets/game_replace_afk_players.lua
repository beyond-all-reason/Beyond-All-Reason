...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:RecvLuaMsg(msg, playerID)
		...
		if msg == "\144" then
			...
			local customtable = select(11, Engine.Shared.GetPlayerInfo(playerID))
	...
	function gadget:Initialize()
		...
		for _, teamID in pairs(teamList) do
			if teamID ~= gaiaTeamID then
				local playerList = Engine.Shared.GetPlayerList(teamID)
				for _, playerID in pairs(playerList) do
					if not select(3, Engine.Shared.GetPlayerInfo(playerID, false)) then
	...
	function FindSubs(real)
		...
		for playerID, _ in pairs(players) do
			local _, active, spec = Engine.Shared.GetPlayerInfo(playerID, false)
			local readyState = Engine.Shared.GetGameRulesParam("player_" .. playerID .. "_readyState")
			local noStartPoint = (readyState == 3) or (readyState == 0)
			local present = active and not spec and not noStartPoint
			if not present then
				local customtable = select(11, Engine.Shared.GetPlayerInfo(playerID)) or {}
				local tsMu = customtable.skill
				local ts = tsMu and tonumber(tsMu:match("%d+%.?%d*"))
				if ts then
					absent[playerID] = ts
			...
			if not absent[playerID] then
				Engine.Synced.SetGameRulesParam("Player" .. playerID .. "willSub", 0)
			end
		end
		...
		for playerID, ts in pairs(absent) do
			...
			for subID, subts in pairs(substitutesLocal) do
				local _, active, spec = Engine.Shared.GetPlayerInfo(subID, false)
			...
			if #validSubs > 0 then
				...
				if real then
					...
					Engine.Synced.AssignPlayerToTeam(sID, teamID)
					players[sID] = teamID
					replaced = true

					local incoming, _ = Engine.Shared.GetPlayerInfo(sID, false)
					local outgoing, _ = Engine.Shared.GetPlayerInfo(playerID, false)
			...
			Engine.Synced.SetGameRulesParam("Player" .. playerID .. "willSub", wouldSub and 1 or 0)
		end
	end
	...
	function gadget:GameFrame(n)
		if n == 1 and replaced then
			...
			for pID, p in pairs(coopStartPoints) do
				...
				local name, _, _, tID = Engine.Shared.GetPlayerInfo(pID, false)
			...
			for tID, p in pairs(teamStartPoints) do
				...
				local playerList = Engine.Shared.GetPlayerList(tID)
				local name = ""
				for _, pID in pairs(playerList) do
					--now do all pIDs for this team which were not coop starts
					if not revealed[pID] then
						local pName, active, spec = Engine.Shared.GetPlayerInfo(pID, false)
	...
	function CheckJoined()
		...
		for _, pID in ipairs(pList) do
			if not players[pID] then
				local _, active, spec, _, aID = Engine.Shared.GetPlayerInfo(pID, false)
	...
	local spec, _ = Engine.Unsynced.GetSpectatingState()
	...
	local function MarkStartPoint(_, x, y, z, name, teamID)
		local _, _, spec = Engine.Shared.GetPlayerInfo(myPlayerID, false)
		if not spec then
			Engine.Unsynced.MarkerAddPoint(x, y, z, colourNames(teamID) .. name, true)
			revealed = true
		end
	end
	...
	local function substitutionOccurred(_, incoming, outgoing)
		if Script.LuaUI("GadgetMessageProxy") then
			Engine.Shared.Echo(Script.LuaUI.GadgetMessageProxy("ui.substitutePlayers.substitutedPlayers", { incoming = incoming, outgoing = outgoing }))
		end
	end

	function gadget:Initialize()
		if isReplay or Spring.Utilities.Gametype.IsFFA() or Engine.Shared.GetGameFrame() > 6 then
	...
	function gadget:GameFrame(n)
		...
		if revealed and Script.LuaUI("GadgetMessageProxy") then
			Engine.Shared.Echo(Script.LuaUI.GadgetMessageProxy("ui.substitutePlayers.substituted"))
		end
