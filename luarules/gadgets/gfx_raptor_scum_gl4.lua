...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:Initialize()
		...
		for unitDefID, unitDef in pairs(UnitDefs) do
			if
				unitDef.customParams.isscavenger
				and not scumSpawnerExclusions[unitDef.name]
				and not unitDef.canMove
				and not string.find(unitDef.name, "lootbox")
				and not scumSpawnerIDs[unitDefID]
				and not unitDef.customParams.objectify
				and not unitDef.canCloak
			then
				if unitDef.xsize and unitDef.zsize then
					scumSpawnerIDs[unitDefID] =
						{ radius = math.min(600, math.max(150, unitDef.xsize * unitDef.zsize * 10)), growthrate = 1.2 }
				else
	...
	local function AddOrUpdateScum(posx, posy, posz, radius, growthrate, scumID)
		...
		if scums[scumID] == nil then
			...
			scum =
				{ posx = posx, posz = posz, radius = radius, spawnframe = gf, growthrate = growthrate, scumID = scumID }
			scums[scumID] = scum
			UpdateBins(scumID)
			numscums = numscums + 1
	...
	function gadget:UnitCreated(unitID, unitDefID, unitTeam)
		if scumSpawnerIDs[unitDefID] and (debugmode or (unitTeam and unitTeam == pveTeamID)) then
			...
			local scumID = AddOrUpdateScum(
				px,
				py,
				pz,
				scumSpawnerIDs[unitDefID].radius,
				scumSpawnerIDs[unitDefID].growthrate,
				unitID
			)
			local scum = scums[scumID]
...
elseif not BAR.Utilities.Gametype.IsScavengers() then -- UNSYNCED
	...
	local function UpdateScumOverlaps()
				...
				if not overlapped[bigScumID] and bigRadius > 512 then
					for smallScumID, smallScum in pairs(scumBin) do
						if not overlapped[smallScumID] and scumRadii[smallScumID] < bigRadius then
							...
							if
								diag(bigScum.posx - smallScum.posx, bigScum.posz - smallScum.posz)
								< (bigRadius - scumRadii[smallScumID] - 128)
							then
		...
		if debugmode then
			Spring.Echo(
				string.format("Of %d scums, %d overlaps found in %d comparisons", numscums, overlapcount, comparisons)
			)
		end
