...
if gadgetHandler:IsSyncedCode() then
	...
	local function AddOrUpdateScum(posx, posy, posz, radius, growthrate, scumID)
		if debugmode then
			Engine.Shared.Echo("AddOrUpdateScum", posx, posy, posz, radius, growthrate, scumID)
		end
		...
		if scums[scumID] == nil then
			posy = posy or Engine.Shared.GetGroundHeight(posx, posz)
			...
			if debugmode then
				Engine.Shared.Echo("Updated scum", scumID, "it was", currentradius, "/", scum.radius, "sized, growing at", growthrate)
			end
		end
		...
		if scum.growthrate < 0 then
			if debugmode then
				Engine.Shared.Echo("Removal of scum ID", scumID, "Scheduled for ", deathtime - gf, "from now")
			end
	...
	function gadget:GameFrame(n)
		if not initialized then
			for i, unitID in ipairs(Engine.Shared.GetAllUnits()) do
...
elseif not Spring.Utilities.Gametype.IsScavengers() then -- UNSYNCED
	...
	local function goodbye(reason)
		Engine.Shared.Echo("Scum GL4 gadget exiting with reason: " .. reason)
	...
	local function UpdateScumOverlaps()
		...
		if debugmode then
			Engine.Shared.Echo(string.format("Of %d scums, %d overlaps found in %d comparisons", numscums, overlapcount, comparisons))
		end
	...
	function gadget:DrawWorldPreUnit()
		...
		if debugmode then
			local mx, my, mb = Engine.Unsynced.GetMouseState()
			local _, coords = Engine.Unsynced.TraceScreenRay(mx, my, true)
			if coords and (IsPosInScum(coords[1], coords[2], coords[3])) then
				Engine.Shared.Echo("Inscum", numscums, IsPosInScum(coords[1], coords[2], coords[3]))
			end
		end
	...
	local function RemoveScum(instanceID)
		if debugmode then
			Engine.Shared.Echo("Removing scum", instanceID)
		end
			...
			for scumID, scum in pairs(scums) do
				if ((scumID % 4) == scumModulo) and scum.growthrate > 0 and not scum.atmaxsize then
					...
					if currentRadius < scum.radius then
						...
						if debugmode then
							Engine.Shared.Echo("Scum ID", scumID, "reached max size", currentRadius, ">=", scum.radius)
						end
						scum.atmaxsize = true
					end
				end
	...
	local function ScumTextures()
		...
		Engine.Shared.Echo("Scum textureresolution set to ", textureresolution)
	end
	...
	local function ScumStats()
		for x = 0, math.ceil(mapSizeX / 1024) do
			for z = 0, math.ceil(mapSizeZ / 1024) do
				...
				Engine.Shared.Echo(string.format("%d scums are in bin %d x %d", scumCount, x, z))
			end
		end
		Engine.Shared.Echo(string.format("Total amount of scums on map is %d", numscums))
		local overlapcount, comparisons = UpdateScumOverlaps()
		Engine.Shared.Echo("overlapcount", overlapcount, "comparisons=", comparisons)
	end

	local function ScumReloadShader()
		Engine.Shared.Echo("ScumReloadShader not implemented")
	end

	local function ScumDrawToggle()
		drawScum = not drawScum
		Engine.Shared.Echo("Scum drawing toggled to", drawScum)
	end
	local function ScumOptimizeOverlap()
		optimizeoverlaps = not optimizeoverlaps
		Engine.Shared.Echo("Scum optimizeoverlaps toggled to", optimizeoverlaps)
		UpdateScumOverlaps()
	end
