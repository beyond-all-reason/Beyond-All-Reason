...
function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID, attackerDefID, attackerTeam, weaponDefID)
	...
	if aliveDrones[unitID] then
		if aliveCarriers[aliveDrones[unitID].owner] then
			aliveCarriers[aliveDrones[unitID].owner][aliveDrones[unitID].index].aliveDrones = aliveCarriers[aliveDrones[unitID].owner][aliveDrones[unitID].index].aliveDrones
				- 1
		end
			...
			if stats.owner then
				...
				if math.random(0, 10) == 0 then
					Spring.GiveOrderToUnit(
						droneID,
						CMD.PATROL,
						{ math.random(0, Game.mapSizeX), 0, math.random(0, Game.mapSizeZ) },
						{ "shift" }
					)
				end
			end
