...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID, attackerDefID, attackerTeam, weaponDefID)
		if attackerID == nil and not ignoreUnits[unitDefID] then -- if reclaimed
			...
			if ux ~= nil then
				...
				SpringSynced.SpawnCEG("metalshards1", x, y, z)
				...
				for i = 1, unitNumFx[unitDefID], 1 do
					...
					SpringSynced.SpawnCEG("metalshards" .. (((i + 1) % 3) + 1), x, y, z)
				end
			end
		end
	end
	...
	function gadget:FeatureDamaged(featureID, featureDefID, featureTeam, damage, weaponDefID, projectileID, attackerID, attackerDefID, attackerTeam)
		if damage > 4 and (damagedFeatures[featureID] == nil or SpringShared.GetGameFrame() - damagedFeatures[featureID] > 15) and select(1, SpringShared.GetFeatureHealth(featureID)) > 0 then -- not sure if "select(1,Spring.GetFeatureHealth(featureID)) > 0" is needed because if featuredestroyed, featuredamaged wont be called probably
			...
			if fx ~= nil then
				...
				if fdef and fdef.minx and fdef.maxx then
					...
					if me ~= nil and me > 0 then
						...
						local posMultiplier = 0.5
						SpringSynced.SpawnCEG("energyshards1", x, y, z)
						for i = 1, numFx, 1 do
							...
							SpringSynced.SpawnCEG("energyshards" .. (((i + 1) % 3) + 1), x, y, z)
						end
					end
			...
			damagedFeatures[featureID] = SpringShared.GetGameFrame()
		end
	end
	...
	function gadget:FeatureDestroyed(featureID, allyteam)
		...
		local fx, fy, fz = SpringShared.GetFeaturePosition(featureID)
		if fy ~= nil and fy + 15 > SpringShared.GetGroundHeight(fx, fz) then
			local fdef = featureList[SpringShared.GetFeatureDefID(featureID)]
	...
	local function reclaimfx(_, fx, fy, fz)
		...
		if fullview or SpringShared.IsPosInLos(fx, fy, fz, SpringUnsynced.GetLocalAllyTeamID()) then
