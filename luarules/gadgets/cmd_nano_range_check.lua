...
function gadget:AllowCommand(unitID, unitDefID, _teamID, cmdID, cmdParams, _cmdOptions)
	...
	if #cmdParams ~= 1 then return true end -- only handle ID targets, fallthrough for area select; Let the intended scripts handle those first, cache resulting commands on ID.
	if not UnitDefs[Spring.GetUnitDefID(cmdParams[1])].isImmobile then return true end -- REVIEW If possible, remove double neg.

	local range = unitDef.buildDistance
	local x, y, z = Spring.GetUnitPosition(unitID)
	local cmdX, cmdY, cmdZ = Spring.GetUnitPosition(cmdParams[1])
	if cmdX == nil then return true end -- in case of feature; Already distanced checked by respective scripts. TODO pre-get exklusion; feature check.
