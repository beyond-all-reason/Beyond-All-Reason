...
local function UpdateVisibility(unitID, unitData, fullview, forceUpdate)
	...
	local unitVisible = fullview or (myAllyTeamID == unitData.allyTeamID) or SpringShared.IsUnitInLos(unitID, myAllyTeamID) or SpringShared.IsUnitInAirLos(unitID, myAllyTeamID)
		...
		if ux then
			...
			if samples then
				...
				for i = 1, #samples do
					...
					if SpringShared.IsPosInLos(sx, cy, sz, myAllyTeamID) or SpringShared.IsPosInAirLos(sx, cy, sz, myAllyTeamID) then
						unitVisible = true
						break
					end
				end
			...
			if not unitVisible and r > 0 then
				...
				if SpringShared.IsPosInLos(ux, uy + r, uz, myAllyTeamID) or SpringShared.IsPosInAirLos(ux, uy + r, uz, myAllyTeamID) or SpringShared.IsPosInLos(ux, uy - r, uz, myAllyTeamID) or SpringShared.IsPosInAirLos(ux, uy - r, uz, myAllyTeamID) then
					unitVisible = true
				end
			end
		end
