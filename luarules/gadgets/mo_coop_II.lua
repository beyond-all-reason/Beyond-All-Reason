...
-- Modoption check
if not Engine.Shared.GetModOptions().coop then
	return false
end
...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:AllowStartPosition(playerID, teamID, readyState, x, y, z)
		...
		if coopStartPoints[playerID] then
			...
			local _, _, _, teamID, allyID = Engine.Shared.GetPlayerInfo(playerID, false)
			local osx, _, osz = Engine.Shared.GetTeamStartPosition(teamID)
			if x ~= osx or z ~= osz then
				local xmin, zmin, xmax, zmax = Engine.Shared.GetAllyTeamStartBox(allyID)
				x = math.clamp(x, xmin, xmax)
				z = math.clamp(z, zmin, zmax)

				SetCoopStartPoint(playerID, x, Engine.Shared.GetGroundHeight(x, z), z) -- record coop start point, display it
				return false
			end
		end
	...
	function IsSteep(x, z)
		...
		local y1 = Engine.Shared.GetGroundHeight(x + d, z)
		if math.abs(y1 - y) > 0.1 then
			a1 = math.atan((y1 - y) / d)
		end
		local y2 = Engine.Shared.GetGroundHeight(x, z + d)
		if math.abs(y2 - y) > 0.1 then
			a2 = math.atan((y2 - y) / d)
		end
		local y3 = Engine.Shared.GetGroundHeight(x - d, z)
		if math.abs(y3 - y) > 0.1 then
			a3 = math.atan((y3 - y) / d)
		end
		local y4 = Engine.Shared.GetGroundHeight(x, z + d)
	...
	local function SpawnTeamStartUnit(playerID, teamID, allyID, x, z)
		local startUnit = Engine.Shared.GetTeamRulesParam(teamID, "startUnit")
		...
		if startUnit == nil then
			...
			if Engine.Shared.GetModOptions().experimentallegionfaction and math.random() > 0.33 then
		...
		-- create
		local unitID = Engine.Synced.CreateUnit(startUnit, x, Engine.Shared.GetGroundHeight(x, z), z, 0, teamID)
		...
		Engine.Synced.SetUnitRulesParam(unitID, "startingOwner", playerID)
	end
