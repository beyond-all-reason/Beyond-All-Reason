if not Engine.Shared.GetModOptions().map_atmosphere then
	return
end
...
local enableGenericConfig = Engine.Shared.GetModOptions().mapatmospherics or "enabled"
...
Engine.Shared.Echo("[ATMOSPHERIC] Current map: " .. currentMapname)
local mapFileName = "" -- (Include at bottom of this file)
for i = 1, #mapList + 1 do
	if i == #mapList + 1 then
		Engine.Shared.Echo("[ATMOSPHERIC] No map config found. Turning off the gadget")
		return
	end
	mapFileName = string.sub(mapList[i], 36, string.len(mapList[i]) - 4):lower()
	if string.find(currentMapname, mapFileName) then
		Engine.Shared.Echo("[ATMOSPHERIC] Success! Map names match!: " .. mapFileName)
		break
	else
...
if not gadgetHandler:IsSyncedCode() then
	...
	local function SetLightingAndAtmosphere(lightandatmos)
		if lightandatmos.atmosphere then
			Engine.Unsynced.SetAtmosphere(lightandatmos.atmosphere)
		end
		if lightandatmos.lighting then
			Engine.Unsynced.SetSunLighting(lightandatmos.lighting)
		end
		if lightandatmos.sunDir then
			Engine.Unsynced.SetSunDirection(lightandatmos.sunDir[1], lightandatmos.sunDir[2], lightandatmos.sunDir[3])
		end
	end
	...
	local function MapAtmosphereConfigSetSun(_, targetbrightness, transitionspeed, redlevel, greenlevel, bluelevel, sundir)
		...
		Engine.Unsynced.SetSunLighting({ groundAmbientColor = { transitionred * gar, transitiongreen * gag, transitionblue * gab } })
		Engine.Unsynced.SetSunLighting({ unitAmbientColor = { transitionred * uar, transitiongreen * uag, transitionblue * uab } })
		Engine.Unsynced.SetSunLighting({ groundDiffuseColor = { transitionred * gdr, transitiongreen * gdg, transitionblue * gdb } })
		Engine.Unsynced.SetSunLighting({ unitDiffuseColor = { transitionred * udr, transitiongreen * udg, transitionblue * udb } })
		Engine.Unsynced.SetSunLighting({ groundSpecularColor = { transitionred * gsr, transitiongreen * gsg, transitionblue * gsb } })
		Engine.Unsynced.SetSunLighting({ unitSpecularColor = { transitionred * usr, transitiongreen * usg, transitionblue * usb } })

		Engine.Unsynced.SetAtmosphere({ skyColor = { transitionred * skycr, transitiongreen * skycg, transitionblue * skycb } })
		Engine.Unsynced.SetAtmosphere({ sunColor = { transitionred * suncr, transitiongreen * suncg, transitionblue * suncb } })
		Engine.Unsynced.SetAtmosphere({ cloudColor = { transitionred * clocr, transitiongreen * clocg, transitionblue * clocb } })
		Engine.Unsynced.SetAtmosphere({ fogColor = { transitionred * fogcr, transitiongreen * fogcg, transitionblue * fogcb } })

		Engine.Unsynced.SetSunLighting({ groundShadowDensity = transition * shadowdensity, modelShadowDensity = transition * shadowdensity })
	end
	...
	local function MapAtmosphereConfigSetFog(_, targetstart, targetend, transitionspeedstart, transitionspeedend)
		...
		Engine.Unsynced.SetAtmosphere({ fogStart = transitionstart * fogstartdefault })
		Engine.Unsynced.SetAtmosphere({ fogEnd = transitionend * fogenddefault })
	end

	function gadget:TextCommand(msg)
		if string.sub(msg, 1, 18) == "atmosplaysoundfile" then
			Engine.Unsynced.PlaySoundFile(string.sub(msg, 20), 0.85, "ui")
		end
	end
	...
	function AtmosSendVoiceMessage(filedirectory)
		Engine.Unsynced.SendCommands("atmosplaysoundfile " .. filedirectory)
	end
	...
	function SpawnCEGInPosition(cegname, posx, posy, posz, damage, paralyzetime, damageradius, sound, soundvolume)
		...
		if sound then
			Engine.Unsynced.PlaySoundFile(sound, soundvolume, posx, posy, posz, "sfx")
		end
		if damage or paralyzetime then
			local units = Engine.Shared.GetUnitsInCylinder(posx, posz, damageradius)
			for i = 1, #units do
				local uhealth, umaxhealth, uparalyze = Engine.Shared.GetUnitHealth(units[i])
				if damage then
					--Spring.Echo("Damaged")
					if uhealth > damage then
						Engine.Synced.SetUnitHealth(units[i], uhealth - damage)
					else
						Engine.Synced.DestroyUnit(units[i])
					end
				end
				if paralyzetime then
					...
					if uparalyze <= umaxhealth then
						...
						Engine.Synced.SetUnitHealth(units[i], { paralyze = paralyzedamage })
					else
	...
	function SpawnCEGInRandomMapPosAvoidUnits(cegname, groundOffset, radius, sound, soundvolume)
		for y = 1, 50 do
			...
			local posz = math_random(0, mapsizeZ)
			local posy = spGetGroundHeight(posx, posz) + (groundOffset or 0)
			local units = Engine.Shared.GetUnitsInCylinder(posx, posz, radius)
			if #units == 0 then
				spSpawnCEG(cegname, posx, posy, posz)
				if sound then
					Engine.Unsynced.PlaySoundFile(sound, soundvolume, posx, posy, posz, "sfx")
