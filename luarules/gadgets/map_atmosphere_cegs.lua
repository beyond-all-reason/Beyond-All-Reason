...
if not gadgetHandler:IsSyncedCode() then
	...
	local function MapAtmosphereConfigSetSun(
		_,
		targetbrightness,
		transitionspeed,
		redlevel,
		...
		Spring.SetSunLighting({
			groundAmbientColor = { transitionred * gar, transitiongreen * gag, transitionblue * gab },
		})
		Spring.SetSunLighting({
			unitAmbientColor = { transitionred * uar, transitiongreen * uag, transitionblue * uab },
		})
		Spring.SetSunLighting({
			groundDiffuseColor = { transitionred * gdr, transitiongreen * gdg, transitionblue * gdb },
		})
		Spring.SetSunLighting({
			unitDiffuseColor = { transitionred * udr, transitiongreen * udg, transitionblue * udb },
		})
		Spring.SetSunLighting({
			groundSpecularColor = { transitionred * gsr, transitiongreen * gsg, transitionblue * gsb },
		})
		Spring.SetSunLighting({
			unitSpecularColor = { transitionred * usr, transitiongreen * usg, transitionblue * usb },
		})
		...
		Spring.SetSunLighting({
			groundShadowDensity = transition * shadowdensity,
			modelShadowDensity = transition * shadowdensity,
		})
	end
	...
	function SpawnCEGInPositionGround(
		cegname,
		posx,
		groundOffset,
		posz,
	...
	function SpawnCEGInRandomMapPosBelowY(
		cegname,
		groundOffset,
		spawnOnlyBelowY,
		damage,
