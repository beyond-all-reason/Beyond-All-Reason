...
if not Spring.Utilities.IsDevMode() or not Spring.Utilities.Gametype.IsSinglePlayer() then
	Engine.Synced.SetGameRulesParam("isSyncedProxyEnabled", false)
	return
end

Engine.Synced.SetGameRulesParam("isSyncedProxyEnabled", true)
...
local function log(level, str, ...)
	...
	Engine.Shared.Log(gadget:GetInfo().name, LOG.NOTICE, str)
end
...
local function processFunctionCall(fn, returnID)
	...
	Engine.Unsynced.SendLuaUIMsg(Proxy.PREFIX.RETURN .. serializedReturn)
end
