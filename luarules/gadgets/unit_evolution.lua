		...
		for _, unitID in pairs(allUnits) do
			...
			for j = 1, #cmds do
				...
				if (unitTargetCommand[cmd.id] or (singleParamUnitTargetCommand[cmd.id] and #params == 1)) and (params[1] == oldUnit) then
					...
					SpringShared.GiveOrderToUnit(unitID, CMD.INSERT, { cmd.tag, cmd.id, opts, params[1], params[2], params[3] }, 0)
					SpringShared.GiveOrderToUnit(unitID, CMD.REMOVE, cmd.tag, 0)
				end
			end
		end
	end
				...
				if command.id < 0 then -- repair case for construction
					...
					for j = 1, #units do
						...
						if SpringShared.GetUnitDefID(areaUnitID) == -command.id then
							SpringShared.GiveOrderToUnit(newUnitID, CMD.REPAIR, areaUnitID, coded)
							notFound = false
							break
						end
					end
					if notFound then
						SpringShared.GiveOrderToUnit(newUnitID, command.id, command.params, coded)
					end
				else
					SpringShared.GiveOrderToUnit(newUnitID, command.id, command.params, coded)
				end
