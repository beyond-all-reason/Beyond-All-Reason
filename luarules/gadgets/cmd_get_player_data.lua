...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:RecvLuaMsg(msg, player)
		if msg:sub(3, 4) == validation then
			if msg:sub(1, 2) == "sd" then
				local name = Engine.Shared.GetPlayerInfo(player, false)
	...
	local function getPlayerIdFromName(targetPlayerName)
		for _, pID in ipairs(Engine.Shared.GetPlayerList()) do
			local name = Engine.Shared.GetPlayerInfo(pID, false)
	...
	local function requestScreenshot(targetPlayerName, width)
		...
		if not targetPlayerID then
			Engine.Shared.Echo("Player not found: " .. targetPlayerName)
			return
		end
		...
		Engine.Unsynced.SendLuaRulesMsg("ss" .. validation .. width .. ";" .. targetPlayerID)
	end
	...
	function StartScreenshot(_, msg)
		...
		local _, _, requestingSpec = Engine.Shared.GetPlayerInfo(requestingPlayerID, false)
		local _, _, mySpec = Engine.Shared.GetPlayerInfo(myPlayerID, false)
		...
		queueScreenshotGameframe = Engine.Shared.GetGameFrame()
	...
	function gadget:DrawScreenPost()
		...
		if sec > 0.03 then
			...
			if queueScreenShotBroadcastChars >= queueScreenShotCharsPerBroadcast or queueScreenShotH >= queueScreenShotHeight then
				...
				Engine.Unsynced.SendLuaRulesMsg(message)
				queueScreenShotBroadcastChars = 0
				queueScreenShotPixels = {}
				data = nil
				if finished == "1" then
		...
		if data then
			if msgType == "screenshot" then
				...
				if screenshotVars.finished or totalTime - 4000 > screenshotVars.dataLast then
					...
					Engine.Shared.Echo(string.format("Received screenshot from %s (%.0f KB, increased replay size)", playerName, compressedKB))
					...
					if Engine.Shared.GetModOptions().date_year then
						screenshotVars.filename = Engine.Shared.GetModOptions().date_year .. "-" .. Engine.Shared.GetModOptions().date_month .. "-" .. Engine.Shared.GetModOptions().date_day .. "_" .. screenshotVars.filename
					else
					...
					for _, pID in ipairs(playerList) do
						...
						if name == playerName then
							teamID = select(4, Engine.Shared.GetPlayerInfo(pID, false))
							if teamID then
								r, g, b = Engine.Unsynced.GetTeamColor(teamID)
							end
							break
						end
					end
	...
	function gadget:DrawScreen()
		...
		if screenshotVars.dlist then
			if screenshotVars.saveQueued then
				...
				Engine.Shared.Echo("Screenshot saved to: " .. file)
			...
			local mouseX, mouseY, mouseButtonL = Engine.Unsynced.GetMouseState()
