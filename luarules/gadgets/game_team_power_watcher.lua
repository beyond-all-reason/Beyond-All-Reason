...
for _, teamID in ipairs(teamList) do
	...
	if teamID ~= scavengerTeam and teamID ~= raptorTeam and select(4, Engine.Shared.GetTeamInfo(teamID, false)) then
		aiTeams[teamID] = true
...
local function highestAlliedTeamPower(teamID, allyID)
	allyID = allyID or select(6, Engine.Shared.GetTeamInfo(teamID))
	local highestPower = 0
	local highestTeamID = nil

	for id, power in pairs(teamPowers) do
		if allyID == select(6, Engine.Shared.GetTeamInfo(id)) and power > 0 then
			if power > highestPower then
				highestPower = power
				highestTeamID = id
			end
...
local function highestAlliedPeakPower(teamID, allyID)
	...
	for id, power in pairs(peakTeamPowers) do
		if allyID == select(6, Engine.Shared.GetTeamInfo(id)) then
			if power > highestPower then
				highestPower = power
				highestTeamID = id
			end
