...
local function UpdateSensorAndJamm(
	unitID,
	unitDefID,
	enabled,
	radarOverride,
	...
	if jammerUnitDef[unitDefID] or jammerOverride then
		Spring.SetUnitSensorRadius(
			unitID,
			"radarJammer",
			(enabled and (jammerOverride or jammerUnitDef[unitDefID])) or 0
		)
	end
			...
			if not unitReloadPaused[unitID] then
				...
				if reloadState < gameFrame then -- unit is already reloaded, so set unit to almost reloaded
					spSetUnitWeaponState(
						unitID,
						i,
						{ reloadTime = newReload, reloadState = gameFrame + UPDATE_PERIOD + 1 }
					)
				else
					local nextReload = gameFrame + (reloadState - gameFrame) * newReload / reloadTime
					spSetUnitWeaponState(
						unitID,
						i,
						{ reloadTime = newReload, reloadState = nextReload + UPDATE_PERIOD }
					)
				end
...
function UpdateUnitAttributes(unitID, frame)
	...
	if
		weaponMods
		or fullDisable
		or selfReloadSpeedChange
		or selfMoveSpeedChange
		or slowState
		or zombieSpeedMult
		or buildpowerMult
		or selfTurnSpeedChange
		or selfIncomeChange
		or disarmed
		or completeDisable
		or selfMaxAccelerationChange
	then
		...
		local buildMult = baseSpeedMult
			* (1 - disarmed)
			* (1 - completeDisable)
			* (selfIncomeChange or 1)
			* (buildpowerMult or 1)
		...
		if
			...
			or currentTurn[unitID] ~= turnMult
			or currentAcc[unitID] ~= maxAccMult
		then
