...
function gadget:AllowResourceTransfer(senderTeamId, receiverTeamId, resourceType, amount)
	...
	local rCur, rStor, rPull, rInc, rExp, rShare = Engine.Shared.GetTeamResources(receiverTeamId, resourceName)
	...
	if amount > maxShare then
		if maxShare > 0 then
			Engine.Synced.ShareTeamResource(senderTeamId, receiverTeamId, resourceName, maxShare)
		end
		return false
	end
...
function gadget:AllowUnitTransfer(unitID, unitDefID, oldTeam, newTeam, capture)
	...
	if capture or spIsCheatingEnabled() or unitCount < Engine.Shared.GetTeamMaxUnits(newTeam) then
		return true
	end
	return false
end
