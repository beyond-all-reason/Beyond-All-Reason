...
if not gadgetHandler:IsSyncedCode() then
	...
	function BroadcastEvent(_, event, player, forceplay)
		if Script.LuaUI("NotificationEvent") and (forceplay or (tonumber(player) and ((tonumber(player) == Spring.GetMyPlayerID()) or Engine.Unsynced.GetSpectatingState()))) then
			if forceplay then
				forceplay = " y"
			else
				forceplay = ""
...
GG["notifications"].queueNotification = function(event, idtype, id, forceplay)
	...
	elseif idtype == "teamID" then
		local playerList = Engine.Shared.GetPlayerList(id)
		for i = 1, #playerList do
			playerIDs[#playerIDs + 1] = playerList[i]
		end
	elseif idtype == "allyTeamID" then
		local teamList = Engine.Shared.GetTeamList(id)
		for i = 1, #teamList do
			local playerList = Engine.Shared.GetPlayerList(teamList[i])
