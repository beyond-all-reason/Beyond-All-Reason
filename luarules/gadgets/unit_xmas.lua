	...
	if not initiated then
		...
		if not detectedCandycane then
			for i = 1, candycaneAmount do
				...
				if y > 5 then
					local groundType, groundType2 = SpringShared.GetGroundInfo(x, z)
					if type(groundType) == "string" and groundType ~= "void" or groundType2 ~= "void" then -- 105 compatibility
						...
						local featureID = SpringSynced.CreateFeature("candycane" .. caneType, x, y, z, random(0, 360))
						SpringSynced.SetFeatureRotation(featureID, random(-12, 12), random(-12, 12), random(-180, 180))
					end
				end
			end
		end
...
function gadget:GameFrame(n)
	if n % 30 == 1 then
		for unitID, frame in pairs(decorations) do
			if frame < n then
				...
				if x then
					local gy = SpringShared.GetGroundHeight(x, z)
					...
					local env = SpringSynced.UnitScript.GetScriptEnv(unitID)
					SpringSynced.UnitScript.CallAsUnit(unitID, env.Sink)
				end
			end
		end
	end
	...
	if enableUnitDecorations then
		for _, data in ipairs(createDecorations) do
			...
			while i < amount do
				...
				uID = SpringSynced.CreateUnit(decorationDefID, data[1], data[2], data[3], 0, teamID)
				if uID ~= nil then
					decorationCount = decorationCount + 1
					decorations[uID] = SpringShared.GetGameFrame() + hasDecoration[data[5]][3] + (random() * (hasDecoration[data[5]][3] * 0.33))
					SpringSynced.SetUnitRotation(uID, random() * 360, random() * 360, random() * 360)
					local impulseMult = hasDecoration[data[5]][2]
					SpringSynced.AddUnitImpulse(uID, (random() - 0.5) * (impulseMult / 2), 1 + (random() * impulseMult), (random() - 0.5) * (impulseMult / 2))
				end
				i = i + 1
			end
		end
	...
	for _, unitID in ipairs(createdDecorations) do
		if not decorations[unitID] then
			decorationCount = decorationCount + 1
			decorations[unitID] = SpringShared.GetGameFrame() + 2000 + (random() * 1000)
			SpringSynced.SetUnitRotation(unitID, random() * 360, random() * 360, random() * 360)
			--Spring.AddUnitImpulse(unitID, (random()-0.5)*2, 3.8+(random()*1), (random()-0.5)*2)
			local impulseMult = 80
			SpringSynced.AddUnitImpulse(unitID, (random() - 0.5) * (impulseMult / 3), 1 + (random() * (impulseMult / 1.6)), (random() - 0.5) * (impulseMult / 3))
		end
	end
	createdDecorations = {}
end
...
function gadget:FeatureCreated(featureID, allyTeam)
	...
	if isComWreck[SpringShared.GetFeatureDefID(featureID)] then
		local px, py, pz = SpringShared.GetFeaturePosition(featureID)
		local rx, ry, rz = SpringShared.GetFeatureRotation(featureID)
		local dx, dy, dz = SpringShared.GetFeatureDirection(featureID)
		local heading = SpringShared.GetFeatureHeading(featureID)
		local teamID = SpringShared.GetFeatureTeam(featureID)
		SpringSynced.DestroyFeature(featureID)
		local xmasFeatureID = SpringSynced.CreateFeature(xmasComwreckDefID, px, py, pz, heading, teamID)
		if xmasFeatureID then
			SpringSynced.SetFeatureRotation(xmasFeatureID, rx, ry, rz)
			SpringSynced.SetFeatureDirection(xmasFeatureID, dx, dy, dz)
			local featureResurrect = SpringShared.GetFeatureResurrect(featureID)
			SpringSynced.SetFeatureResurrect(xmasFeatureID, featureResurrect, "s", 0)
		end
	end
end
