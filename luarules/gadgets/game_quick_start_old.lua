...
local function calculateUnitCostInJuice(unitDef)
...
for commanderName, labs in pairs(commanderAllLabs) do
	for labName, weight in pairs(labs) do
		if unitDefNames[labName] then
			local unitDefID = unitDefNames[labName].id
			local unitDef = UnitDefs[unitDefID]
			FACTORY_DISCOUNT = math.min(FACTORY_DISCOUNT, calculateUnitCostInJuice(unitDef) * FACTORY_DISCOUNT_MULTIPLIER)
		end
	end
end
...
local function generateBuildAreaChunksAroundPosition(centerX, centerZ)
...
local function generateBuildPositionsInChunkSortedByDistance(chunkData, targetX, targetZ)
	local positions = {}
...
local function initializeCommanderWithQuickStartResources(commanderID, teamID)
	...
	local chunksWithDistance = generateBuildAreaChunksAroundPosition(commanderX, commanderZ)
...
local function calculateJuiceCostForUnitDefID(unitDefID)
	local unitDef = UnitDefs[unitDefID]
	return calculateUnitCostInJuice(unitDef)
end

local function deductJuiceCostAndCreateUnitAtPosition(commanderData, unitDefID, buildX, buildY, buildZ, facing, teamID, commanderID, isFree)
	...
	if isFree then
		...
		local juiceCost = calculateJuiceCostForUnitDefID(unitDefID)
...
local function isBuildCommandByID(cmdID)
	if not cmdID then return false end
	return cmdID < 0
end

local function isCommanderWithinCommandStealRange(commanderX, commanderZ, targetX, targetZ)
...
local function trySnapPositionToGridAndValidateBuild(unitDefID, posX, posZ)
	local searchY = spGetGroundHeight(posX, posZ)
	local snappedX, snappedY, snappedZ = spPos2BuildPos(unitDefID, posX, searchY, posZ)
	if snappedX and spTestBuildOrder(unitDefID, snappedX, snappedY, snappedZ, 0) > 0 then
		return true, snappedX, snappedY, snappedZ
	end
	return false, nil, nil, nil
end

local function findFirstValidBuildPositionInPositionList(unitDefID, positions)
	for _, pos in ipairs(positions) do
		local ok, x, y, z = trySnapPositionToGridAndValidateBuild(unitDefID, pos.x, pos.z)
		if ok then
			return x, y, z
		end
	end
...
local function findAndBuildNearLocation(x, y, z, unitDefID, teamID, commanderData, commanderID)
	local unitDef = UnitDefs[unitDefID]
	...
	if isMetalExtractor and not isMetalMap and metalSpotsList and #metalSpotsList > 0 then
		...
		for _, spotData in ipairs(spotsWithDistance) do
			...
			if spTestBuildOrder(unitDefID, spot.x, spot.y, spot.z, 0) > 0 then
				local fullyBuilt = deductJuiceCostAndCreateUnitAtPosition(commanderData, unitDefID, spot.x, spot.y, spot.z, commanderData.defaultFacing or 0, teamID, commanderID, false)
				if fullyBuilt then
					return true
				end
			end
	...
	for _, chunkData in ipairs(chunksWithDistance) do
		local positions = generateBuildPositionsInChunkSortedByDistance(chunkData)
		local snappedX, snappedY, snappedZ = findFirstValidBuildPositionInPositionList(unitDefID, positions)
		if snappedX then
			local fullyBuilt = deductJuiceCostAndCreateUnitAtPosition(commanderData, unitDefID, snappedX, snappedY, snappedZ, commanderData.defaultFacing or 0, teamID, commanderID, false)
			if fullyBuilt then
				return true
			end
		end
...
local function generateBuildQueueFromQuotasAndWeights(commanderID, commanderData)
	local buildQueue = {}
	...
	for i = 1, overflowCount do
		local selectedOption = selectRandomOptionByWeight(randomBuildOptionWeights)
...
local function filterChunksForFactoryPlacement(chunks)
	local result = {}
...
local function ensureFactoryIsBuiltIfRequired(commanderID, commanderData)
	...
	local labsUsed = commanderAllLabs[commanderData.commanderName]
	...
	local factoryOrder = {}
	if not commanderData.isHuman then
	...
	local chunksWithDistance = filterChunksForFactoryPlacement(commanderData.chunksWithDistance)
	local commanderX, commanderY, commanderZ = spGetUnitPosition(commanderID)
	for _, factory in ipairs(factoryOrder) do
		for _, chunkData in ipairs(chunksWithDistance) do
			local positions = generateBuildPositionsInChunkSortedByDistance(chunkData, commanderX, commanderZ)
			local snappedX, snappedY, snappedZ = findFirstValidBuildPositionInPositionList(factory.unitDefID, positions)
			if snappedX then
				local facing = commanderData.defaultFacing or 0
				local isDiscounted = factoryRequired and not commanderData.factoryMade
				local fullyBuilt = deductJuiceCostAndCreateUnitAtPosition(commanderData, factory.unitDefID, snappedX, snappedY, snappedZ, facing, commanderData.teamID, commanderID, isDiscounted)
				if fullyBuilt then
...
local function executeQueuedBuildCommandsFromCommander(commanderID, commanderData, commanderX, commanderZ)
	...
	if commanderData.juice > 0 then
		...
		if next(commands) then
			for i, cmd in ipairs(commands) do
				...
				if isBuildCommandByID(cmd.id) and isCommanderWithinCommandStealRange(commanderX, commanderZ, buildsiteX, buildsiteZ) then
					...
					local ok, snappedX, snappedY, snappedZ = trySnapPositionToGridAndValidateBuild(buildDefID, buildX, buildZ)
					if ok then
						local fullyBuilt = deductJuiceCostAndCreateUnitAtPosition(commanderData, buildDefID, snappedX, snappedY, snappedZ, facing, commanderData.teamID, commanderID, isFree)
						if fullyBuilt then
							if isFactory then
								commanderData.factoryMade = true
							end
...
function gadget:GameFrame(frame)
	if not initialized and frame > PREGAME_DELAY_FRAMES then
		for unitID, teamID in pairs(queueCommanderCreation) do
			initializeCommanderWithQuickStartResources(unitID, teamID)
		end
	...
	for commanderID, commanderData in pairs(commanderMetaList) do
		...
		executeQueuedBuildCommandsFromCommander(commanderID, commanderData, commanderX, commanderZ)
		while commanderData.juice > 0 do
			if #buildQueues[commanderID] == 0 then
				generateBuildQueueFromQuotasAndWeights(commanderID, commanderData)
			end
			...
			if type(queueItem) == "string" then
				...
				unitDefID = trueName and unitDefNames[trueName] and unitDefNames[trueName].id
			else
				unitDefID = queueItem
			end
			
			if unitDefID then
				local success = findAndBuildNearLocation(commanderX, commanderY, commanderZ, unitDefID, commanderData.teamID, commanderData, commanderID)
			end
		end
		ensureFactoryIsBuiltIfRequired(commanderID, commanderData)
	end
