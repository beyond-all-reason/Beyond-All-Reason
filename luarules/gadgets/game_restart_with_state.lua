...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:RecvLuaMsg(msg, playerID)
		...
		if msg == "rws_restore_commit" then
			...
			Spring.Echo(
				"[Restart With State] State received ("
					.. tostring(#data.units)
					.. " units). Will restore at frame "
					.. restoreFrame
			)
			return true
		end
	...
	local function restoreUnits()
		...
		for i = 1, #data.units do
			...
			if not UnitDefs[u.unitDefID] then
				...
				local ok, newID = pcall(
					...
					u.facing or 0,
					u.teamID,
					isUnderConstruction and true or false
				)
				if ok and newID then
					...
					Spring.Echo(
						"[Restart With State] CreateUnit failed for defID "
							.. tostring(u.unitDefID)
							.. ": "
							.. tostring(newID)
					)
				end
			...
			if newID and u.commands and #u.commands > 0 then
				...
				for j = 1, #u.commands do
					...
					-- skip
						...
						local skip = false
						local isUnitTarget = UNIT_TARGET_ALWAYS[c.id]
							or (UNIT_TARGET_SINGLE[c.id] and params and #params == 1)
		...
		Spring.Echo(
			"[Restart With State] Restored " .. tostring(createdCount) .. "/" .. tostring(#data.units) .. " units."
		)
	end
