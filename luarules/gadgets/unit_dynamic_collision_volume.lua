...
if gadgetHandler:IsSyncedCode() then
	...
	local spSetPieceCollisionData = Engine.Synced.SetUnitPieceCollisionVolumeData
	local spGetPieceList = Engine.Shared.GetUnitPieceList
	local spGetUnitDefID = Engine.Shared.GetUnitDefID
	local spGetUnitCollisionData = Engine.Shared.GetUnitCollisionVolumeData
	local spSetUnitCollisionData = Engine.Synced.SetUnitCollisionVolumeData
	local spSetUnitRadiusAndHeight = Engine.Synced.SetUnitRadiusAndHeight
	local spGetUnitRadius = Engine.Shared.GetUnitRadius
	local spGetUnitHeight = Engine.Shared.GetUnitHeight
	local spSetUnitMidAndAimPos = Engine.Synced.SetUnitMidAndAimPos
	local spGetFeatureCollisionData = Engine.Shared.GetFeatureCollisionVolumeData
	local spSetFeatureCollisionData = Engine.Synced.SetFeatureCollisionVolumeData
	local spSetFeatureRadiusAndHeight = Engine.Synced.SetFeatureRadiusAndHeight
	local spGetFeatureRadius = Engine.Shared.GetFeatureRadius
	local spGetFeatureHeight = Engine.Shared.GetFeatureHeight

	local spArmor = Engine.Shared.GetUnitArmored
	local pairs = pairs
	local pieceIndexStr = {}
	for i = 0, 99 do
	...
	function gadget:Initialize()
		...
		for i = 1, #allFeatures do
			...
			local modelpath = FeatureDefs[Engine.Shared.GetFeatureDefID(featID)].modelpath
	...
	function gadget:UnitCreated(unitID, unitDefID, unitTeam)
		if unitDefMidAndAimPos[unitDefID] then
			...
			Engine.Synced.SetUnitMidAndAimPos(
				unitID,
	...
	function gadget:FeatureCreated(featureID, allyTeam)
		if featureDefMidAndAimPos[featureDefID] then
			...
			Engine.Synced.SetFeatureMidAndAimPos(
				featureID,
				midAndAimPos["midx"] or 0,
				midAndAimPos["midy"] or 0,
				midAndAimPos["midz"] or 0,
		...
		if is3doFeature[Engine.Shared.GetFeatureDefID(featureID)] then
			local rs, hs
