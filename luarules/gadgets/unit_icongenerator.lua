...
if gadgetHandler:IsSyncedCode() then
	...
	local function GameFrame(_, n)
		...
		for i = 1, #units do
			if (units[i].frame + 5) == n then
				Engine.Synced.DestroyUnit(units[i].id, false, true)
		...
		for i, cunit in ipairs(createunits) do
			if #units > 10 or (curTeam and cunit.team ~= curTeam) then
				...
				local y = Engine.Shared.GetGroundHeight(0, 0)
				Engine.Synced.LevelHeightMap(x - 50, z - 50, x + 50, z + 50, y)

				local uid = Engine.Synced.CreateUnit(cunit.defname, x, y, z, "south", 0) -- FIXME needs to be a non-gaia team if gaia doesn't have its unitlimit assigned

				if uid then
					units[#units + 1] = { id = uid, defname = cunit.defname, frame = n + cunit.time }
					curTeam = cunit.team

					Engine.Synced.SetUnitNeutral(uid, true)
					Engine.Shared.GiveOrderToUnit(uid, CMD.FIRE_STATE, { 0 }, 0)
					Engine.Shared.GiveOrderToUnit(uid, CMD.STOP, {}, 0)

					local env = Engine.Synced.UnitScript.GetScriptEnv(uid)
					if env then
						lus = true
					end
					...
					if lus then
						if env.Activate then
							Engine.Synced.UnitScript.CallAsUnit(uid, env.Activate)
						end
					else
						Engine.Synced.CallCOBScript(uid, "Activate", 0)
					end

					if cunit.move then
						if lus then
							if env.StartMoving then
								Engine.Synced.UnitScript.CallAsUnit(uid, env.StartMoving)
							end
						else
							Engine.Synced.CallCOBScript(uid, "StartMoving", 0)
						end
					end
					...
					if cunit.attack and not lus then
						...
						Engine.Synced.CallCOBScript(uid, "AimPrimary", 0, Engine.Shared.GetHeadingFromVector(0, 1), angle)
						Engine.Synced.CallCOBScript(uid, "AimWeapon1", 0, Engine.Shared.GetHeadingFromVector(0, 1), angle)

						Engine.Synced.CallCOBScript(uid, "AimSecondary", 0, Engine.Shared.GetHeadingFromVector(0, 1), angle)
						Engine.Synced.CallCOBScript(uid, "AimWeapon2", 0, Engine.Shared.GetHeadingFromVector(0, 1), angle)

						Engine.Synced.CallCOBScript(uid, "AimTertiary", 0, Engine.Shared.GetHeadingFromVector(0, 1), angle)
						Engine.Synced.CallCOBScript(uid, "AimWeapon3", 0, Engine.Shared.GetHeadingFromVector(0, 1), angle)
					end
				end
			end
		end
	...
	local function CreateResources()
      ...
      void main(void) {
		...
		if not pre_shader then
			Engine.Shared.Log(gadget:GetInfo().name, LOG.ERROR, gl.GetShaderLog())
		end
	...
	local function GetUnitDefDims(udid)
		local dims = Engine.Shared.GetUnitDefDimensions(udid)
	...
	local function CheckBoundings(udid, top, left, bottom, right, scale, border)
		...
		if bottom_ + height_ > renderY or left_ < 0 or bottom_ < 0 or left_ + width_ > renderX then
			...
			if offX then
				Engine.Shared.Echo(i, UnitDefs[udid].name .. ": Boundings outside of the texture", offX, offY)
				autoConfigs[udid].offset = { autoConfigs[udid].offset[1] + offX, autoConfigs[udid].offset[2] + offY, 0 }
				return false, left_, bottom_, width_, height_
			else
				Engine.Shared.Echo(i, UnitDefs[udid].name .. ": Render Context too small (you have to increase renderX&renderY)")
			end
		end
	...
	local function CreateIcon(udid, uid)
		...
		if not result and not cfg.empty then
			Engine.Shared.Log(gadget:GetInfo().name, LOG.ERROR, "icongen: " .. UnitDefs[udid].name .. ": give up :<")
		end
	end
	...
	local function BuildIcon(cmd, line, words, playerID)
		if not Engine.Shared.IsCheatingEnabled() then
			Engine.Shared.Echo("Cheating must be enabled")
			return false
		end
		...
		if final_tex or #jobs > 0 then
			Engine.Shared.Echo("Wait until current process is finished")
			return false
		end

		if words[1] and words[1] ~= "all" and not UnitDefNames[words[1]] then
			Engine.Shared.Echo("No such unit found")
			return false
		end
			...
			for _, _scheme in pairs(schemes) do
				for _ratio_name, _ratio in pairs(ratios) do
					...
					if words[1] and words[1] ~= "all" then
						...
						Engine.Shared.Echo("buildicon: " .. words[1] .. "  " .. (words[3] or ""))
					else
					...
					AddJob(function()
						...
						Engine.Unsynced.CreateDir(outdir)
