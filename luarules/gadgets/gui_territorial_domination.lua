...
local function createShader()
	...
	local processedVertexShader =
		vertexShaderSource:gsub("//__ENGINEUNIFORMBUFFERDEFS__", engineUniformBufferDefinitions)
...
local function processSpectatorModeChange()
	...
	if currentSpectating ~= amSpectating or (previousAllyID and currentAllyID ~= previousAllyID) then
		...
		for gridID, gridSquareData in pairs(captureGrid) do
			gridSquareData.isVisible, _ = getSquareVisibility(
				gridSquareData.allyOwnerID,
				gridSquareData.allyOwnerID,
				gridSquareData.visibilityArray
			)
		end
	end
...
function gadget:GameFrame(frame)
	...
	if notifyFrames[frame] then
		...
		spPlaySoundFile(
			"scavdroplootspawn",
			CAPTURE_SOUND_VOLUME,
			gridData.gridMidpointX,
			0,
			gridData.gridMidpointZ,
			0,
			0,
			0,
			"sfx"
