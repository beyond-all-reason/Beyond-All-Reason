...
function gadget:Initialize()
	...
	readyButton = gl.CreateList(function()
		...
		RectRound(-((readyW/2)+bgMargin), 0, ((readyW/2)+bgMargin), ((readyH/2)+bgMargin), 3, 2,2,0,0, {1,1,1,0.035}, {1,1,1,0.11})
		RectRound(-((readyW/2)+bgMargin), -((readyH/2)+bgMargin), ((readyW/2)+bgMargin), -((readyH/4)+bgMargin), 3, 0,0,2,2, {1,1,1,0.045}, {1,1,1,0})
	...
	readyButtonHover = gl.CreateList(function()
		...
		-- gloss
		gl.Blending(GL.SRC_ALPHA, GL.ONE)
		RectRound(-((readyW/2)+bgMargin), 0, ((readyW/2)+bgMargin), ((readyH/2)+bgMargin), 3, 2,2,0,0, {1,1,1,0.05}, {1,1,1,0.18})
		RectRound(-((readyW/2)+bgMargin), -((readyH/2)+bgMargin), ((readyW/2)+bgMargin), -((readyH/4)+bgMargin), 3, 0,0,2,2, {1,1,1,0.07}, {1,1,1,0})
		gl.Blending(GL.SRC_ALPHA, GL.ONE_MINUS_SRC_ALPHA)
		gl.Color(1,1,1,1)
	end)
end
