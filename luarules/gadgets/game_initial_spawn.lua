	...
	if enabled then
		...
		for i =1, #teams do
			if select(4,Spring.GetTeamInfo(teams[i],false)) then	-- is AI?
				...
				if aiName and aiName ~= '' and not string.find(aiName, "Chicken:") and not string.find(aiName, "DAI") and not string.find(aiName, "KAIK") and not string.find(aiName, "NullAI") and not string.find(aiName, "ScavengersAI") then
					enabled = false
