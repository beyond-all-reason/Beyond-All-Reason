...
local enabled = false
local minEngineVersionTitle = '104.0.1-1250'
if (engineVersion < 1000 and engineVersion >= 105) or engineVersion >= 104011250 then
	...
	if enabled then
		...
		for i =1, #teams do
			if select(4,Spring.GetTeamInfo(teams[i],false)) then	-- is AI?
				...
				if aiName and aiName ~= '' and not string.find(aiName, "Chicken:") and not string.find(aiName, "DAI") and not string.find(aiName, "KAIK") and not string.find(aiName, "NullAI") and not string.find(aiName, "ScavengersAI") and not string.find(aiName, "newAI") then
					enabled = false
