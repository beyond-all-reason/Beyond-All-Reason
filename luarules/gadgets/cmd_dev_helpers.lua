		...
		for _, permKey in ipairs({ "devhelpers_" .. subPermission, subPermission }) do
			if
				...
				or (
					...
					and (
						SYNCED.permissions[permKey][accountID]
						or (playername and SYNCED.permissions[permKey][playername])
					)
				)
			then
				hasPermission = true
...
if gadgetHandler:IsSyncedCode() then
			...
			for j = 1, tableDebth do
				Spring.Echo(
					j
						...
						.. j + tableDebth
						.. "\t"
						.. modeArray[j + tableDebth][2]
						.. "\t\t\t"
						.. j + tableDebth + tableDebth
						.. "\t"
						.. modeArray[j + tableDebth + tableDebth][2]
				)
			end
	...
	function gadget:RecvLuaMsg(msg, playerID)
		...
		elseif
			...
			or cmd == "destroyunits"
			or cmd == "removeunits"
			or cmd == "removenearbyunits"
			or cmd == "reclaimunits"
			or cmd == "transferunits"
			or cmd == "select"
			or cmd == "unselect"
			or cmd == "neutralize"
			or cmd == "maxhealth"
			...
			or cmd == "sethealth"
			or cmd == "spawnceg"
			or cmd == "spawnunitexplosion"
			or cmd == "removeunitdef"
			or cmd == "removeobjects"
		then
			subPermission = "units"
		...
		for n = 2, #words do
			...
			if unitID and Spring.ValidUnitID(unitID) then
				...
				elseif action == "wreck" then
					...
					if
						UnitDefs[unitDefID]
						and UnitDefs[unitDefID].corpse
						and FeatureDefNames[UnitDefs[unitDefID].corpse]
					then
	...
	function spawnunitexplosion(words, playerID)
		...
		if unitDefID then
			...
			local unitID =
				Spring.CreateUnit(unitDefID, tonumber(words[3]), tonumber(words[4]), tonumber(words[5]), "n", teamID)
			if unitID then
	...
	function ExecuteRemoveUnitDefName(unitdefname)
		...
		if unitDefID then
			...
			Spring.Echo(
				string.format(
					"Removed %i units, %i wrecks, %i heaps for unitDefName %s",
					removedunits,
					removedwrecks,
					removedheaps,
					unitdefname
	...
	function gadget:Initialize()
		...
		gadgetHandler:AddSyncAction("modbroadcast", function(_, colorKey, soundID, text)
			...
			if text and text ~= "" then
				...
				modbroadcastActive =
					{ text = text, r = color.r, g = color.g, b = color.b, startTime = Spring.GetGameSeconds() }
			end
		end)
		...
		if boxX1 then
			...
			if selectedUnits and #selectedUnits > 0 then
				...
				if targetType == "unit" and unitID and Spring.ValidUnitID(unitID) then
					...
					targetUnits = Spring.GetUnitsInScreenRectangle(
						mx - HOVER_PICK_SCREEN_RADIUS,
						my - HOVER_PICK_SCREEN_RADIUS,
						mx + HOVER_PICK_SCREEN_RADIUS,
						my + HOVER_PICK_SCREEN_RADIUS
					) or {}
					if #targetUnits == 0 then
						local _, pos = Spring.TraceScreenRay(mx, my, true)
						if type(pos) == "table" then
							local nearbyUnits = Spring.GetUnitsInSphere(pos[1], pos[2], pos[3], HOVER_PICK_WORLD_RADIUS)
								or {}
	...
	function processUnits(_, line, words, playerID, action)
		...
		if action == "removenearbyunits" then
			...
			if type(pos) == "table" then
				units = Spring.GetUnitsInSphere(
					pos[1],
					pos[2],
					pos[3],
					words[1] and words[1] or 24,
					words[2] and words[2] or nil
				)
			end
		else
	...
	function dumpFeatures(_, line, words, playerID)
		...
		for k, featureID in pairs(features) do
			...
			) --{ name = 'ad0_aleppo_2', x = 2900, z = 52, rot = "-1" },
		end
	end
	...
	function dumpLoadout(_, line, words, playerID)
		...
		for _, unitID in pairs(Spring.GetAllUnits()) do
			...
			Spring.Echo(
				string.format(
					"\t{ unitDefName = '%s', x = %d, z = %d, facing = '%s', team = %d%s },",
					unitDefName,
					math.floor(x),
					math.floor(z),
					facing,
		...
		for _, featureID in pairs(Spring.GetAllFeatures()) do
			...
			Spring.Echo(
				string.format(
					"\t{ featureDefName = '%s', x = %d, z = %d, facing = '%s' },",
					featureDefName,
					math.floor(x),
					math.floor(z),
					facing
	...
	function fightertest(_, line, words, playerID, action)
		...
		if fightertestactive then
			...
			local s1 = string.format(
				"Fightertest complete, #created = %d, #destroyed = %d",
				fighterteststats.numunitscreated,
				fighterteststats.numunitsdestroyed
			)
			Spring.Echo(s1)
			local res = {}
			local stats = {}
			for n, t in pairs({
				Sim = fighterteststats.simFrameTimes,
				Draw = fighterteststats.drawFrameTimes,
				Update = fighterteststats.updateFrameTimes,
			}) do
				local ms = {
					count = 0,
					total = 0,
				...
				local s2 =
					string.format("%s %d frames, %3.2fms per frame, %4.2fs total", n, ms.count, ms.mean, ms.total)
				res[#res + 1] = s2
				Spring.Echo(s2)
			end
	...
	function globallos(_, line, words, playerID, action)
		...
		Spring.SendLuaRulesMsg(
			PACKET_HEADER .. ":globallos:" .. (globallos and " 1" or " 0") .. (words[2] and ":" .. words[2] or "")
		)
	end
	...
	function spawnunitexplosion(_, line, words, playerID)
		...
		local msg = "spawnunitexplosion "
			...
			.. " "
			.. tostring(z)
			.. ((words[2] and words[2] == "1") and " 1" or " 0")
	...
	function GiveCat(_, line, words, playerID)
		...
		local function isNavalUnit(ud)
			...
			return ud.floatOnWater
				or (ud.minWaterDepth and ud.minWaterDepth > 0)
				or string.find(subfolder, "ships", 1, true)
		end
		...
		addFilter("scout", function(ud)
			return hasRoleTag(ud, "scout")
				or (not ud.isBuilding and ((ud.losRadius or 0) >= 700 or (ud.radarDistance or 0) >= 1800))
		end)
		addFilter({ "artillery", "arty" }, function(ud)
			...
			if
				ud.canFly
				and not ud.hoverAttack
				and unitHasWeaponMatching(ud, function(wDef)
			...
			if
				unitHasWeaponMatching(ud, function(wDef)
