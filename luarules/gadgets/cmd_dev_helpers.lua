...
if gadgetHandler:IsSyncedCode() then
	...
	function fightertest(words)
		...
		if not fightertestenabled then
			SpringShared.Echo(string.format("Fightertest ended, %d units spawned over %d gameframes, Units/frame = %f",
					fightertesttotalunitsspawned,
					SpringShared.GetGameFrame() - fighterteststartgameframe,
					fightertesttotalunitsspawned * (1.0 / (SpringShared.GetGameFrame() - fighterteststartgameframe))
					))
		...
		else SpringShared.Echo(words[2], "is not a valid unitDefName, using", team1unitDefName, "instead") end

		if words[3] and UnitDefNames[words[3]] then	team2unitDefName = words[3]
		else SpringShared.Echo(words[3], "is not a valid unitDefName, using", team2unitDefName, "instead") end
		...
		SpringShared.Echo(string.format("Starting fightertest %s vs %s with %i maxunits and %i units per step in a %d radius, features live %d frames",
				team1unitDefName,
				team2unitDefName,
				maxunits,
				feedstep,
	...
	function spawnceg(words)
		...
		SpringSynced.SpawnCEG(words[2], --cegname
	...
	function GiveCat(_, line, words, playerID)
		...
		if words[1] and UnitDefNames[words[1]] then
			...
			local _, _, _, teamID = SpringShared.GetPlayerInfo(Spring.GetMyPlayerID(), false)
			if words[2] and tonumber(words[2]) then
