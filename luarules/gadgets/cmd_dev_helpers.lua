...
if gadgetHandler:IsSyncedCode() then
	...
	local function SpawnUnitDefsForTeam(teamID, unitDefName)
		local unitcount = SpringShared.GetTeamUnits(teamID)
		if #unitcount < maxunits then
			local cmd = string.format("give %d %s %d @%d,%d,%d", feedstep, unitDefName, teamID, mapcx + placementradius * (getrandom() - 0.5), mapcy, mapcz + placementradius * (getrandom() - 0.5))
	...
	local function SpawnUnitDefsForTeamSynced(teamID, unitDefName)
		...
		if unitcount < maxunits then
			...
			SpringSynced.GiveOrderToUnitArray(newUnitIDs, CMD.REPEAT, { 1 }, 0)
			...
			SpringSynced.GiveOrderToUnitArray(newUnitIDs, CMD.MOVE, { ncx, gh, ncz }, { "shift" })
			...
			SpringSynced.GiveOrderToUnitArray(newUnitIDs, CMD.MOVE, { cx, gh, cz }, { "shift" })
	...
	function fightertest(words)
		...
		if not fightertestenabled then
			SpringShared.Echo(string.format("Fightertest ended, %d units spawned over %d gameframes, Units/frame = %f", fightertesttotalunitsspawned, SpringShared.GetGameFrame() - fighterteststartgameframe, fightertesttotalunitsspawned * (1.0 / (SpringShared.GetGameFrame() - fighterteststartgameframe))))
			ExecuteRemoveUnitDefName(team1unitDefName)
			ExecuteRemoveUnitDefName(team2unitDefName)
			return
		end
		fighterteststartgameframe = SpringShared.GetGameFrame()
		...
		SpringShared.Echo(string.format("Starting fightertest %s vs %s with %i maxunits and %i units per step in a %d radius, features live %d frames", team1unitDefName, team2unitDefName, maxunits, feedstep, placementradius, keepfeatures))
		featuredefstoremove = {}
