	...
	if randomposy >= data.wind_minHeight and Spring.IsSphereInView(randomposx, randomposy + 400, randomposz) then
		...
		if math.random() > 0.1 then
			Spring.PlaySoundFile(
				data.wind_soundBank[math.random(1, #data.wind_soundBank)],
				windSpeed * 0.6,
				randomposx,
				(randomposy + 400),
				randomposz,
			...
			Spring.PlaySoundFile(
				data.wind_rareSoundBank[math.random(1, #data.wind_rareSoundBank)],
				windSpeed * 0.6,
				randomposx,
				(randomposy + 400),
				randomposz,
...
function PlaySeaSound(n) -- We want wind to play at volume depending on wind speed and height.
	...
	if randomposy <= data.sea_minDepth and Spring.IsSphereInView(randomposx, randomposy + 400, randomposz) then
		...
		if math.random() > 0.1 then
			Spring.PlaySoundFile(
				data.sea_soundBank[math.random(1, #data.sea_soundBank)],
				(0.10 + (windSpeed * 0.75)),
				randomposx,
				(randomposy + 400),
				randomposz,
			...
			Spring.PlaySoundFile(
				data.sea_rareSoundBank[math.random(1, #data.sea_rareSoundBank)],
				(0.10 + (windSpeed * 0.75)),
				randomposx,
				(randomposy + 400),
				randomposz,
...
function PlayBeachSound(n) -- We want wind to play at volume depending on wind speed and height.
	...
	if
		randomposy < 0
		and randomposy >= data.beach_maxDepth
		and Spring.IsSphereInView(randomposx, randomposy, randomposz)
	then
		local windSpeed = select(4, Spring.GetWind()) / 25
		if math.random() > 0.1 then
			Spring.PlaySoundFile(
				data.beach_soundBank[math.random(1, #data.beach_soundBank)],
				(0.20 + (windSpeed * 0.6)),
				randomposx,
				(randomposy + 400),
				randomposz,
			...
			Spring.PlaySoundFile(
				data.beach_rareSoundBank[math.random(1, #data.beach_rareSoundBank)],
				(0.20 + (windSpeed * 0.6)),
				randomposx,
				(randomposy + 400),
				randomposz,
