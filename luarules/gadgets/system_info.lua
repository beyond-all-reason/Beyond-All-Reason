...
if gadgetHandler:IsSyncedCode() then
	...
	local chobbyLoaded = (Engine.Unsynced.GetMenuName and string.find(string.lower(Engine.Unsynced.GetMenuName()), "chobby") ~= nil)
	...
	function gadget:Initialize()
		...
		Engine.Shared.Echo(string.format("infologVersionTags:engine=%s,game=%s,lobby=%s,map=%s", Engine.version or "", (Game.gameName or "") .. " " .. (Game.gameVersion or ""), (VFS and VFS.GetNameFromRapidTag and VFS.GetNameFromRapidTag("byar-chobby:test") or ""), Game.mapName or ""))
		...
		if Engine ~= nil and Platform ~= nil then -- v104
			...
			if Platform.availableVideoModes then
				...
				for i, mode in pairs(Platform.availableVideoModes) do
					if mode.h > maxheight then
						maxheight = mode.h
					end
					if mode.w > maxwidth then
						maxwidth = mode.h
					end
				end
	...
	function gadget:MapDrawCmd(playerID, cmdType, px, py, pz, labelText)
		if playerID == myPlayerID and cmdType == "point" and string.len(labelText) > 2 then
			local msg = string.format("m@pm@rk%s:%d:%d:%d:%d:%s:%s", validation, Engine.Shared.GetGameFrame(), playerID, px, pz, myPlayerName, labelText)
			SendLuaRulesMsg(msg)
		end
	end
