...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:UnitDamaged(
		...
		if
			...
			and attackerTeam
			and GetAllyTeamID(attackerTeam) ~= myAllyTeamID
			and not isObjectified[unitDefID]
		then
			if isCommander[unitDefID] then
	...
	function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID, attackerDefID, attackerTeam, weaponDefID)
		...
		if
			isCommander[unitDefID]
			and not select(3, Spring.GetTeamInfo(unitTeam))
			and not Spring.GetUnitRulesParam(unitID, "muteDestructionNotification")
		then
			local myComCount = 0
				...
				if numTeams > 1 and not playingAsHorde then
					...
					for ct, player in pairs(players) do
						if tostring(player) then
							...
							elseif
								not attackerTeam
								and select(6, Spring.GetTeamInfo(unitTeam, false)) == myAllyTeamID
								and (
									not commanderLastDamaged[unitID]
									or commanderLastDamaged[unitID] + 150 < Spring.GetGameFrame()
								)
							then
