...
if (gadgetHandler:IsSyncedCode()) then
	...
	_G.validationLoshack = validation
	...
	function gadget:RecvLuaMsg(msg, player)
		if msg:sub(1,2)=="lh" and msg:sub(3,4)==validation then
	...
	local validation = SYNCED.validationLoshack
	...
	function CasualFullViewCheck()
		...
		if full and not spec then
			Spring.SendLuaRulesMsg("lh"..validation.."imacheat")
		end
	end
	...
	function VisibilityUnsyncedCheck(_, sunitID, sunitTeam)
		if Spring.GetSpectatingState() == true then
			...
			if Spring.IsUnitVisible(unitID) == true and not (unitTeam == Spring.GetMyTeamID()) then
				Spring.SendLuaRulesMsg("lh"..validation.."loshackprevent;"..sunitID..";"..sunitTeam..";"..(tostring(Spring.GetMyTeamID())))
			end
		end
	end
end