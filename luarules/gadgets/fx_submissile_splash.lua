...
local GetProjectilePosition = Engine.Shared.GetProjectilePosition
local GetProjectileDirection = Engine.Shared.GetProjectileDirection
...
function gadget:ProjectileCreated(proID, proOwnerID, weaponDefID)
	if subMissileWeapons[weaponDefID] then
		...
		Engine.Synced.SpawnCEG(splashCEG, x, 0, z)
	end
end
	...
	for proID, CEG in pairs(missiles) do
		...
		if y then
			if y < 0 and random() < 0.95 then
				...
				Engine.Synced.SpawnCEG(CEG, x, y, z, dirX, dirY, dirZ)
			end
		else
