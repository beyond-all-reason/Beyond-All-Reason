...
if not gadgetHandler:IsSyncedCode() then
	...
	local function SetLightingAndAtmosphere(lightandatmos)
		...
		if Script.LuaUI("NightFactorChanged") then
			Script.LuaUI.NightFactorChanged(
				lightandatmos.nightFactor.red,
				lightandatmos.nightFactor.green,
				lightandatmos.nightFactor.blue,
				lightandatmos.nightFactor.shadow,
				lightandatmos.nightFactor.altitude
			)
		end
	...
	local function GetNightLight(fromlight, nightfactor, azimuth, altitude)
		...
		endlight.nightFactor = {
			red = nightfactor[1],
			green = nightfactor[2],
			blue = nightfactor[3],
			shadow = nightfactor[4],
			altitude = altitude,
		}
		return endlight
	end
	...
	function gadget:GameFrame(n)
		...
		for i, nc in ipairs(nightModeConfig) do
			...
			if currentseconds < nc.repeats * nc.period then
				...
				elseif
					phase > nc.dayDuration + nc.transitionDuration
					and phase < nc.dayDuration + nc.transitionDuration + nc.nightDuration
				then
					...
					if phase <= nc.dayDuration + nc.transitionDuration then -- moving to night
						...
						mixfac = 1.0
							- math.smoothstep(
								nc.dayDuration + nc.transitionDuration + nc.nightDuration,
								nc.period,
								phase
							)
					end
	...
	function gadget:Initialize()
		...
		for i, nightConf in ipairs(nightModeConfig) do
			nightConf.endLight =
				GetNightLight(initial_atmosphere_lighting, nightConf.nightFactor, nightConf.azimuth, nightConf.altitude)
