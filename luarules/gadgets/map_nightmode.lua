...
if not gadgetHandler:IsSyncedCode() then
		...
		for _, s in ipairs({ "lighting", "atmosphere" }) do
			...
			for k, v in pairs(l[s]) do
				if type(v) == "table" then
					Engine.Shared.Echo(string.format("  %s = {%s},", k, quicktablestring(v)))
				else
					Engine.Shared.Echo(string.format("  %s = %s,", k, tostring(v)))
				end
			end
		end
		Engine.Shared.Echo("sunDir = " .. quicktablestring(l["sunDir"]))
	end
	...
	local function SetLightingAndAtmosphere(lightandatmos)
		...
		if lightandatmos.lighting then
			Engine.Unsynced.SetSunLighting({})
	...
	local function GetNightLight(fromlight, nightfactor, azimuth, altitude)
		...
		for _, atmlight in ipairs(atmosphere_lighting) do
			for k2, v2 in pairs(endlight[atmlight]) do
				if string.find(k2, "Color", nil, true) then
					local unitmod = 0
					if string.find(k2, "unit", nil, true) then
						unitmod = 0.66
					end
					for i = 1, #v2 do
		...
		if fromazimuth > 0 then
			...
			if toazimuth > math.pi then
				toazimuth = toazimuth - 2 * math.pi
			end
		else -- turn it right
			toazimuth = fromazimuth - azimuth
			if toazimuth < -1 * math.pi then
				toazimuth = toazimuth + 2 * math.pi
			end
		end
	...
	local function SetNightMode(cmd, line, words, playerID)
		...
		Engine.Shared.Echo("SetNightMode", cmd, line, words, playerID)
		if #words <= 1 then
			Engine.Shared.Echo("Resetting Lighting")
			SetLightingAndAtmosphere(initial_atmosphere_lighting)
			return
		end

		Engine.Shared.Echo("Expecting /luarules NightMode nightR nightG nightB azimuth altitude shadowfactor")
		...
		Engine.Shared.Echo(newNightLight)
	...
	local function PrintSun(cmd, line, words, playerID)
		Engine.Shared.Echo("Current sun settings are")
		local sun = GetLightingAndAtmosphere()
		EchoSun(sun)
	end

	function gadget:GameFrame(n)
		if transitionenabled == false then
			return
		end
		for i, nc in ipairs(nightModeConfig) do
			...
			if currentseconds < nc.repeats * nc.period then
				...
				elseif phase > nc.dayDuration + nc.transitionDuration and phase < nc.dayDuration + nc.transitionDuration + nc.nightDuration then
					-- still night, dont do shit
				else
					-- we are in transition
					if nc.mixedlight == nil then
