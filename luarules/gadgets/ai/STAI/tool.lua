...
function Tool:RectsOverlap(rectA, rectB)
	return rectA.x1 < rectB.x2 and rectB.x1 < rectA.x2 and rectA.z1 < rectB.z2 and rectB.z1 < rectA.z2
end
...
function Tool:UnitPos(UNIT)
	...
	if not UNIT.unit:Internal() then
		return
	end
...
function Tool:FillCircle(grid, gridElmos, position, radius, sets, adds)
	...
	if cradius > 0 then
		...
		while x >= z do
			...
			if err >= 0 then
				if x ~= lastZ then
					grid = self:Plot4(grid, cx, cz, lastZ, x, sets, adds)
				end
				err = err - x
				x = x - 1
				err = err - x
