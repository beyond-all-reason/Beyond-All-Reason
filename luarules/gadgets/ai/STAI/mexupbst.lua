...
function MexUpBST:Update()
	...
	if not self.active then
		...
		if self.ai.schedulerhst.behaviourTeam ~= self.ai.id or self.ai.schedulerhst.behaviourUpdate ~= "MexUpBST" then
			return
		end
		self:StartUpgradeProcess()
	end
...
function MexUpBST:Priority()
	if self.upType and self.ai.tool:listHasValue(self.ai.armyhst.buildersRole.expand[self.name], self.id) then
		return 99
	elseif self.upType and self.ai.tool:listHasValue(self.ai.armyhst.buildersRole.eco[self.name], self.id) and self.ai.tool:countMyUnit({ self.ai.armyhst._mex_ }) < 3 and self.ai.ecohst.Metal.full < 0.3 then
		return 150
	else
		return 0
	end
