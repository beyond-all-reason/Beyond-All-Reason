...
function LabsBST:ExitCheck()
	for i, v in pairs(self.ai.armyhst.unitTable[self.name].unitsCanBuild) do
		if not Engine.Shared.TestMoveOrder(self.ai.armyhst.unitTable[v].defId, self.position.x, self.position.y, self.position.z) then
...
function LabsBST:Update()
	...
	--	return
	--end
	if self.ai.schedulerhst.behaviourTeam ~= self.ai.id or self.ai.schedulerhst.behaviourUpdate ~= "LabsBST" then
		return
	end
	local f = self.game:Frame()
	self:preFilter() -- work or no resource??
	if Engine.Shared.GetFactoryCommandCount(self.id) > 1 then
		return
