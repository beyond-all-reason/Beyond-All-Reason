...
function OverviewHST:EvaluateSituation()
	...
	local bombingTooExpensive = self.ai.bomberhst:GetCounter() == self.ai.UnitiesHST.maxBomberCounter
	local attackTooExpensive = attackCounter == self.ai.UnitiesHST.maxAttackCounter
	...
	self.needSiege = (self.ai.totalEnemyImmobileThreat > self.ai.totalEnemyMobileThreat * 3.5 and self.ai.totalEnemyImmobileThreat > 50000) or attackCounter >= self.ai.UnitiesHST.siegeAttackCounter or controlMetalSpots
	...
	if self.ai.Metal.income > 50 and self.ai.haveAdvFactory and (needUpgrade or self.ai.BigEco) and self.ai.enemyBasePosition then
		if not self.ai.haveExpFactory then
			for i, factory in pairs(self.ai.factoriesAtLevel[self.ai.maxFactoryLevel]) do
				for expFactName, _ in pairs(self.ai.UnitiesHST.expFactories) do
					for _, mtype in pairs(self.ai.UnitiesHST.factoryMobilities[expFactName]) do
