		...
		if tg then
			...
			if  spaceEquals then
				if not self:unitNearCheck(utype,tgPos,spaceEquals)then
					...
					local p = ai.buildsitehst:ClosestBuildSpot(builder, tgPos, utype , minDist, nil, nil, unitTable[builderName].losRadius)
		...
		for index, pos in pairs(list) do
			if Distance(pos,builder:GetPosition()) < unitTable[builder:Name()].losRadius then
				if not spaceEquals or not self:unitNearCheck(utype,pos,spaceEquals)then
					local p = ai.buildsitehst:ClosestBuildSpot(builder, pos, utype , minDist, nil, nil, unitTable[utype:Name()][spaceEquals])
					if p then return p end
				end
			end
		end
