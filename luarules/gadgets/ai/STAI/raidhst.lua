...
function RaidHST:NeedMore(mtype, add)
	...
	if mtype == nil then
		for mtype, count in pairs(self.counter) do
			if self.counter[mtype] == nil then self.counter[mtype] = self.ai.UnitiesHST.baseRaidCounter end
			self.counter[mtype] = self.counter[mtype] + add
			self.counter[mtype] = math.min(self.counter[mtype], self.ai.UnitiesHST.maxRaidCounter)
...
function RaidHST:NeedLess(mtype)
	if mtype == nil then
		for mtype, count in pairs(self.counter) do
			...
			self.counter[mtype] = math.max(self.counter[mtype], self.ai.UnitiesHST.minRaidCounter)
			self:EchoDebug(mtype .. " raid counter: " .. self.counter[mtype])
		end
	else
		if self.counter[mtype] == nil then self.counter[mtype] = self.ai.UnitiesHST.baseRaidCounter end
...
function RaidHST:GetCounter(mtype)
	...
	if self.counter[mtype] == nil then
		return self.ai.UnitiesHST.baseRaidCounter
	else
