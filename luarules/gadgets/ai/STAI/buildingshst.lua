...
function BuildingsHST:LandWaterFilter(pos, unitTypeToBuild, builder)
	...
	if water then
		self:EchoDebug(builderName .. " is in water")
	else
		self:EchoDebug(builderName .. " is on land")
	end
	if waterBuildOrder then
		self:EchoDebug(unitName .. " would be in water")
	else
		self:EchoDebug(unitName .. " would be on land")
	end
...
function BuildingsHST:CanBuildHere(unittype, x, y, z) -- returns boolean
	local newX, newY, newZ = Engine.Shared.Pos2BuildPos(unittype:ID(), x, y, z)
	local buildable = Engine.Shared.TestBuildOrder(unittype:ID(), newX, newY, newZ, 1) --TODO check if it really necessary
...
function BuildingsHST:CheckBuildPos(
	pos,
	unitTypeToBuild,
	builder --[[, originalPosition]]
) --TODO clean this
	if not pos then
		return
	end
	if not self.ai.maphst:isInMap(pos) then
		return
	end
...
function BuildingsHST:CheckForDuplicates(unitName)
	...
	for i, plan in pairs(self.builders) do
		...
		if isFactory and factoryPlanned then
			return true
		end
		if plan.unitName == unitName then
			return true
		end
	end
