...
function LosHST:viewPos(upos)
	local LosOrRadar, inLos, inRadar, jammed = Engine.Shared.GetPositionLosState(upos.x, upos.y, upos.z, self.ai.allyId)
	if Engine.Shared.IsPosInAirLos(upos.x, upos.y, upos.z, self.ai.allyId) then
		return 1
	end
...
function LosHST:setPosLayer(unitName, Pos)
	...
	if Engine.Shared.GetGroundHeight(Pos.x, Pos.z) < 0 then --TEST  WARNING
		floating = true
	end
	return 0, floating
end
...
function LosHST:setPosLayer2(unitName, x, y, z)
	...
	if Engine.Shared.GetGroundHeight(x, z) < 0 then --TEST  WARNING
		floating = true
	end
	return 0, floating
end
...
function LosHST:setCellRadar(grid, unit, X, Z)
	...
	local speedX, speedY, speedZ, SPEED = Engine.Shared.GetUnitVelocity(unit:ID())
