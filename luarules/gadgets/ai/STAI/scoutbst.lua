...
function ScoutBST:Init()
	...
	self.armed = self.ai.UnitiesHST.unitTable[self.name].isWeapon
	self.keepYourDistance = self.ai.UnitiesHST.unitTable[self.name].losRadius * 0.5
	if mtype == "air" then
		self.airDistance = self.ai.UnitiesHST.unitTable[self.name].losRadius * 1.5
		...
		if f > self.lastUpdateFrame + 30 then
			...
			if self.target == nil and attackTarget == nil then
				...
				if topos ~= nil then
					...
					self.target = self.ai.Tool:RandomAway(self.ai, topos, self.keepYourDistance) -- don't move directly onto the spot
					unit:Move(self.target)
					self.attacking = false
				else
		...
		if f > self.lastCircleFrame + 60 then
			...
			if self.target then
				...
				if dist < self.airDistance then
					unit:Move(self.ai.Tool:RandomAway(self.ai, self.target, 100))
				end
			else
				unit:Move(self.ai.Tool:RandomAway(self.ai, upos, 500))
			end
