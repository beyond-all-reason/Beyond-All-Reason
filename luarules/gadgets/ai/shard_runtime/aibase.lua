...
function AIBase:Init() end

function AIBase:Update() end

function AIBase:GameEnd() end

function AIBase:GameMessage(text) end

function AIBase:UnitCreated(unit, unitDefId, teamId, builderId) end
...
function AIBase:MyUnitBuilt(engineunit) end

function AIBase:FriendlyUnitBuilt(engineunit) end

function AIBase:EnemyUnitBuilt(engineunit) end

function AIBase:NeutralUnitBuilt(engineunit) end
...
function AIBase:UnitDead(engineunit) end

function AIBase:UnitIdle(engineunit) end

function AIBase:UnitDamaged(engineunit, enginedamage) end

function AIBase:UnitEnteredLos(unitID, unitTeam, allyTeam, unitDefID) end

function AIBase:UnitLeftLos(unitID, unitTeam, allyTeam, unitDefID) end

function AIBase:UnitEnteredRadar(unitID, unitTeam, allyTeam, unitDefID) end

function AIBase:UnitLeftRadar(unitID, unitTeam, allyTeam, unitDefID) end

function AIBase:UnitMoveFailed(engineunit) end
...
if tracy then
	Engine.Shared.Echo("Enabled Tracy support for AIBase")
	AIBase.lastGCinfo = 0
	local logRAM = true
	--local function tracyZoneBeginMem() return end
	--local function tracyZoneEndMem() return end
	function AIBase:tracyZoneBeginN(fname)
		tracy.ZoneBeginN(self:Name() .. ":" .. fname)
	end
	function AIBase:tracyZoneEnd()
		tracy.ZoneEnd()
	end
