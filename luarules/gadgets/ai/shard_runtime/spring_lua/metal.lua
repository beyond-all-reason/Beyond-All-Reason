...
Engine.Shared.Echo("STAI metal map", metalmapSizeX, metalmapSizeZ, metalmapSizeX * metalmapSizeZ, extractorRadiusMetal, maxSpotArea, minSpotArea, halfHexHeight, halfExtractorRadiusMetal)

local mCeil = math.ceil

local spGetMetalAmount = Engine.Shared.GetMetalAmount
...
local function Flood4Metal(x, z, id)
	if x > metalmapSizeX or x < 1 or z > metalmapSizeZ or z < 1 then
		return
	end
	if isInBlob[x] and isInBlob[x][z] then
		return
	end
	local metalAmount = spGetMetalAmount(x, z)
	if metalAmount and metalAmount > 0 then
		if not isInBlob[x] then
			isInBlob[x] = {}
		end
		if not blobs[id] then
			blobs[id] = {}
		end
...
local function FloodHexBlob(x, z, id)
	...
	if not hexes[id] then
		hexes[id] = {}
	end
	if not hexes[id][x] then
		hexes[id][x] = {}
	end
	-- if hexes[id][x][z] or spGetMetalAmount(x,z) == 0 then return end
	if hexes[id][x][z] or not isInBlob[x] or isInBlob[x][z] ~= id then
		return
	end
...
local function Flood1HexBlob(x, z, id)
	...
	if hexes[id][x][z] or spGetMetalAmount(x, z) == 0 then
		return
	end
	local blobArea = CheckCircle(x, z, extractorRadiusMetal, id)
	-- Spring.Echo(x, z, id, blobArea, minSpotArea)
...
local function GetSpots()
	...
	Engine.Shared.Echo(#blobs, "blobs spots")
	-- isInBlob = {}
	local spots = {}
	local spotsCount = 0
	for id = 1, #blobs do
		...
		if blobArea < maxSpotArea then
			...
			Engine.Shared.Echo(#blobSpots, "spots in blob", id)
	...
	for i = 1, #spots do
		local spot = spots[i]
		if not spot.y then
			Engine.Unsynced.MarkerAddPoint(spot.x, spGetGroundHeight(spot.x, spot.z), spot.z, "bad spot")
		end
	end
	return spots
end
