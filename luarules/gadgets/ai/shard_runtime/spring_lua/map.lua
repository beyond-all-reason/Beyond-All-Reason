...
local Spring_GetGroundHeight = Engine.Shared.GetGroundHeight
...
function map:FindClosestBuildSite(unittype, builderpos, searchradius, minimumdistance, validFunction) -- returns Position
	...
	validFunction = validFunction or function(position)
		return position
	end
	...
	for radius = 50, searchradius, minimumdistance do
		...
		for angle = initAngle, initAngle + twicePi, angleInc do
			...
			if realAngle > twicePi then
				realAngle = realAngle - twicePi
			end
...
function map:CanBuildHere(unittype, position) -- returns boolean
	local newX, newY, newZ = Engine.Shared.Pos2BuildPos(unittype:ID(), position.x, position.y, position.z)
	local blocked = Engine.Shared.TestBuildOrder(unittype:ID(), newX, newY, newZ, 1) == 0
...
function map:TestMoveOrder(UnitDefID, p) -- returns boolean
	return Engine.Shared.TestMoveOrder(UnitDefID, p.x, p.z, p.y)
end
...
function map:RequestPath(moveClass, POS1, POS2, radius) -- returns a path
	...
	local metapath = Engine.Shared.RequestPath(moveClass, POS1.x, POS1.y, POS1.z, POS2.x, POS2.y, POS2.z, radius)
...
function map:GetPathObject(moveID, start_x, start_y, start_z, end_x, end_y, end_z, radius)
	...
	radius = radius or 8
	return Engine.Shared.RequestPath(moveID, start_x, start_y, start_z, end_x, end_y, end_z, radius)
end
...
function map:TestPath(moveID, start_x, start_y, start_z, end_x, end_y, end_z, radius, uname)
	...
	wp = wp[#wp]
	wp[2] = Engine.Shared.GetGroundHeight(wp[1], wp[3])
	local x = end_x - wp[1]
	local y = end_y - wp[2]
	local z = end_z - wp[3]
	local dist = math_sqrt((x * x) + (y * y) + (z * z))
...
function map:PathTest(moveID, start_x, start_y, start_z, end_x, end_y, end_z, radius)
	...
	radius = radius or 8
	if Engine.Shared.RequestPath(moveID, start_x, start_y, start_z, end_x, end_y, end_z, radius) then
		return true
	end
end
...
function map:GetMapFeatures()
	local fv = Engine.Shared.GetAllFeatures()
	if not fv then
		return {}
	end
	local f = {}
...
function map:GetMapFeaturesAt(position, radius)
	local fv = Engine.Shared.GetFeaturesInSphere(position.x, position.y, position.z, radius)
	if not fv then
		return {}
	end
	local f = {}
...
function map:GetWind()
	local _, _, _, strenght = Engine.Shared.GetWind()
	return strenght
end
...
function map:MaximumHeight() -- returns maximum map height
	local minHeight, maxHeight = Engine.Shared.GetGroundExtremes()
	return maxHeight
end
...
function map:GetGroundHeight(x, z)
	return Engine.Shared.GetGroundHeight(x, z)
end
