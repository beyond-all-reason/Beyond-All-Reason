ShardUnit = class(function(a, id)
	a.id = id
	a.className = "unit"
	local udefid = Engine.Shared.GetUnitDefID(id)
	a.UnitDefID = udefid
...
function ShardUnit:Team()
	return Engine.Shared.GetUnitTeam(self.id)
end

function ShardUnit:Radius()
	return Engine.Shared.GetUnitRadius(self.id)
end

function ShardUnit:AllyTeam()
	return Engine.Shared.GetUnitAllyTeam(self.id)
end

function ShardUnit:isNeutral()
	return Engine.Shared.GetUnitNeutral(self.id)
end
...
function ShardUnit:Stunned()
	local stunned_or_inbuild, stunned, inbuild = Engine.Shared.GetUnitIsStunned(self.id)
	return stunned
end
...
function ShardUnit:IsAlive()
	if Engine.Shared.GetUnitIsDead(self.id) == false then
		return true
	else
...
function ShardUnit:GetLos(bitmask)
	...
	return Engine.Shared.GetUnitLosState(self.id, self:GetUnitAllyTeam(), bitmask)
end
...
function ShardUnit:Cloaked()
	return Engine.Shared.GetUnitIsCloaked(self.id)
end

function ShardUnit:GetUnitIsBuilding()
	return Engine.Shared.GetUnitIsBuilding(self.id)
end

function ShardUnit:GetCurrentBuildPower()
	return Engine.Shared.GetUnitCurrentBuildPower(self.id)
end

function ShardUnit:IsBlocking()
	return Engine.Shared.GetUnitBlocking(self.unitid)
end

function ShardUnit:CurrentStockpile()
	local numStockpiled, numStockpileQued, buildPercent = Engine.Shared.GetUnitStockpile(self.id)
...
function ShardUnit:IsBeingBuilt()
	return Engine.Shared.GetUnitIsBeingBuilt(self.id)
end
...
function ShardUnit:FactoryWait()
	local topQueue = Engine.Shared.GetFactoryCommands(self.id, 1)[1]
...
function ShardUnit:getFactoryCommands()
	return Engine.Shared.GetFactoryCommands(self.id, 1)
end
...
function ShardUnit:GetUnitCommands(count)
	if count == 0 then
		Engine.Shared.Echo(self:Name(), self.id, "Get Unit Commands received 0 as parameter index")
		return {} -- FIXME: Why am I receiving 0 count?
	end

	count = count or 1
	local currentOrder = Engine.Shared.GetUnitCommands(self.id, count)
	return currentOrder
end
...
function ShardUnit:CurrentCommand()
	return Engine.Shared.GetUnitCurrentCommand(self.id)
end

function ShardUnit:GetPosition()
	local bpx, bpy, bpz = Engine.Shared.GetUnitPosition(self.id)
	local isDead = Engine.Shared.GetUnitIsDead(self.id)
	if not bpx then
...
function ShardUnit:GetHealtsParams()
	local health, maxHealth, paralyzeDamage, captureProgress, buildProgress = Engine.Shared.GetUnitHealth(self.id)
...
function ShardUnit:BuildProgress()
	local isBuilding, buildProgress = Engine.Shared.GetUnitIsBeingBuilt(self.id)
...
function ShardUnit:MaxWeaponsRange()
	return Engine.Shared.GetUnitMaxRange(self.id)
end
...
function ShardUnit:GetFacing(id)
	return Engine.Shared.GetUnitBuildFacing(id)
	-- 	0, "s", "south"
	--     1, "e", "east"
	--     2, "n", "north"
	--     3, "w", "west"
...
function ShardUnit:GetResourceUsage(idx)
	local metalMake, metalUse, energyMake, energyUse = Engine.Shared.GetUnitResources(self.id)
...
function ShardUnit:TestMoveOrder(p)
	return Engine.Shared.TestMoveOrder(self.UnitDefID, p.x, p.y, p.z, nil, nil, nil, true, true, false)
end
