...
function game:Frame() -- returns int/game frame number
	return Engine.Shared.GetGameFrame() -- Spring Gadget API
end

function game:Test() -- debug
	Engine.Shared.Echo("Testing API")
	return true
end
...
function game:GetTypeByName(typename) -- returns unittype
	if not UnitDefNames[typename] then
		Engine.Shared.Echo('shard: debug: could not find "' .. typename .. '" in UnitDefNames')
		return nil
	end
...
function game:GetUnitLos(id, bitmask)
	...
	return Engine.Shared.GetUnitLosState(id, self.ai.allyId, bitmask)
end
...
function game:GetTeamUnitDefCount(team, unitDef)
	return Engine.Shared.GetTeamUnitDefCount(team, unitDef)
end

function game:GetTeamUnitsByDefs(team, unitDef)
	return Engine.Shared.GetTeamUnitsByDefs(team, unitDef)
end

function game:GetUnitSeparation(Id1, Id2, d2d, surface)
	return Engine.Shared.GetUnitSeparation(Id1, Id2, d2d, surface)
end
...
function game:AddMarker(position, label) -- adds a marker
	Engine.Unsynced.MarkerAddPoint(position.x, position.y, position.z, label)
	return true
end
...
function game:GetResource(idx) --  returns a Resource object
	local currentLevel, storage, pull, income, expense, share, sent, received = Engine.Shared.GetTeamResources(self.ai.id, Shard.resourceIds[idx])
...
function game:GetResourceByName(name) -- returns a Resource object, takes the name of the resource
	...
	local currentLevel, storage, pull, income, expense, share, sent, received = Engine.Shared.GetTeamResources(self.ai.id, name)
	return Shard:shardify_resource({ currentLevel = currentLevel, storage = storage, pull = pull, income = income, expense = expense, share = share, sent = sent, received = received })
end
...
function game:Pause() -- pauses the game
	Engine.Unsynced.SendCommands("pause")
end
