...
function gadget:Initialize()
	...
	if BAR.Utilities.Gametype.IsFFA() then
	...
	if nHumanTeams <= 1 then
		gameType = "single player" -- and gaia
	elseif nHumanTeams == 2 and nAITeams == 0 then
		gameType = "1v1"
	else
		gameType = "team"
	end
...
function gadget:UnitDamaged(
	...
	if not info[attackerDefID] then
		return
	end
...
function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID, attackerDefID, attackerTeam, weaponDefID)
	...
	info[attackerDefID].killed_cost = info[attackerDefID].killed_cost
		+ unitMetalCost[unitDefID]
		+ unitEnergyCost[unitDefID] / 60
end
