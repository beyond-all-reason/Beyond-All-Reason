...
function gadget:UnitStunned(unitID, unitDefID, teamID, stunned)
	...
	if stunned then
		if not paralyzedUnits[unitID] then
			local beingBuilt, _ = Engine.Shared.GetUnitIsBeingBuilt(unitID)
...
function gadget:UnitFinished(unitID, unitDefID, unitTeam)
	if storageDefs[unitDefID] then
		local _, maxHealth, paralyzeDamage, _, _ = Engine.Shared.GetUnitHealth(unitID)
...
function gadget:Initialize()
	...
	for i = 1, #allUnits do
		...
		if select(5, Engine.Shared.GetUnitHealth(unitID)) == 1 then
			local unitDefID = Engine.Shared.GetUnitDefID(unitID)
			if storageDefs[unitDefID] and spGetUnitIsStunned(unitID) then
				reduceStorage(unitID, unitDefID, Engine.Shared.GetUnitTeam(unitID))
			end
		end
	end
end
...
function gadget:Shutdown()
	...
	for unitID, unitDefID in pairs(paralyzedUnits) do
		if spGetUnitIsStunned(unitID) then
			restoreStorage(unitID, unitDefID, Engine.Shared.GetUnitTeam(unitID))
		end
	end
end
