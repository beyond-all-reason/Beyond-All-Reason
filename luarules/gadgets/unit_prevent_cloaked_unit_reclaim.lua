...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:AllowCommand(unitID, unitDefID, unitTeam, cmdID, cmdParams)
		if
			(#cmdParams == 1)
			and GetUnitIsCloaked(cmdParams[1])
			and (GetUnitAllyTeam(unitID) ~= GetUnitAllyTeam(cmdParams[1]))
			and not IsUnitInRadar(cmdParams[1], GetUnitAllyTeam(unitID))
		then
			return false
		end
		return true
			...
			for _, bID in pairs(units) do
				...
				if canReclaim[unitDefID] then
					...
					if
						cmd == CMD.RECLAIM
						and target == unitID
						and (GetUnitAllyTeam(bID) ~= GetUnitAllyTeam(unitID))
						and not IsUnitInRadar(unitID, GetUnitAllyTeam(bID))
					then
