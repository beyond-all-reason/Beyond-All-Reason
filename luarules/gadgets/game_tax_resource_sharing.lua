...
function gadget:GameFrame(f)
	if (f - 1) % UPDATE_PERIOD == 0 then
		...
		for j, teamID in ipairs(teamList) do
			local teamEnergyCurrentLevel, teamEnergyStorage, teamEnergyPull, teamEnergyIncome, teamEnergyExpense, teamEnergyShare, teamEnergySent, teamEnergyReceived, teamEnergyExcess =
				spGetTeamResources(teamID, "energy")
			local teamMetalCurrentLevel, teamMetalStorage, teamMetalPull, teamMetalIncome, teamMetalExpense, teamMetalShare, teamMetalSent, teamMetalReceived, teamMetalExcess =
			...
			for i, otherTeamID in ipairs(teamList) do
				...
				if otherTeamID ~= teamID and spAreTeamsAllied(teamID, otherTeamID) and not isDead then
					local otherTeamMetalCurrentLevel, otherTeamMetalStorage, _, _, _, otherTeamMetalShare =
						spGetTeamResources(otherTeamID, "metal")
					local otherTeamEnergyCurrentLevel, otherTeamEnergyStorage, _, _, _, otherTeamEnergyShare =
						spGetTeamResources(otherTeamID, "energy")
					eShare = eShare
						+ math_max(
							0.0,
							(otherTeamEnergyStorage * math_min(0.99, otherTeamEnergyShare))
								- otherTeamEnergyCurrentLevel
						)
					mShare = mShare
						+ math_max(
							0.0,
							(otherTeamMetalStorage * math_min(0.99, otherTeamMetalShare)) - otherTeamMetalCurrentLevel
						)
				end
			end
