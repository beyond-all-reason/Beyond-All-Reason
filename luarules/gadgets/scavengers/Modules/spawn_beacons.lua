		...
		if BeaconSpawnChance == 0 or canSpawnBeaconHere == false then
			for i = 1,100 do
				...
				if canSpawnBeaconHere then
					canSpawnBeaconHere = posOccupied(posx, posy, posz, 80)
				end
				if canSpawnBeaconHere then
					if globalScore then
						...
						if scavengerGamePhase == "initial" then
							...
							elseif numOfSpawnBeacons < unitSpawnerModuleConfig.minimumspawnbeacons*0.2 then
								canSpawnBeaconHere = posLosCheckOnlyLOS(posx, posy, posz,80)
							elseif numOfSpawnBeacons < unitSpawnerModuleConfig.minimumspawnbeacons*0.4 then
								canSpawnBeaconHere = posLosCheckNoRadar(posx, posy, posz,80)
							else
								canSpawnBeaconHere = posLosCheck(posx, posy, posz,80)
							end
						end
				...
				if canSpawnBeaconHere then
					...
					if unitSpawnerModuleConfig.beacondefences == true and scavengerGamePhase ~= "initial" then
						...
						Spring.CreateUnit("scavengerdroppod_scav", posx-80, posy, posz, math_random(0,3),GaiaTeamID)
						Spring.CreateUnit("scavengerdroppod_scav", posx+80, posy, posz, math_random(0,3),GaiaTeamID)
						Spring.CreateUnit("scavengerdroppod_scav", posx, posy, posz+80, math_random(0,3),GaiaTeamID)
						Spring.CreateUnit("scavengerdroppod_scav", posx, posy, posz-80, math_random(0,3),GaiaTeamID)
