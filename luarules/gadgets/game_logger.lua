...
if gadgetHandler:IsSyncedCode() then
	...
	local spSendLuaRulesMsg = Engine.Unsynced.SendLuaRulesMsg
	...
	local mySpec, fullview = Engine.Unsynced.GetSpectatingState()

	function gadget:PlayerChanged(playerID)
		if playerID == myPlayerID then
			myTeamID = Spring.GetMyTeamID()
			mySpec, fullview = Engine.Unsynced.GetSpectatingState()
		end
	end
	...
	function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID, attackerDefID, attackerTeam, weaponDefID)
		...
		if (not mySpec and attackerTeam == myTeamID) and (unitTeam ~= attackerTeam) then
			...
			if (isEcoUnit[unitDefID] or isCommander[unitDefID]) and spAreTeamsAllied(unitTeam, attackerTeam) and isCommander[attackerDefID] then
				...
				local msg = string.format("l0g%s:friendlyfire:%d:%s:%d:%d:%d", validation, Engine.Shared.GetGameFrame(), "ud", unitTeam, attackerTeam, unitDefID)
				--Spring.Echo(msg)
	...
	function gadget:UnitLoaded(unitID, unitDefID, unitTeam, transportID, transportTeam)
		if not mySpec and transportTeam == myTeamID and unitTeam ~= transportTeam and isCommander[unitDefID] then
			local _, _, _, isAiTeam = Engine.Shared.GetTeamInfo(unitTeam, false)
			if isAiTeam then
				return
			end
			local msg = string.format("l0g%s:allycommloaded:%d:%s:%d:%d:%d", validation, Engine.Shared.GetGameFrame(), "ud", unitTeam, transportTeam, unitDefID)
			spSendLuaRulesMsg(msg)
		end
	end
end
