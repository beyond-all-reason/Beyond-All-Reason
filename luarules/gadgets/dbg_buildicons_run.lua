...
local unitConfigs =
	{ -- copy from configs/icon_generator.lua, included.. because else it will miss frames when animating
	...
	if nextFrame and n > nextFrame and index <= #unitnames then
		...
		if not string.find(unitnames[index], " ") then
			...
			if unitName then
				if
					unitConfigs
					and unitConfigs[UnitDefNames[unitName].id]
					and unitConfigs[UnitDefNames[unitName].id].wait
				then
