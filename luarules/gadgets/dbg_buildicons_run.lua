...
function buildslowly(_, _, params)
	nextFrame = Engine.Shared.GetGameFrame()
	if #params == 1 then
		if type(tonumber(params[1])) == "number" then
			index = tonumber(params[1])
		end
	end
	Engine.Shared.Echo("building icons all slow-like, starting from " .. index)
...
function buildanim(_, _, params)
	...
	if #params == 1 then
		...
		Engine.Shared.Echo("building icon with animation frames all slow-like, starting from " .. index)
...
function buildanimslowly(_, _, params)
	...
	Engine.Shared.Echo("building icons with animation frames all slow-like, starting from " .. index)
	...
	for unitName, _ in pairs(UnitDefNames) do
		...
		if not unitnumber or counter >= unitnumber then
			...
			if VFS.FileExists(filepath, VFS.RAW) then
				Engine.Shared.Echo("File exists for: " .. unitName .. " " .. filepath)
			else
				if not skipUnits[unitName] then
					Engine.Shared.Echo("Building filepath: " .. filepath)
					buildUnitAnim(unitName)
				end
			end
		end
...
function gadget:GameFrame(n)
	if nextFrame and n > nextFrame and index <= #unitnames then
		Engine.Unsynced.SendCommands("luarules buildicon " .. unitnames[index])
		index = index + 1
		counter = counter + 1
