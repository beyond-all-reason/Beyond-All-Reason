...
local data = {
	...
	map_minHeight = Engine.Shared.GetGroundExtremes(), -- no touchies
	map_maxHeight = select(2, Engine.Shared.GetGroundExtremes()), -- no touchies
	...
	wind_soundDelay = 60,
	wind_minHeight = select(2, Engine.Shared.GetGroundExtremes()) * 0.6,
	wind_maxHeight = select(2, Engine.Shared.GetGroundExtremes()),
...
function CollectGeoVentPositions()
	...
	for i = 1, #allFeatures do -- loop through all features on the map
		if FeatureDefs[Engine.Shared.GetFeatureDefID(allFeatures[i])].geoThermal then -- isGeoSpot
			local x, y, z = Engine.Shared.GetFeaturePosition(allFeatures[i])
			local allowGeo = true
...
function PlayGeoVentSound()
	...
	if Engine.Unsynced.IsSphereInView(randomposx, randomposy, randomposz) then
		Engine.Unsynced.PlaySoundFile("geoventshort", 0.25, randomposx, randomposy, randomposz, "sfx")
	end
...
function PlayWindSound(n) -- We want wind to play at volume depending on wind speed and height.
	...
	local randomposy = Engine.Shared.GetGroundHeight(randomposx, randomposz)
	if randomposy >= data.wind_minHeight and Engine.Unsynced.IsSphereInView(randomposx, randomposy + 400, randomposz) then
		local windSpeed = select(4, Engine.Shared.GetWind()) / 25
		if math.random() > 0.1 then
			Engine.Unsynced.PlaySoundFile(data.wind_soundBank[math.random(1, #data.wind_soundBank)], windSpeed * 0.6, randomposx, (randomposy + 400), randomposz, "sfx")
		else
			Engine.Unsynced.PlaySoundFile(data.wind_rareSoundBank[math.random(1, #data.wind_rareSoundBank)], windSpeed * 0.6, randomposx, (randomposy + 400), randomposz, "sfx")
		end
...
function PlaySeaSound(n) -- We want wind to play at volume depending on wind speed and height.
	...
	local randomposz = math.random(0, data.map_sizeZ)
	local randomposy = Engine.Shared.GetGroundHeight(randomposx, randomposz)
	if randomposy <= data.sea_minDepth and Engine.Unsynced.IsSphereInView(randomposx, randomposy + 400, randomposz) then
		local windSpeed = select(4, Engine.Shared.GetWind()) / 25
		if math.random() > 0.1 then
			Engine.Unsynced.PlaySoundFile(data.sea_soundBank[math.random(1, #data.sea_soundBank)], (0.10 + (windSpeed * 0.75)), randomposx, (randomposy + 400), randomposz, "sfx")
		else
			Engine.Unsynced.PlaySoundFile(data.sea_rareSoundBank[math.random(1, #data.sea_rareSoundBank)], (0.10 + (windSpeed * 0.75)), randomposx, (randomposy + 400), randomposz, "sfx")
		end
...
function PlayBeachSound(n) -- We want wind to play at volume depending on wind speed and height.
	...
	local randomposz = math.random(0, data.map_sizeZ)
	local randomposy = Engine.Shared.GetGroundHeight(randomposx, randomposz)
	if randomposy < 0 and randomposy >= data.beach_maxDepth and Engine.Unsynced.IsSphereInView(randomposx, randomposy, randomposz) then
		local windSpeed = select(4, Engine.Shared.GetWind()) / 25
		if math.random() > 0.1 then
			Engine.Unsynced.PlaySoundFile(data.beach_soundBank[math.random(1, #data.beach_soundBank)], (0.20 + (windSpeed * 0.6)), randomposx, (randomposy + 400), randomposz, "sfx")
		else
			Engine.Unsynced.PlaySoundFile(data.beach_rareSoundBank[math.random(1, #data.beach_rareSoundBank)], (0.20 + (windSpeed * 0.6)), randomposx, (randomposy + 400), randomposz, "sfx")
		end
