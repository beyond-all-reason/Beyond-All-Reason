...
function gadget:GameFrame(frame)
	if frame % 30 == 21 then
		for queenID, _ in pairs(aliveQueens) do
			local queenx, queeny, queenz = SpringShared.GetUnitPosition(queenID)
			local queenTeamID = SpringShared.GetUnitTeam(queenID)
			local surroundingUnits = CallAsTeam(queenTeamID, SpringShared.GetUnitsInSphere, queenx, queeny, queenz, 750, -4)
			for i = 1, #surroundingUnits do
				if aliveTargets[surroundingUnits[i]] then
					SpringSynced.GiveOrderToUnit(queenID, CMD.STOP, 0, 0)
					SpringSynced.GiveOrderToUnit(queenID, CMD.ATTACK, { surroundingUnits[i] }, 0)
					break
				end
			end
		end
