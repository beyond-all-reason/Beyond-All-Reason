...
function gadget:GameFrame(frame)
	if frame % 30 == 21 then
		for queenID, _ in pairs(aliveQueens) do
			local queenx, queeny, queenz = Engine.Shared.GetUnitPosition(queenID)
			local queenTeamID = Engine.Shared.GetUnitTeam(queenID)
			local surroundingUnits = CallAsTeam(queenTeamID, Engine.Shared.GetUnitsInSphere, queenx, queeny, queenz, 750, -4)
			for i = 1, #surroundingUnits do
				if aliveTargets[surroundingUnits[i]] then
					Engine.Shared.GiveOrderToUnit(queenID, CMD.STOP, 0, 0)
					Engine.Shared.GiveOrderToUnit(queenID, CMD.ATTACK, { surroundingUnits[i] }, 0)
					break
				end
			end
		end
