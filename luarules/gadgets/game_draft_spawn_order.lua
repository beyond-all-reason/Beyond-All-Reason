...
local gaiaAllyTeamID = select(6, spGetTeamInfo(Engine.Shared.GetGaiaTeamID(), false))
...
local function isAllyTeamSkillZero(allyTeamID)
	local tteams = Engine.Shared.GetTeamList(allyTeamID)
...
local function sendTeamOrder(teamOrder, allyTeamID_ready, log)
	...
	if log and not alone then
		Engine.Shared.Log(gadget:GetInfo().name, LOG.INFO, "Order [id:" .. allyTeamID_ready .. "]: " .. orderMsg)
	end
	Engine.Unsynced.SendLuaUIMsg("DraftOrderPlayersOrder " .. allyTeamID_ready .. " " .. orderIds)
end
...
local function SendDraftMessageToPlayer(allyTeamID, target_num)
	...
	Engine.Unsynced.SendLuaUIMsg("DraftOrderPlayerTurn " .. allyTeamID .. " " .. target_num) -- we send allyTeamID orderIndex
end
...
local function checkVotesAndAdvanceTurn(allyTeamID)
	...
	if votedPercentage >= VOTE_YES_PRCTN_REQ then
		if announceVoteResults then
			Engine.Shared.Echo("" .. votedPercentage .. "% (req: " .. VOTE_YES_PRCTN_REQ .. "%) of players on ally team " .. allyTeamID .. " have voted to skip current player turn.")
		end
...
local function PreInitDraftOrderData()
	local tteams = Engine.Shared.GetTeamList()
	for _, teamID in ipairs(tteams) do
...
local function InitDraftOrderData(allyTeamID_ready)
	...
	Engine.Unsynced.SendLuaUIMsg("DraftOrderAllyTeamJoined " .. allyTeamID_ready)
...
local function checkVotesAndStartDraft(allyTeamID)
	...
	if votedPercentage >= VOTE_YES_PRCTN_REQ then
		if announceVoteResults then
			Engine.Shared.Echo("" .. votedPercentage .. "% (req: " .. VOTE_YES_PRCTN_REQ .. "%) of players on ally team " .. allyTeamID .. " have voted to skip wait for late-joiners.")
		end
...
function draftModeInitialize()
	...
	if draftMode == "random" then -- Random draft
		Engine.Unsynced.SendLuaUIMsg("DraftOrder_Random") -- Discord: https://discord.com/channels/549281623154229250/1163844303735500860
	elseif draftMode == "captain" then -- Fair mod
		Engine.Unsynced.SendLuaUIMsg("DraftOrder_Captain") -- Discord: https://discord.com/channels/549281623154229250/1163844303735500860
	elseif draftMode == "skill" then -- Skill-based placement order
		Engine.Unsynced.SendLuaUIMsg("DraftOrder_Skill") -- Discord: https://discord.com/channels/549281623154229250/1134886429361713252
	elseif draftMode == "fair" then -- Fair mod
		Engine.Unsynced.SendLuaUIMsg("DraftOrder_Fair") -- Discord: https://discord.com/channels/549281623154229250/1123310748236529715
	end
end
...
function DraftRecvLuaMsg(msg, playerID, playerIsSpec, playerTeam, allyTeamID)
	...
	if msg == "i_have_joined_fair" then
		...
		if allyTeamIsInGame[allyTeamID] ~= true then
			...
			local teamList = Engine.Shared.GetTeamList(allyTeamID)
			local count = 0
			local tcount = 0
			...
			if not waitIsMandatory and tcount >= VOTE_QUORUM and count > 0 and tcount > 0 and (((count / tcount) * 100) >= VOTE_YES_PRCTN_REQ) then
				Engine.Unsynced.SendLuaUIMsg("DraftOrderShowCountdown " .. allyTeamID)
			end
		end
	elseif msg == "send_me_the_info_again" then -- someone luaui /reload'ed, send them the queue and index again
		if allyTeamIsInGame[allyTeamID] then
			Engine.Unsynced.SendLuaUIMsg("DraftOrderAllyTeamJoined " .. allyTeamID)
