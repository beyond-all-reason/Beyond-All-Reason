		...
		if announceVoteResults then
			Spring.Echo(
				""
					...
					.. "% (req: "
					.. VOTE_YES_PRCTN_REQ
					.. "%) of players on ally team "
					.. allyTeamID
					.. " have voted to skip current player turn."
			)
		end
	...
	if draftMode == "random" or draftMode == "captain" then
		...
		if draftMode == "captain" and (#allyTeamSpawnOrder[allyTeamID_ready] > 1) then
			...
			if captainTeamIDindex and highestSkill > 0 and captainTeamIDindex ~= 1 then
				allyTeamSpawnOrder[allyTeamID_ready][1], allyTeamSpawnOrder[allyTeamID_ready][captainTeamIDindex] =
					allyTeamSpawnOrder[allyTeamID_ready][captainTeamIDindex], allyTeamSpawnOrder[allyTeamID_ready][1]
			end
		end
	...
	elseif draftMode == "skill" then
		...
		if isAllyTeamSkillZero(allyTeamID_ready) then
			...
			for i = 1, #allyTeamSpawnOrder[allyTeamID_ready] - 1 do
				...
				if
					GetSkillByTeam(allyTeamSpawnOrder[allyTeamID_ready][i])
					== GetSkillByTeam(allyTeamSpawnOrder[allyTeamID_ready][i + 1])
				then
		...
		if announceVoteResults then
			Spring.Echo(
				""
					...
					.. "%) of players on ally team "
					.. allyTeamID
					.. " have voted to skip wait for late-joiners."
			)
		end
...
function DraftRecvLuaMsg(msg, playerID, playerIsSpec, playerTeam, allyTeamID)
	if
		not waitIsMandatory
		and canVoteForceStartDraft
		and msg == "vote_wait_too_long"
		and votedToForceStartDraft[allyTeamID][playerTeam] ~= nil
	then
	...
	if draftMode == "skill" or draftMode == "random" or draftMode == "captain" then
		if allyTeamSpawnOrderPlaced[allyTeamID] and allyTeamSpawnOrderPlaced[allyTeamID] > 0 then
			...
			elseif
				canVoteSkipTurn
				and msg == "vote_skip_turn"
				and votedToForceSkipTurn[allyTeamID][playerTeam] ~= nil
			then
	...
	if msg == "i_have_joined_fair" then
		...
		if allyTeamIsInGame[allyTeamID] ~= true then
			...
			if
				not waitIsMandatory
				and tcount >= VOTE_QUORUM
				and count > 0
				and tcount > 0
				and (((count / tcount) * 100) >= VOTE_YES_PRCTN_REQ)
			then
