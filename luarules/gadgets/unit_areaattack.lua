...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:GameFrame(f)
		for i, o in pairs(attackList) do
			...
			if o.radius > 0 then
				...
				Engine.Shared.GiveOrderToUnit(o.unit, CMD.INSERT, { 0, CMD.ATTACK, 0, o.x + math_cos(phase) * amp, o.y, o.z + math_sin(phase) * amp }, { "alt" })
			end
		end
		for i, o in pairs(closeList) do
			closeList[i] = nil
			Engine.Synced.SetUnitMoveGoal(o.unit, o.x, o.y, o.z, o.radius)
		end
	end
	...
	function gadget:CommandFallback(u, ud, team, cmd, param, opt)
		if cmd == CMD_AREA_ATTACK_GROUND then
			local x, _, z = Engine.Shared.GetUnitPosition(u)
			if not x then
				return true, true
			end
	...
	function gadget:UnitCreated(u, ud, team)
		if canAreaAttack[ud] then
			Engine.Synced.InsertUnitCmdDesc(u, aadesc)
		end
	end
	...
	function gadget:Initialize()
		Engine.Unsynced.SetCustomCommandDrawData(CMD_AREA_ATTACK_GROUND, CMDTYPE.ICON_UNIT_OR_AREA, { 1, 0, 0, 0.8 }, true)
	end
end
