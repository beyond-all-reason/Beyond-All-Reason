	...
	for _, weapon in pairs(unitDef.weapons) do
		...
		if clusterDefName then
			...
			if clusterCount < minSpawnNumber or clusterCount > maxSpawnNumber then
				SpringShared.Log(gadget:GetInfo().name, LOG.WARNING, weaponDef.name .. ": cluster_count of " .. clusterCount .. ", clamping to " .. minSpawnNumber .. "-" .. maxSpawnNumber)
			...
			if clusterDef then
				if spawnableTypes[clusterDef.type] then
					...
					SpringShared.Log(gadget:GetInfo().name, LOG.ERROR, "Invalid weapon spawn type: " .. clusterDef.type)
				end
			else
				SpringShared.Log(gadget:GetInfo().name, LOG.ERROR, "Could not find weapon def matching cluster_def: " .. clusterDefName)
			end
		end
	end
end
...
for weaponDefID in pairs(removeIDs) do
	SpringShared.Log(gadget:GetInfo().name, LOG.ERROR, "Preventing nested explosions: " .. WeaponDefs[weaponDefID].name)
...
function gadget:Initialize()
	if not next(clusterWeaponDefs) then
		SpringShared.Log(gadget:GetInfo().name, LOG.INFO, "Removing gadget. No weapons found.")
	...
	if not GG.Shields then
		SpringShared.Log("ScriptedWeapons", LOG.ERROR, "Shields API unavailable (cluster)")
		return
	end
