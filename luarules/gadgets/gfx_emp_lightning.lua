...
local function spawnImpactCluster(
	spawn,
	px,
	py,
	pz,
...
local function scheduleTailPulses(
	nowFrame,
	px,
	py,
	pz,
	...
	for i = 1, config.tailPulseCount do
		...
		local pulseIntensity =
			mathMax(config.tailMinIntensityScale, intensityScale * (config.tailIntensityDecay ^ decayPow))
		local pulseBursts =
			mathMax(config.tailMinBursts, mathFloor(burstCount * (config.tailBurstDecay ^ decayPow) + 0.5))
		local pulseScatter = scatterRadiusScale * (config.tailScatterDecay ^ decayPow)

		local interval = config.tailIntervalFrames
			+ mathFloor((mathRandom() * 2.0 - 1.0) * config.tailIntervalJitterFrames)
		if interval < 1 then
			interval = 1
		end
