  ...
  local GetGroundHeight = Spring.GetGroundHeight
	...
	function gadget:FeaturePreDamaged(featureID, featureDefID, featureTeam, Damage, weaponDefID, projectileID, attackerID, attackerDefID, attackerTeam)
		...
		if fx ~= nil then
			...
			--Begin Hornet addition
			--was tree touched by a com spawn blast?


			--WeaponDefNames['com_spawn_blast'].id
			Spring.Echo('Hornet poi tree touched')
			Spring.Echo(attackerDefID)
			Spring.Echo(WeaponDefNames['com_spawn_blast'].id)
			...
			if treeScaleY[featureDefID] then
				...
				ddissapearSpeed = 0.15 + Spring.GetFeatureHeight(featureID) / math_random(3700, 4700)
			end
			ppx, ppy, ppz = Spring.GetUnitPosition(attackerID)
			...
			treesdying[featureID] = {
				...
				strength = 1,--math.max(1, treeMass[featureDefID] / dmg),
				fire = fire,
				size = size,
				dissapearSpeed = ddissapearSpeed,
				destroyFrame = GetGameFrame() + falltime + 150 + (dissapearSpeed * 4000)
			}
