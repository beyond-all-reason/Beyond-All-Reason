...
if  (gadgetHandler:IsSyncedCode()) then
	local treefireExplosion = {
		tiny = {
			...
			hitUnit = 1,
			hitFeature = 1,
			craterAreaOfEffect = 40,
			damageAreaOfEffect = 40,
			edgeEffectiveness = 1,
			explosionSpeed = 1,
			impactOnly = false,
			ignoreOwner = false,
		...
		small = {
			...
			hitUnit = 1,
			hitFeature = 1,
			craterAreaOfEffect = 52,
			damageAreaOfEffect = 52,
			edgeEffectiveness = 1,
			explosionSpeed = 1,
			impactOnly = false,
			ignoreOwner = false,
        ...
        for featureID, featureinfo in pairs(treesdying) do
			if not GetFeaturePosition(featureID) then
				...
				if featureinfo.fire then
					fireFrequency = math.floor(2 + ((gf - featureinfo.frame) / 40))
				end
				...
				if featureinfo.frame + thisfeaturefalltime > gf then
					...
					if fy ~= nil then
						if featureinfo.fire then
							if gf%fireFrequency == fireFrequency-2 then
