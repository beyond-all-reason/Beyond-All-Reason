		...
		for weaponNumber, weaponData in pairs(weapons) do
			...
			if WeaponDefs[weaponDefID].customParams.smart_preferred_weapon then
				...
				if def.speed and def.speed ~= 0 then
					...
					Spring.Echo(def.name, smartWeaponDefs[unitDefID].canMove)
				end
			end
...
local function failureToFireCheck(attackerID, attackerData, defData)
    if attackerData.preferredReloadFrame < gameFrame - defData.failedToFireFrameThreshold then
        attackerData.preferredReloadFrame = math.max(
            Spring.GetUnitWeaponState(attackerID, defData.preferredWeapon, 'reloadFrame'),
            Spring.GetUnitWeaponState(attackerID, defData.deferredWeapon, 'reloadFrame')
        )
    end
...
local function updateAimingState(attackerID)
    ...
    elseif pTargetType == 2 then
        preferredCanShoot = spGetUnitWeaponHaveFreeLineOfFire(attackerID, defData.preferredWeapon, _, _, _, pTarget[1], pTarget[2], pTarget[3])
    end

    local failureToFire = failureToFireCheck(attackerID, attackerData, defData)

    if pIsUserTarget and preferredCanShoot then
        if failureToFire then
            attackerData.errorTallyMultiplier = math.min(attackerData.errorTallyMultiplier + 1, errorTallyMultiplierCap)
    ...
    Spring.Echo(attackerData.errorTallyMultiplier, attackerData.aggroBias)
end
