...
local CommandsToCatchMap =
	{ -- CMDTYPES: ICON_MAP, ICON_AREA, ICON_UNIT_OR_MAP, ICON_UNIT_OR_AREA, ICON_UNIT_FEATURE_OR_AREA, ICON_BUILDING
		[CMD.MOVE] = true,
		[CMD.PATROL] = true,
		[CMD.FIGHT] = true,
		[CMD.ATTACK] = true,
		...
		if isNoRushRestricted() and not TeamIDsToExclude[unitTeam] then
			...
			elseif CommandsToCatchMap[cmdID] and #cmdParams >= 3 then
				...
				if cmdParams[4] and not cmdParams[6] then -- might be map pos with radius, check radius range too
					if
						not RushStartboxCheck(cmdParams[1] + cmdParams[4], cmdParams[2], cmdParams[3], allyTeamID)
						or not RushStartboxCheck(cmdParams[1] - cmdParams[4], cmdParams[2], cmdParams[3], allyTeamID)
						or not RushStartboxCheck(cmdParams[1], cmdParams[2], cmdParams[3] + cmdParams[4], allyTeamID)
						or not RushStartboxCheck(cmdParams[1], cmdParams[2], cmdParams[3] - cmdParams[4], allyTeamID)
					then
						allowed = false
					end
