...
function gadget:GetInfo()
	return {
		...
		layer = -100,
		enabled = Engine.Shared.GetModOptions().norushtimer > 0,
	}
end

-- Get Startbox Area of every player
local positionCheckLibrary = VFS.Include("luarules/utilities/damgam_lib/position_checks.lua")
local norushtimer = Engine.Shared.GetModOptions().norushtimer * 1800 -- modoption is stating minutes, and we need frames. 60 seconds * 30 frames = 1800
...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:AllowCommand(unitID, unitDefID, unitTeam, cmdID, cmdParams, cmdOptions, cmdTag, synced)
		...
		if frame < norushtimer and not TeamIDsToExclude[unitTeam] then
			local _, _, _, _, _, allyTeamID = Engine.Shared.GetTeamInfo(unitTeam, false)
			...
			elseif CommandsToCatchUnit[cmdID] and #cmdParams == 1 then
				...
				if Engine.Shared.GetUnitDefID(targetUnitID) then
					local x, y, z = Engine.Shared.GetUnitPosition(targetUnitID)
					if not positionCheckLibrary.StartboxCheck(x, y, z, allyTeamID) then
						allowed = false
					end
				elseif Engine.Shared.GetFeatureDefID(targetUnitID - Game.maxUnits) and CommandsToCatchFeature[cmdID] then -- maybe it's a feature that we want to reclaim?
					local x, y, z = Engine.Shared.GetFeaturePosition(targetUnitID - Game.maxUnits)
					if not positionCheckLibrary.StartboxCheck(x, y, z, allyTeamID) then
						allowed = false
					end
				end
			elseif CommandsToCatchFeature[cmdID] and #cmdParams == 1 then
				...
				if Engine.Shared.GetFeatureDefID(targetFeatureID - Game.maxUnits) then
					local x, y, z = Engine.Shared.GetFeaturePosition(targetFeatureID - Game.maxUnits)
					if not positionCheckLibrary.StartboxCheck(x, y, z, allyTeamID) then
						allowed = false
					end
				end
