...
function gadget:ProjectileCreated(proID, proOwnerID, weaponDefID)
	if missileWeapons[weaponDefID] then
		missiles[proID] = { Engine.Shared.GetGameFrame() - 4 + Engine.Shared.GetProjectileTimeToLive(proID), missileWeapons[weaponDefID] }
	end
end
	...
	for proID, missile in pairs(missiles) do
		if gf > missile[1] then
			...
			if y and y > 0 then
				...
				Engine.Synced.SpawnCEG(missile[2], x, y, z, dirX, dirY, dirZ)
			else
