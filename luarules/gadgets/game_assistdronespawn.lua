if not (SpringShared.GetModOptions().assistdronesenabled == "enabled" or (SpringShared.GetModOptions().assistdronesenabled == "pve_only" and Spring.Utilities.Gametype.IsPvE())) then
	return
end
...
function SpawnAssistDrone(unitID, unitDefID, unitTeam)
	...
	if CountItemsInArray(teamIDDroneList[unitTeam]) < droneCount then
		...
		if droneID then
			...
			SpringShared.GiveOrderToUnit(droneID, CMD.GUARD, unitID, {})
