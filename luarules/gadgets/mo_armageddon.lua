...
function gadget:UnitCreated(unitID, unitDefID, teamID, builderID)
	...
	if unitDefID == meteorDefID then
		...
		local m = gf + 3 * 30 + 2
		if not toKill[m] then
			toKill[m] = {}
		end
		local k = #(toKill[gf + 3 * 30 + 2]) + 1
		toKill[gf + 3 * 30 + 2][k] = unitID --just to make sure, although its own explosion should kill it on the frame before
