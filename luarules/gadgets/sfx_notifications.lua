...
local function GetAllyTeamID(teamID)
	return select(6, Engine.Shared.GetTeamInfo(teamID, false))
end
...
if gadgetHandler:IsSyncedCode() then
	...
	function gadget:UnitGiven(unitID, unitDefID, newTeam, oldTeam)
		if not _G.transferredUnits or not _G.transferredUnits[unitID] then -- exclude upgraded units (t2 mex/geo) because allied players could have done this
			local players = Engine.Shared.GetPlayerList(newTeam)
	...
	function gadget:ProjectileCreated(proID, proOwnerID, weaponDefID)
		if nukeWeapons[Engine.Shared.GetProjectileDefID(proID)] then
			local ownerTeamID = Engine.Shared.GetUnitTeam(proOwnerID)
	...
	local enableLastcomNotif = (Engine.Shared.GetModOptions().deathmode == "com")
	...
	local myAllyTeamList = Engine.Shared.GetTeamList(myAllyTeamID)
	...
	function gadget:UnitDamaged(unitID, unitDefID, unitTeam, damage, paralyzer, weaponDefID, projectileID, attackerID, attackerDefID, attackerTeam)
		...
		if isCommander[unitDefID] then
			commanderLastDamaged[unitID] = Engine.Shared.GetGameFrame()
		end
	...
	function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID, attackerDefID, attackerTeam, weaponDefID)
		...
		if isCommander[unitDefID] and not select(3, Engine.Shared.GetTeamInfo(unitTeam)) and not Engine.Shared.GetUnitRulesParam(unitID, "muteDestructionNotification") then
			local myComCount = 0
			...
			for _, teamID in ipairs(myAllyTeamList) do
				...
				for unitDefID, _ in pairs(isCommander) do
					local comCount = Engine.Shared.GetTeamUnitDefCount(teamID, unitDefID)
			...
			if (not isSpec) and (unitTeam ~= myTeamID) then
				if numTeams > 1 and not playingAsHorde then
					local players = PlayersInAllyTeamID(GetAllyTeamID(Engine.Shared.GetUnitTeam(unitID)))
					for ct, player in pairs(players) do
						if tostring(player) then
							--if not unitInView then
							if Engine.Shared.GetUnitRulesParam(unitID, "unit_evolved") then
							elseif not attackerTeam and select(6, Engine.Shared.GetTeamInfo(unitTeam, false)) == myAllyTeamID and (not commanderLastDamaged[unitID] or commanderLastDamaged[unitID] + 150 < Engine.Shared.GetGameFrame()) then
				...
				--if not unitInView then
				local players = AllButAllyTeamID(GetAllyTeamID(Engine.Shared.GetUnitTeam(unitID)))
				for ct, player in pairs(players) do
					if tostring(player) and not Engine.Shared.GetUnitRulesParam(unitID, "unit_evolved") then
				...
				for ct, player in pairs(players) do
					if tostring(player) then
						if Engine.Shared.GetUnitRulesParam(unitID, "unit_evolved") then
						elseif not attackerTeam and (not commanderLastDamaged[unitID] or commanderLastDamaged[unitID] + 150 < Engine.Shared.GetGameFrame()) then
