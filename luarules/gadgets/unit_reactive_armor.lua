...
local callFromLus = Engine.Synced.UnitScript.CallAsUnit
...
local function checkReactiveArmor(unitID, unitDefID, params)
	local hasMethod
	local lusEnv = Engine.Synced.UnitScript.GetScriptEnv(unitID)
	...
	if lusEnv then
		...
		hasMethod = function(name)
			return Engine.Synced.GetCOBScriptID(unitID, name) ~= nil
		end
	end
	...
	if next(missing) then
		...
		Engine.Shared.Log("Reactive Armor", LOG.ERROR, ("Unit script missing %s: %s"):format(table.concat(missing, ", "), UnitDefs[unitDefID].name))
		success = false
	end
	...
	if (params.pieces - 1) * gameSpeed > params.frames then
		...
		Engine.Shared.Log("Reactive Armor", LOG.ERROR, ("Too many armor pieces (%d) for restore time: "):format(params.pieces, UnitDefs[unitDefID].name))
		success = false
	end
...
local function getUnitDebugInfo(unitID)
	...
	return {
		...
		health = Engine.Shared.GetUnitHealth(unitID),
...
local function showDebugInfo(unitID)
	...
	if info.unitCountdown then
		...
		Engine.Unsynced.MarkerAddPoint(info.x, info.y, info.z, display)
		Engine.Shared.Echo("Reactive Armor", info)
	end
end
...
function gadget:Initialize()
	...
	callFromLus = Engine.Synced.UnitScript.CallAsUnit
	...
	for _, unitID in ipairs(Engine.Shared.GetAllUnits()) do
		...
		if armoredUnitDefs[uDefID] then
			reloadUnitState(unitID, uDefID, Engine.Shared.GetUnitTeam(unitID))
		end
	end
end
