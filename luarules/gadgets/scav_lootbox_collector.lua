...
for unitDefName, tier in pairs({
	lootboxbronze_scav = 1,
	lootboxsilver_scav = 1,
	lootboxgold_scav = 2,
	lootboxplatinum_scav = 2,
}) do
	if UnitDefNames[unitDefName] then
...
function gadget:GameFrame(frame)
	if
		frame % 30 == 12
		and Spring.GetGameRulesParam("scavBossAnger") >= 1
		and Spring.GetGameRulesParam("scavTechAnger") >= config.airStartAnger
	then
					...
					if lootboxPosX and not GG.IsPosInRaptorScum(lootboxPosX, lootboxPosY, lootboxPosZ) then
						if math.random(0, aliveLootboxesCount) == 0 and not handledLootboxesList[lootboxID] then
							for transportDefID, transportTier in pairs(transportsList) do
								if
									math.random(0, SetCount(transportsList)) == 0
									and transportTier == lootboxTier
									and not handledLootboxesList[lootboxID]
								then
									for spawnerID, _ in pairs(aliveSpawners) do
										if
											math.random(0, SetCount(aliveSpawners)) == 0
											and not handledLootboxesList[lootboxID]
										then
											targetLootboxID = lootboxID
											local spawnerPosX, spawnerPosY, spawnerPosZ =
												Spring.GetUnitPosition(spawnerID)
											for j = 1, 5 do
												if math.random() <= config.spawnChance then
													local transportID = Spring.CreateUnit(
														transportDefID,
														spawnerPosX + math.random(-1024, 1024),
														spawnerPosY + 100,
														spawnerPosZ + math.random(-1024, 1024),
														math.random(0, 3),
														scavTeamID
													)
													if transportID then
														...
														for i = 1, 100 do
															...
															local randomY =
																math.max(0, Spring.GetGroundHeight(randomX, randomZ))
															if GG.IsPosInRaptorScum(randomX, randomY, randomZ) then
																Spring.GiveOrderToUnit(
																	transportID,
																	CMD.UNLOAD_UNITS,
																	{ randomX, randomY, randomZ, 1024 },
																	{ "shift" }
																)
															end
															if i == 100 then
