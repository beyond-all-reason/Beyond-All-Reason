...
if (not gadgetHandler:IsSyncedCode()) or (not Spring.Utilities.Gametype.IsScavengers()) or SpringShared.GetModOptions().unit_restrictions_noair then
	return false
											...
											for j = 1, 5 do
												if math.random() <= config.spawnChance then
													...
													if transportID then
														...
														SpringShared.GiveOrderToUnit(transportID, CMD.LOAD_UNITS, { targetLootboxID }, { "shift" })
														for i = 1, 100 do
															...
															if GG.IsPosInRaptorScum(randomX, randomY, randomZ) then
																SpringShared.GiveOrderToUnit(transportID, CMD.UNLOAD_UNITS, { randomX, randomY, randomZ, 1024 }, { "shift" })
															end
															if i == 100 then
																SpringShared.GiveOrderToUnit(transportID, CMD.MOVE, { randomX + math.random(-256, 256), randomY, randomZ + math.random(-256, 256) }, { "shift" })
															end
														end
													end
												end
