...
function gadget:UnitCreated(unitID, unitDefID)
	if unitRspeed[unitDefID] then
		...
		Engine.Synced.MoveCtrl.SetGroundMoveTypeData(unitID, "maxSpeed", unitSpeed[unitDefID])
		Engine.Synced.MoveCtrl.SetGroundMoveTypeData(unitID, "maxReverseSpeed", 0)
	end
end
...
function gadget:GameFrame(f)
	for unitID, unitDefID in pairs(refreshList) do
		...
		if cmd and cmd[1] and cmd[1]["options"] and cmd[1]["options"].ctrl then
			Engine.Synced.MoveCtrl.SetGroundMoveTypeData(unitID, "maxSpeed", unitRspeed[unitDefID])
			Engine.Synced.MoveCtrl.SetGroundMoveTypeData(unitID, "maxReverseSpeed", unitRspeed[unitDefID])
		else
			Engine.Synced.MoveCtrl.SetGroundMoveTypeData(unitID, "maxSpeed", unitSpeed[unitDefID])
			Engine.Synced.MoveCtrl.SetGroundMoveTypeData(unitID, "maxReverseSpeed", 0)
		end
