			...
			if #blueprints > 0 then
				...
				for i = 1, #blueprints do
					...
					Spring.MarkerAddPoint(
						blueprintpositions[i].posx,
						Spring.GetGroundHeight(blueprintpositions[i].posx, blueprintpositions[i].posz),
						blueprintpositions[i].posz,
						"#" .. i
					)
				end
	...
	function RequestScavTest(cmd, line, words, playerID)
		if words and #words == 1 then
			...
			Spring.SendMessageToPlayer(
				playerID,
				"Please specify which file you want to spawn (Enter the filename only without .lua)"
			)
		end
	end
end
