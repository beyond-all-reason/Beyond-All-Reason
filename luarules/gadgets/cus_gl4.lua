...
local autoReload = { enabled = false, vssrc = "", fssrc = "", lastUpdate = Engine.Unsynced.GetTimer(), updateRate = 0.5 }
...
local function GetUniformBinID(objectDefID, reason)
	if objectDefID and objectDefToUniformBin[objectDefID] then
		...
		if debugmode then
			Engine.Shared.Echo("Failed to find a uniform bin id for objectDefID", objectDefID, reason)
		end
		return "otherunit"
	end
end
...
local function updateFeatureFadeDistances()
	local drawDist = Engine.Unsynced.GetConfigInt("FeatureDrawDistance", 10000)
	local fadeDist = Engine.Unsynced.GetConfigInt("FeatureFadeDistance", math.floor(drawDist * 0.8))
...
local function initMaterials()
	...
	if itsXmas then
		Engine.Shared.Echo("CUS GL4 enabled XMAS mode")
	end
...
local function dumpShaderCodeToInfolog(defs, src, filename) -- no IO in unsynced gadgets :/
	Engine.Shared.Echo(filename)
	Engine.Shared.Echo(defs)
	Engine.Shared.Echo(src)
end
...
local function CompileLuaShader(shader, definitions, plugIns, addName, recompilation)
	...
	if definitions == nil or definitions == {} then
		Engine.Shared.Echo(addName, "nul definitions", definitions)
	end
	...
	if compilationResult ~= true then
		Engine.Shared.Echo("Custom Unit Shaders. " .. addName .. " shader compilation failed")
...
local function initBinsAndTextures()
	...
	Engine.Shared.Echo("[CUS GL4] Init Feature bins")
	...
	Engine.Shared.Echo("[CUS GL4] Init Unit bins")
...
local function PreloadTextures()
	Engine.Shared.Echo("[CUS GL4] Cache Textures")
...
local function GetObjectDefName(objectID)
	...
	elseif objectID >= 0 then
		if Engine.Shared.ValidUnitID(objectID) then
			local udid = Engine.Shared.GetUnitDefID(objectID)
			return UnitDefs[udid].name
		else
			return "Invalid UnitID:" .. tostring(objectID)
		end
	else
		if Engine.Shared.ValidFeatureID(-1 * objectID) then
			local fdid = Engine.Shared.GetFeatureDefID(-1 * objectID)
...
local function AssignObjectToBin(objectID, objectDefID, flag, shader, textures, texKey, uniformBinID, calledfrom)
	...
	if objectID >= 0 then
		if (not Engine.Shared.ValidUnitID(objectID)) or (Engine.Shared.GetUnitIsDead(objectID) == true) then
			return
		end
	else
		if not Engine.Shared.ValidFeatureID(-objectID) then
			return
		end
	end
	...
	if objectDefID == nil then
		Engine.Shared.Echo("AssignObjectToBin", objectID, objectDefID, flag, shader, textures, texKey, uniformBinID, calledfrom)
	end
	...
	if texKey == nil or uniformBinID == nil then
		if badassigns[objectID] == nil then
			Engine.Shared.Echo("[CUS GL4]Failure to assign to ", objectID, objectDefID, flag, shader, textures, texKey, uniformBinID, calledfrom)
			Engine.Shared.Echo("REPORT THIS TO BEHERITH: bad object:", GetObjectDefName(objectID))
	...
	if unitDrawBinsFlagShaderUniforms[texKey] == nil then
		local t0 = Engine.Unsynced.GetTimerMicros()
		local mybinVAO = gl.GetVAO()
		local mybinIBO = gl.GetVBO(GL.ARRAY_BUFFER, true)

		if (mybinIBO == nil) or (mybinVAO == nil) then
			Engine.Shared.Echo("Failed to allocate IBO or VAO for CUS GL4", mybinIBO, mybinVAO)
			--Spring.Debug.TraceFullEcho()
			gadgetHandler:RemoveGadget()
			return
		end
		...
		if debugmode then
			Engine.Shared.Echo("Init of bin", shader, flag, texKey, uniformBinID, "took", Engine.Unsynced.DiffTimers(Engine.Unsynced.GetTimerMicros(), t0, nil), "ms")
		end
	end
	...
	if unitDrawBinsFlagShaderUniformsTexKey.objectsIndex[objectID] then
		Engine.Shared.Echo("Trying to add a unit to a bin that it is already in!")
	else
		if debugmode then
			Engine.Shared.Echo("AssignObjectToBin success:", objectID, objectDefID, flag, shader, texKey, uniformBinID)
		end
	end
	...
	if numobjects + 1 >= maxElements then
		...
		if objectID >= 0 then -- this tells us if we are gonna be using features or units
			for i, unitID in ipairs(objectsArray) do
				if Engine.Shared.ValidUnitID(unitID) == true and Engine.Shared.GetUnitIsDead(unitID) ~= true then
			...
			for i, featureID in ipairs(objectsArray) do
				if Engine.Shared.ValidFeatureID(-featureID) then
					newObjectsCount = newObjectsCount + 1
	...
	if debugmode and flag == 0 then
		Engine.Shared.Echo("AssignObjectToBin", objectID, objectDefID, texKey, uniformBinID, shader, flag, numobjects)
		...
		Engine.Shared.Echo(objids)
	end
end

local spGetUnitDefID = Engine.Shared.GetUnitDefID
local spValidUnitID = Engine.Shared.ValidUnitID
local spSetUnitEngineDrawMask = Engine.Unsynced.SetUnitEngineDrawMask
local spGetFeatureDefID = Engine.Shared.GetFeatureDefID
local spValidFeatureID = Engine.Shared.ValidFeatureID
local spSetFeatureEngineDrawMask = Engine.Unsynced.SetFeatureEngineDrawMask
local spSetFeatureNoDraw = Engine.Unsynced.SetFeatureNoDraw
local spSetFeatureFade = Engine.Unsynced.SetFeatureFade
...
local function AddObject(objectID, drawFlag, reason)
	...
	if debugmode then
		Engine.Shared.Echo("AddObject", objectID, objectDefID, drawFlag, reason)
	end
	if objectDefID == nil then
		return
	...
	if objectID >= 0 then
		...
		if spValidFeatureID(-1 * objectID) == false then
			Engine.Shared.Echo("Invalid feature for drawmask", objectID, objectDefID)
		end
...
local function RemoveObjectFromBin(objectID, objectDefID, texKey, shader, flag, uniformBinID, reason)
	...
	texKey = texKey or fastObjectDefIDtoTextureKey[objectDefID]
	if debugmode then
		Engine.Shared.Echo("RemoveObjectFromBin", objectID, objectDefID, texKey, shader, flag, uniformBinID, reason)
	end
		...
		if unitDrawBins[flag][shader][uniformBinID] then
			if unitDrawBins[flag][shader][uniformBinID][texKey] then
				...
				if objectIndex == numobjects then -- last element
					...
					if debugmode and flag == 0 then
						Engine.Shared.Echo("Moving", objectIDatEnd, "from", numobjects, " to", objectIndex, "while removing", objectID)
					end
					...
					if objectID >= 0 then -- unit
						if Engine.Shared.ValidUnitID(objectIDatEnd) == true and Engine.Shared.GetUnitIsDead(objectIDatEnd) ~= true then
							unitDrawBinsFlagShaderTexKey.IBO:InstanceDataFromUnitIDs(objectIDatEnd, objectTypeAttribID, objectIndex - 1)
						else
							if debugmode then
								Engine.Shared.Echo("Tried to remove invalid unitID", objectIDatEnd, "while removing", objectID)
							end
						end
					else -- feauture
						if Engine.Shared.ValidFeatureID(-objectIDatEnd) == true then
							unitDrawBinsFlagShaderTexKey.IBO:InstanceDataFromFeatureIDs(-1 * objectIDatEnd, objectTypeAttribID, objectIndex - 1)
						else
							if debugmode then
								Engine.Shared.Echo("Tried to remove invalid featureID", -objectIDatEnd, "while removing", -objectID)
							end
						end
					end
			...
			if debugmode then
				Engine.Shared.Echo("Failed to find uniformBinID for", objectID, objectDefID, texKey, shader, flag, uniformBinID)
			end
		end
	else
		if debugmode then
			...
			Engine.Shared.Echo("Failed to find shader for", objectID, objectDefID, texKey, shader, flag, uniformBinID, defName)
		end
...
local function UpdateObject(objectID, drawFlag, reason)
	if debugmode then
		Engine.Shared.Echo("UpdateObject", objectID, drawFlag, reason)
	end
	if drawFlag >= 128 then --icon
		return
	end
...
local function RemoveObject(objectID, reason) -- we get pos/neg objectID here
	...
	if debugmode then
		Engine.Shared.Echo("RemoveObject", objectID, reason)
	end
	local objectDefID = objectIDtoDefID[objectID]
	if objectDefID == nil then
		return
	...
	for k = 1, drawBinKeysLen do --drawBinKeys = {1, 1 + 4, 16}
		...
		if debugmode then
			Engine.Shared.Echo("RemoveObject Flags", objectID, flag, overrideDrawFlagsCombined[flag])
		end
		if overrideDrawFlagsCombined[flag] then
...
local function ProcessUnits(units, drawFlags, reason)
	...
	for i = 1, numUnits do
		...
		if debugmode then
			Engine.Shared.Echo("ProcessUnits", unitID, drawFlag, reason)
		end
...
local function RecompileShaders(recompilation)
	...
	Engine.Shared.Echo("[CUS GL4] Compiling Shaders")
...
local function initGL4()
	...
	if Platform.glHaveGL4 ~= true then
		Engine.Shared.Echo("[CUS GL4] No GL4 support for this gpu as indicated by Platform.glHaveGL4, disabling.")
		return
	end
	...
	Engine.Shared.Echo("[CUS GL4] Initializing materials")
	...
	if modelsVertexVBO == nil or modelsIndexVBO == nil then
		Engine.Shared.Echo("CUS GL4 failed to initialize VBO, exiting")
		gadgetHandler:RemoveGadget()
		return
	end
	...
	Engine.Shared.Echo("[CUS GL4] Initializing bins")

	initBinsAndTextures()

	Engine.Shared.Echo("[CUS GL4] Collecting units")
	Engine.Unsynced.ClearUnitsPreviousDrawFlag()
	Engine.Unsynced.ClearFeaturesPreviousDrawFlag()
	Engine.Shared.Echo("[CUS GL4] Ready")
	initiated = true
end
...
local function ReloadCUSGL4(optName, line, words, playerID)
	...
	Engine.Shared.Echo("[CustomUnitShadersGL4] Reloading")
	gadget:Shutdown()
	gadget:Initialize()
	initGL4()
end
...
local function DisableCUSGL4(optName, _, _, playerID)
	...
	Engine.Shared.Echo("[CustomUnitShadersGL4] Disabling")
	gadget:Shutdown()
end
...
local function CUSGL4updaterate(optName, line, words, playerID)
	...
	Engine.Shared.Echo("[CustomUnitShadersGL4] Updaterate set to", updaterate)
end
...
local function DebugCUSGL4(optName, line, words, playerID)
	...
	Engine.Shared.Echo("[CustomUnitShadersGL4] Debugmode set to", debugmode)
end
...
local function DumpCUSGL4(optName, line, words, playerID)
	...
	Engine.Shared.Echo("[CustomUnitShadersGL4] Dumping unit bins:", debugmode)

	if unitDrawBins == nil then
		return
	end
	for drawflag, bin in pairs(unitDrawBins) do
		Engine.Shared.Echo(string.format("%i = { -- drawFlag", drawflag))
		for shadername, uniformbin in pairs(bin) do
			Engine.Shared.Echo(string.format("  %s = { -- shadername", shadername))
			for uniformbinid, texandobjset in pairs(uniformbin) do
				Engine.Shared.Echo(string.format("    %s = { -- uniformbin", uniformbinid))
				for texturekey, minibin in pairs(texandobjset) do
					Engine.Shared.Echo(string.format("      %i = { -- textureset", texturekey))
					for minibinattr, minibinvalue in pairs(minibin) do
						if type(minibinvalue) == "table" then
							Engine.Shared.Echo(string.format("        %s = {", minibinattr))
							if minibinattr == "objectsIndex" then
								for k, v in pairs(minibinvalue) do
									local objdefname = (k >= 0 and Engine.Shared.GetUnitDefID(k) and UnitDefs[Engine.Shared.GetUnitDefID(k)].name) or (Engine.Shared.GetFeatureDefID(-1 * k) and FeatureDefs[Engine.Shared.GetFeatureDefID(-1 * k)].name) or "???"
									Engine.Shared.Echo(string.format("          %i = %i, --(%s)", k, v, objdefname))
								end
							elseif minibinattr == "objectsArray" then
								for k, v in pairs(minibinvalue) do
									local objdefname = (v >= 0 and Engine.Shared.GetUnitDefID(v) and UnitDefs[Engine.Shared.GetUnitDefID(v)].name) or (Engine.Shared.GetFeatureDefID(-1 * v) and FeatureDefs[Engine.Shared.GetFeatureDefID(-1 * v)].name) or "???"
									Engine.Shared.Echo(string.format("          %i = %i, --(%s)", k, v, objdefname))
								end
							else
								for k, v in pairs(minibinvalue) do
									Engine.Shared.Echo(string.format("          %s = %s,", tostring(k), tostring(v)))
								end
							end
							Engine.Shared.Echo("        },")
						else
							Engine.Shared.Echo(string.format("        %s = %s,", tostring(minibinattr), tostring(minibinvalue)))
						end
					end
					Engine.Shared.Echo("      },")
				end
...
local function MarkBinCUSGL4(optName, line, words, playerID)
	...
	Engine.Shared.Echo("[CustomUnitShadersGL4] Marking Bins", optName, line, words, playerID)
	...
	local function markBin(drawPass)
			...
			for uniformbinid, texandobjset in pairs(uniformbin) do
				for texturekey, minibin in pairs(texandobjset) do
					for objectID, _ in pairs(minibin.objectsIndex) do
						local px, py, pz
						if objectID > 0 then
							px, py, pz = Engine.Shared.GetUnitPosition(objectID)
						else
							px, py, pz = Engine.Shared.GetFeaturePosition(-1 * objectID)
						end
						if px then
							Engine.Unsynced.MarkerAddPoint(px, py, pz, tostring(drawPass) .. "/" .. tostring(shadername) .. "/" .. tostring(uniformbinid) .. "/" .. tostring(texturekey) .. "/" .. tostring(objectID))
							count = count + 1
						end
					end
				end
		...
		Engine.Shared.Echo("Added markers for", count, "units in drawPass", drawPass)
	end
...
local function FreeTextures() -- pre we are using 2200mb
	...
	Engine.Shared.Echo("Freeing textures")
	...
	for unitDefID, uniformBin in pairs(objectDefToUniformBin) do
		if uniformBin == "raptor" then
			...
			Engine.Shared.Echo("Freeing ", textureTable[0], textureTable[1], s1, s2)
		end
	end
	...
	for i, featureID in ipairs(features) do
		local existingFeatureDefID = Engine.Shared.GetFeatureDefID(featureID)
	...
	for i, fname in ipairs(unittexfiles) do
		if string.find(fname, "chicken", nil, true) then
			...
			Engine.Shared.Echo("Freeing ", fname, s1)
		end
	end
end
...
function gadget:Initialize()
	...
	if not initiated and tonumber(Engine.Unsynced.GetConfigInt("cus2", 1) or 1) == 1 then
		initGL4()
	end
...
function gadget:Shutdown()
	if debugmode then
		Engine.Shared.Echo(unitDrawBins, "unitDrawBins")
	end
...
function gadget:UnitFinished(unitID)
	...
	buildProgresses[unitID] = nil
	UpdateUnit(unitID, Engine.Unsynced.GetUnitDrawFlag(unitID))
end
...
function gadget:UnitGiven(unitID)
	local flag = Engine.Unsynced.GetUnitDrawFlag(unitID)
...
function gadget:UnitCloaked(unitID)
	uniformCache[1] = Engine.Shared.GetGameFrame()
	gl.SetUnitBufferUniforms(unitID, uniformCache, 12)
	UpdateUnit(unitID, Engine.Unsynced.GetUnitDrawFlag(unitID))
	if debugmode then
		Engine.Shared.Echo("UnitCloaked", unitID, Engine.Unsynced.GetUnitDrawFlag(unitID))
	end
end

function gadget:UnitDecloaked(unitID)
	UpdateUnit(unitID, Engine.Unsynced.GetUnitDrawFlag(unitID))
	uniformCache[1] = -1 * Engine.Shared.GetGameFrame()
	gl.SetUnitBufferUniforms(unitID, uniformCache, 12)
	if debugmode then
		Engine.Shared.Echo("UnitDecloaked", unitID, Engine.Unsynced.GetUnitDrawFlag(unitID))
	end
end
...
function gadget:DrawWorldPreUnit()
	...
	if updateframe == 0 then
		...
		if autoReload.enabled then
			if Engine.Unsynced.DiffTimers(Engine.Unsynced.GetTimer(), autoReload.lastUpdate) > autoReload.updateRate then
				-- Check for fs and vs src identity
				autoReload.lastUpdate = Engine.Unsynced.GetTimer()
				...
				if (defaulttemplate.shader.vertex ~= defaultMaterialTemplate.shader.vertex) or (defaulttemplate.shader.fragment ~= defaultMaterialTemplate.shader.fragment) then
					-- recompile on change:
					Engine.Shared.Echo("Changes to CUS shaders detected, recompiling...")
		...
		if manualReload then
			manualReload = false
			units, drawFlagsUnits = Engine.Unsynced.GetRenderUnits(overrideDrawFlag, true)
			features, drawFlagsFeatures = Engine.Unsynced.GetRenderFeatures(overrideDrawFlag, true)
		else
			units, drawFlagsUnits = Engine.Unsynced.GetRenderUnitsDrawFlagChanged(true)
			features, drawFlagsFeatures = Engine.Unsynced.GetRenderFeaturesDrawFlagChanged(true)
		end
		...
		if debugmode and (#destroyedUnitIDs > 0 or #units > 0) then
			Engine.Shared.Echo("Processing destroyedUnitIDs", #units, #destroyedUnitIDs)
		end
		...
		if firstDraw then
			...
			local firstunits = Engine.Unsynced.GetVisibleUnits()
		...
		local deltat = Engine.Unsynced.DiffTimers(Engine.Unsynced.GetTimerMicros(), t0, nil) -- in ms
		--Spring.Echo(deltat)
		if (deltat > 2) and perfdebug then
			local usecperobjectchange = (1000 * deltat) / totalobjects
			Engine.Shared.Echo("[CUS GL4] [", Engine.Unsynced.GetDrawFrame(), "]", totalobjects, " Update time 2 < ", deltat, string.format("ms, per object change: %.2fus ", usecperobjectchange), totalobjects, "objs")
			-- PERF CONCULUSION:
...
function gadget:SunChanged() -- Note that map_nightmode.lua gadget has to change sun twice in a single draw frame to update all
	local df = Engine.Unsynced.GetDrawFrame()
...
function gadget:DrawOpaqueUnitsLua(deferredPass, drawReflection, drawRefraction)
	...
	if preloadedTextures == false then
		PreloadTextures()
	end
