	...
	for _, configFile in pairs(availableConfigFiles) do
		...
		if string.find(sanitizedCurrentMapName, basename) then
			Spring.Log(
				...
				LOG.INFO,
				string.format(
					"found FFA start points config %s provided by BAR for current map %s",
					configFile,
					currentMapName
				)
			)
...
local function tryLoadConfigFromMap(currentMapName)
	...
	if VFS.FileExists(configFile) then
		Spring.Log(
			gadget:GetInfo().name,
			LOG.INFO,
			string.format("found FFA start points config %s provided by current map %s", configFile, currentMapName)
		)
		return VFS.Include(configFile), true
	end
	return nil, false
...
local function tryLoadConfig(currentMapName, requiredStartPointCount)
	...
	if ffaStartPoints and ffaStartPoints[requiredStartPointCount] then
		Spring.Log(
			...
			LOG.WARNING,
			string.format(
				"backwards compatibility layer: using legacy FFA start points config for %s start points",
	...
	if not layout then
		Spring.Log(
			...
			LOG.INFO,
			string.format(
				"did not find a layout for %s start points for current map %s",
				requiredStartPointCount,
				currentMapName
			)
		)
	...
	elseif #layout ~= requiredStartPointCount then
		Spring.Log(
			...
			LOG.ERROR,
			string.format(
				"incorrect number of start points found in layout (actual: %s, expected: %s)",
				#layout,
				requiredStartPointCount
			)
		)
