...
function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attUnitID, attUnitDefID, attTeamID, weaponDefID)
	...
	if Script.LuaUI("UnitDestroyedByTeam") then
		...
		if
			not isAllyUnit
			and (not (spec and specFullView) and (spGetUnitLosState(unitID, spGetMyAllyTeamID(), true) % 2 == 1))
		then
			scriptUnitDestroyed(unitID, unitDefID, unitTeam, nil, nil, nil, weaponDefID)
		end
	end
...
function gadget:UnitFinished(unitID, unitDefID, unitTeam)
	...
	-- Also important: units that are created and finished in the same gameframe (like /give, or created fully built) are not in los yet, so this won't trigger!
