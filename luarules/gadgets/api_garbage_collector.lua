...
if not gadgetHandler:IsSyncedCode() then
	...
	local function CheckRamLimitGC()
		...
		if ramuse > garbagelimit then
			...
			Engine.Shared.Echo(string.format("BAR using %d MB RAM > %d MB limit, performing garbage collection down to %d MB and adjusting limit to %d MB", math.floor(ramuse / 1000), math.floor(garbagelimit / 1000), math.floor(notgarbagemem / 1000), math.floor(newgarbagelimit / 1000)))
