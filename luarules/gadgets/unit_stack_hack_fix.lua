...
function gadget:GameFrame(n)
	for i = 1, #affectedUnits do
		...
		local nearestAlly = Engine.Shared.GetUnitNearestAlly(unitID, isAffectedUnit[unitDefID][1])
		if nearestAlly then
			if not canMove[Engine.Shared.GetUnitDefID(nearestAlly)] then
				if not Engine.Shared.GetUnitTransporter(unitID) then
					local x, _, z = Engine.Shared.GetUnitPosition(unitID)
					local ax, _, az = Engine.Shared.GetUnitPosition(nearestAlly)
					...
					local movementTargetY = Engine.Shared.GetGroundHeight(x + movementTargetX, z + movementTargetZ)
					...
					if aboveMinWaterDepth and belowMaxWaterDepth and onMap then
						Engine.Synced.SetUnitPosition(unitID, x + movementTargetX, z + movementTargetZ)
					end
				end
			end
		end
