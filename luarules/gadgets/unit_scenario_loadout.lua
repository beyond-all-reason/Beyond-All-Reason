	...
	if n == 1 then
		...
		if next(additionalStorage) then
			for teamID, additionalstorage in pairs(additionalStorage) do
				...
				GG.SetTeamResourceData(teamID, { resourceType = ResourceType.METAL, storage = mstore + additionalstorage.metal })
				GG.SetTeamResourceData(teamID, { resourceType = ResourceType.ENERGY, storage = estore + additionalstorage.energy })
			end
		...
		for i = 1, #teamList do
			...
			if mstore < 500 then
				GG.SetTeamResourceData(teamID, {
					[ResourceType.METAL] = { resourceType = ResourceType.METAL, storage = 500 },
				})
			end
			if estore < 500 then
				GG.SetTeamResourceData(teamID, {
					[ResourceType.ENERGY] = { resourceType = ResourceType.ENERGY, storage = 500 },
				})
			end
		end
	end
