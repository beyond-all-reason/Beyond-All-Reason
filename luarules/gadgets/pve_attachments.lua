local gadget = gadget ---@type Gadget

function gadget:GetInfo()
	return {
		name = "Unit Attachments",
		desc = "Allows to attach structures to other structures, merging their healthbars",
		author = "Damgam",
		date = "2023",
		license = "GNU GPL, v2 or later",
		layer = 0,
		enabled = false, -- we don't need it for now, but might need it later.
	}
end

if not gadgetHandler:IsSyncedCode() then
	return
end

local unitAttachments = {
	-- [UnitDefNames["raptor_hive"].id] = {
	--     { unitType = "raptor_antinuke",   posx = 0, posz = 0, dir = 0 },
	-- },
}

local deleteAttachments = {}
local unitAttachedTo = {}

function gadget:UnitPreDamaged(unitID, unitDefID, unitTeam, damage, paralyzer, weaponID, projectileID, attackerID, attackerDefID, attackerTeam)
	if unitAttachedTo[unitID] and not paralyzer then
		local h, mh = Engine.Shared.GetUnitHealth(unitAttachedTo[unitID])
		if h and mh then
			Engine.Synced.SetUnitHealth(unitAttachedTo[unitID], h - damage)
		end
		damage = 0
	end
	return damage, 1
end

function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID)
	for attachment, attachedTo in pairs(unitAttachedTo) do
		if unitAttachedTo[attachment] == unitID then
			table.insert(deleteAttachments, attachment)
		end
	end
	if #deleteAttachments > 0 then
		for i = 1, #deleteAttachments do
			if deleteAttachments[i] then
				unitAttachedTo[deleteAttachments[i]] = nil
				Engine.Synced.DestroyUnit(deleteAttachments[i], false, true)
			end
		end
		deleteAttachments = {}
	end
end

function gadget:UnitCreated(unitID, unitDefID, unitTeam)
	if unitAttachments[unitDefID] then
		for i = 1, #unitAttachments[unitDefID] do
			local unitType = unitAttachments[unitDefID][i].unitType
			local posx = unitAttachments[unitDefID][i].posx
			local posz = unitAttachments[unitDefID][i].posz
			local dir = unitAttachments[unitDefID][i].dir or 0
			local bx, by, bz = Engine.Shared.GetUnitPosition(unitID)
			local attachmentID = Engine.Synced.CreateUnit(unitType, bx + posx, by, bz + posz, dir, unitTeam)
			if attachmentID then
				Engine.Synced.SetUnitBlocking(attachmentID, false, false)
				unitAttachedTo[attachmentID] = unitID
			end
		end
	end
end

function gadget:GameFrame(frame)
	for attachment, attachedTo in pairs(unitAttachedTo) do
		local h, mh = Engine.Shared.GetUnitHealth(attachedTo)
		if h and mh then
			Engine.Synced.SetUnitMaxHealth(attachment, mh)
			Engine.Synced.SetUnitHealth(attachment, h)
		end
	end
end
