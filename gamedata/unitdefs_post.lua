...
local function applyAimFromEstimates()
	-- Where each weapon's muzzle ends up once its script has aimed, so the engine's
	-- pre-aim line-of-fire test does not run from the AimFromWeapon piece (aimFromEstimate
	-- weapon tag). Generated by tools/aimfit, see tools/aimfit/README.md; a value written in
	-- the unit file itself takes precedence, and tweakunits can still override both.
	local estimates = VFS.Include("gamedata/aim_from_estimates.lua")
	for unitName, weapons in pairs(estimates.weapons) do
		local unitDef = UnitDefs[unitName]
		if unitDef and unitDef.weapons then
			for slot, estimate in pairs(weapons) do
				local weapon = unitDef.weapons[slot]
				if weapon and weapon.aimfromestimate == nil then
					weapon.aimfromestimate = estimate
				end
			end
		end
	end
