...
local function scavWeaponDefPost(name, wDef)
	...
	if wDef.commandfire and not wDef.customparams.scavforcecommandfire then
		wDef.commandfire = false
	end
	--wDef.metalpershot = 0
	...
	if wDef.weapontype == "Cannon" then
		...
		if wDef.explosiongenerator then
			if
				string.find(wDef.explosiongenerator, "genericshellexplosion")
				or string.find(wDef.explosiongenerator, "expldgun")
			then
