...
local function scavUnitDef_Post(name, uDef)
	...
	if uDef.buildpic then
		...
		if (not string.find(uDef.buildpic, "scavengers")) and (not string.find(uDef.buildpic, "raptor")) and (not string.find(uDef.buildpic, "critters")) and (not string.find(uDef.buildpic, "lootboxes")) and (not string.find(uDef.buildpic, "other")) and (not string.find(uDef.buildpic, "alternative")) and (VFS.FileExists("unitpics/scavengers/" .. uDef.buildpic)) then
	...
	if uDef.explodeas then
		if string.find(string.lower(uDef.explodeas), "explosiongeneric") or string.find(string.lower(uDef.explodeas), "buildingexplosiongeneric") or string.find(string.lower(uDef.explodeas), "explosiont3") or string.find(string.lower(uDef.explodeas), "bantha") or string.find(string.lower(uDef.explodeas), "lootbox") then
			uDef.explodeas = uDef.explodeas .. "-purple"
		end
	end

	if uDef.selfdestructas then
		if string.find(string.lower(uDef.selfdestructas), "explosiongeneric") or string.find(string.lower(uDef.selfdestructas), "buildingexplosiongeneric") or string.find(string.lower(uDef.selfdestructas), "explosiont3") or string.find(string.lower(uDef.selfdestructas), "bantha") or string.find(string.lower(uDef.selfdestructas), "lootbox") then
	...
	if uDef.builder then
		...
		if uDef.workerspeed then
			...
			if uDef.metalcost then
				uDef.metalcost = uDef.metalcost * 1.5
			end
			if uDef.energycost then
	...
	if uDef.metalcost then
		uDef.metalcost = math.ceil(uDef.metalcost)
	end
	if uDef.energycost then
		uDef.energycost = math.ceil(uDef.energycost)
	end
	if uDef.buildtime then
		uDef.buildtime = math.ceil(uDef.buildtime)
	end
