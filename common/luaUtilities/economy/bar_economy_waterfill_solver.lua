...
function Gadgets.ProcessEconomy(springRepo, teamsList, policies)
  ...
  -- Update teamsList with ledger data
  -- Excess is always 0 in the waterfill algorithm as we only allocate what can be stored.
  
  local teamMap = {}
  for i = 1, #teamsList do
    teamMap[teamsList[i].id] = teamsList[i]
  end

  for teamId, ledger in pairs(allLedgers) do
    local team = teamMap[teamId]
    if team then
      if ledger[ResourceType.METAL] then
        local l = ledger[ResourceType.METAL]
        team.metal.sent = l.sent
        team.metal.received = l.received
        team.metal.excess = 0
      end
      if ledger[ResourceType.ENERGY] then
        local l = ledger[ResourceType.ENERGY]
        team.energy.sent = l.sent
        team.energy.received = l.received
        team.energy.excess = 0
      end
    end
  end
