...
function PolicyHelpers.PackUnitPolicyResult(team, myTeamID, player)
  ...
  PolicyHelpers.PackPolicyResult(TransferEnums.TransferCategory.UnitTransfer, policyResult, player)
end

local packByDomain = {
  unit = PolicyHelpers.PackUnitPolicyResult,
  metal = PolicyHelpers.PackMetalPolicyResult,
  energy = PolicyHelpers.PackEnergyPolicyResult,
}

---Re-pack policy for rows affected by a SharePolicyChanged event: all rows when our own
---team (the sender) changed, otherwise just the changed team's row.
---@param player table all player rows
---@param myTeamID number
---@param changedTeamID number team whose policy changed
---@param domain string "unit" | "metal" | "energy"
