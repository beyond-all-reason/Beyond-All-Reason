...
local function TryDenyPolicy(ctx, resourceType)
  ...
  local resourceSharingEnabled = modOpts[ModeEnums.ModOptions.ResourceSharingEnabled]
  if resourceSharingEnabled == false or resourceSharingEnabled == "0" then
...
function Gadgets.CalcResourcePolicy(ctx, resourceType)
  ...
  local modOpts = ctx.springRepo.GetModOptions()
  local MOD = ModeEnums.ModOptions

  local taxRate = ctx.taxRate or tonumber(modOpts[MOD.TaxResourceSharingAmount]) or 0
  if taxRate < 0 then taxRate = 0 end
  if taxRate > 1 then taxRate = 1 end

  local threshold
  if resourceType == TransferEnums.ResourceType.METAL then
    threshold = math.max(0, tonumber(modOpts[MOD.PlayerMetalSendThreshold]) or 0)
  else
    threshold = math.max(0, tonumber(modOpts[MOD.PlayerEnergySendThreshold]) or 0)
  end
  ...
  while groupsProcessed < groupsPerFrame do
    ...
    for _, senderID in ipairs(allyGroup) do
      for _, receiverID in ipairs(allyGroup) do
        ...
        local metalPolicy = Gadgets.CalcResourcePolicy(ctx, ResourceType.METAL)
        Gadgets.CachePolicyResult(springRepo, senderID, receiverID, ResourceType.METAL, metalPolicy)
        
        local energyPolicy = Gadgets.CalcResourcePolicy(ctx, ResourceType.ENERGY)
