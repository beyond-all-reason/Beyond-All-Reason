...
---They are cheap to mass-produce and trivially "tossed around", so grouping
---them with builders lets certain tech trees be favoured (and skews spam) when
---construction units are shared separately from combat units.
...
function sharing.classifyUnitDef(unitDef)
	...
	-- Transports share with combat units; armed ones already match isCombatUnitDef.
	-- Fast T2 engineers (see combatGroupBuilders) are folded in here too, ahead of
	-- the constructor check, so they share alongside the commander/combat group.
	if sharing.isCombatUnitDef(unitDef)
		or sharing.isTransportDef(unitDef)
		or sharing.isCombatGroupBuilderDef(unitDef) then
...
function sharing.isConstructorDef(unitDef)
	if unitDef.isFactory then return false end
