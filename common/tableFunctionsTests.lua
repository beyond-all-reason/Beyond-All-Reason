local function testMergeInPlace(deep)
	...
	local function checkAllYes(tbl)
		for key, value in pairs(tbl) do
			if type(value) == "table" then
				...
				assert(
					value == "yes",
					string.format("expected all values to be 'yes', but go '%s' for key %s", value, key)
				)
				yesCount = yesCount + 1
			end
		end
	...
	if deep then
		assert(
			mergeData.shouldBeAdded ~= mergeTarget.shouldBeAdded,
			"expected mergeData.shouldBeAdded and mergeTarget.shouldBeAdded to be different instance, due to deep merge"
		)
		assert(
			mergeData.shouldBeMerged.shouldBeAdded ~= mergeTarget.shouldBeMerged.shouldBeAdded,
...
local function testTableRemoveFirst()
	...
	for name, test in pairs(tests) do
		...
		for _, value in pairs({ "b", "c", "a" }) do
			...
			Spring.Echo(
				string.format("[test] [table.removeFirst] %s: %s (removed: %s)", name, table.toString(test), value)
			)
			...
			if name == "sequence" then
				...
				for i, _ in pairs(test) do
					...
					assert(
						i == prev_i + 1,
						string.format("expected table %s to have continuous indexes, but it does not", name)
					)
					prev_i = i
				end
			end
...
local function testTableShuffle()
	...
	assert(
		standardDeviationResults < threshold,
		string.format(
			"expected results standard deviation to be below %s (actual: %s)",
			threshold,
			standardDeviationResults
		)
	)
	assert(
		standardDeviationResults0 < threshold,
		string.format(
			"expected results0 standard deviation to be below %s (actual: %s)",
			threshold,
