...
local function getSettings()
	...
	local springAllyTeamList = Engine.Shared.GetAllyTeamList()
	local allyTeamList = {}
	local allyTeamSizes = {}
		...
		if #teamList > 0 and allyTeamID ~= gaiaAllyTeamID then
			...
			for _, teamID in ipairs(teamList) do
				if select(4, Engine.Shared.GetTeamInfo(teamID, false)) then
					...
					local teamPlayers = Engine.Shared.GetPlayerList(teamID)
	...
	if #entirelyHumanAllyTeams == 1 and #Engine.Shared.GetTeamList(entirelyHumanAllyTeams[1]) > 1 then
		isCoop = true
	end

	if holidaysList and Engine.Shared.GetModOptions and Engine.Shared.GetModOptions().date_day then
		local currentDay = Engine.Shared.GetModOptions().date_day
		local currentMonth = Engine.Shared.GetModOptions().date_month
		local currentYear = Engine.Shared.GetModOptions().date_year
