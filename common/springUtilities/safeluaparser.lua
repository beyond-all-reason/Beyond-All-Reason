...
if not Spring then
	local function deep_equal(a, b)
		...
		if type(a) ~= "table" or type(b) ~= "table" then
			return false,
				string.format("Type mismatch: %s vs %s (a=%s, b=%s)", type(a), type(b), tostring(a), tostring(b))
		end
	...
	local function testFunctionParsing()
		...
		local test7 =
			'function(t) for k,v in pairs(t) do if type(v) == "table" then v[3], v[4] = 1.0 - v[3], 1.0 - v[4] end end end'
