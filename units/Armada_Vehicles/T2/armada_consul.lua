return {
	armada_consul = {
		maxacc = 0.07029,
		maxdec = 0.14059,
		energycost = 4300,
		metalcost = 250,
		builddistance = 136,
		builder = true,
		buildpic = "armada_consul.DDS",
		buildtime = 6800,
		canmove = true,
		category = "ALL TANK MOBILE NOTSUB NOWEAPON NOTSHIP NOTAIR NOTHOVER SURFACE EMPABLE",
		collisionvolumeoffsets = "0 0 -3",
		collisionvolumescales = "32 30 30",
		collisionvolumetype = "Box",
		corpse = "DEAD",
		energymake = 15,
		energystorage = 100,
		explodeas = "smallexplosiongeneric-builder",
		footprintx = 3,
		footprintz = 3,
		idleautoheal = 5,
		idletime = 1800,
		leavetracks = true,
		health = 1080,
		maxslope = 18,
		speed = 78.0,
		maxwaterdepth = 18,
		movementclass = "TANK2",
		objectname = "Units/armada_consul.s3o",
		script = "Units/armada_consul.cob",
		seismicsignature = 0,
		selfdestructas = "smallExplosionGenericSelfd-builder",
		sightdistance = 500,
		terraformspeed = 750,
		trackoffset = 10,
		trackstrength = 5,
		tracktype = "armada_gremlin_tracks",
		trackwidth = 35,
		turninplace = true,
		turninplaceanglelimit = 90,
		turninplacespeedlimit = 1.83678,
		turnrate = 635,
		workertime = 150,
		buildoptions = {
			[1] = "armada_solarcollector",
			[2] = "armada_metalextractor",
			[3] = "armada_vehicleplant",
			[4] = "armada_constructionturret",
			[5] = "armada_beholder",
			[6] = "armada_veil",
			[7] = "armada_fortificationwall",
			[8] = "armada_advancedradartower",
			[9] = "armada_mediummine",
			[10] = "armada_anemone",
			[11] = "armada_beamer",
			[12] = "armada_rattlesnake",
			[13] = "armada_ferret",
			[14] = "armada_arbalest",
			[15] = "armada_constructionvehicle",
			[16] = "armada_pawn",
			[17] = "armada_crossbow",
			[18] = "armada_webber",
			[19] = "armada_sprinter",
			[20] = "armada_gunslinger",
			[21] = "armada_constructionship",
			[22] = "armada_corsair",
			[23] = "armada_amphibiousbot",
		},
		customparams = {
			unitgroup = 'buildert2',
			area_mex_def = "armada_metalextractor",
			model_author = "FireStorm",
			normaltex = "unittextures/Arm_normal.dds",
			subfolder = "armada_vehicles/t2",
			techlevel = 2,
		},
		featuredefs = {
			dead = {
				blocking = true,
				category = "corpses",
				collisionvolumeoffsets = "0 0 -3",
				collisionvolumescales = "32 30 30",
				collisionvolumetype = "Box",
				damage = 800,
				energy = 0,
				featuredead = "HEAP",
				featurereclamate = "SMUDGE01",
				footprintx = 3,
				footprintz = 3,
				height = 20,
				hitdensity = 100,
				metal = 153,
				object = "Units/armada_consul_dead.s3o",
				reclaimable = true,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
			heap = {
				blocking = false,
				category = "heaps",
				collisionvolumescales = "55.0 4.0 6.0",
				collisionvolumetype = "cylY",
				damage = 700,
				energy = 0,
				featurereclamate = "SMUDGE01",
				footprintx = 3,
				footprintz = 3,
				height = 4,
				hitdensity = 100,
				metal = 61,
				object = "Units/armada_3x3A.s3o",
				reclaimable = true,
				resurrectable = 0,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
		},
		sfxtypes = {
			pieceexplosiongenerators = {
				[1] = "deathceg2-builder",
				[2] = "deathceg3-builder",
				[3] = "deathceg4-builder",
			},
		},
		sounds = {
			canceldestruct = "cancel2",
			underattack = "warning1",
			cant = {
				[1] = "cantdo4",
			},
			count = {
				[1] = "count6",
				[2] = "count5",
				[3] = "count4",
				[4] = "count3",
				[5] = "count2",
				[6] = "count1",
			},
			ok = {
				[1] = "tarmmove",
			},
			select = {
				[1] = "tarmsel",
			},
		},
	},
}
