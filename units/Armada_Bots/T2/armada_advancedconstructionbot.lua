return {
	armada_advancedconstructionbot = {
		maxacc = 0.4968,
		maxdec = 3.105,
		energycost = 6900,
		metalcost = 430,
		builddistance = 136,
		builder = true,
		buildpic = "armada_advancedconstructionbot.DDS",
		buildtime = 9500,
		canmove = true,
		category = "BOT MOBILE ALL NOTSUB NOWEAPON NOTSHIP NOTAIR NOTHOVER SURFACE EMPABLE",
		collisionvolumeoffsets = "0 0 0",
		collisionvolumescales = "27 35 23",
		collisionvolumetype = "box",
		corpse = "DEAD",
		energymake = 14,
		energystorage = 100,
		energyupkeep = 14,
		explodeas = "mediumexplosiongeneric-builder",
		footprintx = 2,
		footprintz = 2,
		idleautoheal = 5,
		idletime = 1800,
		health = 930,
		maxslope = 20,
		speed = 33.0,
		maxwaterdepth = 25,
		movementclass = "BOT3",
		objectname = "Units/armada_advancedconstructionbot.s3o",
		radardistance = 50,
		script = "Units/armada_advancedconstructionbot.cob",
		seismicsignature = 0,
		selfdestructas = "mediumExplosionGenericSelfd-builder",
		sightdistance = 323.70001,
		terraformspeed = 900,
		turninplace = true,
		turninplaceanglelimit = 90,
		turninplacespeedlimit = 0.759,
		turnrate = 1138.5,
		upright = true,
		workertime = 180,
		buildoptions = {
			"armada_fusionreactor",
			"armada_advancedfusionreactor",
			"armada_cloakablefusionreactor",
			"armada_experimentalgantry",
			"armada_advancedgeothermalpowerplant",
			"armada_prude",
			"armada_advancedmetalextractor",
			"armada_advancedenergyconverter",
			"armada_hardenedenergystorage",
			"armada_hardenedmetalstorage",
			"armada_advancedradartower",
			"armada_veil",
			"armada_fortificationwall",
			"armada_airrepairpad",
			"armada_pinpointer",
			"armada_tracer",
			"armada_keeper",
			"armada_rattlesnake",
			"armada_pitbull",
			"armada_pulsar",
			"armada_arbalest",
			"armada_mercury",
			"armada_paralyzer",
			"armada_citadel",
			"armada_armageddon",
			"armada_basilica",
			"armada_ragnarok",
			"armada_decoyfusionreactor",
			"armada_botlab",
			"armada_advancedbotlab",
		},
		customparams = {
			unitgroup = 'buildert2',
			area_mex_def = "armada_advancedmetalextractor",
			model_author = "Kaiser",
			normaltex = "unittextures/Arm_normal.dds",
			subfolder = "armada_bots/t2",
			techlevel = 2,
		},
		featuredefs = {
			dead = {
				blocking = true,
				category = "corpses",
				collisionvolumeoffsets = "1.64561462402 6.61621093734e-06 -1.44562530518",
				collisionvolumescales = "29.6044616699 29.4648132324 28.049697876",
				collisionvolumetype = "Box",
				damage = 700,
				energy = 0,
				featuredead = "HEAP",
				featurereclamate = "SMUDGE01",
				footprintx = 2,
				footprintz = 2,
				height = 20,
				hitdensity = 100,
				metal = 189,
				object = "Units/armada_advancedconstructionbot_dead.s3o",
				reclaimable = true,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
			heap = {
				blocking = false,
				category = "heaps",
				collisionvolumescales = "35.0 4.0 6.0",
				collisionvolumetype = "cylY",
				damage = 500,
				energy = 0,
				featurereclamate = "SMUDGE01",
				footprintx = 2,
				footprintz = 2,
				height = 4,
				hitdensity = 100,
				metal = 76,
				object = "Units/armada_2x2B.s3o",
				reclaimable = true,
				resurrectable = 0,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
		},
		sfxtypes = {
			pieceexplosiongenerators = {
				[1] = "deathceg2-builder",
				[2] = "deathceg3-builder",
				[3] = "deathceg4-builder",
			},
		},
		sounds = {
			build = "nanlath1",
			canceldestruct = "cancel2",
			capture = "capture1",
			repair = "repair1",
			underattack = "warning1",
			working = "reclaim1",
			cant = {
				[1] = "cantdo4",
			},
			count = {
				[1] = "count6",
				[2] = "count5",
				[3] = "count4",
				[4] = "count3",
				[5] = "count2",
				[6] = "count1",
			},
			ok = {
				[1] = "kbarmmov",
			},
			select = {
				[1] = "kbarmsel",
			},
		},
	},
}
