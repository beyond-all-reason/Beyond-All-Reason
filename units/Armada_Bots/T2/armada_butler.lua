return {
	armada_butler = {
		maxacc = 0.4968,
		autoheal = 5,
		maxdec = 5.175,
		energycost = 3000,
		metalcost = 210,
		builddistance = 136,
		builder = true,
		buildpic = "armada_butler.DDS",
		buildtime = 4300,
		canmove = true,
		category = "BOT MOBILE ALL NOTSUB NOWEAPON NOTAIR NOTHOVER SURFACE EMPABLE",
		collisionvolumeoffsets = "0 0 -1",
		collisionvolumescales = "21 26 17",
		collisionvolumetype = "CylY",
		corpse = "DEAD",
		energymake = 12,
		energystorage = 25,
		explodeas = "smallbuilder",
		footprintx = 2,
		footprintz = 2,
		idleautoheal = 5,
		idletime = 1800,
		health = 335,
		maxslope = 14,
		speed = 75.0,
		maxwaterdepth = 22,
		movementclass = "BOT3",
		objectname = "Units/armada_butler.s3o",
		radardistance = 50,
		script = "Units/armada_butler.cob",
		seismicsignature = 0,
		selfdestructas = "smallbuilderSelfd",
		sightdistance = 377,
		terraformspeed = 600,
		turninplace = true,
		turninplaceanglelimit = 90,
		turninplacespeedlimit = 1.7424,
		turnrate = 1265,
		upright = true,
		workertime = 140,
		buildoptions = {
			[1] = "armada_solarcollector",
			[2] = "armada_windturbine",
			[3] = "armada_metalextractor",
			[4] = "armada_energyconverter",
			[5] = "armada_beholder",
			[6] = "armada_compass",
			[7] = "armada_radarjammerbot",
		},
		customparams = {
			unitgroup = 'buildert2',
			model_author = "FireStorm",
			normaltex = "unittextures/Arm_normal.dds",
			subfolder = "armada_bots/t2",
			techlevel = 2,
		},
		featuredefs = {
			dead = {
				blocking = true,
				category = "corpses",
				collisionvolumeoffsets = "0 0 0",
				collisionvolumescales = "21 5 17",
				collisionvolumetype = "Box",
				damage = 250,
				energy = 0,
				featuredead = "HEAP",
				featurereclamate = "SMUDGE01",
				footprintx = 2,
				footprintz = 2,
				height = 20,
				hitdensity = 100,
				metal = 131,
				object = "Units/armada_butler_dead.s3o",
				reclaimable = true,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
			heap = {
				blocking = false,
				category = "heaps",
				collisionvolumescales = "35.0 4.0 6.0",
				collisionvolumetype = "cylY",
				damage = 150,
				energy = 0,
				featurereclamate = "SMUDGE01",
				footprintx = 2,
				footprintz = 2,
				height = 4,
				hitdensity = 100,
				metal = 52,
				object = "Units/armada_2x2D.s3o",
				reclaimable = true,
				resurrectable = 0,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
		},
		sfxtypes = {
			pieceexplosiongenerators = {
				[1] = "deathceg2-builder",
				[2] = "deathceg3-builder",
				[3] = "deathceg4-builder",
			},
		},
		sounds = {
			build = "nanlath1",
			canceldestruct = "cancel2",
			capture = "capture1",
			repair = "repair1",
			underattack = "warning1",
			working = "reclaim1",
			cant = {
				[1] = "cantdo4",
			},
			count = {
				[1] = "count6",
				[2] = "count5",
				[3] = "count4",
				[4] = "count3",
				[5] = "count2",
				[6] = "count1",
			},
			ok = {
				[1] = "kbarmmov",
			},
			select = {
				[1] = "kbarmsel",
			},
		},
	},
}
