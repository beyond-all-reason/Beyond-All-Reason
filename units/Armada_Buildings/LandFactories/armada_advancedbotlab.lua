return {
	armada_advancedbotlab = {
		maxacc = 0,
		maxdec = 0,
		buildangle = 1024,
		energycost = 15000,
		metalcost = 2900,
		builder = true,
		buildpic = "armada_advancedbotlab.DDS",
		buildtime = 16200,
		canmove = true,
		category = "ALL NOTLAND NOTSUB NOWEAPON NOTSHIP NOTAIR NOTHOVER SURFACE EMPABLE",
		collisionvolumescales = "85 75 100",
		collisionvolumetype = "Box",
		corpse = "DEAD",
		energystorage = 200,
		explodeas = "largeBuildingexplosiongeneric",
		footprintx = 6,
		footprintz = 6,
		idleautoheal = 5,
		idletime = 1800,
		health = 4250,
		maxslope = 15,
		maxwaterdepth = 0,
		metalstorage = 200,
		objectname = "Units/armada_advancedbotlab.s3o",
		--radardistance = 50,
		script = "Units/armada_advancedbotlab.cob",
		seismicsignature = 0,
		selfdestructas = "largeBuildingExplosionGenericSelfd",
		sightdistance = 286,
		terraformspeed = 1000,
		workertime = 300,
		yardmap = "yooooy oooooy yooooy cccccc cccccc cccccc",
		buildoptions = {
			[1] = "armada_advancedconstructionbot",
			[2] = "armada_butler",
			[3] = "armada_sprinter",
			[4] = "armada_amphibiousbot",
			[5] = "armada_welder",
			[6] = "armada_gunslinger",
			[7] = "armada_recluse",
			[8] = "armada_hound",
			[9] = "armada_sharpshooter",
			[10] = "armada_fatboy",
			[11] = "armada_webber",
			[12] = "armada_archangel",
			[13] = "armada_tumbleweed",
			[14] = "armada_decoycommander",
			[15] = "armada_umbrella",
			[16] = "armada_radarjammerbot",
			[17] = "armada_ghost",
			[18] = "armada_compass",
		},
		customparams = {
			usebuildinggrounddecal = true,
			buildinggrounddecaltype = "decals/armada_advancedbotlab_aoplane.dds",
			buildinggrounddecalsizey = 8.6,
			buildinggrounddecalsizex = 8.6,
			buildinggrounddecaldecayspeed = 30,
			unitgroup = 'buildert2',
			model_author = "Cremuss",
			normaltex = "unittextures/Arm_normal.dds",
			subfolder = "armada_buildings/landfactories",
			techlevel = 2,
		},
		featuredefs = {
			dead = {
				blocking = true,
				category = "corpses",
				collisionvolumeoffsets = "0 -17 -1",
				collisionvolumescales = "73 56 89",
				collisionvolumetype = "CylZ",
				damage = 2285,
				energy = 0,
				featuredead = "HEAP",
				featurereclamate = "SMUDGE01",
				footprintx = 5,
				footprintz = 6,
				height = 20,
				hitdensity = 100,
				metal = 1773,
				object = "Units/armada_advancedbotlab_dead.s3o",
				reclaimable = true,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
			heap = {
				blocking = false,
				category = "heaps",
				damage = 1143,
				energy = 0,
				featurereclamate = "SMUDGE01",
				footprintx = 5,
				footprintz = 5,
				height = 4,
				hitdensity = 100,
				metal = 887,
				object = "Units/arm5X5A.s3o",
				reclaimable = true,
				resurrectable = 0,
				seqnamereclamate = "TREE1RECLAMATE",
				world = "All Worlds",
			},
		},
		sfxtypes = {
			pieceexplosiongenerators = {
				[1] = "deathceg2",
				[2] = "deathceg3",
				[3] = "deathceg4",
			},
		},
		sounds = {
			canceldestruct = "cancel2",
			underattack = "warning1",
			unitcomplete = "untdone",
			count = {
				[1] = "count6",
				[2] = "count5",
				[3] = "count4",
				[4] = "count3",
				[5] = "count2",
				[6] = "count1",
			},
			select = {
				[1] = "plabactv",
			},
		},
	},
}
