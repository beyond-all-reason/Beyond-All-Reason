...
void main(void){
	...
	//outColor.rgb = iblSpecular;
		...
		if (buildProgress > -0.5){
			...
			float sintimefast = 0.5 + 0.5 * sin(simFrame * 0.166); // pulses faster
			...
			vec3 pulseTeamColor = mix(teamCol.rgb, teamCol.rgb * 1.68, sintimefast );
	...
	// SELECTION EFFECTS!
		...
		if (selectedness > 0.25){
			...
			selectionHighlight.rgb = clamp(teamCol.rgb, 0.75, 2.0);
