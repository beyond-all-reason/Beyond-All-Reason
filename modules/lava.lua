...
Engine.Shared.Echo("Lava Mapname", mapName)
...
local function getLavaConfig(mapName)
	...
	if mapName then
		...
		if VFS.FileExists(gameConfigPath(mapName)) then
			...
			Engine.Shared.Log("Lava", LOG.INFO, "Loaded map config for", mapName)
		elseif mapName ~= mapNameNoVersion and VFS.FileExists(gameConfigPath(mapNameNoVersion)) then
			...
			Engine.Shared.Log("Lava", LOG.INFO, "Loaded map config for", mapNameNoVersion)
		end
	end
	...
	if mapConfig and gameConfig and gameConfig.overrideMap then
		...
		Engine.Shared.Log("Lava", LOG.INFO, "Game config overrides map")
	end
	...
	for tide in string.gmatch(modoptionDataTrim, "{(.-)}") do
		...
		for value in string.gmatch(tide, "[^,]+") do
			if not tonumber(value) then
				Engine.Shared.Echo("Lava Advanced Tide Rhythm data is not valid, non-number value: ", value)
				return false
			else
		...
		if #partRhythm ~= 3 then
			Engine.Shared.Echo("Lava Advanced Tide Rhythm data is not valid, invalid tide definition: ", partRhythm)
			return false
		elseif not ((partRhythm[1] >= 0) and (partRhythm[2] > 0) and (partRhythm[3] >= 0)) then
			Engine.Shared.Echo("Lava Advanced Tide Rhythm data is not valid, negative or zero values: ", partRhythm)
			return false
		end
		table.insert(advancedRhythm, partRhythm)
	end
	if next(advancedRhythm) == nil then
		Engine.Shared.Echo("Lava Advanced Tide Rhythm data is empty")
		return false
	else
...
local function lavaModGen(modOptions)
	...
	if tweakLavaRaw ~= "" and tweakLavaRaw ~= "0" then
		...
		if advancedRhythm then
			...
			Engine.Shared.Echo("Lava Advanced Tide Rhythm data is not valid, using default values")
...
local modTideRhythm = (Engine.Shared.GetModOptions().map_waterislava and Engine.Shared.GetModOptions().map_lavatiderhythm) or "default"

if mapLavaConfig and not voidWaterMap then
	applyConfig(mapLavaConfig)
	if modTideRhythm == "enabled" then
		lavaModGen(Engine.Shared.GetModOptions())
...
elseif Engine.Shared.GetModOptions().map_waterislava and not voidWaterMap then
	isLavaMap = true
	if modTideRhythm == "enabled" then
		lavaModGen(Engine.Shared.GetModOptions())
