...
function Hooks.New(config, behaviourByID)
	return {
		...
		---A queen arrives fresh, and the first one brings a miniboss out of
		---every burrow the spawn chance favours (the director already sends
		---each burrow's escort).
		...
		---A queen that is not staggered calls her brood, in bursts the
		---roster sizes.
		---@param bossID integer
		---@param state WaveDirectorState
		...
		---Spawners (matriarchs, broods) call minions. The chance is divided
		---by how many of that def are alive, so a swarm of spawners does not
		---multiply.
		---@param unitID integer
		---@param defID integer
		---@param state WaveDirectorState
