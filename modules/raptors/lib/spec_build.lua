...
-- Squad lifetimes, in waves. Raptors keep every specialist for the same
-- long stretch; only the base squad turns over each wave.
local SQUAD_LIFE = 10
local SPECIALIST_LIFE = 100

-- Raptors have no tier table; this is where the wheel stops treating the
-- game as "still opening".
local OPENING_ANGER = 5

-- Legacy pool -> director bucket. Raptors are land-only: a sea map gets the
-- land roster spawned on land, as it always did.
...
function SpecBuild.Build(opts)
	...
	if bossCount > 0 then
		boss = {
			...
			-- The legacy bank drained against the plain queen count.
