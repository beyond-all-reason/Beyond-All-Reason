...
-- Frames between the last queen dying and the raptor team being killed.
...
---Raptor allies are not players: a human on the raptors' ally team is moved
---onto the raptor team itself, and any second AI there is killed off.
...
---The hunt is over: the raptor team dies, and its whole ally team with it
---unless a live scavengers AI is still fighting there.
...
function gadget:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID)
	...
	if unitDefID == config.burrowDef then
		-- A razed hive answers with a turret wave, at the roster's own odds.
		if math.random() <= config.spawnChance then
			GG.Waves.SpawnStructures(DIRECTOR)
		end
	elseif unitDef.isBuilding and byPlayer then
		-- A raptor structure razed is a tenth of a hive's worth of anger.
...
---directions, and no raptor hurts another unless its def is allowed to.
