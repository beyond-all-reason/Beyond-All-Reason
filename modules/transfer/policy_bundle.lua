...
local Serializers = {
	["unit.allow"] = function(p, lock)
		return {
			[UNIT_KEY[p.tier or 0]] = {
				value = p.category or ModeEnums.UnitFilterCategory.All,
	...
	["resource.tax"] = function(p, lock)
		-- The policy takes the resource, but the transport does not yet: there
		-- is one tax modoption per tier and none per resource. Rather than
		-- write a metal rate into the key that prices both, say so.
		assert(
			p.category == nil,
	...
	["reclaim.allow"] = function(_p, lock)
		return {
			[Opt.AlliedUnitReclaimMode] = { value = ModeEnums.AlliedUnitReclaimMode.Enabled, locked = lock.structure },
		}
	end,
	["reclaim.deny"] = function(_p, lock)
		return {
			[Opt.AlliedUnitReclaimMode] = { value = ModeEnums.AlliedUnitReclaimMode.Disabled, locked = lock.structure },
		}
	end,
	["resurrect.allow"] = function(_p, lock)
		return {
			[Opt.AllowPartialResurrection] = {
