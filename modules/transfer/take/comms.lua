...
local categoryDisplayNames = {
	...
	[ModeEnums.UnitCategory.Buildings] = "buildings",
	[ModeEnums.UnitCategory.Constructors] = "constructors",
	[ModeEnums.UnitCategory.Resource] = "resource",
	[ModeEnums.UnitCategory.NonCombat] = "non-combat",
...
---Resolve the global take policy; declared as a named stage in
---policies/take.lua, so the mode grammar and this agree by construction.
---@param modOptions table
---@return TakePolicy
function Comms.GetPolicy(modOptions)
	local ModuleHandler = VFS.Include("modules/module_handler.lua")
	return ModuleHandler.Evaluate(ModuleHandler.LoadPolicies("transfer").take, { modOptions = modOptions })
end
...
function Comms.FormatMessage(result)
	...
	if result.mode == ModeEnums.TakeMode.StunDelay then
		if result.stunned > 0 then
			return string.format("%s took %d units and resources from %s; %d %s units stunned for %ds", taker, result.transferred, source, result.stunned, Comms.CategoryDisplayName(result.category), result.delaySeconds)
		end
		return string.format("%s took %d units and resources from %s", taker, result.transferred, source)
	end
	...
	if result.mode == ModeEnums.TakeMode.TakeDelay then
		if result.isSecondPass then
			return string.format("%s took remaining %d %s units and resources from %s", taker, result.transferred, Comms.CategoryDisplayName(result.category), source)
		end
		if result.remainingSeconds then
			return string.format("%s: %ds remaining before %s's %s units can be taken", taker, result.remainingSeconds, source, Comms.CategoryDisplayName(result.category))
		end
		return string.format("%s took %d/%d units from %s; %d %s units available for /take in %ds", taker, result.transferred, result.total, source, result.delayed, Comms.CategoryDisplayName(result.category), result.delaySeconds)
	end
