...
function gadget:GetInfo()
	if Game.nativeExcessSharing ~= false then
		Spring.Echo(
			"ERROR: Resource Transfer Controller requires nativeExcessSharing=false (Lua-owned resource sharing); economy GG API unavailable"
		)
	end
	return {
...
local function ProcessEconomy(frame)
	...
	local results =
		ManualShareLedger.FoldInto(WaterfillSolver.SolveToResults(springRepo, teams, SharedConfig.getTeamTaxRate))
	...
	-- same tick, so the factors read post-redistribution currents
...
function gadget:Initialize()
	if not spAddTeamResourceExcessStats then
		Spring.Echo(
			"ERROR: Resource Transfer Controller requires Spring.AddTeamResourceExcessStats (engine resource-excess-callin + excess-stats port); excess stats will be unavailable"
		)
	end
...
if TechBlockingShared.AnyTaxConfigured(modOptions) then
	function gadget:AllowUnitBuildStep(builderID, builderTeam, unitID, unitDefID, part)
		...
		if
			not spAreTeamsAllied(builderTeam --[[@as integer]], unitTeam)
		then
			return true
		end
