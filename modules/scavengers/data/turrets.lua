--- Creep structures by tier: what the director builds on ground it holds.
---
--- type gates them against the unit restrictions (no-air kills antiair, and
--- so on for nukes and long-range artillery). A fractional spawnedPerWave is a
--- per-wave CHANCE of one, not a fraction of a turret. Never list the same
--- turret twice.
