--- Synced contract of the scavengers module.
---
--- One verb that matters: turn a pack name into a running director. The
--- multiplayer gadget does not come through here — it has a bot, a discovered
--- team and a host's modoptions, and starts itself. This is the path for
--- everything else, which today means a mission asking for pressure.
---
--- The two paths share the roster, the spec builder and the hooks. What
--- differs is only what a mission has to say for itself: who the pressure is
--- aimed at, and where it comes from.
...
---The team the waves spawn for: the first team that is not Gaia and is not
---allied with the team being attacked. Failing that, Gaia — which is always
---present and is nobody's ally, so a mission with no second team still gets
---its pressure.
...
---How many teams the pressure is aimed at.
---@param againstAllyTeam integer
---@return integer
...
---@return table<string, WavePackRef>
api.Packs = function()
	return Packs.Nouns
end
...
---Build the spec for a pack, ready to hand to GG.Waves.Start.
---
---Separate from Start so a caller can inspect it. The mission loader does
---not today, but a spec you cannot look at is a spec you cannot debug.
...
api.BuildSpec = function(request)
	...
	if UnitDefNames[BEACON_DEF] == nil then
		-- On the screen, not only in the log. This is the difference between a
		-- mission that visibly refuses and one that silently never spawns
		-- anything, and the silent version cost a day of looking at placement
		-- code that was working correctly the whole time.
		Spring.Echo("[scavengers] NO SCAVENGERS: this game has no scavenger unit defs."
...
---Build and start in one call — what a mission's Waves.Begin resolves to.
---@param request ScavengersStartRequest
