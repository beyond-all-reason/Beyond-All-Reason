...
-- A quarter of the map per side: big enough for the placement cascade to find
-- ground, small enough that "from the northeast" still means the northeast.
...
api.BuildSpec = function(request)
	...
	-- The purple defs are derived only when the game asks (a scavengers AI,
	-- ruins/forceallunits/zombies); a mission must say so in its game setup.
