...
-- Declared so a typo in a data file's `type` field is a nil index at build time, not silence.
local BUCKETS = {
	"basicLand",
	"basicSea",
	"basicAirLand",
...
---Sorted keys: a roster whose contents depend on table iteration order can differ between
---two clients of the same game.
---@param t table
---@return any[]
...
-- Sea squads travel at a quarter and a half: a fleet at full size cannot path out of a bay.
local EXPANSIONS = {
	{
		list = "land",
		role = "Raid",
		variants = {
			{ bucket = "basicLand", size = full },
			{ bucket = "specialLand", size = double },
		},
	},
	{
		list = "land",
	...
	},
	{
		...
		role = "Raid",
		variants = {
			{ bucket = "basicSea", size = quarter },
			{ bucket = "specialSea", size = half },
		},
	},
	{
		list = "sea",
...
function DefsBuild.Build(opts)
	...
	-- Copied: a second Build must not inherit the first one's role-expansion additions.
	...
	-- Flattened onto the config because every reader addresses it as config.gracePeriod.
