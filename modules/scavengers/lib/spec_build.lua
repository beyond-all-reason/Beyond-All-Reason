--- The seam: a scavengers config on one side, a WaveSpec on the other.
---
--- This is the whole of what makes scavengers a FLAVOR rather than a
--- spawner. The director knows anger clocks and squad composition; it does
--- not know what a beacon is, that commanders come in real and decoy pairs,
--- or that the boss staggers. Those live here — as data in the spec, and as
--- hooks the director calls at the moments it cannot decide alone.
---
--- Pure: the config and the discovered teams arrive as arguments, so a spec
--- can be built and inspected without a game running.
...
---Translate the config's squad pools into the director's vocabulary.
---
---One field differs — the config says `unit`, the director says `def` — and
---the pools hold thousands of entries. Since a squad appears once per weight
---point BY REFERENCE, translating per distinct table instead of per entry
---keeps this proportional to the number of squads rather than to their
---total weight.
...
---Fold the config's six behaviour categories into one record per unit-def id.
---
---The director asks one question — "what does this unit do?" — and gets one
---answer. The categories are the roster's way of writing it; this is the
---director's way of reading it.
...
---The endless ladder in the director's shape. One field is renamed and the
---rest travel as they are.
...
function SpecBuild.Build(opts)
	...
	local params = {
		...
		-- Read by the spawn drain and the squad manager.
		squadLife = SQUAD_LIFE,
		spawnTimeMultiplier = modOptions.scav_spawntimemult or 1,
		-- Scav-only, read by the flavor's own gadgets.
