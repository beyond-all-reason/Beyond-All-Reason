...
--- The seam between the roster and the director. What is asserted here is
--- that a scavengers config crosses it without losing anything the director
--- needs — and that the numbers on the other side are the ones the monolith
--- computed at the same points in its own code.
...
describe("scavengers spec_build", function()
	describe("the clocks", function()
		...
		it("divides the boss hour by the boss dial the way the roster already had it", function()
			-- Faithful to a quirk worth keeping visible: the config's boss time
			-- ALREADY has the host's multiplier in it, and the tech clock
			-- divides by that multiplier again. The two cancel over the boss
			-- ramp and leave grace scaled — so the reference is
			-- bossRamp + grace/multiplier, and a host who halves the boss time
			-- gets a marginally SLOWER roster clock, not a faster one.
	...
	describe("the wave envelope", function()
		it("scales with team count the way the roster says", function()
			local single = build()
	...
	describe("the buckets", function()
		...
		it("shares one translated unit list per squad, not one per weight point", function()
			...
			-- A weighted squad appears many times by reference; translating
			-- per entry instead of per squad would allocate thousands of
			-- duplicate tables at every Start.
			local distinct = {}
			...
			assert.is_true(count < #pool, "expected shared unit tables, got " .. count .. " for " .. #pool .. " entries")
		end)
	end)
	...
	describe("behaviours", function()
		it("fold the roster's categories into one record per unit", function()
			...
			-- Berserk is a REACTION, not a role: charging what hit you is
			-- handled by the flavor's own damage callin, and it says nothing
			-- about how the squad moves. So it produces no director record.
