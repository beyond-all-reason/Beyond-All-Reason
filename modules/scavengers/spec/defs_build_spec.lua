...
local function squadKey(entry)
	...
	table.sort(units)
	return table.concat({
		tostring(entry.minAnger), tostring(entry.maxAnger), tostring(entry.weight), table.concat(units, ","),
	}, "|")
end
	...
	for _, difficulty in ipairs({ "normal", "epic" }) do
		describe("against the frozen roster on " .. difficulty, function()
			...
			it("flattens the same live values onto the config", function()
				for _, key in ipairs({
					"gracePeriod", "bossTime", "scavSpawnRate", "burrowSpawnRate", "turretSpawnRate",
					"bossSpawnMult", "angerBonus", "maxXP", "spawnChance", "damageMod", "healthMod",
					"maxBurrows", "minScavs", "maxScavs", "scavPerPlayerMultiplier", "bossName",
					"bossResistanceMult",
				}) do
			...
			it("carries the same scalar settings", function()
				for _, key in ipairs({
					"useScum", "useWaveMsg", "spawnSquare", "spawnSquareIncrement", "burrowSize",
					"bossFightWaveSizeScale", "defaultScavFirestate", "airStartAnger",
					"scavSpawnMultiplier", "burrowSpawnType",
				}) do
					assert.are.equal(legacy[key], built[key], "config." .. key)
				end
			...
			it("fills every pool — an empty pool would make the comparisons below vacuous", function()
				for _, pool in ipairs(POOLS) do
					assert.is_true(#legacy.squadSpawnOptionsTable[pool] > 20,
