...
local function taxRateFor(_, teamId)
	...
	local terms = ModuleHandler.Enrich("economy", "distribution", springRepo.GetModOptions(), ctx)
...
local function amended(results)
	...
	local amendedResults = ModuleHandler.Enrich("economy", "redistribution", springRepo.GetModOptions(), ctx)
