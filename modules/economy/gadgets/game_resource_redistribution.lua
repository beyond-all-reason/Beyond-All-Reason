...
local function taxRateFor(_, teamId)
	...
	for _, provision in ipairs(ModuleHandler.LoadEnrichers("economy", "distribution")) do
		local results = { provision.evaluate(ctx) }
		for i, field in ipairs(provision.names) do
			if field == Contract.Distribution.TaxRate and results[i] ~= nil then
				return tonumber(results[i]) or 0
			end
		end
	end
	return 0
...
local function amended(results)
	...
	for _, provision in ipairs(ModuleHandler.LoadEnrichers("economy", "redistribution")) do
		local out = { provision.evaluate(ctx) }
		for i, field in ipairs(provision.names) do
			if field == Contract.Redistribution.Results and out[i] ~= nil then
				results = out[i]
				ctx.results = results
			end
		end
