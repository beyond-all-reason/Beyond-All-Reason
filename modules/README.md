...
* A module is a directory that answers questions for one concern, like "may this transport pick that up."
* A policy is a file under `policies/`: the pipelines a module builds, its own or another module's, each opened with `Policies.On(...)`.
* A pipeline is one rule: a pure function that answers one question, written as an ordered list of named steps, each of which can refuse, pass, or answer. No step is a rule by itself.
* A contract is the list of those step names and the facts the pipeline reads, so another module or a mod can say "put my step after this one" or "replace that one."
* A context is the plain table of facts the gadget hands the pipeline when it asks: who, where, what the modoptions say.
* A mode is a preset of modoptions that says which dials are locked or hidden.
...
Each module owns one concern and answers its questions through pipelines in its own contract. What each owns, and what it requires:

| Module | Owns | Requires |
|---|---|---|
| `defs` | Def post-processing as a fold: what the base game does to every unit and weapon def, and where a module adds its own. | the runtime |
| `game` | Which game this is: the game axis, one selector, the presets, the export the lobby reads. | [in review](https://github.com/beyond-all-reason/Beyond-All-Reason/pull/9111) |
...
Modules land one at a time, each with its own contract, policies and specs; a candidate is a concern with a name and no code yet.
...
**Order is precedence.** No guard ever says yes; each can only refuse and otherwise hand the question on. So no step is a rule by itself. The whole statement is the rule, read top to bottom, and "yes, regardless of the rest" is a matter of placement, not a verb. The load pipeline with one Answer added (and a `Stunned` stage added to the contract, for demonstration only):

```lua
Policies.On(load)
	.Unless(load.Submerged, submerged)
	.Answer(load.Stunned, function(ctx)
		if ctx.stunned then
...
The manifest. Names the module (it must match the directory) and lists what it requires. No manifest, no module: any other directory under `modules/` is ignored.
```lua
return { name = "defs", description = "Def post-processing as pipelines", requires = {} }
```

**`contract.lua`**

The one file to read to know what a module decides, which of those decisions others may change, and which facts it takes from them. Declares each pipeline's step names, how its steps combine, and the context it reads. see **In contract.lua** below.
...
`modules/transport/policies/transport.lua`
...
`modules/transfer/actions/units.lua`

**`state.lua`**

What the module keeps in memory, declared once as a class and anchored once per Lua state. A file-level table that is written after load lives here, never in a `local`: `VFS.Include` is uncached, so a local is one copy per includer. see **In a gadget, widget or lib** below.
...
```lua
Defs.PrebakeUnitDefs()` from `gamedata/unitdefs_post.lua
```

**`lib/`**

The module's own helpers. Ordinary include paths, no loader involvement.
`modules/defs/lib/base.lua`

**`enums.lua`**

The module's names as values, so nothing refers to them by string. `modules/enums.lua` at the root is the enum of modules themselves.
```lua
Modules.Defs`, `TransportEnums.ModOptions.CommanderTransportSlow
```

**`modes/`**

Presets, one file each, written in the mode grammar. A preset makes the module that ships it live.
`modules/game/modes/ffa.lua`
...
`modules/transport/gadgets/transport_rules.lua`
...
**`PolicyBuilder.Contract(Modules.X, { ... })`** · `(Modules, table) → Contract`

Stamps every member with its owner and category, so a pipeline's identity travels with its stage enum wherever it is included. An optional third argument, `function(Policies)`, is the module's policy inline; the loader runs it like a file under `policies/`, and including the contract never does.
```lua
return PolicyBuilder.Contract(Modules.Defs, { UnitDef = PolicyBuilder.Fold(UnitDef) })
```

**`Single(stages)`** · `stages → PolicyStages<C, T>`

A list of guards ending in an Answer. Guards refuse; the first Answer that answers ends it. For a question with one answer: a yes or no, or a value. Most pipelines.
```lua
Load = PolicyBuilder.Single(Load)
```

**`Product(stages)`** · `stages → PolicyStages<C, number>`

Every stage is an Answer returning a number, and the numbers multiply. Nothing refuses; no factor at all is a broken owner and an error at the ask. For a modifier: loaded speed is the base speed times the commander drag.
```lua
LoadedSpeed = PolicyBuilder.Product(LoadedSpeed)
```

**`Fold(stages)`** · `stages → PolicyStages<C, C>`

Every stage is an Answer that receives one context, may edit it, and passes it on. Nothing refuses, nothing ends early, the context comes back. For post-processing: every unit def goes through every stage.
```lua
UnitDef = PolicyBuilder.Fold(UnitDef)
```

**`Facts(names)`** · `names → PolicyFacts<C>`

Not a pipeline. The facts a decision reads, which other modules may fill before the pipeline is asked. A fact informs a decision; it is not the decision.
```lua
TeamPairing = PolicyBuilder.Facts({ TaxRate = "taxRate" })
```

**`Contributes(target, names)`** · `(PolicyStages, names) → names`
...
The shape of the facts a pipeline reads, declared beside the stages, so `ctx` is typed inside every predicate without an annotation on the predicate.
```lua
---@class DefContext` with `---@field def table
```
...
return PolicyBuilder.Contract(Modules.Defs, {
	...
	-- the policy, inline: the loader runs this with the registrar; including the contract never does
...
**`Policies.On(stages)`** · `PolicyStages<C, T> → PolicyPipeline<C, T>` · `PolicyFacts<C> → PolicyEnrichment<C>`
...
**`.Unless(stage, fn)`** · `(C → bool) → Step<C, T>`
...
```lua
.Unless(load.Submerged, function(ctx) return ctx.submerged end)
```

**`.If(stage, fn)`** · `(C → bool) → Step<C, T>`
...
**`.Answer(stage, fn)`** · `(C → T?) → Step<C, T>`

The only step that can answer. Returns the pipeline's result, or nil to pass to the next step. If every Answer declines, the pipeline refuses: nothing said yes is a no. A Single pipeline always ends in one.
```lua
...
```lua
.Refusal(function(ctx) return { may = false, reason = "submerged" } end)
```

**`.Before(stage)`, `.After(stage)`** · `Step → Step`

Where the step just added goes. Without either, a new step joins the end of the checks, just before the answer.
```lua
.Unless(Mine.NoTanks, isTank).Before(load.Submerged)
```

**`.Replace(stage, fn)`** · `(C → T?) → Step<C, T>`

Swap the closure under an existing name, keeping its position.
```lua
.Replace(load.MovingEnemy, function() return false end)
```
...
**`Policies.On(facts)`** · `PolicyFacts<C> → PolicyEnrichment<C>`
...
**`.Provide(fact, fn)`** · `(C → V?) → Provision<C>`
...
**`.Default(fact, fn)`** · `(C → V) → Provision<C>`
...
A module by name, from `modules/enums.lua`. Code never names a module by string.
```lua
local Modules = VFS.Include("modules/enums.lua").Modules
```

**`ModuleHandler.LoadPolicies(Modules.X)`** · `Modules → { [category]: AssembledPipeline<C, T> }`

The module's assembled pipelines, keyed by data-case category, every contributor's steps applied. Read once at file scope.
```lua
local pipelines = ModuleHandler.LoadPolicies(Modules.Transport) ---@type TransportPipelines
```

**`ModuleHandler.Evaluate(pipeline, ctx, ...)`** · `(AssembledPipeline<C, T>, C) → T | false`

Asks. Runs the steps in order under the contract's strategy and returns the result, or the refusal.
```lua
return ModuleHandler.Evaluate(pipelines.load, ctx) == true
```

**`ModuleHandler.Enrich(facts, modOptions, ctx, ...)`** · `(PolicyFacts<C>, modOptions, C) → { [fact]: V }`
...
**`ModuleHandler.LoadActions(Modules.X)`** · `Modules → { byName, list }`
...
**`ModuleHandler.State(Modules.X)`** · `Modules → table`
...
**`Actions.RegisterValidate(fn)`, `Actions.RegisterExecute(fn)`** · `(request → bool, string?)` · `(request → result)`
