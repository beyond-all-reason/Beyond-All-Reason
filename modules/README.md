...
| `modes/` | Presets, one file each, written in the mode grammar. A preset makes the module that ships it live. | `modules/game/modes/ffa.lua` |
| `modoptions.lua` | The module's fragment of the game's options. The root `modoptions.lua` appends every module's fragment, so a module that ships options needs no change to the root file. | `modules/game/modoptions.lua` |
