...
local commanderDrag = modOptions[ModeEnums.ModOptions.CommanderTransportSlow] == true
...
local STEALTHY_TRANSPORTS = { "armdfly" }
local NANOS = { "cornanotc", "armnanotc" }
if modOptions.experimentallegionfaction then
	NANOS[#NANOS + 1] = "legnanotc"
end

---@param name string
---@param roots string[]
---@return boolean
local function namedAfter(name, roots)
	for _, root in ipairs(roots) do
		if string.find(name, root, 1, true) then
			return true
		end
	end
	return false
...
for unitDefID, unitDef in pairs(UnitDefs) do
	...
	stealthyTransport[unitDefID] = namedAfter(unitDef.name, STEALTHY_TRANSPORTS) or nil
	isNano[unitDefID] = namedAfter(unitDef.name, NANOS) or nil
