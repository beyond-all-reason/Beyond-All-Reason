...
def _pick_nested(groups, size):
    """
    Choose one primary packing per slot size such that the packings nest:
    every primary slot of size 2c is exactly tiled by two primary slots of
    size c, and no primary c-slot straddles a primary 2c-slot's edge.

    With that structure, filling primaries in descending unit size can never
    strand seats (the buddy-allocator argument), which is what
    ReorganizeAndLoad relies on. Returns {name: True} for primaries, or None
    when no nested packing exists.
    """
    beam_index = {}
    for group in groups.values():
        for l in group:
            for b in l['beams']:
                beam_index.setdefault(b, len(beam_index))
    mask = {l['name']: sum(1 << beam_index[b] for b in l['beams'])
            for group in groups.values() for l in group}
    classes = sorted(groups, reverse=True)
    primary = {}
    ...
    def pick_class(ci):
        ...
        c = classes[ci]
        cap = size // c
        parents = [mask[n] for n in primary if n.startswith(f'{classes[ci - 1]}_')] if ci else []
        names = sorted(l['name'] for l in groups[c])
        chosen = []

        def rec(i, used, count):
            if count == cap:
                if any((p & used) != p for p in parents):
                    return False
                for n in chosen:
                    primary[n] = True
                if pick_class(ci + 1):
                ...
                for n in chosen:
                    del primary[n]
                return False
            if i == len(names) or count + (len(names) - i) < cap:
                return False
            m = mask[names[i]]
            ok = (m & used) == 0
            if ok:
                for p in parents:
                    inter = m & p
                    if inter and inter != m:
                        ok = False
                        break
            if ok:
                chosen.append(names[i])
                if rec(i + 1, used | m, count + 1):
                    return True
                chosen.pop()
            return rec(i + 1, used, count)

        return rec(0, 0, 0)

    return primary if pick_class(0) else None


def build_pad_defs(links_per_size):
    """
    Returns dict  size → {links: [...sorted...], requires: {name: [...]}}
    Sorted: largest slot first, primary (non-overlapping) first within each
    slot size. Every link carries 'overlapping': False for the primaries of
    the nested packing, True for the alternates that share beams with them.
    """
    pad_defs = {}
    for size in range(1, MAX_SIZE + 1):
        ...
        primary = _pick_nested(groups, size)
        if primary is None:
            print(f"WARNING: size{size}: no nested packing, falling back to greedy")

        ordered = []
        for slot_size in sorted(groups, reverse=True):
            group = sorted(groups[slot_size], key=lambda l: l['name'])
            if primary is None:
                selected, rest = _pick_non_overlapping(group, size // slot_size)
            else:
                selected = [l for l in group if l['name'] in primary]
                rest = [l for l in group if l['name'] not in primary]
            for l in selected:
                l['overlapping'] = False
            for l in rest:
                l['overlapping'] = True
...
def generate_lua_defs(pad_defs, footprints, output_path):
    lines = [
        '-- Generated by universalPadGenerator.py along with the s3o pattern',
    ...
    for size in range(1, MAX_SIZE + 1):
        ...
        for l in links:
            ...
            if req:
                ...
                req_lua = '{}'
            flag = ', overlapping = true' if l.get('overlapping') else ''
            lines.append(
                f'                {{ name = "{l["name"]}", size = {l["size"]}, requires = {req_lua}{flag} }},')
        lines.append(f'            }},')
        lines.append(f'        }},')
...
def main():
    ...
    defs_path = os.path.join(base, 'loadPadDefs.lua')
