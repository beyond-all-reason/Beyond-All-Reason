...
local function validateInstanceVBOTable(iT, calledfrom)
	...
	if (indextoInstanceIDsize ~= instanceIDtoIndexsize) or (instanceIDtoIndexsize ~= indextoUnitID) then
		Spring.Echo(
			"Table size mismatch during validation of",
			iT.myName,
			indextoInstanceIDsize,
...
local function popElementInstance(iT, instanceID, noUpload)
	...
	if oldElementIndex == lastElementIndex then
		...
		if lastElementInstanceID == nil then --
			Spring.Echo(
				"We somehow have a nil element at the back of the array, which is completely invalid, probably about to crash",
				iT.myName
			)
		...
		if iT.indextoUnitID then
			...
			if popunitID == nil then
				Spring.Echo(
					"TODO: what the f is happening here?, how the f could we have popped a nil from the back of?",
					iT.myName
				) -- TODO TODO
			end
...
local function MakeTexRectVAO(minX, minY, maxX, maxY, minU, minV, maxU, maxV, name)
	...
	minX, minY, maxX, maxY, minU, minV, maxU, maxV =
		minX or -1, minY or -1, maxX or 1, maxY or 1, minU or 0, minV or 0, maxU or 1, maxV or 1
