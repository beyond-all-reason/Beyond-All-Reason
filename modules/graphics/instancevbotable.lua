...
local function resizeInstanceVBOTable(iT)
	...
	if iT.indextoUnitID then
		...
		for i, objectID in ipairs(iT.indextoUnitID) do
			...
			if iT.featureIDs then isValidID = SpringShared.ValidFeatureID(objectID)
			else isValidID = SpringShared.ValidUnitID(objectID) end
			if isValidID then
...
local function pushElementInstance(iT,thisInstance, instanceID, updateExisting, noUpload, unitID)
	...
	if unitID ~= nil then
		local isvalidid
		if iT.featureIDs then isvalidid = SpringShared.ValidFeatureID(unitID)
		else isvalidid = SpringShared.ValidUnitID(unitID) end
