local function makeInstanceVBOTable(layout, maxElements, myName, objectTypeAttribID, objecttype)
	...
	if newInstanceVBO == nil then
		Engine.Shared.Echo("makeInstanceVBOTable, cannot get VBO for", myName)
		return nil
	end
...
local function comparetables(t1, t2, name)
	for k, v in pairs(t1) do
		if t2[k] == nil then
			Engine.Shared.Echo("Key ", k, "with value", v, "existing in t1 does not exist in t2 in ", name)
		elseif t2[k] ~= v then
			Engine.Shared.Echo("Value ", v, "for", k, "existing in t1 does not match value for t2", t2[k], " in ", name)
		end
	end

	for k, v in pairs(t2) do
		if t1[k] == nil then
			Engine.Shared.Echo("Key ", k, "with value", v, "existing in t2 does not exist in t1 in ", name)
		elseif t1[k] ~= v then
			Engine.Shared.Echo("Value ", v, "for", k, "existing in t2 does not match value for t1", t1[k], " in ", name)
		end
	end
end
...
local function dbgt(t, name)
	...
	Engine.Shared.Echo(tostring(gf) .. " " .. name .. " #" .. tostring(count) .. " {" .. res .. "}")
	return res
end
...
local function resizeInstanceVBOTable(iT)
	...
	for i, unitID in ipairs(iT.indextoUnitID) do
		...
		if objecttype == "unitID" then
			...
			if Engine.Shared.ValidUnitID(unitID) ~= true then
				Engine.Shared.Echo("Invalid unitID", unitID, "at", i, "during resizing", iT.myName)
			else
		...
		elseif objecttype == "featureID" then
			if Engine.Shared.ValidFeatureID(unitID) ~= true then
				Engine.Shared.Echo("Invalid featureID", unitID, "at", i, "during resizing", iT.myName)
			else
...
local function pushElementInstance(iT, thisInstance, instanceID, updateExisting, noUpload, unitID, objecttype, teamID)
	...
	if (objecttype == "unitID" and (Engine.Shared.ValidUnitID(unitID) == false)) or (objecttype == "featureID" and (Engine.Shared.ValidFeatureID(unitID) == false)) or (objecttype == "unitDefID" and (UnitDefs[unitID] == nil)) or (objecttype == "featureDefID" and (FeatureDefs[unitID] == nil)) then
		Engine.Shared.Echo("Tried to push invalid", objecttype, unitID, "into", iT.myName, "with instanceID", instanceID, "returning nil")
		return nil
	end

	if #thisInstance ~= iT.instanceStep then
		Engine.Shared.Echo("Trying to upload an oddly sized instance into", iT.myName, #thisInstance, "instead of ", iT.instanceStep)
	end
	...
	if thisInstanceIndex == nil then -- new, register it
		...
		if updateExisting == nil then
			Engine.Shared.Echo("Tried to add existing element to an instanceTable", iT.myName, instanceID)
			return nil
		else
...
local function popElementInstance(iT, instanceID, noUpload)
	...
	if iT.instanceIDtoIndex[instanceID] == nil then -- if key is instanceID yet does not exist, then warn and bail
		Engine.Shared.Echo("Tried to remove element ", instanceID, "From instanceTable", iT.myName, "but it does not exist in it")
		return nil
	end
	if iT.usedElements == 0 then -- Dont remove the last element
		Engine.Shared.Echo("Tried to remove element ", instanceID, "From instanceTable", iT.myName, "but it should be empty")
		return nil
	end
		...
		if noUpload ~= true then
			...
			if iT.indextoUnitID then
				...
				if objecttype == "unitID" or objecttype == "featureID" then
					if objecttype == "unitID" and Engine.Shared.ValidUnitID(myunitID) then
						iT.instanceVBO:InstanceDataFromUnitIDs(myunitID, iT.objectTypeAttribID, oldElementIndex - 1)
					--iT.VAO:AddUnitDefsToSubmission(unitID)
					elseif objecttype == "featureID" and Engine.Shared.ValidFeatureID(myunitID) then
						...
						if iT.debugZombies then
							...
							Engine.Shared.Echo("Tried to pop back an invalid unitID ", myunitID, "from", iT.myName, "while removing instance", instanceID, ". Ensure that you remove invalid units from your instance tables")
							Spring.Debug.TraceFullEcho()
						end
					end
...
local function dumpAndCompareInstanceData(iT)
	...
	for i = 1, iT.usedElements do
		Engine.Shared.Echo(string.format("%s, instanceID =%s, element %d of %d", iT.myName, tostring(iT.indextoInstanceID[i]), i, iT.usedElements))
		local index = 1
...
local function validateInstanceVBOIDTable(iT, calledfrom)
	...
	for i = 1, iT.usedElements do
		if iT.indextoInstanceID[i] == nil then
			Engine.Shared.Echo(errorKey, "There is a hole in indextoInstanceID", iT.myName, "at", i, "out of", iT.usedElements, calledfrom)
			--Spring.Echo()
			if iT.indextoUnitID[i] == nil then
				Engine.Shared.Echo(errorKey, "It is also missing from indextoUnitID")
			else
				Engine.Shared.Echo(errorKey, "But it does exist in indextoUnitID with an unitID of ", iT.indextoUnitID[i])
				Engine.Shared.Echo(errorKey, "This is valid?", Engine.Shared.GetUnitPosition(iT.indextoUnitID[i]))
			end
		else
			local instanceID = iT.indextoInstanceID[i]
			local objecttype = iT.indextoObjectType[i]
			if iT.instanceIDtoIndex[instanceID] == nil then
				Engine.Shared.Echo(errorKey, "There is a hole instanceIDtoIndex", iT.myName, "at", i, " iT.instanceIDtoIndex[instanceID] == nil ")
			elseif iT.instanceIDtoIndex[instanceID] ~= i then
				Engine.Shared.Echo(errorKey, "There is a problem in indextoInstanceID", iT.myName, "at i =", i, "  iT.indextoInstanceID[instanceID] ~= i, it is instead: ", iT.indextoInstanceID[instanceID])
			end
	...
	if string.len(idsremoved) > 1 then
		Engine.Shared.Echo(errorKey, " removed invalid instanceIDs", idsremoved)
	end
	--[[
