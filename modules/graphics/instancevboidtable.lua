...
local function pushElementInstance(iT, thisInstance, instanceID, updateExisting, noUpload, unitID, objecttype, teamID)
	...
	if
		(objecttype == "unitID" and (Spring.ValidUnitID(unitID) == false))
		or (objecttype == "featureID" and (Spring.ValidFeatureID(unitID) == false))
		or (objecttype == "unitDefID" and (UnitDefs[unitID] == nil))
		or (objecttype == "featureDefID" and (FeatureDefs[unitID] == nil))
	then
		Spring.Echo(
			"Tried to push invalid",
			objecttype,
			unitID,
			"into",
			iT.myName,
	...
	if #thisInstance ~= iT.instanceStep then
		Spring.Echo(
			"Trying to upload an oddly sized instance into",
			iT.myName,
			#thisInstance,
			"instead of ",
			iT.instanceStep
...
local function popElementInstance(iT, instanceID, noUpload)
	...
	if oldElementIndex == lastElementIndex then -- EARLY OPT DEVILRY BAD!
		...
		if lastElementInstanceID == nil then
			...
			Spring.Echo(
				"popElementInstance: no instanceID recorded for last element",
				lastElementIndex,
				"of",
				iT.myName,
				"while removing",
		...
		if noUpload ~= true then
			...
			if iT.indextoUnitID then
					...
					if iT.lastpopgameframe == nil then
						...
						if iT.lastpopgameframe ~= gf then -- New gameframe
							...
							if iT.numZombies and iT.numZombies > 0 then -- WE HAVE ZOMBIES AAAAARGH
								...
								local s = 'Error: popElementInstance [string "LuaUI/  We have '
									.. tostring(iT.numZombies)
									.. " zombie units left over in "
									.. iT.myName
									.. " after removing:"
									.. tostring(instanceID)
									.. " zombies:"
				...
				if objecttype == "unitID" or objecttype == "featureID" then
					...
					elseif objecttype == "featureID" and Spring.ValidFeatureID(myunitID) then
						...
						if iT.debugZombies then
							...
							Spring.Echo(
								"Tried to pop back an invalid unitID ",
								myunitID,
								"from",
								iT.myName,
								"while removing instance",
								instanceID,
				...
				elseif objecttype == "featureDefID" then
					-- Element offset is the 4th argument (3rd is the palette index).
					iT.instanceVBO:InstanceDataFromFeatureDefIDs(
						myunitID,
...
local function dumpAndCompareInstanceData(iT)
	...
	for i = 1, iT.usedElements do
		Spring.Echo(
			string.format(
				"%s, instanceID =%s, element %d of %d",
				iT.myName,
				tostring(iT.indextoInstanceID[i]),
				i,
				iT.usedElements
			)
		)
		...
		for j, layout in ipairs(iT.layout) do
			...
			local layoutstr = string.format(
				"  ID=%d, name = %s, size = %d, type = %d, luaData = {%s}, gpuData={%s}",
				layout.id,
				layout.name,
				layout.size,
				layout.type or GL.FLOAT,
				luaData,
				gpuData
			)
...
local function validateInstanceVBOIDTable(iT, calledfrom)
	...
	for i = 1, iT.usedElements do
		if iT.indextoInstanceID[i] == nil then
			Spring.Echo(
				errorKey,
				"There is a hole in indextoInstanceID",
				iT.myName,
				"at",
				i,
				"out of",
			...
			if iT.instanceIDtoIndex[instanceID] == nil then
				Spring.Echo(
					errorKey,
					"There is a hole instanceIDtoIndex",
					iT.myName,
					"at",
					i,
			...
			elseif iT.instanceIDtoIndex[instanceID] ~= i then
				Spring.Echo(
					errorKey,
					"There is a problem in indextoInstanceID",
					iT.myName,
					"at i =",
					i,
					"  iT.indextoInstanceID[instanceID] ~= i, it is instead: ",
					iT.indextoInstanceID[instanceID]
				)
			end
