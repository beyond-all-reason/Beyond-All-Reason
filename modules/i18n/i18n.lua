...
local loadTranslationTable

local function loadTranslationValue(languageCode, composedKey, value)
	languageTranslations[languageCode][composedKey] = value
end

local function loadNestedTranslations(languageCode, composedKey, value)
	loadTranslationTable(languageCode, composedKey, value)
end

local function ignoreTranslationValue()
end

local translationValueHandlers = {
	string = loadTranslationValue,
	table = loadNestedTranslations,
}

loadTranslationTable = function(languageCode, currentContext, data)
	local composedKey
	for k,v in pairs(data) do
		composedKey = (currentContext and (currentContext .. '.') or "") .. tostring(k)
		local valueHandler = translationValueHandlers[type(v)] or ignoreTranslationValue
		valueHandler(languageCode, composedKey, v)
	end
end
...
local function loadLanguageFiles(languageCode)
	...
	for _, file in ipairs(languageFiles[languageCode] or {}) do
...
function i18n.loadWidgetLanguage(language)
