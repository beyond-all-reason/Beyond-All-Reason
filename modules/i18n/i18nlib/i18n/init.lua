...
function i18n.translate(key, data)
	...
	if (Engine.Unsynced.GetConfigInt("language_english_unit_names", 1) == 1) and key:sub(1, #"units.names.") == "units.names." then
		usedLocale = "en"
	end
	...
	for i = 1, #fallbacks do
		...
		if value then
			...
			if not missingTranslation[fallback] and not (fallback == "en" and data.default) then
				Engine.Shared.Log("i18n", "notice", '"' .. key .. '" is not translated in ' .. fallback)
