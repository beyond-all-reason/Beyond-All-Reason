...
local function loadPolicyFiles(vfsMode)
	...
	for _, name in ipairs(names) do
		local stagesPath = manifests[name].dir .. "policy_stages.lua"
			...
			for _, entry in ipairs(built) do
				if entry.kind == "pipeline" then
					...
					if identity == nil then
						error(
							filePath
								.. ": Policies.Pipeline needs the stages of a pipeline — a table from a module's policy_stages.lua"
						)
					end
					...
					if identity == nil or not identity.context then
						error(
							filePath
								.. ": Policies.Enrich needs a context token — a Context(...) table from a module's policy_stages.lua"
						)
					end
					local ops = chain.Build()
					if #ops == 0 then
...
---@param category string the context token's name in the owner's policy_stages.lua
---@param vfsMode string?
