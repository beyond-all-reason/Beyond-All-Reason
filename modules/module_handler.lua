...
---One policy file, run against a fresh facade: every Pipeline and Enrich it
---builds is collected, checked, and handed back with its identity. The file
---itself returns nothing; a returned value would be cached by the include
---shim and the registration lost on the next load.
---@param name string module name
---@param filePath string
---@param vfsMode string?
	...
	for _, entry in ipairs(built) do
		...
		if entry.kind == "pipeline" then
			...
			if identity == nil or not identity.facts then
				error(filePath .. ": Policies.Enrich needs facts — a Facts(...) table from a module's contract.lua")
			end
			local ops = chain.Build()
			if #ops == 0 then
...
local function loadPolicyFiles(vfsMode)
	...
	for _, name in ipairs(names) do
		...
		table.sort(files)
		for _, filePath in ipairs(files) do
			local fileChains, fileEnrichments = loadPolicyFile(name, filePath, vfsMode)
