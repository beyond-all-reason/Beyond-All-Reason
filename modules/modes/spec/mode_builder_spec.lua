...
describe("mode builder", function()
	...
	describe("Ranked", function()
		...
		it("prohibition is a policy: ranked_game pinned off, locked", function()
			local mode = grammar()("R").Ranked(false)
			assert.is_false(mode.allowRanked)
			assert.are.same({ value = false, locked = true }, mode.modOptions.ranked_game)
		end)

		it("prohibition unlocks like any policy", function()
			local mode = grammar()("R").Ranked(false).Unlocked()
			assert.is_false(mode.modOptions.ranked_game.locked)
		end)
	end)
end)
