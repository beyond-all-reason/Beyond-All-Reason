...
-- Neutrality is what makes attackers advance instead of parking: Weapon::AutoTarget
-- and MobileCAI::AutoGenerateTarget skip neutral units below fire-at-neutral.
...
---Hooks are only ever added, never removed with the ledger: the handler DEFERS
---its list update while iterating (callinDepth > 0) but the method field goes
---nil immediately, so the next gadget reached in that loop dies on it.
