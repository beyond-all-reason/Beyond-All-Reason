--- The combat_rules gadget: what protection actually does to a unit, and
--- which callins it takes part in. The gadget is loaded into a mocked
--- environment and driven through its own GG surface.
---
--- AllowWeaponTarget is the contract's sharp edge. It fires only for
--- weaponDefIDs some gadget registered with Script.SetWatchAllowTarget (a
--- per-handle flag with no refcount, LuaHandleSynced.cpp SetWatchDef), and
--- gadgetHandler:AllowWeaponTarget is last-writer-wins with no nil filter —
--- so a layer-100 participant overwrites every other gadget's priority while
--- it is loaded, and gets nothing for it. Combat stays out of that loop.
...
local function newGadget()
	local h = {
		units = {}, -- unitID -> alive
		neutral = {}, -- unitID -> current neutral flag
		paralyze = {}, -- unitID -> last paralyze damage set
		updated = {}, -- callin name -> UpdateCallIn count
		frame = 0,
	}
	...
	---The handler's call shape for the damage callin, or nil when unhooked.
...
describe("combat_rules", function()
	...
	describe("the damage floor", function()
		...
		it("forgets a dead unit's neutrality, so a recycled ID cannot inherit it", function()
			...
			-- The ID comes back as a different, non-neutral unit; a protection
			-- that opened before it existed must not restore the dead one's state.
			h.Combat.Protect(7)
			h.spawn(7)
			h.Combat.Unprotect(7)
			assert.is_false(h.neutral[7])
