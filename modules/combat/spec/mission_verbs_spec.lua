--- Combat's mission verbs, and the lifetime .Until is supposed to bound.
--- The tense is the whole point, so the statements run through the real
--- trigger engine and the real ledger: a release exists only once the
--- protection it bounds has been applied.
...
---One trigger file over a live engine, wired exactly as the loader wires it:
---the contribution's env, then the file's Finalize and the contribution's.
---`tick` is the evaluation cadence, `kill` a UnitDestroyed latch plus bus
---forward, `latch` the same death with no callin hooked.
...
describe("combat mission verbs", function()
	it("Protect builds an effect over the roster name", function()
		local log = {}
		local ctx = { Protect = function(name)
	...
	it("Unprotect builds the primitive Until sugars over", function()
		local log = {}
		local ctx = { Unprotect = function(name)
	...
	describe("the lifetime .Until bounds", function()
		it("arms nothing at load: the file's own statements are all that is armed", function()
			local h = newFile()
			h.When(Unit("hub").IsDestroyed())
				.Do(h.Combat.Protect(Unit("vip")).Until(Unit("boss").IsDestroyed()))
			h.Finalize()
		...
		it("the release polls: it arms after the loader fixed its callin hooks", function()
			-- Nothing in this file watches UnitDestroyed at load, so the loader
			-- hooks no forwarder for it and an event-driven release never runs.
			local go = { inputs = { "mission.objective_changed" }, evaluate = function()
				return true
			end }
			local h = newFile()
		...
		it("overlapping lifetimes on one unit release independently", function()
			local ends = {}
			local firstOver = { evaluate = function()
				return ends.first == true
			end }
			local secondOver = { evaluate = function()
...
describe("combat's mission DSL contribution", function()
	...
	it("has no load-time commit step, so a failed load leaves nothing behind", function()
		-- The loader's transaction rolls back the staging engine, not the
		-- modules that contribute to it: combat must not need rolling back.
		local registered = 0
