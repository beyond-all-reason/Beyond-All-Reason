...
local importCommandsToObject = function(object)
	if not globalCmdDeprecatedShown and not object.gadgetHandler then
		...
		Engine.Shared.Log("CMD", LOG.DEPRECATED, msg)
...
for code, cmdID in pairs(gameCommands) do
	if CMD[cmdID] then
		Engine.Shared.Log("CMD", LOG.ERROR, "Duplicate command id: " .. code .. " " .. tostring(cmdID) .. "!")
	end
	if CMD[code] then
		Engine.Shared.Log("CMD", LOG.ERROR, "Duplicate command code: " .. code .. " " .. tostring(cmdID) .. "!")
	end
