--- The engine half, driven with a fake map. Ground.New takes its checks
--- injected for exactly this reason: the decision about what makes a spot
--- usable is worth asserting without a game.
...
local function permissiveChecks()
	return {
		...
		FlatAreaCheck = function() return true end,
		OccupancyCheck = function() return true end,
	}
end
...
local function stubHeight(h)
	...
	Spring.GetGroundHeight = function() return h or 100 end
end

describe("what makes a spot usable", function()
	before_each(function() stubHeight(100) end)
	after_each(function() Spring.GetGroundHeight = savedHeight end)
	...
	it("refuses ground that is the wrong surface, and says so", function()
		...
		checks.LandOrSeaCheck = function() return "sea" end
	...
	it("refuses ground that is half in the water for land AND for sea", function()
		...
		local checks = permissiveChecks()
		checks.LandOrSeaCheck = function() return "mixed" end
		local ground = Ground.New({ positionChecks = checks })
	...
	it("\"solid\" takes land or sea, but never the shoreline between them", function()
		...
		for _, surface in ipairs({ "land", "sea" }) do
			checks.LandOrSeaCheck = function() return surface end
			assert.is_true(ground.Usable(0, 0, { footprint = 64, surface = "solid" }),
				surface .. " should satisfy solid")
		end
		for _, surface in ipairs({ "mixed", "death" }) do
			checks.LandOrSeaCheck = function() return surface end
			assert.is_false(ground.Usable(0, 0, { footprint = 64, surface = "solid" }),
	...
	it("only measures flatness when the caller asked for it", function()
		...
		local asked = 0
		local checks = permissiveChecks()
		checks.FlatAreaCheck = function() asked = asked + 1; return false end
		local ground = Ground.New({ positionChecks = checks })
	...
	it("defaults clearance to the footprint, not to something roomier", function()
		...
		local seen
		local checks = permissiveChecks()
		checks.OccupancyCheck = function(_x, _y, _z, r) seen = r; return true end
		local ground = Ground.New({ positionChecks = checks })
	...
	it("stops at the first refusal, so a cheap test never runs an expensive one", function()
		local occupancy = 0
		local checks = permissiveChecks()
		checks.MapEdgeCheck = function() return false end
		checks.OccupancyCheck = function() occupancy = occupancy + 1; return true end
		local ground = Ground.New({ positionChecks = checks })
