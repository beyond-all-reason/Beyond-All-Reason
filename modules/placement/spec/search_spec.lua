--- The search order is the contract, so it is asserted directly rather than
--- inferred from where units happened to land.
...
describe("placement search order", function()
	...
	it("never offers a candidate beyond the radius it was given", function()
		...
		assert.is_true(worst <= 300 * 300,
			"offered a spot outside the requested radius: " .. math.sqrt(worst))
	end)
