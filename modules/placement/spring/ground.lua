--- The engine half: what makes a spot usable, asked of the map.
---
--- Spring lives here. Nothing above spring/ may include this file.
---
--- The tests themselves are NOT reimplemented — luarules/Utilities/damgam_lib/
--- position_checks.lua already knows how to ask whether ground is flat, clear,
--- land or sea, and inside the map. This wraps them into one question with one
--- answer, and records WHICH test refused, because a caller that cannot place
--- something needs to be able to say why rather than shrug.
...
function Ground.New(deps)
	...
	--- Why a spot was refused, in the order the tests run. Counting rather than
	--- returning the first reason: over a whole search the counts say what is
	--- wrong with the AREA, which is the useful thing. "312 too steep, 4
	--- occupied" is a cliff; "0 too steep, 316 occupied" is a crowded base.
	---@return table
	function ground.NewTally()
		return { offMap = 0, steep = 0, occupied = 0, wrongSurface = 0, tried = 0 }
	end

	--- Is this a spot the caller's thing can stand on?
	---
	...
	function ground.Usable(x, z, want, tally)
		...
		if not checks.MapEdgeCheck(x, y, z, footprint) then
			if tally then tally.offMap = tally.offMap + 1 end
			return false
		end
		...
		if surface ~= "any" then
			...
			if not ok then
				if tally then tally.wrongSurface = tally.wrongSurface + 1 end
				return false
			end
		end
	...
	--- What the tally says, as a sentence a human can act on.
	---@param tally table
	---@return string
	function ground.Explain(tally)
		...
		return string.format(
			...
			tally.tried, tally.offMap, tally.wrongSurface, tally.steep, tally.occupied)
	end
