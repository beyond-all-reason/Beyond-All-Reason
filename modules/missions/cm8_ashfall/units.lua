...
Spawn(UnitDef("corllt"), "gaia").At(0.45, 0.40).Grouped("outpost_auto").Neutral()

Spawn(UnitDef("corrad"), "gaia").At(0.42, 0.39).Grouped("outpost_auto").Neutral()

Spawn(UnitDef("corsolar"), "gaia").At(0.39, 0.44).Grouped("outpost_auto").Neutral()

Spawn(UnitDef("corsolar"), "gaia").At(0.45, 0.44).Grouped("outpost_auto").Neutral()

-- Beats 4 and 6, the Armada enclave: two discoveries, in the order the
-- ground forces them. The beacon sits on the enclave's rim — spotting it
-- closes find_the_enclave; the device is deeper in, and spotting THAT
-- closes find_the_tenebrium_device. Killing the commander wins through
-- matchflow.

Spawn(UnitDef("armrad"), "enemy").At(0.72, 0.72).Named("enclave_beacon")

Spawn(UnitDef("armfus"), "enemy").At(0.75, 0.75).Named("tenebrium_device")
...
-- The player's own commander, claimed for the same reason: the game already
-- spawned it. Named so losing it can mean something — the mission owns the
-- verdict, and an unnamed commander's death is a fact no trigger can speak.
