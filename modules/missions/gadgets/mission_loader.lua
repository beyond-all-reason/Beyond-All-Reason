...
local forwarders = {
	UnitDestroyed = function(_, unitID, _, _, attackerID, _, attackerTeam)
		...
		if name ~= nil then
			...
			Spring.Log(LOG_TAG, LOG.INFO, "roster unit destroyed: " .. name
				.. (attackerTeam ~= nil and (" (attacker team " .. tostring(attackerTeam) .. ")") or " (no attacker)"))
		end
...
local function loadMission(missionName, preserve)
	...
	if VFS.FileExists(manifestPath) then
		...
		if ok and type(manifest) == "table" and type(manifest.sides) == "table" then
			...
			if want ~= nil and type(side) == "string" and side ~= ""
				and side:lower() ~= want.name:lower() then
				Spring.Echo("[" .. LOG_TAG .. "] " .. missionName .. " is written for a "
					.. want.name .. " player; this game's player is " .. side
...
function gadget:Initialize()
	...
	gadgetHandler:AddChatAction("mission", missionChatAction, "missions: /mission load <name> | /mission reload | /mission restart")
	rearmAfterLuaReload()
end
