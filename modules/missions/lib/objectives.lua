--- Declaration order gates REVEAL ONLY (presentation); completion gating is
--- game logic and stays explicit. A referenced id with no declaration is a
--- load error, exactly the roster's contract.
...
function Objectives.ForFile(filename, effects)
	...
	local Objective = function(id)
		...
		-- The handle carries its id, as a roster handle carries its name:
		-- what a spec reads the board by.
		local chain = { id = id }
		idByChain[chain] = id
		...
		---A gate on the whole objective: every way to complete it must also
		---find this true. Position-free, exactly as When is on a trigger.
		---@param condition MissionCondition
		---@return MissionObjectiveDeclaration
	...
	local Finalize = function(exports)
		...
		finalized = true
		-- Anything may export; an objective handle reads on the other side as
		-- the declaration it is, a bare reference to an undeclared id is still
		-- the error below.
		assert(
