--- The savegame rule: trigger progress lives in the engine's plain `state` table,
--- never in closures. Watcher index and dirty flags are DERIVED, never serialized.
...
function TriggerEngine.New()
	...
	engine.Register = function(descriptor)
		...
		-- A repeating trigger is asked every cadence too: nothing announces
		-- that a floor has elapsed, and its condition may simply still hold.
