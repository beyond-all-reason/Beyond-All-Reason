--- Trigger identity = filename + declaration order, stamped at Finalize: the
--- key hot reload unregisters by. A chain without a Do is a load error.
...
function DSL.ForFile(filename, sink)
	...
	local When = function(condition)
		...
		---Measured from when the conditions FIRST held, reset if they stop holding;
		---a repeating trigger re-arms after each fire, so .After doubles as a rate limit.
		...
		---A repeating trigger's floor between fires, so a condition that holds
		---does not fire every cadence.
		---@param seconds number
		---@return TriggerChain
	...
	local Finalize = function()
		...
		for order, build in ipairs(chains) do
			...
			if #build.conditions > 1 then
				-- Inputs compose as the UNION of the parts; any poll-only part (nil inputs)
				-- makes the whole trigger poll. The closure captures configuration, never
				-- progress (the savegame rule).
