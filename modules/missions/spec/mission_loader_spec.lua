...
local function newLoader(sources, requires)
	...
	local moduleHandler = {
		...
		-- The REAL action files, loaded the way the framework loads them: the
		-- loader dispatches through actions/, so stubbing them out would test
		...
		LoadActions = function()
			...
			for _, name in ipairs({ "load", "reload" }) do
				...
				-- The framework gives an action file the globals plus what it
				-- injects — NOT the gadget's environment. Mirroring that here
				-- is what makes an action reaching for GG fail in the spec the
				-- way it fails in the game.
	...
	-- the loader reads BAR.Utilities, so that is where the stub belongs.
	local bar = {
		Utilities = { Gametype = {
