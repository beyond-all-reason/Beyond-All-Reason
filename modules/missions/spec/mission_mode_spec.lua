...
describe("missions mode preset", function()
	...
	it("serializes to exactly the options its policies own", function()
		assert.same({
			...
			-- A mission's roster and its wave packs can name any unit, and the
			-- scavenger defs are derived only when something asks for them. The
			-- mode asks, so nobody has to remember to tick a box in the lobby —
			-- and cannot reach for ruins=enabled instead, which derives them by
			-- scattering derelict bases over the map.
			...
			-- the picker: exposed open at its default, so the lobby renders
			-- the mission list and the loader arms the choice at game start
	...
	it("is selectable on the game axis", function()
		...
		assert.is_truthy(table.concat(keys, ","):find(mission.key, 1, true),
			"the preset's own key must be selectable")
	end)
