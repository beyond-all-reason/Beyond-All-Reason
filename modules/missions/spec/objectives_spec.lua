--- The objectives definition-site DSL: declaration order, reference
--- validation, and the entries the loader compiles the board from. Pure
--- lib — the derived triggers and the published params are the loader
--- spec's business.
...
describe("objectives DSL", function()
	it("declaration order is the board order, titles default prettified", function()
	...
	it("When before CompletedWhen is a load error — it ANDs onto the completion", function()
		local file = Objectives.ForFile("t/objectives.lua")
		local ok, err = pcall(function()
			file.Objective("gated").When(condition())
		end)
		assert.is_false(ok)
		assert.matches("When before CompletedWhen", tostring(err))
	end)
	...
	it("gates, overrides and foreshadow ride the entry", function()
		...
		assert.are.equal(2, #decls[1].completions[1])
	...
	it("a second CompletedWhen is another way, not another requirement", function()
		-- Disjuncts OR; each When ANDs onto the LATEST one — the same shape
		-- as triggers, where OR is two statements.
		local file = Objectives.ForFile("t/objectives.lua")
