...
local function rulesNames(prefix)
	...
	return {
		...
		bossStaggerPercentage = prefix .. "BossStaggerPercentage",
		bossStaggerActive = prefix .. "BossStaggerActive",
...
---@param host table
---@param burrowID integer|nil
---@return integer|nil the burrow itself when it is live, else any live one
local function anyBurrow(host, burrowID)
	local burrows = host.director.state.burrows
	if burrowID ~= nil then
		return burrows[burrowID] and burrowID or nil
	end
	local list = burrowList(host)
	if #list == 0 then
		return nil
	end
...
local function spawnBoss(host, world)
	...
	notify(host, Events.BossSpawned, spec.events.bossKind or "boss")
...
local function execute(host, world, waveOrder)
	...
	elseif kind == "wave" then
		...
		notify(host, Events.WaveSpawned, spec.events.useWaveMsg and "wave" or nil, waveOrder.count)
...
hot.UnitCreated = function(_, unitID, unitDefID, unitTeam)
	...
	for _, name in ipairs(hostNames) do
		...
		if unitTeam ~= host.spec.teamID then
			...
			if host.eco then
				host.eco.Add(unitID, UnitDefs[unitDefID])
			end
		end
	end
...
local function bossDestroyed(host, unitID)
	...
	notify(host, Events.BossDefeated, "bossKilled")
...
hot.UnitDestroyed = function(_, unitID, unitDefID, unitTeam, _, _, attackerID)
	...
	for _, name in ipairs(hostNames) do
		...
		if host.director.OnUnitDestroyed(unitID) then
			notify(host, Events.WaveCleared, nil)
		end
...
local function makeHost(spec)
	...
	local host = { spec = spec, director = director }

	-- Targets by economic value: the tracker is the host's own, and it starts
	-- with what is already standing — a mission's director arrives mid-game.
	if spec.targets and spec.targets.policy == "eco" then
		host.eco = EcoTargets.New()
		for _, unitID in ipairs(Spring.GetAllUnits()) do
			if Spring.GetUnitTeam(unitID) ~= spec.teamID then
				host.eco.Add(unitID, UnitDefs[Spring.GetUnitDefID(unitID)])
			end
		end
	end
	if spec.hooks.targetsOf == nil then
		spec.hooks.targetsOf = host.eco and host.eco.Targets or function()
			return targets, highValueTargets
		end
	end

	-- A flavor whose legacy panel read names the prefix cannot produce
	-- (raptorQueenAnger, not raptorBossAnger) overrides them one by one.
	local names = rulesNames(spec.rulesParamPrefix)
	for key, value in pairs(spec.rulesNames or {}) do
		names[key] = value
	end
	host.names = names
...
function gadget:Initialize()
	GG.Waves = {
		...
		---@param burrowID integer|nil where it comes out; nil for any live burrow
		---@param defName UnitDefName
		---@param count integer
		---@return boolean spawned false with no burrow to come out of
		SpawnNamed = function(name, burrowID, defName, count)
			local host = hosts[name]
			burrowID = host and anyBurrow(host, burrowID) or nil
			if burrowID == nil then
				return false
			end
		...
		---An off-wave squad out of one burrow: what a fresh burrow or a first
		---boss throws, on a flavor's own cue (a burrow dying, a boss arriving).
		---@param name string
		...
		SpawnOffWave = function(name, burrowID)
			...
			host.director.ComposeOffWave(makeWorld(host, Spring.GetGameFrame()), burrowID)
			return true
		end,
		...
		SpawnStructures = function(name)
			local host = hosts[name]
			if host ~= nil and host.spec.structures ~= nil then
				local state = host.director.state
				structures.SpawnWave(host.spec, state, Spring.GetGameSeconds(), function(unitID)
					state.owned[unitID] = true
					drain.SetExperience(state, unitID)
				end)
			end
		end,
		---Aggression a flavor earns outside the director's own sources (a
		---raptor structure razed counts a tenth of a burrow).
		---@param name string
		---@param amount number
		AddAggression = function(name, amount)
			local host = hosts[name]
			if host ~= nil then
				local anger = host.director.state.anger
				anger.aggression = anger.aggression + amount
			end
		end,
		---A unit told to flee keeps fleeing until this frame: the squad
		---manager leaves it alone, and the sweep releases it after.
		---@param name string
		---@param unitID integer
		---@param untilFrame integer
		SetCowardCooldown = function(name, unitID, untilFrame)
			local host = hosts[name]
			if host ~= nil then
				host.director.state.cowardCooldown[unitID] = untilFrame
			end
		end,
		---A read-only view of one director, for the referee gadgets that sit
		---above this one: names, the boss policy, the live boss set.
		---@param name string
		---@return table|nil
		Host = function(name)
			local host = hosts[name]
			...
			return {
				name = name,
				teamID = host.spec.teamID,
				names = host.names,
				boss = host.spec.boss,
				events = host.spec.events,
				hooks = host.spec.hooks,
				reactions = host.spec.reactions,
				capture = host.spec.capture,
				params = state.params,
				bossIDs = state.boss.ids,
				cycle = state.cycle,
				state = state,
			}
		end,
