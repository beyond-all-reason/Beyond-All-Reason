...
-- Immobile only: a squad chasing a scout is a squad doing nothing.
...
---No unhook toggle: the handler defers list updates inside a callin but nils
---the method field immediately, so a gadget that unhooks itself mid-callin
---errors on the next gadget:GameFrame in that loop, and directors stop
---mid-GameFrame routinely (Waves.End, the last boss dying).
...
local function spawnBoss(host, world)
	...
	-- The boss counts as a burrow so squads keep a door to come out of.
...
---The action is behind a coin so fifty idle units do not all re-path on the same frame.
---@param host table
---@param world WaveWorld
