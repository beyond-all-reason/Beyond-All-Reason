...
local function tickUnit(host, unitID)
	...
	-- Mission scenery and anything set neutral would otherwise convert with
	-- no callin ever firing, since this is a direct transfer.
...
function gadget:GameFrame(frame)
	...
	for _, host in ipairs(capturing) do
		...
		if pass ~= nil then
			-- convert() fires UnitGiven mid-walk, and inserting a key during
			-- next/pairs corrupts the iterator: walk a snapshot.
			local ids = {}
