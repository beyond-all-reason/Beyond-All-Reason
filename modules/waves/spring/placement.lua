...
function Placement.New(deps)
	...
	---corner.
	---
	---And it STOPS. The widening is a preference bending, not a preference
	---dissolving: unbounded, it reached the whole map in six retries, which on
	---a mission with a compressed grace period is a matter of seconds — the
	---author wrote "from the northeast" and beacons appeared next to the
	---player's base. Past MAX_ORIGIN_HALF the box is no longer describing a
	---direction, so it does not grow any further; a box with no valid ground
	---at all is the deterministic sweep's problem, not the retry's.
	...
	local function goodGround(x, y, z, spread, allyTeamID, wantScum, tally)
		if not checks.FlatAreaCheck(x, y, z, spread, FLAT_TOLERANCE, true) then
			if tally then tally.steep = tally.steep + 1 end
			return false
		end
		if not checks.OccupancyCheck(x, y, z, spread) then
			if tally then tally.occupied = tally.occupied + 1 end
			return false
		end
		if wantScum and GG.IsPosInRaptorScum and not GG.IsPosInRaptorScum(x, y, z) then
			if tally then tally.noScum = tally.noScum + 1 end
			return false
		end
		return true
	end
	...
	local function probe(box, spread, attempts, allyTeamID, wantScum, visibility, tally, accept)
		if box == nil or box.x1 + spread >= box.x2 - spread or box.z1 + spread >= box.z2 - spread then
			if tally then tally.tooSmall = true end
			return false
		end
		for _ = 1, attempts do
			...
			if goodGround(x, y, z, spread, allyTeamID, wantScum, tally) then
				...
				elseif visibility(x, y, z, spread) then
					...
					if tally then tally.seen = tally.seen + 1 end
				end
			end
		end
		return false
	...
	local function findBurrowSpot(spec, state, t)
		...
		-- Surface and creep are PER SAMPLE, not a verdict on the whole cadence.
		-- They used to run once, after the probe had already returned, and a
		-- refusal threw the found spot away and gave up until the next tick —
		-- so on a watery map, where most of the shoreline is half land and half
		-- sea, nearly every cadence ended with nothing placed and nothing said.
		-- A sample the surface refuses is just another sample.
		...
		-- Last resort: somewhere the burrow FITS, sight be damned.
		--
		-- Every cascade above can fail for reasons that will still be true next
		-- cadence — a map under global LOS has no unseen ground at all, and a
		-- small box may be entirely watched. A beacon the players can see is a
		-- worse beacon; no beacon is not a director.
		--
		-- It relaxes the ground as well as the sight test, which it did not
		-- before, and that was the bug. spread is 1.5x the burrow: spacing, not
		-- fit. Demanding a clear flat circle three times the footprint radius
		-- is a quality preference, and a last resort that keeps insisting on it
		-- is not a last resort — on a small or crowded map every attempt
		-- including this one failed, and the director placed nothing at all.
		-- Deterministic sweep FIRST when the placement module is available, then
		-- the relaxed random probe. Not either/or: a Placement built without the
		-- module (a spec, an older caller) must still have a last resort, and
		-- the sweep can legitimately come back empty on a box with no solid
		-- ground in it at all.
	...
	placement.SpawnBurrow = function(spec, state, t, techAnger)
		...
		if not found then
			...
			if said <= MAX_PLACEMENT_WARNINGS then
				...
				-- Say WHICH test refused the spots, not a guess at one. The
				-- previous message blamed crowding every time, including on
				-- maps where nothing was crowded and the ground was simply too
				-- steep — which sends whoever reads it looking in the wrong
				-- place, and cost exactly that.
				local why = state.lastPlacementTally or {}
				local because = why.tooSmall
						and "the box is smaller than a burrow needs"
					or string.format(
						"%d too steep, %d too close to something, %d half in the water, %d in the players' sight",
						why.steep or 0, why.occupied or 0, why.wetOrDeadly or 0, why.seen or 0)
				Spring.Log("waves", LOG.WARNING, string.format(
					...
					tostring(state.params.placement), because,
	...
	placement.TrySpawnBurrow = function(spec, state, t, techAnger)
		-- Patience before bending the box. The formula is multiplayer's: a
		-- four-minute grace period buys a dozen retries. A mission compresses
		-- grace to a fraction of that, so it evaluates to zero or less and the
		-- box widened on the very first failure — the named corner was
		-- abandoned before it had been properly tried. The floor is what makes
		-- "from the northeast" get a real attempt at any grace period.
		...
		if not state.fullySpawned then
			...
			-- The FIRST burrow is impatient on purpose — a director with nothing
			-- on the field yet is a director doing nothing — but impatient is
			-- not instant. This used to be a bare `or state.firstSpawn`, which
			-- bypassed the count entirely and widened on the opening failure,
			-- and firstSpawn stays true until a burrow actually lands: the
			-- named corner never survived the opening on any map that made it
			-- work for it.
