...
local MAX_TRIES = 30
-- Runs on the slow tick and the retry widens the box, so a few warnings are enough.
local MAX_PLACEMENT_WARNINGS = 3
local FLAT_TOLERANCE = 30
local MIN_BOX_SIDE = 512
-- A quarter per side is half the map: past this "from the northeast" would
-- describe beacons arriving from anywhere.
local MAX_ORIGIN_HALF = 0.25
-- Placement failures are often transient (a unit walks off the spot); widening is permanent.
...
function Placement.New(deps)
	...
	local startboxLib = deps.startboxLib
	...
	placement.InitialBox = function(spec, state)
		local fromShape = false
		local x1, z1, x2, z2
		if state.startBox ~= nil then
			-- Possibly WIDENED by a caller that could not place; re-deriving would undo that.
		...
		if fromShape then
			-- The bounds of the team's REAL start box, which may not be a
			-- rectangle: while startBox still matches these, points are drawn
			-- from the shape itself, not the rectangle around it.
			state.startBoxPristine = { x1 = x1, z1 = z1, x2 = x2, z2 = z2 }
		elseif spec.burrows.box ~= nil then
			state.startBoxPristine = nil
		end
	...
	---A sampler drawing from the ally team's real start box shape, or nil
	---when the probed box is not that shape's pristine bounds.
	local function shapeSampler(state, box, allyTeamID, spread)
		if startboxLib == nil then
			return nil
		end
		local pristine = state.startBoxPristine
		if
			pristine == nil
			or box == nil
			or box.x1 ~= pristine.x1
			or box.z1 ~= pristine.z1
			or box.x2 ~= pristine.x2
			or box.z2 ~= pristine.z2
		then
			return nil
		end
		return function()
			return startboxLib.GetRandomPos(allyTeamID, spread, 1)
		end
	end

	local function probe(box, spread, attempts, allyTeamID, wantScum, visibility, tally, accept, sampler)
	...
	local function findBurrowSpot(spec, state, t)
		...
		if state.params.placement ~= "avoid" then
			...
			if not found then
				...
				end, tally, accept, shapeSampler(state, state.startBox, allyTeamID, spread))
			end
		else
		...
		if not found and nearestValid then
			-- Deterministic sweep: the random probes above can miss a spot that
			-- exists, and this finds the SAME one on every client and in a replay.
		...
		if not found then
			...
			if not found then
				found, x, y, z = probe(state.startBox, fits, 100, allyTeamID, false, function()
					return true
				end, tally, accept, shapeSampler(state, state.startBox, allyTeamID, fits))
			end
		end
	...
	placement.SpawnBurrow = function(spec, state, t, techAnger)
		...
		for _, name in ipairs(names) do
			...
			-- GameFrame: it would take the callin down every cadence, forever.
	...
	placement.TrySpawnBurrow = function(spec, state, t, techAnger)
		...
		if state.firstSpawn and burrowID then
			-- Fixed appointment for the opening wave, whatever the cadence or
			-- intensity dial would otherwise make of it.
