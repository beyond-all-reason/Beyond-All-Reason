...
function Squads.New()
	...
	---High-value targets get a rising share of the picks as more of them exist, so a base full of fusions
	---pulls waves onto itself; failing everything, a random map position keeps units moving.
	---@param spec WaveSpec
	...
	squads.RandomTarget = function(spec)
		...
		for _ = 1, 10 do
			...
			if #pool > 0 then
				...
				if
					Spring.ValidUnitID(target)
					and not Spring.GetUnitIsDead(target)
					and not Spring.GetUnitNeutral(target)
				then
	...
	---Recycles an emptied slot where one exists so the squad table does not grow without bound.
	...
	squads.GiveHealerOrders = function(spec, unitID)
		...
		local function at()
			return {
				pos.x + mRandom(-ORDER_JITTER, ORDER_JITTER),
				pos.y,
				pos.z + mRandom(-ORDER_JITTER, ORDER_JITTER),
				20000,
			}
		end
		...
		if squad.regroupEnabled then
			...
			for _, unitID in ipairs(squad.units) do
				if
					...
					and not Spring.GetUnitNeutral(unitID)
				then
			...
			if count > 0 then
				...
				local strungOut = xmin < xaverage - REGROUP_SPREAD
					or xmax > xaverage + REGROUP_SPREAD
					or zmin < zaverage - REGROUP_SPREAD
					or zmax > zaverage + REGROUP_SPREAD
				if strungOut then
