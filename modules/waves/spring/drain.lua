...
function Drain.New(deps)
	...
	---The square grows every failed try: the twentieth unit stands further out than the first, which is the visual of a wave pouring out.
	---@param spec WaveSpec
	...
	local function applyBehaviour(spec, state, defName)
		...
		-- an air squad that regroups on the ground is a dead air squad, whatever roster it came from
	...
	---One squad per call is what paces the trickle.
	---@param spec WaveSpec
	---@param state WaveDirectorState
	...
	drain.Run = function(spec, state, onSpawned)
		...
		repeat
			...
			if entry == nil then
				-- or the half-assembled squad would sit there until the next wave adopted it
			...
			if unitDef == nil then
				...
				if x ~= nil then
					...
					if unitID then
						-- squadID 1 closes the previous squad; without this the whole wave would be one squad with one target
