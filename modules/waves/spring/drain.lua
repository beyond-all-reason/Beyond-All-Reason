--- The spawn drain: turning queue entries into units on the map.
---
--- A PORT. Two things make it worth keeping exactly as it was. First the
--- trickle — five frames per unit, not a wave at once — which is what stops a
--- forty-unit wave freezing the frame it lands on and what makes a wave look
--- like it is coming out of the ground. Second the expanding-square probe:
--- start tight around the burrow and widen until the unit fits, so a burrow
--- boxed in by wreckage still spawns instead of silently failing.
---
--- Squads are assembled HERE, at the queue's own boundaries: a squadID of 1
--- means "the previous squad is finished", which is why the composer must
--- restart that counter per squad.
...
function Drain.New(deps)
	...
	---Somewhere near a burrow this unit actually fits. The square grows every
	---failed try, so the twentieth unit of a wave stands further out than the
	...
	---Fold one spawned unit's behaviour into the squad being assembled.
	---@param spec WaveSpec
	---@param state WaveDirectorState
