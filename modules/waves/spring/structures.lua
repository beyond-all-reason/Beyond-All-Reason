--- Creep structures: the turrets and factories the director sprinkles
--- between waves, so the map itself gets harder and not only the units on it.
---
--- A PORT. The scaling curve is the monolith's — the number allowed alive
--- opens with anger, quadratically below a hundred and linearly above — and
--- so is the two-mode placement: on creep where the flavor has creep, and in
--- the director's own line of sight where it does not.
...
function Structures.New(deps)
	...
	---Find ground for one structure and put it there.
	---@param spec WaveSpec
		...
		if spec.burrows.useScum and GG.IsPosInRaptorScum then
			...
			for _ = 1, PLACEMENT_ATTEMPTS do
				...
				ok = checks.FlatAreaCheck(x, y, z, spread, FLAT_TOLERANCE, true)
					and checks.OccupancyCheck(x, y, z, spread)
					and checks.VisibilityCheckEnemy(x, y, z, spread, spec.allyTeamID, true, true, true)
					-- Reversed on purpose: this one must be INSIDE the
					-- director's line of sight, and the check answers "not in".
	...
	---One structure wave: every eligible def gets its per-wave allowance, and
	---each attempt is a coin weighted by how far into the grace period the
	---game is — so the map does not sprout turrets in the first minute.
	---@param spec WaveSpec
	---@param state WaveDirectorState
							...
							if unitID then
								...
								if unitDef.isFactory then
									Spring.GiveOrderToUnit(unitID, CMD.FIGHT,
										{ x + mRandom(-256, 256), y, z + mRandom(-256, 256) }, { "meta" })
								else
									Spring.GiveOrderToUnit(unitID, CMD.PATROL,
										{ x + mRandom(-128, 128), y, z + mRandom(-128, 128) }, { "meta" })
								end
							end
...
---Per-player scaling for a structure allowance.
---@param base number
...
---How many of a def may stand at once, given the anger.
---
---Two regimes, and the seam at a hundred is where the roster clock stops
---being a percentage and starts being an open-ended multiplier: below it the
---allowance opens across the def's own bracket, above it it simply tracks
---anger.
