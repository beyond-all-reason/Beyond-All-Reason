...
---Runs before any decision, so a wave the director declines to spawn still moves the wheel.
...
---Below the second tier's threshold the wave is deliberately a trickle, so the first minutes are pressure, not a fight.
...
function Wheel.Shape(state, input, rng)
	...
	if opening then
		-- pinned under the anger fraction so wave one is a handful of units, not a wave
