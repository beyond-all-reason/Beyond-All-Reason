--- The archetype wheel: what KIND of wave this is.
---
--- Eight counters, one per flavor (air, special, basic, small, larger, huge,
--- epic, plus the base gate that rate-limits the whole thing). Every wave
--- decrements all eight; the first flavor whose counter has expired — and
--- whose coin lands — claims the wave and re-arms itself with a fresh random
--- cooldown. The effect is a rotation that never repeats on a fixed period,
--- so a defender cannot set a clock by it.
---
--- Pure: the RNG is injected, so a seeded generator makes the whole sequence
--- reproducible under busted.
