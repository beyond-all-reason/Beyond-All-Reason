--- The boss fight's arithmetic, Spring-free: the health-phase curve, the
--- per-weapon resistance bank, and the stagger bank. One instance per
--- director; the gadget feeds it damage and ticks and applies what it says.
...
---A healthy boss takes double, a nearly-dead one a quarter — the fight has a shape.
...
---The mirror: a boss that is nearly dead hits hardest.
---@param healthPercent number|nil
---@return number
...
function BossReferee.New(policy)
	...
	---A new cycle: banks and statuses reset, the damage tally survives.
	...
	---Damage TO a boss. Order is the monolith's: the health curve, then the
	---resistance bank, then the stagger bank, with the resistance applied
	---differently while the boss is down.
	---@param damage number
	...
	---One second of the stagger bank. Returns what changed so the gadget can
	---paralyze, strike lightning, and publish.
	...
	referee.TickStagger = function()
		...
		if stagger.active then
			...
			if stagger.currentTimer > 0 then
				...
				-- Recovery escalates: longer next time, and a bigger bank.
	...
	---The UI's view, as the panels read it.
	---@return table
