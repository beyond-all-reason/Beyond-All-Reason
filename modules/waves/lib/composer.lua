--- Composition: turning a wave's SHAPE into a list of spawns.
---
--- The monolith drew a candidate at random and re-rolled up to a thousand
--- times until one happened to sit in the anger bracket — which is rejection
--- sampling with a silent failure mode (a bracket with no candidates burns a
--- thousand rolls and spawns nothing). This filters first and draws once.
--- The distribution is the same wherever the old loop was likely to succeed,
--- because buckets carry an entry once per weight point: a uniform draw over
--- the filtered list IS the weighted draw.
---
--- Pure: buckets and populations are plain data, the RNG and the surface
--- lookup are injected. Output is the exact legacy queue-entry shape, so the
--- ported drain and squad code consume it untouched.

local Composer = {}

-- A 1-in-100 wave reaches thirty points of anger past its bracket: the "super
-- squad", the roll that puts something you have not teched to on the field.
...
---Draw one squad from a bucket at this anger. nil when the bucket has
---nothing eligible — an honest answer the caller can act on, where the old
---loop returned nil only after a thousand wasted rolls.
...
---Which pool a burrow draws from, given the wave's air/special rolls and the
---ground it stands on. Air and special are independent coins; surface is not
---a roll at all.
...
---Expand a chosen squad into queue entries.
---
---The first unit of a squad is unconditional and every later one takes the
---spawn-chance coin: a squad always shows up, but only a rich game gets it
---at full strength. squadID counts WITHIN the squad — the drain reads 1 as
---"start a new squad here", so the counter must restart per squad.
...
---Try to place one population singleton (a commander, a decoy) at a burrow.
---
---Three gates, all of which must pass: the def is in bracket, the field is
---not already at its cap, and the whole population stays under a ceiling that
---only opens as anger climbs — which is what keeps the early game free of
---commanders without hard-coding a minute.
...
local function tryPopulation(input, entries, burrowID, key, claimed, claimedCount)
	...
	for _, slot in ipairs(slots) do
		if input.rng() <= input.spawnChance
...
local function composeForBurrow(input, entries, burrowID, withHealers, claims)
	...
	if bucketName ~= nil then
		...
		if squad ~= nil then
			added = added + Composer.Expand(entries, squad, {
				burrow = burrowID,
				team = input.teamID,
	...
	-- Healers ride along on the first pass only: they repair and resurrect,
	-- so one escort per burrow is support and four is a stalemate.
	if withHealers and input.rng() <= input.spawnChance and (surface == "land" or surface == "sea") then
		...
		if healers ~= nil then
			added = added + Composer.Expand(entries, healers, {
				burrow = burrowID,
				team = input.teamID,
				spawnChance = input.spawnChance,
				wave = input.wave,
	...
	-- A coin decides whether this burrow may field a real commander or only a
	-- decoy — the players cannot tell which until it dies.
...
---Compose a full wave: every burrow contributes, and the whole map keeps
---contributing until the envelope is full or the loop guard trips.
---
...
---One burrow's off-wave squad: what a burrow throws out when it is first
---built, or when the boss lands. Same composition, one pass, and a tighter
---population ceiling — these are extras, not the wave.
---@param input WaveComposeInput
---@return { entries: WaveQueueEntry[], count: integer }
...
---A named unit, straight to the queue: the minion path, where a spawner
---names exactly what it wants and how many.
---@param burrowID integer
