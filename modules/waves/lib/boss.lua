--- The boss lifecycle, minus the flavor.
---
--- What lives here: when a boss is due, how much health it arrives with, and
--- the bookkeeping for however many of them there are. What deliberately does
--- NOT live here: resistance banks and stagger machines — those are a flavor
--- module's business (scavengers keeps its own), and a waves module that
--- shipped them would be a scavengers module wearing a generic name.
...
---How much health a boss lands with: a fraction of its maximum set by how
---far the roster clock has run, floored so an early boss is still a boss.
...
---Every boss of this cycle is down.
---@param state { spawned: integer, killed: integer }
---@param config WaveBossConfig
---@return boolean
