...
---pairs order is not a wire value in a synced director, so population tables are flattened
---to sorted lists ONCE so composition draws from the same order everywhere.
...
function Director.NewState(spec, rng)
	...
	return {
		...
		waveAlive = {},
		waveUnits = {},
		-- A mission's director shares a team with the mission's own roster, so "on my team" is not "mine".
		owned = {},
		boss = Boss.NewState(),
		stopped = false,
	}
