...
describe("waves mode grammar", function()
	...
	it("locks structural choices and leaves the numeric dials host-tunable", function()
		local Mode = grammar()
		local mode = Mode("Flavor")
			.Difficulty(Pack.Horde, "normal")
			.Pace(Pack.Horde, 1.0, 1.0)

		assert.is_true(mode.modOptions.flavor_difficulty.locked, "which difficulty is a structural choice")
		assert.is_false(mode.modOptions.flavor_spawntimemult.locked, "pace is a dial")
	end)

	it("opens a structural choice with .Unlocked and pins a dial with .Locked", function()
		local Mode = grammar()
		local mode = Mode("Flavor").Difficulty(Pack.Horde, "normal").Unlocked().Pace(Pack.Horde, 1, 1).Locked()
