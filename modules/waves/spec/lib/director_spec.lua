...
describe("waves box widening", function()
	...
	it("grows with the retry round, up to the point the direction stops reading", function()
		...
		-- ...and then it STOPS. Unbounded, this reached the whole map in six
		-- rounds, and a mission's compressed grace period spends six rounds in
		-- seconds: "from the northeast" ended up meaning "next to the player".
		-- Half the map per axis is the widest a named corner can be and still
		-- be describing a corner.
	...
	it("keeps the far side of the map out of a named corner's reach, forever", function()
		-- The property that actually protects the player: whatever the retry
		-- count, the box never arrives at the opposite corner.
