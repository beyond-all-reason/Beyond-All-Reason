...
--- Chain verbs every PvE flavor shares. A mode that needs a bot on the field
--- says so here: this is the one thing the modoptions cannot express. PvE
--- flavors are activated by the presence of their AI, not by an option —
--- which is what keeps every existing lobby working — so the preset has to
--- tell the lobby to field one. It goes into the exported JSON beside the
--- options, and nothing in the game reads it.
Pve.ChainVerbs = {
	---@param chain table
