...
if gadgetHandler:IsSyncedCode() then
	...
	-- The sim keeps running after the verdict lands and triggers keep
	-- evaluating, so a late Victory/Defeat is expected rather than a fault.
	-- Leave a retired surface behind: a logged no-op, not an assert raised
	-- inside an unprotected synced callin.
	...
	function gadget:GameFrame(gf)
		if ceremony.IsStarted() then
			if scriptedWinners ~= nil then
				...
				Spring.Log(LOG_TAG, LOG.WARNING,
