...
if gadgetHandler:IsSyncedCode() then
	...
	-- neverend and sandbox disable the ELIMINATION checks but the gadget stays
	-- alive (the pre-module gadget removed itself) so scripted verdicts still end the game.
	...
	local gameOverPipeline ---@type AssembledPipeline<GameOverContext, GameOverVerdict>
	...
	-- The sim keeps running after the verdict, so a late Victory/Defeat is expected:
	-- a logged no-op, not an assert raised inside an unprotected synced callin.
	local function retiredSurface()
		local function retired(verb)
			return function()
				Spring.Log(LOG_TAG, LOG.INFO, "MatchFlow." .. verb .. " after the match ended; ignored")
			end
		end
		return { Victory = retired("Victory"), Defeat = retired("Defeat") }
	end

	function gadget:Shutdown()
		GG.MatchFlow = retiredSurface()
	end

	function gadget:GameFrame(gf)
		if ceremony.IsStarted() then
			if scriptedWinners ~= nil then
				-- First verdict wins. Dropping this silently would show a
				-- victory over a mission's defeat with nothing in the log.
				Spring.Log(
					LOG_TAG,
					LOG.WARNING,
					"scripted verdict arrived after the ceremony began; the running outcome stands"
				)
				scriptedWinners = nil
			end
			ceremony.GameFrame(gf)
		else
			-- Legacy, preserved exactly: player state refreshed on a cadence, the decision every frame.
			if eliminationEnabled then
				if fixedallies then
			...
			gameOverCtx.infos = liveness and liveness.Infos() or {}
	...
	function gadget:RecvLuaMsg(msg, playerID)
		-- The legacy gadget removed itself under neverend/sandbox, taking this path with it.
