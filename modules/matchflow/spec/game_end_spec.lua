--- The gadget glue: the wiring between liveness, the policy pipeline and the
--- ceremony. The libraries under it are specced separately; what is pinned
--- here is what the gadget itself decides — chiefly what happens to a scripted
--- verdict that arrives at the wrong moment, and what MatchFlow callers reach
--- after the gadget has retired.

local realVFS = VFS

local GADGET = "modules/matchflow/gadgets/game_end.lua"

---Load the gadget's synced half over a mocked engine.
...
local function newGadget(opts)
	...
	local moduleHandler = {
		...
		-- The pipeline's answer is the fixture's, so the gadget's own wiring is
		-- what is under test rather than the policy.
	...
	local vfs = setmetatable({
		Include = function(path, fileEnv)
			...
			if path == "modules/matchflow/lib/ceremony.lua" then
				return { New = function() return ceremony end }
			end
	...
	env.UnitDefs = {}
	-- Engine constant tables read at load; the values do not matter here.
	env.CMD = setmetatable({}, { __index = function() return 0 end })
	...
	env.Spring = setmetatable({
		...
		Utilities = { Gametype = { IsFFA = function() return false end } },
...
describe("game end gadget", function()
	describe("the MatchFlow surface after the gadget retires", function()
		it("leaves a surface behind, so a late caller is not an assert", function()
			...
			h.gadget:Shutdown()
			-- Nil here would raise inside mission_loader's unprotected
			-- GameFrame and take LuaRules down with it.
			assert.is_table(h.env.GG.MatchFlow)
	...
	describe("a verdict that arrives after the ceremony began", function()
		it("does not override the outcome already running", function()
			...
			-- The ceremony keeps its frames; the pipeline is never consulted.
