...
---@param level integer
---@param requiredLevel integer
---@return boolean
local function unlocked(unitDefID, teamID, level, requiredLevel)
	---@type TechUnlockContext
	local ctx = { unitDefID = unitDefID, teamID = teamID, level = level, requiredLevel = requiredLevel }
	return ModuleHandler.Evaluate(pipelines.unlock, ctx) == true
end
	...
	for _, teamID in ipairs(teamList) do
		if not ignoredTeams[teamID] then
			...
			for unitDefID, requiredLevel in pairs(blockTechDefs) do
				if unlocked(unitDefID, teamID, techLevel, requiredLevel) then
...
function gadget:GameStart()
	...
	for _, teamID in ipairs(teamList) do
		if not ignoredTeams[teamID] then
			...
			for unitDefID, requiredLevel in pairs(blockTechDefs) do
				if not unlocked(unitDefID, teamID, techLevel, requiredLevel) then
					GG.BuildBlocking.AddBlockedUnit(unitDefID, teamID, "tech_level_" .. requiredLevel)
				end
			end
		end
