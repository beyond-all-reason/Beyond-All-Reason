...
Policies.Pipeline(techCore).Select(techCore.TechCoreLadder, function(request)
...
Policies.Pipeline(unlock)
	.Unless(unlock.BelowTier, function(ctx)
		return ctx.level < ctx.requiredLevel
	end)
	.Select(unlock.Allowed, function()
		return true
	end)
