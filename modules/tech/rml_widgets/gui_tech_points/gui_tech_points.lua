...
local RML_PATH = "modules/tech/rml_widgets/gui_tech_points/gui_tech_points.rml"
...
local function updatePopups()
	...
	if
		not widgetState.initialPopupShown
		and widgetState.gameStartTime
		and (popupsEnabled or Spring.GetGameFrame() > POPUP_DELAY_FRAMES)
	then
		popupsEnabled = true
...
local function updateUI()
	...
	local changed = (level ~= prevLevel)
		or (points ~= prevCount)
		or (math.floor(progress + 0.5) ~= math.floor(prevProgress + 0.5))
	if not changed then
		return
	end
