--- Tech tier resolution: the general rule, shared by every tier scheme.
--- Which options a scheme puts on the ladder is the scheme's business
--- (see policies/tech_core.lua); this only knows that _at_t3 beats
--- _at_t2 beats the base key. The tax rate that reads a team's tier
--- lives with transfer, which owns what a transfer costs.
local TechTier = {}
