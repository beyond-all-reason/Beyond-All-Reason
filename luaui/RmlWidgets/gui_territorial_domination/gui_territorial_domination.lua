...
local spGetAIInfo = Engine.Shared.GetAIInfo
	...
	for i = 1, #teamList do
		...
		if isAI then
			...
			if (not mySpecStatus) and anonymousMode ~= "disabled" and teamID ~= myTeamID then
				local anonymousColorR = Engine.Unsynced.GetConfigInt("anonymousColorR", COLOR_BYTE_MAX) / COLOR_BYTE_MAX
				local anonymousColorG = Engine.Unsynced.GetConfigInt("anonymousColorG", 0) / COLOR_BYTE_MAX
				local anonymousColorB = Engine.Unsynced.GetConfigInt("anonymousColorB", 0) / COLOR_BYTE_MAX
...
local function checkDocumentVisibility()
	...
	local currentTime = Engine.Shared.GetGameSeconds()
...
local function calculateUILayout()
	...
	local screenWidth, screenHeight = Engine.Unsynced.GetViewGeometry()
...
local function showRoundEndPopup(roundNumber, isFinalRound)
	...
	Engine.Unsynced.PlaySoundFile("sounds/global-events/scavlootdrop.wav", 0.8, "ui")
	Engine.Unsynced.PlaySoundFile("sounds/replies/servlrg3.wav", 1.0, "ui")
end
...
local function updateRoundInfo()
	...
	if roundEndTime > 0 then
		timeRemainingSeconds = math.max(0, roundEndTime - Engine.Shared.GetGameSeconds())
...
function widget:Initialize()
	...
	local language = Engine.Unsynced.GetConfigString("language", "en")
	...
	resetCache()
	widgetState.lastUpdateTime = Engine.Shared.GetGameSeconds()
	widgetState.lastScoreUpdateTime = Engine.Shared.GetGameSeconds()
	widgetState.lastTimeUpdateTime = Engine.Shared.GetGameSeconds()
