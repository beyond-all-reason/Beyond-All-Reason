...
function M.sync(doc, ctx, spState, setSummary)
	...
	if doc then
		...
		do
			...
			local anyFilterOn = spState.smartEnabled == true and (sf.avoidWater or sf.avoidCliffs or sf.preferSlopes or sf.altMinEnable or sf.altMaxEnable) and true or false
	...
	do
		...
		setSummary(toolLabel, "#fdc04c", "CH ", tostring(spState.channel or "?"), "", shapeNames[spState.shape] or "Circle", "R ", tostring(spState.radius or 0), "Str ", string.format("%.2f", spState.strength or 0), "Int ", string.format("%.1f", spState.intensity or 0))
	end
