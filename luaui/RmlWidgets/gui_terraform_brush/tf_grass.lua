...
function M.sync(doc, ctx, gbState, setSummary, sumEl)
	...
	if doc then
		...
		if symStGb then
			syncAndFlash(getCachedEl(doc, "gb-slider-symmetry-radial-count"), "gb-symmetry-radial-count", tostring(symStGb.symmetryRadialCount or 2))
			syncAndFlash(getCachedEl(doc, "gb-slider-symmetry-mirror-angle"), "gb-symmetry-mirror-angle", tostring(symStGb.symmetryMirrorAngle or 0))
		end
