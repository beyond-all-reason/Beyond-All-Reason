...
function M.sync(doc, ctx, gbState, setSummary, sumEl)
	...
	local clayBtnG = doc and getCachedEl(doc, "btn-clay-mode")
	if clayBtnG then
	...
	do
		...
		if s then
			...
			if doc and ctx.syncWarnChip then
				...
				local anyFilter = (
					gbState.smartEnabled
					and (sfE.avoidCliffs or sfE.preferSlopes or sfE.altMinEnable or sfE.altMaxEnable)
				...
				ctx.syncWarnChip(
					doc,
					"warn-chip-gb-instruments",
					"section-gb-instruments",
					instActive and true or false
				)
			end
	...
	if doc then
		...
		do
			...
			syncAndFlash(getCachedEl(doc, "slider-grass-density"), "gb-density", tostring(sv))
		end
		...
		do
			...
			syncAndFlash(
				getCachedEl(doc, "slider-gb-curve"),
				"gb-curve",
				tostring(math.floor((gbState.curve or 1.0) * 10 + 0.5))
			)
			...
			syncAndFlash(
				getCachedEl(doc, "slider-gb-length"),
				"gb-length",
				tostring(math.floor((gbState.lengthScale or 1.0) * 10 + 0.5))
			)
		end
		...
		do
			...
			if smartToggle then
				smartToggle:SetAttribute(
					"src",
					smartOn and "/luaui/images/terraform_brush/check_on.png"
			...
			local function syncSmartCheck(id, key)
				...
				if el then
					el:SetAttribute(
						"src",
						sf[key] and "/luaui/images/terraform_brush/check_on.png"
							or "/luaui/images/terraform_brush/check_off.png"
					)
				end
			end
		...
		do
			...
			if colorToggle then
				colorToggle:SetAttribute(
					"src",
					colorOn and "/luaui/images/terraform_brush/check_on.png"
						or "/luaui/images/terraform_brush/check_off.png"
				)
			end
			...
			local swatchEl = getCachedEl(doc, "gb-tex-color-swatch")
			if swatchEl then
			...
			if exToggle then
				exToggle:SetAttribute(
					"src",
					exOn and "/luaui/images/terraform_brush/check_on.png"
						or "/luaui/images/terraform_brush/check_off.png"
				)
			end
		...
		do
			...
			local slH = getCachedEl(doc, "slider-gb-history")
			if slH then
		...
		if symStGb then
			syncAndFlash(
				getCachedEl(doc, "gb-slider-symmetry-radial-count"),
				"gb-symmetry-radial-count",
				tostring(symStGb.symmetryRadialCount or 2)
			)
			syncAndFlash(
				getCachedEl(doc, "gb-slider-symmetry-mirror-angle"),
				"gb-symmetry-mirror-angle",
				tostring(symStGb.symmetryMirrorAngle or 0)
			)
		end
	...
	if gbState.shape and dm and dm.activeShape ~= gbState.shape then
		dm.activeShape = gbState.shape
	end
	...
	do
		...
		if not hasGrass then
			if sumEl then
				...
				ctx.setInnerRmlIfChanged(
					sumEl,
					"status-summary",
					'<span class="tf-ss-mode" style="color: #fdc04c;">GRASS</span>'
						.. sep
						.. '<span class="tf-ss-val" style="color: #fdc04c;">No grass data for this map</span>'
				)
			end
		else
			setSummary(
				...
				"R ",
				tostring(gbState.radius or 0),
				"Density ",
				string.format("%.0f", (gbState.density or 0) * 100) .. "%"
			)
		end
	end
