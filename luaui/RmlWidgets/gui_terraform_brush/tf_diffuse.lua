...
local function buildSig(layers, activeId)
	...
	for i = 1, #layers do
		...
		parts[#parts + 1] = string.format("%d:%s:%s:%.2f:%.2f:%.2f:%.2f:%s:%s", layer.id or 0, tostring(layer.name or ""), layer.enabled and "1" or "0", (layer.color and layer.color[1]) or 0, (layer.color and layer.color[2]) or 0, (layer.color and layer.color[3]) or 0, layer.opacity or 0, tostring(layer.texturePath or ""), (activeId == layer.id) and "A" or "-")
	end
...
local function rebuildLayerList(doc, ctx, layers, activeId)
	...
	for i = 1, #layers do
		...
		row:SetAttribute("style", "display: flex; flex-direction: row; align-items: center; gap: 6dp; padding: 4dp 6dp;")

		-- Color swatch
		local swatch = doc:CreateElement("div")
		swatch:SetAttribute("style", "width: 18dp; height: 18dp; background-color: " .. _rgbCss(layer.color) .. "; border: 1dp #000;")
		row:AppendChild(swatch)

		-- Enable toggle (checkbox icon)
		local enableImg = doc:CreateElement("img")
		enableImg:SetClass("tf-icon-sm", true)
		enableImg:SetAttribute("src", layer.enabled and "/luaui/images/terraform_brush/check_on.png" or "/luaui/images/terraform_brush/check_off.png")
...
function M.sync(doc, ctx, dfpState, setSummary)
	...
	if setSummary then
		setSummary("DIFFUSE", "#fdc04c", "", shortName(activeLayer and activeLayer.name, 18), "R ", tostring(dfpState.radius or 128), "Str ", string.format("%d%%", math.floor((dfpState.strength or 0.6) * 100 + 0.5)), "", dfpState.erase and "ERASE" or "PAINT")
	end
end
