...
function M.attach(doc, ctx)
	...
	for _, entry in ipairs({
		...
		if sliderEl and trackSliderDrag then
			trackSliderDrag(sliderEl, entry[2])
		end
	end
end
...
local function rebuildLayerList(doc, ctx, layers, activeId)
	...
	for i = 1, #layers do
		...
		if activeId == layer.id then
			row:SetClass("active", true)
		end
		row:SetAttribute(
			"style",
		...
		swatch:SetAttribute(
			"style",
			"width: 18dp; height: 18dp; background-color: " .. _rgbCss(layer.color) .. "; border: 1dp #000;"
		)
		...
		enableImg:SetAttribute(
			"src",
			layer.enabled and "/luaui/images/terraform_brush/check_on.png"
...
function M.sync(doc, ctx, dfpState, setSummary)
	...
	if not dfpState or not WG.DiffusePainter then
		return
	end
	...
	if dm then
		...
		if dm.dfpRadiusStr ~= radiusStr then
			dm.dfpRadiusStr = radiusStr
		end
		if dm.dfpStrengthStr ~= strengthStr then
			dm.dfpStrengthStr = strengthStr
		end
		if dm.dfpCurveStr ~= curveStr then
			dm.dfpCurveStr = curveStr
		end
		if dm.dfpErase ~= (dfpState.erase == true) then
			dm.dfpErase = (dfpState.erase == true)
		end

		local activeName = shortName(activeLayer and activeLayer.name or "", 28)
		if dm.dfpActiveLayerName ~= activeName then
			dm.dfpActiveLayerName = activeName
		end
		...
		do
			...
			if dm.dfpFractalStr ~= fracStr then
				dm.dfpFractalStr = fracStr
			end
		...
		if dm.dfpChNormals ~= chN then
			dm.dfpChNormals = chN
		end
		if dm.dfpChSpecular ~= chS then
			dm.dfpChSpecular = chS
		end
		if dm.dfpChEmission ~= chE then
			dm.dfpChEmission = chE
		end
		local specIntStr = tostring(math.floor((dfpState.specIntensity or 0.25) * 100 + 0.5))
		if dm.dfpSpecIntStr ~= specIntStr then
			dm.dfpSpecIntStr = specIntStr
		end
		local glowStr = tostring(math.floor(((activeLayer and activeLayer.glowStrength) or 0) * 100 + 0.5))
		if dm.dfpGlowStr ~= glowStr then
			dm.dfpGlowStr = glowStr
		end

		-- Grass attach
		local gAttach = dfpState.grassAttach and true or false
		if dm.dfpGrassAttach ~= gAttach then
			dm.dfpGrassAttach = gAttach
		end
		local grassStr = tostring(math.floor(((activeLayer and activeLayer.grassDensity) or 0) * 100 + 0.5))
		if dm.dfpGrassStr ~= grassStr then
			dm.dfpGrassStr = grassStr
		end
	...
	if doc then
		...
		syncAndFlash(
			getCachedEl(doc, "dfp-slider-strength"),
			"dfp-strength",
			tostring(math.floor((dfpState.strength or 1.0) * 100 + 0.5))
		)
		syncAndFlash(
			getCachedEl(doc, "dfp-slider-curve"),
			"dfp-curve",
			tostring(math.floor((dfpState.curve or 1.5) * 10 + 0.5))
		)
		do
			local amt, freq = dfpState.fractalAmount, dfpState.fractalFreq
			syncAndFlash(
				getCachedEl(doc, "dfp-slider-fractal"),
				"dfp-fractal",
				tostring(math.floor((amt or 0) * 100 + 0.5))
			)
			local freqSlider = math.floor(1 + ((freq or 0.003) - 0.0002) / (0.01 - 0.0002) * 49 + 0.5)
			freqSlider = math.max(1, math.min(50, freqSlider))
		...
		-- Channel sliders
		syncAndFlash(
			getCachedEl(doc, "dfp-slider-specint"),
			"dfp-specint",
			tostring(math.floor((dfpState.specIntensity or 0.25) * 100 + 0.5))
		)
		if activeLayer then
			syncAndFlash(
				getCachedEl(doc, "dfp-slider-glow"),
				"dfp-glow",
				tostring(math.floor((activeLayer.glowStrength or 0) * 100 + 0.5))
			)
			syncAndFlash(
				getCachedEl(doc, "dfp-slider-grass"),
				"dfp-grass",
				tostring(math.floor((activeLayer.grassDensity or 0) * 100 + 0.5))
			)
		end
		...
		if activeLayer then
			...
			syncAndFlash(
				getCachedEl(doc, "dfp-slider-thermo-angle"),
				"dfp-thermo-angle",
				tostring(math.floor(activeLayer.thermoAngle or 30))
			)
			syncAndFlash(
				getCachedEl(doc, "dfp-slider-thermo-falloff"),
				"dfp-thermo-falloff",
				tostring(math.floor(activeLayer.thermoFalloff or 8))
			)
		end
	...
	if setSummary then
		setSummary(
			...
			"R ",
			tostring(dfpState.radius or 128),
			"Str ",
			string.format("%d%%", math.floor((dfpState.strength or 0.6) * 100 + 0.5)),
			"",
