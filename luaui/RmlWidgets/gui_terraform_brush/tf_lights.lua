...
function M.attach(doc, ctx)
	...
	local function populateBuiltinPresets(filter)
		if not llBuiltinList or not WG.LightPlacer then
			return
		end
		...
		for i, preset in ipairs(presets) do
			...
			if
				...
				or (preset.desc or ""):lower():find(filter, 1, true)
			then
		...
		for _, entry in ipairs(shown) do
			...
			local html = '<div id="'
				...
				.. (preset.name or "Unnamed")
				.. "</div>"
				.. '<div class="ll-preset-desc">'
				.. (preset.desc or "")
				.. " ("
				.. #preset.lights
				.. " lights)</div>"
	...
	local function populateUserPresets(filter)
		if not llUserList or not WG.LightPlacer then
			return
		end
		filter = filter or getLLSearchFilter()
		...
		if not presets or #presets == 0 then
			llUserList.inner_rml =
				'<div class="text-xs text-keybind" style="padding: 4dp;">No saved user presets yet.</div>'
			return
		end
		local shown = {}
	...
	local function applyGlobeDirection()
		...
		if not gw or gw <= 0 or not gh or gh <= 0 then
			return
		end
...
function M.sync(doc, ctx, lpState, setSummary)
	if ctx.syncTBMirrorControls then
		ctx.syncTBMirrorControls(doc, "lp")
	end
	local widgetState = ctx.widgetState
	local uiState = ctx.uiState
	local WG = ctx.WG
	...
	if dm then
		...
		if dm.lpLightType ~= lt then
			dm.lpLightType = lt
		end
		local md = lpState.mode or "place"
		if dm.lpMode ~= md then
			dm.lpMode = md
		end
		local dist = lpState.distribution or "random"
		if dm.lpDistMode ~= dist then
			dm.lpDistMode = dist
		end
	...
	if dm and dm.lpDirectedLight ~= directedLight then
		dm.lpDirectedLight = directedLight
	end
	...
	if widgetState.dmHandle then
		...
		if dm.lpBrightnessStr ~= v then
			dm.lpBrightnessStr = v
		end
		v = tostring(math.floor(lpState.lightRadius))
		if dm.lpLightRadiusStr ~= v then
			dm.lpLightRadiusStr = v
		end
		v = tostring(math.floor(lpState.elevation))
		if dm.lpElevationStr ~= v then
			dm.lpElevationStr = v
		end
		v = tostring(lpState.lightCount)
		if dm.lpCountStr ~= v then
			dm.lpCountStr = v
		end
		v = tostring(math.floor(lpState.radius))
		if dm.lpBrushRadiusStr ~= v then
			dm.lpBrushRadiusStr = v
		end
	end
	...
	if syncAndFlash then
		...
		syncAndFlash(
			getCachedEl(doc, "slider-lp-brush-radius"),
			"lp-brush-radius",
			tostring(math.floor(lpState.radius))
		)
		syncAndFlash(
			getCachedEl(doc, "slider-lp-brightness"),
			"lp-brightness",
			tostring(math.floor(lpState.brightness * 100 + 0.5))
		)
		syncAndFlash(
			getCachedEl(doc, "slider-lp-light-radius"),
			"lp-light-radius",
			tostring(math.floor(lpState.lightRadius))
		)
		...
		syncAndFlash(
			getCachedEl(doc, "slider-lp-color-r"),
			"lp-color-r",
			tostring(math.floor((lpState.color[1] or 0) * 1000 + 0.5))
		)
		syncAndFlash(
			getCachedEl(doc, "slider-lp-color-g"),
			"lp-color-g",
			tostring(math.floor((lpState.color[2] or 0) * 1000 + 0.5))
		)
		syncAndFlash(
			getCachedEl(doc, "slider-lp-color-b"),
			"lp-color-b",
			tostring(math.floor((lpState.color[3] or 0) * 1000 + 0.5))
		)
		do
			local tbs = WG.TerraformBrush and WG.TerraformBrush.getState and WG.TerraformBrush.getState() or {}
			syncAndFlash(
				getCachedEl(doc, "lp-slider-symmetry-radial-count"),
				"lp-symmetry-radial-count",
				tostring(tbs.symmetryRadialCount or 2)
			)
			syncAndFlash(
				getCachedEl(doc, "lp-slider-symmetry-mirror-angle"),
				"lp-symmetry-mirror-angle",
				tostring(math.floor(tbs.symmetryMirrorAngle or 0))
			)
		end
	...
	if widgetState.dmHandle and WG.LightPlacer then
		...
		if widgetState.dmHandle.lpPlacedCountStr ~= v then
			widgetState.dmHandle.lpPlacedCountStr = v
		end
	end
	...
	setSummary(
		...
		"Bright ",
		string.format("%.1f", lpState.brightness or 1),
		"Rad ",
		tostring(lpState.lightRadius or 200),
		"Elev ",
		tostring(lpState.elevation or 100)
	)
	...
	if doc and ctx.setDisabledIds then
		...
		ctx.setDisabledIds(doc, {
			"slider-lp-brightness",
			"slider-lp-brightness-numbox",
			"slider-lp-elevation",
			"slider-lp-elevation-numbox",
			"slider-lp-color-r",
			"slider-lp-color-r-numbox",
			"slider-lp-color-g",
			"slider-lp-color-g-numbox",
			"slider-lp-color-b",
			"slider-lp-color-b-numbox",
		}, remove)
		-- Count/cadence: scatter only
		ctx.setDisabledIds(doc, {
			"slider-lp-count",
			"slider-lp-count-numbox",
			"slider-lp-cadence",
			"slider-lp-cadence-numbox",
		}, not scatter)
		-- Brush radius (pick/scatter radius): irrelevant in point mode
		ctx.setDisabledIds(doc, {
			"slider-lp-brush-radius",
			"slider-lp-brush-radius-numbox",
		}, mode == "point")
	end
end
