...
function M.attach(doc, ctx)
	if ctx.attachTBMirrorControls then
		ctx.attachTBMirrorControls(doc, "dc")
	end
...
function M.sync(doc, ctx, setSummary)
	if ctx.syncTBMirrorControls then
		ctx.syncTBMirrorControls(doc, "dc")
	end
	local widgetState = ctx.widgetState
	...
	if dpState then
		setSummary("DECALS", "#fdc04c", "", (dpState.mode or "idle"):upper(), "R ", tostring(dpState.radius or 0), "Sel ", tostring(#(dpState.selectedDecals or {})))
		...
		local function setNum(id, val)
			...
			if e then
				e:SetAttribute("value", tostring(val))
			end
		end
		...
		if dm then
			...
			if dm.dcLibMode ~= m then
				dm.dcLibMode = m
			end
			local d = dpState.distribution or "random"
			if dm.dcDistribution ~= d then
				dm.dcDistribution = d
			end
		end
		-- Align toggle icon
		...
		if alignBtn then
			alignBtn:SetAttribute("src", dpState.alignToNormal and "/luaui/images/terraform_brush/check_on.png" or "/luaui/images/terraform_brush/check_off.png")
		end
		...
		if nbDcHist then
			nbDcHist:SetAttribute("value", tostring(dcUndoCnt))
		end
		...
		if ctx.setDisabledIds then
			...
			ctx.setDisabledIds(doc, {
				"dc-slider-rotation",
				"dc-slider-rotation-numbox",
				"dc-slider-rotrand",
				"dc-slider-rotrand-numbox",
				"btn-dc-rot-ccw",
				"btn-dc-rot-cw",
				"btn-dc-rotrand-down",
				"btn-dc-rotrand-up",
			...
			ctx.setDisabledIds(doc, {
				"dc-slider-count",
				"dc-slider-count-numbox",
				"btn-dc-count-down",
				"btn-dc-count-up",
				"btn-dc-dist-random",
				"btn-dc-dist-regular",
				"btn-dc-dist-clustered",
			}, not scatter)
			...
			ctx.setDisabledIds(doc, {
				"dc-slider-sizemin",
				"dc-slider-sizemin-numbox",
				"dc-slider-sizemax",
				"dc-slider-sizemax-numbox",
				"dc-slider-alpha",
				"dc-slider-alpha-numbox",
				"btn-dc-sizemin-down",
				"btn-dc-sizemin-up",
				"btn-dc-sizemax-down",
				"btn-dc-sizemax-up",
