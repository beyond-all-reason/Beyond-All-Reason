...
function M.attach(doc, ctx)
	if ctx.attachTBMirrorControls then
		ctx.attachTBMirrorControls(doc, "cl")
	end
...
function M.sync(doc, ctx, clState, setSummary)
	if ctx.syncTBMirrorControls then
		ctx.syncTBMirrorControls(doc, "cl")
	end
	local widgetState = ctx.widgetState
	...
	do
		...
		if doc then
			...
			if widgetState.dmHandle and clState then
				...
				if widgetState.dmHandle.clRotationStr ~= v then
					widgetState.dmHandle.clRotationStr = v
				end
			end
			...
			if widgetState.dmHandle and clState then
				...
				if widgetState.dmHandle.clHeightStr ~= v then
					widgetState.dmHandle.clHeightStr = v
				end
			end
			...
			if clState and widgetState.dmHandle then
				...
				if dm.clMirrorX ~= mx then
					dm.clMirrorX = mx
				end
				if dm.clMirrorZ ~= mz then
					dm.clMirrorZ = mz
				end
			end
			...
			if clState and clState.layers and widgetState.dmHandle then
				...
				if dm.clLayerTerrain ~= (ly.terrain == true) then
					dm.clLayerTerrain = ly.terrain == true
				end
				if dm.clLayerMetal ~= (ly.metal == true) then
					dm.clLayerMetal = ly.metal == true
				end
				if dm.clLayerFeatures ~= (ly.features == true) then
					dm.clLayerFeatures = ly.features == true
				end
				...
				if dm.clLayerDecals ~= (ly.decals == true) then
					dm.clLayerDecals = ly.decals == true
				end
				if dm.clLayerWeather ~= (ly.weather == true) then
					dm.clLayerWeather = ly.weather == true
				end
				if dm.clLayerLights ~= (ly.lights == true) then
					dm.clLayerLights = ly.lights == true
				end
			end
			...
			if clState and widgetState.dmHandle then
				...
				if widgetState.dmHandle.clQuality ~= tq then
					widgetState.dmHandle.clQuality = tq
				end
			end
	...
	do
		...
		setSummary("CLONE", "#fdc04c", "", stateLabels[cs] or cs:upper(), "Rot ", tostring(math.floor(clState and clState.pasteRotation or 0)) .. "\194\176", "Quality ", (clState and clState.terrainQuality or "full"):upper())
	end
	...
	if doc and ctx.setDisabledIds and clState then
		...
		ctx.setDisabledIds(doc, {
			"slider-cl-rotation",
			"slider-cl-rotation-numbox",
			"btn-cl-rot-ccw",
			"btn-cl-rot-cw",
			"slider-cl-height",
			"slider-cl-height-numbox",
			"btn-cl-height-down",
			"btn-cl-height-up",
			"btn-cl-mirror-x",
			"btn-cl-mirror-z",
