...
local function computeTileWidth()
	...
	local tileW =
		math.floor((innerPx - (TILE_COLUMNS - 1) * gapPx - TILE_COLUMNS * tileBorderPx - safetyPx) / TILE_COLUMNS)
	if tileW < 24 then
		tileW = 24
	end
	return tileW
...
local function renderVirtualWindow(listEl, doc, startIdx, endIdx, selectedSet)
	...
	virtualTopSpacer:SetAttribute(
		"style",
		string.format("width: 100%%; height: %dpx; flex-shrink: 0;", math.floor(topRows * rowHeightPx + 0.5))
	)
	...
	virtualBotSpacer:SetAttribute(
		"style",
		string.format("width: 100%%; height: %dpx; flex-shrink: 0;", math.floor(bottomRows * rowHeightPx + 0.5))
	)
...
local function rebuildFeatureList(filter)
	...
	for _, entry in ipairs(toShow) do
		...
		if
			lowerFilter == ""
			or entry.name:lower():find(lowerFilter, 1, true)
			or (shown and shown:lower():find(lowerFilter, 1, true))
		then
...
function widget:Update()
	...
	local mainPanel = WG.TerraformerShared
		and WG.TerraformerShared.getElementRect
	...
	if not userDragged and mainPanel and widgetState.rootElement then
		...
		if myWidth and myWidth > 0 then
			local gap = 8
			widgetState.rootElement:SetAttribute(
				"style",
				string.format("left: %dpx; top: %dpx;", mainPanel.left - myWidth - gap, mainPanel.top)
			)
		end
	end
