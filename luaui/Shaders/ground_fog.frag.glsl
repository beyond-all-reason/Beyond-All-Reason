...
uniform float heightFogFalloff;      // 1 = linear, 2 = hugs ground, 0.5 = fuller top

// Nubis-style ground cloud uniforms (lifted from Horizon Zero Dawn / Beckebans
// VolumetricClouds). cloudCoverage is the low-cutoff threshold that gates
// where banks exist; cloudScale controls the base noise frequency on XZ;
// cloudErodeAmount is how much the high-freq Worley eats into the edges;
// cloudWindSpeed scrolls the whole field; cloudMorphSpeed slowly rotates the
// noise on its Z axis so banks mutate in place; cloudTime is seconds pushed
// from lua each frame so the morph advances independent of wind.
uniform float cloudCoverage;
uniform float cloudScale;
uniform float cloudErodeAmount;
uniform float cloudWindSpeed;
uniform float cloudMorphSpeed;
uniform float cloudTime;

#define MAX_FOG_BLASTS 64
// Per-blast: 4 floats (x, y, z, currentRadius). Declared as a flat float array
// because SetUniformFloatArrayAlways reliably pushes float[] but not vec4[].
// Unused slots have radius = 0 and are skipped by the early-out.
uniform float fogBlasts[MAX_FOG_BLASTS * 4];
// Per-blast carve strength [0..1] separate from radius, so the fade-out can
// dissipate the fog-removal opacity while the sphere visually shrinks only
// part way.
uniform float fogBlastOpacity[MAX_FOG_BLASTS];
uniform int   fogBlastCount;
...
// remap(x, a,b, c,d): linear remap of x from [a..b] into [c..d]. Used by the
// Nubis cloud pipeline to combine coverage and erosion without losing density.
float cloudRemap(float x, float oldMin, float oldMax, float newMin, float newMax) {
	return newMin + ((x - oldMin) / max(oldMax - oldMin, 1e-5)) * (newMax - newMin);
}

// Ground-cloud density at a single world position. This is the BAR-port of
// the Horizon Zero Dawn / Nubis sampleCloudDensity:
//   1. base low-freq shape  — noise64cube at cloudScale
//   2. coverage low-cutoff  — remap so only values above cloudCoverage survive
//   3. height gradient      — zachte onder+bovenkant, voorkomt zichtbare bak-zijkanten
//   4. high-freq erode      — packedNoise (Worley) at 4x freq eats the edges
// All noise coords stay in [0..1] by using worldXZ / MAP_SIZE + wind offsets
// that are also fractional. worldY is normalised against the fog layer height.
float sampleGroundCloudDensity(vec3 worldPos, vec2 windXZ, float morph) {
	// Vertical fraction inside the fog layer: 0 at bottom, 1 at top.
	float layerH = max(heightFogTop - heightFogBottom, 1.0);
	float hFrac = clamp((worldPos.y - heightFogBottom) / layerH, 0.0, 1.0);

	// Soft height envelope — smooth ramp-up at the bottom and ramp-down at
	// the top so the density field has no hard box silhouette. heightFogFalloff
	// biases how much of the layer sits at the bottom vs the top.
	float hLow  = smoothstep(0.0, 0.25, hFrac);
	float hHigh = 1.0 - smoothstep(0.6, 1.0, hFrac);
	float heightGradient = hLow * hHigh;
	heightGradient = pow(heightGradient, max(heightFogFalloff, 0.01));
	if (heightGradient <= 0.0) return 0.0;

	// Base noise coordinate: XZ scaled into texture space, offset by wind and
	// a slow morph on the Z axis so banks reshape over time. Morph is divided
	// down because noise64cube is a 3D sampler and Z movement slices between
	// different 2D layers fast — even 0.01/frame flickers. Keep it sub-glacial.
	vec3 baseCoord = vec3(worldPos.xz * cloudScale + windXZ, morph * 0.004);
	float base = texture(noise64cube, baseCoord).r;
	// A second octave at ~2x freq for low-freq variety.
	float base2 = texture(noise64cube, baseCoord * 2.13 + vec3(0.37, 0.71, 0.13)).r;
	// Contrast-stretch the noise to [0..1] first. noise64cube returns values
	// clustered around 0.4–0.6, so without this step the coverage remap has
	// no dynamic range to work with. 2.0 * (n - 0.5) + 0.5 = widen around 0.5.
	float baseNoise = clamp((base * 0.65 + base2 * 0.35 - 0.5) * 2.2 + 0.5, 0.0, 1.0);

	// Coverage as a soft smoothstep instead of a hard remap. cloudCoverage
	// now linearly moves the midpoint: 0 = almost nothing survives,
	// 1 = almost everything. A smoothstep window of 0.35 keeps bank edges soft.
	float covMid = mix(0.85, 0.15, cloudCoverage);
	float shaped = smoothstep(covMid - 0.18, covMid + 0.18, baseNoise);
	if (shaped <= 0.0) return 0.0;

	// High-freq erode via the packed Worley noise. Keep the erode scale
	// moderate — a too-small tile means pixel-sized speckle. 1/8192 gives a
	// ~8k elmo tile which is big enough to show whole cells, not just noise.
	vec3 erodeCoord = vec3(
		fract(worldPos.x * (1.0/8192.0) + windXZ.x * 0.25 + morph * 0.001),
		fract(worldPos.z * (1.0/8192.0) + windXZ.y * 0.25),
		fract(hFrac * 0.25 + morph * 0.002)
	);
	float erode = getPackedNoise(erodeCoord).x;
	// Apply only to the edges: where shaped is already near 1 we leave it
	// alone, where it's mid-range we carve with erode. Classic Nubis move.
	shaped = shaped - erode * cloudErodeAmount * (1.0 - shaped);
	shaped = clamp(shaped, 0.0, 1.0);

	return shaped * heightGradient;
}
...
// ----------------- BEGIN HEIGHT-BASED FOG -------------------------------
	...
	// Nubis-style volumetric raymarch: step along the view ray between
	// rayStart and rayEnd, sampling sampleGroundCloudDensity at each step
	// and accumulating into the trapezoid totals that downstream code uses.
	// Bayer + blue-noise jitter on the starting t kills the step-boundary
	// banding that plain uniform stepping would cause.
	#define GROUND_CLOUD_STEPS 10
	vec2 cloudWindXZ = windFractFull.xy * cloudWindSpeed;
	float cloudMorph = cloudTime * cloudMorphSpeed;

	float startDensity = 0.0;
	float endDensity   = 0.0;
	{
		float stepT = 1.0 / float(GROUND_CLOUD_STEPS);
		// Weaker jitter: more steps already smooth the ray, heavy jitter
		// just adds pixel-scale variance that reads as speckle.
		float jitter = (thisQuadOffset + blueNoiseSample.x * 0.25) * stepT * 0.5;
		float accum = 0.0;
		for (int s = 0; s < GROUND_CLOUD_STEPS; s++) {
			float t = clamp((float(s) + 0.5) * stepT + jitter, 0.0, 1.0);
			vec3 samplePos = mix(rayStart, rayEnd, t);
			accum += sampleGroundCloudDensity(samplePos, cloudWindXZ, cloudMorph);
		}
		float avg = accum * stepT;
		// The downstream formula is (startDensity + endDensity) * 2 * rayLength
		// (a trapezoid over the ray). Feeding the average into both halves
		// gives us a clean rectangle integration equal to avg * 4 * rayLength.
		startDensity = avg;
		endDensity   = avg;

		// DEBUG off: use final integrated density downstream.
		#if 0
		fragColor = vec4(avg, avg, avg, 1.0); return;
		#endif
	}

	// Explosion shock-wave carve-out: analytic ray/sphere test per blast.
	// For each blast we compute the fraction of the view ray that lies
	// inside the sphere (using the perpendicular distance from the sphere
	// center to the infinite line, plus the chord length formula), and
	// carve fog proportional to that fraction. This is true volumetric
	// behavior — the effect reads as a 3D sphere from every angle,
	// independent of fog layer thickness.
	#define FOG_BLAST_MAX_CARVE 0.6
	float blastCarve = 1.0;
	vec3 rayDir = rayEnd - rayStart;
	float rayLenSq = max(dot(rayDir, rayDir), 1.0);
	float rayLenInv = inversesqrt(rayLenSq);
	vec3 rayDirN = rayDir * rayLenInv;
	for (int i = 0; i < fogBlastCount; i++) {
		int base = i * 4;
		vec3 blastPos = vec3(fogBlasts[base + 0], fogBlasts[base + 1], fogBlasts[base + 2]);
		float r = fogBlasts[base + 3];
		if (r <= 0.0) continue;

		// Project blast center onto the ray (clamped to the [start,end] segment).
		vec3 toBlast = blastPos - rayStart;
		float tProj = clamp(dot(toBlast, rayDirN), 0.0, 1.0 / rayLenInv);
		vec3 closest = rayStart + rayDirN * tProj;
		float perpDist = distance(closest, blastPos);

		// How much of the sphere volume this ray pierces. 1.0 when the ray
		// passes dead through the center, 0.0 at the grazing edge. Soft
		// gradient via smoothstep so the silhouette of the sphere is round.
		float pierce = 1.0 - smoothstep(0.0, r, perpDist);
		float opacity = fogBlastOpacity[i];
		blastCarve *= (1.0 - FOG_BLAST_MAX_CARVE * pierce * opacity);
	}
	startDensity *= blastCarve;
	endDensity   *= blastCarve;
...
// ----------------- END HEIGHT-BASED FOG -------------------------------
	...
	// ----------------- BEGIN COMPOSITING (GROUND FOG ONLY) ---------------------
	// Stripped version: only the height-based fog layer is composited.
	// Distance fog, cloud raymarching, cloud shadows, and underwater shadow
	// absorption are intentionally not blended into the output. Their upstream
	// work above is dead code and will be eliminated by the GLSL compiler.
	fragColor = heightBasedFogColor;
	// Clamp fog opacity so particles/scene behind always keep (1 - fogMaxOpacity)
	// of their original light. Prevents additive CEG particles from being fully
	// drowned by thick mist while still allowing distance attenuation.
	fragColor.a = min(fragColor.a, fogMaxOpacity);

	#if (FULLALPHA == 1)
		fragColor.a = 1.0;
	#endif
	#if (RESOLUTION == 1) // When not using combine shader, we must pre-multiply color with alpha!
