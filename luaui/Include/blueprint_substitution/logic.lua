...
if not BpDefs then
	error(
		"[BlueprintSubLogic] CRITICAL: Failed to load required blueprint definitions from: "
			.. defPath
			.. ". Substitution logic cannot initialize."
	)
end
...
function BlueprintSubLogic.validateCategoryDefinitions()
	...
	if next(duplicateCategories) then
		...
		Spring.Log(
			"BlueprintSubLogic",
			LOG.ERROR,
			string.format("[BlueprintSubLogic ERROR] Found duplicate category keys: %s", table.concat(dupes, ", "))
		)
	end
	...
	for unitName, categories in pairs(multiAssigned) do
		...
		Spring.Log(
			"BlueprintSubLogic",
			LOG.WARNING,
			string.format("Unit '%s' is assigned to multiple categories: %s", unitName, table.concat(categories, ", "))
		)
	end
	if count > 0 then
		Spring.Log(
			"BlueprintSubLogic",
			LOG.WARNING,
			string.format("Found %d units with multiple category assignments (This may be intended)", count)
		)
	else
...
Spring.Log(
	"BlueprintSubLogic",
	LOG.INFO,
	string.format("Generated Master Building Data for %d buildings.", buildingCount)
)
Spring.Log(
	"BlueprintSubLogic",
	LOG.INFO,
	"Internal data structures for substitution logic generated. Module ready to be used."
)
...
local function _getActualSubstitutedUnitName(originalUnitName, targetSide)
	...
	if not equivalentUnitName or equivalentUnitName == "" then
		Spring.Log(
			...
			LOG.WARNING,
			string.format(
				"_getActualSubstitutedUnitDefID: No mapping for unit '%s' to target side '%s'.",
				unitNameLower,
				targetSide
			)
		)
...
local function _generateSubstitutionSummary(aggregatedStats, itemTypeString, sourceSide, targetSide)
	...
	local verboseMessage = string.format(
		...
		stats.substituted,
		numFailedToMap,
		numSkippedOrUnchangedConsidered,
		stats.unchangedNotBuilding
	)
	...
	elseif stats.totalConsidered > 0 then
		...
		verboseMessage = verboseMessage
			.. string.format(" (No relevant %s items to process for substitution)", itemTypeString:lower())
	end
	...
	if stats.totalConsidered > 0 then
		...
		if substitutionActuallyFailed then
			details =
				string.format("%d/%d substituted, %d failed", stats.substituted, stats.totalConsidered, numFailedToMap)
		else
		...
		simpleMessage =
			string.format("%s from %s to %s: No relevant units to process.", itemTypeString, sourceSide, targetSide)
	end
...
function BlueprintSubLogic.processBlueprintSubstitution(originalBlueprint, targetSide)
	local sourceSide = originalBlueprint
		and originalBlueprint.sourceInfo
		and originalBlueprint.sourceInfo.primarySourceSide

	if not (originalBlueprint and originalBlueprint.units and targetSide) then
		Spring.Log(
			"BlueprintSubLogic",
			LOG.ERROR,
			"processBlueprintSubstitution: Called with invalid arguments (nil blueprint, units, or targetSide)."
		)
		local errorStats = {
			totalConsidered = 0,
			substituted = 0,
			...
			hadMappingFailures = true,
		}
		return {
			stats = errorStats,
			summaryMessage = "Internal error: Invalid arguments for substitution.",
	...
	if not sourceSide then
		...
		local errorStats = {
			totalConsidered = originalBlueprint.units and #originalBlueprint.units or 0,
			substituted = 0,
			failedNoMapping = originalBlueprint.units and #originalBlueprint.units or 0,
			failedInvalidEquivalent = 0,
			unchangedSameSide = 0,
			unchangedOther = 0,
			unchangedNotBuilding = 0,
	...
	Spring.Log(
		...
		LOG.DEBUG,
		string.format(
			"Processing blueprint substitution (in-place) from %s to %s for %d units.",
			tostring(sourceSide),
			tostring(targetSide),
			originalBlueprint and #originalBlueprint.units or 0
		)
	)
...
function BlueprintSubLogic.processBuildQueueSubstitution(originalBuildQueue, sourceSide, targetSide)
	if not (originalBuildQueue and sourceSide and targetSide) then
		Spring.Log(
			"BlueprintSubLogic",
			LOG.ERROR,
			"processBuildQueueSubstitution: Called with incomplete arguments (nil sourceSide or targetSide). Review caller logic."
		)
		local errorStats = {
			totalConsidered = 0,
			substituted = 0,
		...
		return {
			stats = errorStats,
			summaryMessage = "Internal error: Incomplete arguments for substitution.",
			substitutionFailed = true,
		}
	end
	...
	Spring.Log(
		...
		LOG.DEBUG,
		string.format(
			"Processing build queue substitution (in-place) from %s to %s for %d items.",
			sourceSide,
			targetSide,
			#originalBuildQueue
		)
	...
	for _, bq_item in ipairs(originalBuildQueue) do
		if type(bq_item) == "table" and #bq_item >= 1 then
			...
			if originalUnitDefID and originalUnitDefID > 0 then
				...
				if not originalUnitDef then
					...
					if buildingData then
						local outcome =
							_getBuildingSubstitutionOutcome(originalUnitName, buildingData, targetSide, sourceSide)
						if outcome.status == "substituted" then
						...
						Spring.Log(
							...
							LOG.DEBUG,
							string.format(
								"processBuildQueueSubstitution: No MasterBuildingData for %s. Item not substituted.",
								originalUnitName:lower()
							)
						)
					end
				end
			...
			Spring.Log(
				"BlueprintSubLogic",
				LOG.WARNING,
				string.format("processBuildQueueSubstitution: Skipping malformed item: %s", tostring(bq_item))
			)
		end
	end
