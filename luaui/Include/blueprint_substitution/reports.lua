...
function REPORTS.generateMappingReport()
	...
	for unitNameLower, buildingCoreData in pairs(masterBuildingDataMinimal) do
		...
		if unitDef then
			...
			Spring.Log(
				"BlueprintReports",
				"warning",
				"[Mapping Report] Could not find UnitDef for ID: "
					.. tostring(buildingCoreData.unitDefID)
					.. " Name: "
					.. unitNameLower
			)
		end
	...
	for side, total in pairs(sideTotals) do
		...
		table.insert(
			reportLines,
			string.format(
				"[Mapping Report Summary] %s: %d / %d buildings categorized (%.2f%%)",
				side,
				complete,
				total,
				percentage
	...
	if unknownCount > 0 then
		table.insert(
			reportLines,
			string.format("[Mapping Report Summary] Buildings with UNKNOWN side: %d", unknownCount)
		)
	end
