...
local function MakeAtlasOnDemand(config)
	...
	AtlasOnDemand.renderImageTaskList[1] =
		{ id = AtlasOnDemand.blankimg, w = config.sizex, h = config.sizey, x = 0, y = 0 } --, srcmode = GL.ZERO, dstmode = GL.ZERO}
	...
	function AtlasOnDemand:ReserveSpace(id, xsize, ysize)
		if self.uvcoords[id] then
			Spring.Echo(
				string.format("AtlasOnDemand %s Warning: ID %s is already added to this atlas", self.name, tostring(id))
			)
	...
	function AtlasOnDemand:AddImage(image, xsize, ysize)
		...
		if self.uvcoords[image] then
			Spring.Echo(
				string.format(
					"AtlasOnDemand %s Warning: image %s is already added to this atlas",
					self.name,
					tostring(image)
				)
			)
		...
		if xsize and ysize then
			...
			if texInfo and (texInfo.xsize ~= xsize or texInfo.ysize ~= ysize) then
				Spring.Echo(
					string.format(
						"AtlasOnDemand %s Warning: image %s size does not match %dx%d given, %dx%d from TextureInfo",
						self.name,
						tostring(image),
						xsize or -1,
						ysize or -1,
	...
	function AtlasOnDemand:PlaceImage(image, x, y, xsize, ysize)
		...
		if texInfo then
			...
			Spring.Echo(
				string.format(
					"AtlasOnDemand %s Warning:   unable to read gl.TextureInfo(%s)",
					self.name,
					tostring(image)
				)
			)
	...
	function AtlasOnDemand:AddText(text, params)
		...
		if not params then -- render with default fot
			if self.uvcoords[text] then
				...
				if not self.defaultfont then
					Spring.Echo(
						string.format(
							"AtlasOnDemand %s Warning: text %s without font cannot be added because atlas has no default font set",
							self.name,
							tostring(id)
						)
					)
		...
		if self.uvcoords[textparams.id] then
			Spring.Echo(
				string.format(
					"AtlasOnDemand %s Warning: text %s is already added to this atlas",
					self.name,
					tostring(textparams.id)
				)
			)
		...
		if self.debug then
			Spring.Echo(
				string.format(
					"Pre adjusted textsize for %s at size %d is w=%d h=%d descender=%d",
					textparams.text,
					textparams.size,
					width,
					height,
		...
		if self.debug then
			Spring.Echo(
				string.format(
					'AddText: "%s", size=%d, w=%d h=%d d=%d pad =%f',
					textparams.text,
					textparams.size,
					width,
					height,
	...
	function AtlasOnDemand:RenderTextTasks()
		...
		for i, task in ipairs(taskList) do
			...
			if not colormatches then
				font:SetTextColor(
					textparams.textcolor[1],
					textparams.textcolor[2],
					textparams.textcolor[3],
					textparams.textcolor[4]
				)
			...
			if not outlinematches then
				font:SetOutlineColor(
					textparams.outlinecolor[1],
					textparams.outlinecolor[2],
					textparams.outlinecolor[3],
					textparams.outlinecolor[4]
				)
			...
			if self.debug then
				Spring.Echo(
					string.format(
						"Task params: id = %s w=%.3f h=%.3f x=%.3f y=%.3f",
						task.id,
						task.w,
						task.h,
						task.x,
				...
				Spring.Echo(
					string.format(
						"renderText: text = %s, x = %f, y = %.3f, size = %.3f, opts = %s",
						textparams.text,
						x,
						y,
						textparams.size,
...
if UNITTEST then
	local atlas = MakeAtlasOnDemand({
		sizex = 124,
		sizey = 512,
		xresolution = 96,
		yresolution = 24,
		name = "QuadTree Atlas Tester",
	})
