...
local function MakeAtlasOnDemand(config)
	...
	function AtlasOnDemand:ReserveSpace(id, xsize, ysize)
		...
		if foundspot then
				...
				for x = self.firstemptycolumn, self.firstemptycolumn + xstep -1 do
					...
					if columnfull then
						if self.debug then SpringShared.Echo("Column Full!", self.firstemptycolumn) end
	...
	function AtlasOnDemand:AddText(text, params)
		...
		if self.debug then SpringShared.Echo(string.format("Pre adjusted textsize for %s at size %d is w=%d h=%d descender=%d", textparams.text, textparams.size, width, height, textdescender)) end
		local pad = 1
		...
		if self.debug then SpringShared.Echo(string.format('AddText: "%s", size=%d, w=%d h=%d d=%d pad =%f', textparams.text, textparams.size, width, height, textdescender, pad)) end
	...
	function AtlasOnDemand:RenderTextTasks()
		...
		for i, task in ipairs(taskList) do
			...
			if currentParams.font ~= textparams.font then
				...
				if self.debug then SpringShared.Echo("Font set to", textparams.font.path,textparams.size) end
