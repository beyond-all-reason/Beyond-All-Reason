...
function test()
	SpringShared.Echo("[test_wait] waiting 5 frames")
	Test.waitFrames(5)
	...
	SpringShared.Echo("[test_wait] waiting for UnitCreated on unitID=" .. createdUnitID)
	Test.waitUntilCallin("UnitCreated", function(unitID, unitDefID, unitTeam, builderID)
		SpringShared.Echo("Saw UnitCreated for unitID=" .. unitID)
		return unitID == createdUnitID
	end, 10)

	startFrame = SyncedProxy.Spring.GetGameFrame()

	SpringShared.Echo("[test_wait] waiting 3 frames, but the hard way")
	Test.waitUntil(function()
		return (SpringShared.GetGameFrame() - startFrame > 3)
	end)

	SpringShared.Echo("[test_wait] waiting 1000 ms")
	Test.waitTime(1000)
end
