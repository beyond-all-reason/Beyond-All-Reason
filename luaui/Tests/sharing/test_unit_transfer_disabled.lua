...
local UnitTransferUnsynced = Sharing.UnitTransferUnsynced
...
local function cleanup()
	SyncedRun(function()
		local orig = _G._origGetModOptions --[[@as function?]]
		if orig then
...
local function test()
	...
	assert(Spring.GetUnitTeam(unitID) == team0, "unit should still belong to team0 when sharing is disabled, got team: " .. tostring(Spring.GetUnitTeam(unitID)))
end
