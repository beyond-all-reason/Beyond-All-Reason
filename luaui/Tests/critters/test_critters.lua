...
function runCritterTest()
	...
	local function countAliveCritters()
		...
		for _, unitID in ipairs(Engine.Shared.GetAllUnits()) do
			local defID = Engine.Shared.GetUnitDefID(unitID)
	...
	SyncedRun(
		function(locals)
			...
			local function createUnit(def, x, z)
				x = midX + x
				z = midZ + z
				local y = Engine.Shared.GetGroundHeight(x, z) + 40
				Engine.Synced.CreateUnit(def, x, y, z, "south", GaiaTeamID)
			end
	...
	assertSuccessBefore(5, 5, function()
		return #Engine.Shared.GetAllUnits() == 36
	end)
	...
	assertSuccessBefore(30, 10, function()
		...
		Engine.Shared.Echo("Pressure units spawned:", aliveCount)
		...
		SyncedRun(
			function(locals)
				for _, unitID in ipairs(locals.pressureUnits) do
					if Engine.Shared.ValidUnitID(unitID) then
						Engine.Synced.DestroyUnit(unitID, false, true, nil, true)
					end
				end
			end,
			500,
	...
	-------------------------------------------------------
	Test.waitFrames(WAIT_FRAMES - (Engine.Shared.GetGameFrame() % WAIT_FRAMES))
