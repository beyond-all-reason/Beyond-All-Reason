...
function test()
	...
	-- B is the last listed target: it runs as a plain engine Attack ahead of the
	-- preserved Move, without a trailing controller, like a native queue would.
	Test.waitUntil(function()
		local queue = Spring.GetUnitCommands(sourceID, -1)
		return #queue == 2
			and queue[1].id == CMD.ATTACK
			and queue[1].params[1] == targetB
			and queue[2].id == CMD.MOVE
			and queue[2].params[1] == moveX
	end, 120)
