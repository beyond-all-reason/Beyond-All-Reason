...
local function createScenario()
	...
	local targetIDs = {}
	for index = 1, 4 do
		-- Spread the targets along x on dry ground; the shared list only accepts
		-- targets the weapon can engage, and the stump cannot hit units in water.
		local x = centerX + 2400 + (index - 1) * 120
		local z = centerZ + 120
		targetIDs[index] = assert(Spring.CreateUnit("corraid", x, Spring.GetGroundHeight(x, z), z, "west", 1))
		-- Target lists only accept targets inside sensors; keep all four visible so
		-- the first list holds several targets and keeps its controller.
		Spring.SetUnitAlwaysVisible(targetIDs[index], true)
	end
...
function test()
	...
	-- Let the sensor state of the new units settle before the list is built.
	Test.waitFrames(32)
