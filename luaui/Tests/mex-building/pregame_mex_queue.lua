...
function test()
	...
	local sx, sy, sz = Engine.Unsynced.WorldToScreenCoords(targetMex.x - 200, targetMex.y, targetMex.z - 200)
	...
	sx, sy, sz = Engine.Unsynced.WorldToScreenCoords(snappedPosition.x, snappedPosition.y, snappedPosition.z)
	Engine.Unsynced.WarpMouse(sx, sy)
