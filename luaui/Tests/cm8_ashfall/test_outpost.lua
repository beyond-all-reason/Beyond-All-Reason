...
function test()
	...
	assert(gaiaArmed == 0, ("%d unheld outpost weapons were still willing to open fire"):format(gaiaArmed))
	assert(countOutpost(playerTeamID) == 0, "the outpost must not change hands while the command hub is unseen")
	...
	assert(countOutpost(gaiaTeamID) == 0, "every unit in the group moves, or the player inherits half a base")
	...
	for _, unitID in ipairs(Spring.GetTeamUnits(playerTeamID) or {}) do
		...
		if unitDef and OUTPOST_DEFS[unitDef.name] and #(unitDef.weapons or {}) > 0 then
			local states = Spring.GetUnitStates(unitID) or {}
			assert((states.firestate or 0) > 0, "a handed-over outpost must be willing to defend its new owner")
		end
	end
