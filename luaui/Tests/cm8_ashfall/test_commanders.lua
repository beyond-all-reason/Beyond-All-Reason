...
function test()
	...
	Spring.Echo(
		("[cm8] occupied seat: before=%d after=%d incumbent=%s bound=%s"):format(
			before,
			after,
			tostring(incumbent),
			tostring(bound)
	...
	assert(bound == incumbent, "armada_commander must be bound to the commander the team already had")
