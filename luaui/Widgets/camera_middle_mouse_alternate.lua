...
local spWarpMouse = Engine.Unsynced.WarpMouse
...
function widget:Update(dt)
	if active then
		local speedFactor = Engine.Unsynced.GetConfigInt("OverheadScrollSpeed", 10)
...
function widget:MouseWheel(up, value)
	...
	local altDown, ctrlDown, metaDown, shiftDown = Engine.Unsynced.GetModKeyState()
	...
	if altDown then
		...
		Engine.Unsynced.SetCameraState(newCameraState, 0)
		return true
	end
	...
	local _, groundPos = Engine.Unsynced.TraceScreenRay(mouseX, mouseY, true)
