...
local function savegrassCmd(_, _, params)
	...
	texture = Utilities.NewTGA(mathFloor(mapSizeX / grassConfig.patchResolution), mathFloor(mapSizeZ / grassConfig.patchResolution), 1)
	local offset = 0
...
function widget:DrawWorldPreUnit()
	...
	if cy < ((grassConfig.grassShaderParams.FADEEND * distanceMult) + gh) and grassVAO ~= nil and #grassInstanceData > 0 then
		...
		if instanceCount <= 0 or startInstanceIndex == #grassInstanceData / 4 then
			return
		end
		...
		if placementMode and SpringShared.GetGameFrame() % 30 == 0 then
