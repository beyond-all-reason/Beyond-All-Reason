...
local function GetSave(path)
	...
	if not success then
		Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "Error getting save " .. path .. ": " .. err)
	else
...
local function SaveGame(filename, description, requireOverwrite)
	...
	local success, err = pcall(function()
		Engine.Unsynced.CreateDir(SAVE_DIR)
		...
		saveData.gameID = (Engine.Shared.GetGameRulesParam("save_gameID") or (Game.gameID and Game.gameID or Engine.Shared.GetGameRulesParam("GameID")))
		saveData.gameframe = spGetGameFrame()
		saveData.totalGameframe = spGetGameFrame() + (Engine.Shared.GetGameRulesParam("totalSaveGameFrame") or 0)
		saveData.playerName = Engine.Shared.GetPlayerInfo(Spring.GetMyPlayerID(), false)
		...
		if requireOverwrite then
			Engine.Unsynced.SendCommands(SAVE_TYPE .. filename .. " -y")
		else
			Engine.Unsynced.SendCommands(SAVE_TYPE .. filename)
		end
		Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, "Saved game to " .. path)

		--DisposeWindow()
	end)
	if not success then
		Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "Error saving game: " .. err)
	end
end
...
local function LoadGameByFilename(filename)
	...
	if saveData then
		if Engine.Unsynced.GetMenuName and Engine.Unsynced.SendLuaMenuMsg and Engine.Unsynced.GetMenuName() then
			Engine.Unsynced.SendLuaMenuMsg(LOAD_GAME_STRING .. filename)
		else
			local ext = GetSaveExtension(SAVE_DIR .. "/" .. filename)
			if not ext then
				Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "Error loading game: cannot find save file.")
				return
			end
			local success, err = pcall(function()
	...
	[GAME]
	{
		...
		MyPlayerName=__PLAYERNAME__;
				...
				Engine.Unsynced.Reload(script)
			end)
			if not success then
				Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "Error loading game: " .. err)
			end
		end
	else
		Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "Save game " .. filename .. " not found")
	end
...
local function DeleteSave(filename)
	if not filename then
		Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "No filename specified for save deletion")
	end
	local success, err = pcall(function()
	...
	if not success then
		Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "Error deleting save " .. filename .. ": " .. err)
	end
end

local function savegameCmd(_, _, params)
	Engine.Shared.Echo("Trying to save:", params[1])
	...
	if Engine.Unsynced.GetMenuName and string.find(string.lower(Engine.Unsynced.GetMenuName()), "chobby") ~= nil then
		Engine.Unsynced.SendLuaMenuMsg("gameSaved")
	end
end
