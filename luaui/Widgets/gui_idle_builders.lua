	...
	for unitDefID, unitDef in pairs(UnitDefs) do
		...
		if not (cp.virtualunit == "1") then
			...
			if
				...
				and (unitDef.canAssist or unitDef.buildOptions[1] or (showRez and unitDef.canResurrect))
		...
		for i = 1, maxIcons do
			if existingIcons[i] then
				local iconRect = {
					backgroundRect[1]
						+ backgroundPadding
						+ ((iconSize - backgroundPadding) * iconCounter)
						+ startOffsetX,
					backgroundRect[2] + (posY - height > 0 and backgroundPadding or 0),
					backgroundRect[1]
						+ backgroundPadding
						+ (iconSize - backgroundPadding)
						+ ((iconSize - backgroundPadding) * iconCounter)
						+ startOffsetX,
				...
				if unitCount > 1 then
					...
					font:Print(
						...
						iconRect[1] + iconMargin + (fontSize * 0.18),
						iconRect[4] - iconMargin - (fontSize * 0.92),
						fontSize,
						"o"
					)
					font:End()
...
local function Update()
	...
	if sec > 0.33 then
		...
		if WG.buildmenu then
			if
				buildmenuBottomPosition ~= WG.buildmenu.getBottomPosition()
				or buildmenuIsShowing ~= WG.buildmenu.getIsShowing()
			then
				widget:ViewResize()
				doUpdate = true
			end
