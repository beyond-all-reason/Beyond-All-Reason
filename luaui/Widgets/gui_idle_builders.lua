	...
	if numIcons > 0 then
		...
		for i = 1, maxIcons do
			if existingIcons[i] then
				...
				local zoom = i == hoveredIcon and 0.105 or 0.05
...
local function isWorkerUnitIdle(unitID, unitDefID)
	return inIdleWorkerTask[unitID] or (unitConf[unitDefID] and spGetFactoryCommandCount(unitID) or spGetUnitCommandCount(unitID)) == 0
end
