	...
	for player, data in pairs(drawCmds) do
		...
		if sum > blocklimit then
			...
			if not wasBlocked then
				Echo(BAR.I18N("ui.mapDrawBlocker.block", { player = (WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(player) or GetPlayerInfo(player, false) }))
			end
		end
		if sum < unblocklimit and data.blocked and (currentCounter - data.blocked > unblocklimit) then
			data.blocked = false
			Echo(BAR.I18N("ui.mapDrawBlocker.unblock", { player = (WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(player) or GetPlayerInfo(player, false) }))
		end
	end
end
