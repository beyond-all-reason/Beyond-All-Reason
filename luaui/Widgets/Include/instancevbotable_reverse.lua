...
local function validateInstanceVBOTable(iT, calledfrom)
	...
	if (indextoInstanceIDsize ~= instanceIDtoIndexsize) or  then
		Spring.Echo("Table size mismatch during validation of", iT.myName, indextoInstanceIDsize, instanceIDtoIndexsize)
	end
...
function pushElementInstance(iT,thisInstance, instanceID, updateExisting, noUpload)
	...
	if noUpload ~= true then --upload or mark as dirty
		...
		--Spring.Echo("pushElementInstance,unitID, iT.unitIDattribID, thisInstanceIndex", thisInstanceIndex)
	else
		iT.dirty = true
	end
