...
function widget:PlayerChanged()
	if Engine.Unsynced.GetSpectatingState() and spGetGameFrame() > 0 then
...
local function GetUnitOrFeaturePosition(id)
	if id < Game.maxUnits then
		...
		return Engine.Shared.GetFeaturePosition(id - Game.maxUnits)
	end
end
...
function widget:CommandNotify(id, params, options)
	...
	if options.shift then
		...
		if modifiers.prepend_queue then
			Engine.Unsynced.GiveOrder(CMD.INSERT, { prependPos, id, opt, unpack(params) }, { "alt" })
		...
		Engine.Unsynced.GiveOrder(CMD.INSERT, { 0, id, opt, unpack(params) }, { "alt" })
	...
	local units = Engine.Unsynced.GetSelectedUnits()
	for i = 1, #units do
		local unit_id = units[i]
		local commands = Engine.Shared.GetUnitCommands(unit_id, 100)
	...
	if id < 0 then
		Engine.Unsynced.SetActiveCommand(Engine.Unsynced.GetCmdDescIndex(id), 1, true, false, options.alt, options.ctrl, false, false)
	end
