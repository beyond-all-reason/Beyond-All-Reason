			...
			if aboveRectangle(mousePos, boxData) then
				...
				for borderName, borderData in pairs(borderRemap) do
					...
					if coordDir == "min" then
						mask[borderName] = mousePos[coordName]
							< (boxData.absSizes[coordName][coordDir] + draggingBorderSize * -1 * dir)
					else
						mask[borderName] = mousePos[coordName]
							> (boxData.absSizes[coordName][coordDir] + draggingBorderSize * -1 * dir)
					end
		...
		for _, teamID in ipairs(GetTeamList(allyTeamID)) do
			if teamID ~= GetGaiaTeamID() then
				...
				if history then
					...
					playerName = (WG.playernames and WG.playernames.getPlayername)
							and WG.playernames.getPlayername(leader)
						or playerName
...
local function DrawBackground()
	...
	local x1, y1, x2, y2 =
		...
		mathFloor(guiData.mainPanel.absSizes.x.max),
		mathFloor(guiData.mainPanel.absSizes.y.max)
	UiElement(
		x1 - bgpadding,
		y1 - bgpadding,
		x2 + bgpadding,
...
function ReGenerateBackgroundDisplayList()
	...
	for lineCount = 1, prevNumLines do
		...
		if evenLineColour and lineCount > 2 then
			...
			RectRound(
				mathFloor(boxSizes.x.min),
				mathFloor(boxSizes.y.max - lineCount * lineHeight),
				mathFloor(boxSizes.x.max),
				mathFloor(boxSizes.y.max - (lineCount - 1) * lineHeight),
				bgpadding,
				0,
				0,
				bottomCorner,
				bottomCorner,
				{ colour[1], colour[2], colour[3], colour[4] * ui_opacity },
				{ colour[1], colour[2], colour[3], colour[4] * 3 * ui_opacity }
			)
	...
	if selectedLine and selectedLine < 3 and selectedColumn and selectedColumn > 0 and selectedColumn <= numColums then
		if sortAscending then
			glColor(
				sortHighLightColour[1],
				sortHighLightColour[2],
				sortHighLightColour[3],
				sortHighLightColour[4] * ui_opacity
			)
		else
			glColor(
				sortHighLightColourDesc[1],
				sortHighLightColourDesc[2],
				sortHighLightColourDesc[3],
				sortHighLightColourDesc[4] * ui_opacity
			)
		end
		local x1, x2 = getColumnBounds(selectedColumn)
		RectRound(
			mathFloor(x1),
			mathFloor(boxSizes.y.max - 2 * lineHeight),
			mathFloor(x2),
			mathFloor(boxSizes.y.max),
			bgpadding,
			0,
			0,
			1,
	...
	for selectedIndex, headerName in ipairs(header) do
		if sortVar == headerName then
			if sortAscending then
				...
				glColor(
					activeSortColourDesc[1],
					activeSortColourDesc[2],
					activeSortColourDesc[3],
					activeSortColourDesc[4] * ui_opacity
				)
			end
...
function ReGenerateTextDisplayList()
	...
	for column, headerName in ipairs(header) do
		...
		font:Print(
			headerRemap[headerName][1],
			columnX,
			baseYSize + heightCorrection - lineCount * lineHeight,
			(fontSize * fontSizePercentage),
			"dco"
		)
		font:Print(
			headerRemap[headerName][2],
			columnX,
			baseYSize + heightCorrection - (lineCount + 1) * lineHeight,
			(fontSize * fontSizePercentage),
			"dco"
		)
	end
			...
			for i, varName in ipairs(header) do
				...
				if i == 1 then
					...
					font2:Print(
						color .. value,
						columnX,
						baseYSize + (heightCorrection * 1.66) - lineCount * lineHeight,
						(fontSize * fontSizePercentage),
						"dco"
					)
					font2:End()
