...
for unitDefID, unitDef in ipairs(UnitDefs) do
	-- Only include neutral defs that should cause areas to become targeted orders;
	-- do not include untargetable units: critters, crashing aircraft, decorations.
	unitNeutrality[unitDefID] = unitDef.customParams.objectify ~= nil
end
...
function widget:CommandNotify(cmdID, cmdParams, cmdOpts)
	...
	if cmdID == CMD_ATTACK then
		...
		for i = 1, #selectedUnits do
			...
			-- Keep for godmode selections without LOS access:
	...
	local newCommands = append and {} or {{ commandClearOrders[cmdID], nil, 0 }}
	local count = #newCommands

	for i = 1, #areaUnits do
		local unitID = areaUnits[i]
		if unitNeutrality[spGetUnitDefID(unitID)] and spGetUnitNeutral(unitID) then
			excludeTargets = true
		else
			includeTargets = true
			count = count + 1
