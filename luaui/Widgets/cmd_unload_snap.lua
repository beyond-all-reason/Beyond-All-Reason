...
function widget:Update()
	...
	for unitID, data in pairs(queuedUnitShapes) do
		...
		for i = 1, #Q do
			...
			if
				cmd.id == data.cmdID
				and math.floor(cmd.params[1] / 16) * 16 == data.cmdParams[1]
				and math.floor(cmd.params[3] / 16) * 16 == data.cmdParams[3]
			then
				found = true
				break
			end
	...
	if blocked == 0 then
		newPosX, newPosY, newPosZ = Spring.ClosestBuildPos(
			Spring.GetMyTeamID(),
			unloadPadDefID,
			cursorPos.x,
			cursorPos.y,
...
function MouseRelease(x, y, button)
	if button == 1 and buildCmd and buildCmd[1] then
		...
		Spring.GiveOrderToUnitArray(
			...
			buildCmd[1][1],
			{ buildCmd[1][2], buildCmd[1][3], buildCmd[1][4] },
			shift and { "shift" } or {}
		)
	end
...
function RegisterQueuedUnitShape(unitID, unitShape)
	if queuedUnitShapes[unitID] then
		WG.StopDrawUnitShapeGL4(queuedUnitShapes[unitID].unitShapeID)
		queuedUnitShapes[unitID] = nil
	end
	local unitShapeID = WG.DrawUnitShapeGL4(
		unitShape[1],
		unitShape[2],
		unitShape[3],
		unitShape[4],
	...
	queuedUnitShapes[unitID] = {
		...
		cmdParams = {
			math.floor(unitShape[2] / 16) * 16,
			math.floor(unitShape[3] / 16) * 16,
			math.floor(unitShape[4] / 16) * 16,
		},
