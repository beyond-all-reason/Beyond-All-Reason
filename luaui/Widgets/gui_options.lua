...
for key, code in ipairs(languageCodes) do
	languageNames[key] = I18N.languages[code]
end
...
for key, code in ipairs(devLanguageCodes) do
	devLanguageNames[key] = I18N.languages[code]
end
...
local devMode = Utilities.IsDevMode()
...
local reclaimFieldHighlightOptions = {
	I18N('ui.settings.option.reclaimfieldhighlight_always'),
	I18N('ui.settings.option.reclaimfieldhighlight_resource'),
	I18N('ui.settings.option.reclaimfieldhighlight_reclaimer'),
	I18N('ui.settings.option.reclaimfieldhighlight_resbot'),
	I18N('ui.settings.option.reclaimfieldhighlight_order'),
	I18N('ui.settings.option.reclaimfieldhighlight_disabled')
}

local spectatorHUDConfigOptions = {
	I18N('ui.settings.option.spectator_hud_config_basic'),
	I18N('ui.settings.option.spectator_hud_config_advanced'),
	I18N('ui.settings.option.spectator_hud_config_expert'),
	I18N('ui.settings.option.spectator_hud_config_custom'),
}

local startScript = VFS.LoadFile("_script.txt")
if not startScript then
	local modoptions = ''
	for key, value in pairs(GetModOptionsCopy()) do
		local v = value
...
function DrawWindow()
	...
	if devMode or devUI then
		title = devOptionColor .. I18N('ui.settings.option.devmode')
	elseif advSettings then
		title = color2 .. I18N('ui.settings.basic') .. "  /  " .. color .. I18N('ui.settings.advanced')
	else
		title = color .. I18N('ui.settings.basic') .. color2 .. "  /  " .. I18N('ui.settings.advanced')
	end
	...
	if changesRequireRestart then
		...
		font:SetOutlineColor(0, 0, 0, 0.4)
		font:Print(I18N('ui.settings.madechanges'), screenX + math.floor(screenWidth*0.5), screenY - screenHeight + (12 * widgetScale), 15 * widgetScale, "cn")
	end
...
function applyOptionValue(i, newValue, skipRedrawWindow, force)
	...
	if options[i].id ~= 'preset' and presets.lowest[options[i].id] ~= nil and manualChange then
		if options[getOptionByID('preset')] then
			options[getOptionByID('preset')].value = I18N('ui.settings.option.select_custom')
...
function init()
	...
	if devMode or devUI then
		...
		presetNames = {
			I18N('ui.settings.option.select_lowest'),
			I18N('ui.settings.option.select_low'),
			I18N('ui.settings.option.select_medium'),
			I18N('ui.settings.option.select_high'),
			I18N('ui.settings.option.select_ultra'),
			I18N('ui.settings.option.select_custom')
		}
	...
	optionGroups = {
		{ id = 'gfx', name = I18N('ui.settings.group.graphics'), numOptions = 0 },
		{ id = 'ui', name = I18N('ui.settings.group.interface'), numOptions = 0 },
		{ id = 'game', name = I18N('ui.settings.group.game'), numOptions = 0 },
		{ id = 'control', name = I18N('ui.settings.group.control'), numOptions = 0 },
		{ id = 'sound', name = I18N('ui.settings.group.audio'), numOptions = 0 },
		{ id = 'notif', name = I18N('ui.settings.group.notifications'), numOptions = 0 },
		{ id = 'accessibility', name = I18N('ui.settings.group.accessibility'), numOptions = 0 },
		{ id = 'custom', name = I18N('ui.settings.group.custom'), numOptions = 0 },
		{ id = 'dev', name = I18N('ui.settings.group.dev'), numOptions = 0 },
	}
	...
	options = {
		--GFX
		{ id = "preset", group = "gfx", category = types.basic, name = I18N('ui.settings.option.preset'), type = "select", options = presetNames,
			onload = function(i)
		...
		{ id = "label_gfx_screen", group = "gfx", name = I18N('ui.settings.option.label_screen'), category = types.basic },
		{ id = "label_gfx_screen_spacer", group = "gfx", category = types.basic },
		{ id = "display", group = "gfx", category = types.basic, name = I18N('ui.settings.option.display'), type = "select", options = displayNames, value = currentDisplay,
			onchange = function(i, value)
		...
		{ id = "resolution", group = "gfx", category = types.basic, name = widgetOptionColor .. "  " .. I18N('ui.settings.option.resolution'), type = "select", options = resolutionNames, value = Spring.GetConfigInt('SelectedScreenMode', 1), description = I18N('ui.settings.option.resolution_descr'),
		...
		{ id = "dualmode_enabled", group = "gfx", category = types.dev, name = I18N('ui.settings.option.dualmode'), type = "bool", value = Spring.GetConfigInt("DualScreenMode"), description = I18N('ui.settings.option.dualmode_enabled_descr'),
		  onchange = function(_, value)
			  Spring.SetConfigInt("DualScreenMode", value and 1 or 0)
		  end,
		},
		{ id = "dualmode_left", group = "gfx", category = types.dev, name = widgetOptionColor .. "  " .. I18N('ui.settings.option.dualmode_left'), type = "bool", value = Spring.GetConfigInt("DualScreenMiniMapOnLeft"), description = I18N('ui.settings.option.dualmode_left_descr'),
		  onchange = function(_, value)
			  Spring.SetConfigInt("DualScreenMiniMapOnLeft", value and 1 or 0)
		  end,
		},
		{ id = "dualmode_minimap_aspectratio", group = "gfx", category = types.dev, name = widgetOptionColor .. "  " .. I18N('ui.settings.option.dualmode_minimap_aspectratio'), type = "bool", value = Spring.GetConfigInt("DualScreenMiniMapAspectRatio"), description = I18N('ui.settings.option.dualmode_minimap_aspectratio_descr'),
		  onchange = function(_, value)
			  Spring.SetConfigInt("DualScreenMiniMapAspectRatio", value and 1 or 0)
		  end,
		},
		{ id = "vsync", group = "gfx", category = types.basic, name = I18N('ui.settings.option.vsync'),  type = "select", options = { I18N('ui.settings.option.select_off'), I18N('ui.settings.option.select_enabled'), I18N('ui.settings.option.select_adaptive')}, value = 2, description = I18N('ui.settings.option.vsync_descr'),
		  onload = function(i)
		...
		{ id = "vsync_fraction", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.vsync_fraction'), min = 1, max = 4, step = 1, type = "slider", value = Spring.GetConfigInt("VSyncFraction", 1), description = I18N('ui.settings.option.vsync_fraction_descr'),
		  onchange = function(i, value)
		...
		{ id = "limitoffscreenfps", group = "gfx", category = types.advanced, widget = "Limit idle FPS", name = I18N('ui.settings.option.limitoffscreenfps'), type = "bool", value = GetWidgetToggleValue("Limit idle FPS"), description = I18N('ui.settings.option.limitoffscreenfps_descr') },
		{ id = "limitidlefps", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.limitidlefps'), type = "bool", value = (Spring.GetConfigInt("LimitIdleFps", 0) == 1), description = I18N('ui.settings.option.limitidlefps_descr'),
		  onchange = function(i, value)
			  Spring.SetConfigInt("LimitIdleFps", (value and 1 or 0))
		  end,
		},

		{ id = "msaa", group = "gfx", category = types.basic, name = I18N('ui.settings.option.msaa'), type = "select", options = { I18N('ui.settings.option.select_off'), 'x2', 'x4', 'x8'}, restart = true, value = tonumber(Spring.GetConfigInt("MSAALevel", 0) or 0), description = I18N('ui.settings.option.msaa_descr'),
		  onload = function(i)
		...
		{ id = "supersampling", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.supersampling'), type = "bool", restart = false, value = (Spring.GetConfigFloat("MinSampleShadingRate", 0.0) == 1.0), description = I18N('ui.settings.option.supersampling_descr'),
		  onchange = function(i, value)
			Spring.SetConfigFloat("MinSampleShadingRate", (value and 1.0 or 0.0))
		  end,
		},

		{ id = "cas_sharpness", group = "gfx", category = types.advanced, name = I18N('ui.settings.option.cas_sharpness'), min = 0.5, max = 1.1, step = 0.01, type = "slider", value = 1.0, description = I18N('ui.settings.option.cas_sharpness_descr'),
		  onload = function(i)
		...
		{ id = "sepiatone", group = "gfx", category = types.advanced, widget = "Sepia Tone", name = I18N('ui.settings.option.sepiatone'), type = "bool", value = GetWidgetToggleValue("Sepia Tone") },
		{ id = "sepiatone_gamma", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sepiatone_gamma'), min = 0.1, max = 0.9, step = 0.02, type = "slider", value = 0.5,
		  onload = function(i)
		...
		{ id = "sepiatone_saturation", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sepiatone_saturation'), min = 0, max = 1, step = 0.02, type = "slider", value = 0.5,
		  onload = function(i)
		...
		{ id = "sepiatone_contrast", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sepiatone_contrast'), min = 0.1, max = 0.9, step = 0.02, type = "slider", value = 0.5,
		  onload = function(i)
		...
		{ id = "sepiatone_sepia", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sepiatone_sepia'), min = 0, max = 0.5, step = 0.02, type = "slider", value = 0.5, description = I18N('ui.settings.option.sepiatone_sepia_descr'),
		  onload = function(i)
		...
		{ id = "sepiatone_shadeui", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sepiatone_shadeui'), type = "bool", value = 0,
		  onload = function(i)
		...
		{ id = "label_gfx_lighting", group = "gfx", name = I18N('ui.settings.option.label_lighting'), category = types.basic },
		{ id = "label_gfx_lighting_spacer", group = "gfx", category = types.basic },


		{ id = "advmapshading", group = "gfx", category = types.basic, name = I18N('ui.settings.option.advmapshading'), type = "bool", value = (Spring.GetConfigInt("AdvMapShading", 1) == 1), description = I18N('ui.settings.option.advmapshading_descr'),
		 onchange = function(i, value)
		...
		{ id = "cusgl4", group = "gfx", name = I18N('ui.settings.option.cus'), category = types.basic, type = "bool", value = (Spring.GetConfigInt("cus2", 1) == 1), description = I18N('ui.settings.option.cus_descr'),
		  onchange = function(i, value)
			  if value == 0.5 then
		...
		{ id = "shadows_opacity", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.shadows_opacity'), type = "slider", min = 0.3, max = 1, step = 0.01, value = gl.GetSun("shadowDensity"), description = '',
		  onchange = function(i, value)
			  Spring.SetSunLighting({ groundShadowDensity = value, modelShadowDensity = value })
		  end,
		},

		{ id = "ssao", group = "gfx", category = types.basic, widget = "SSAO", name = I18N('ui.settings.option.ssao'), type = "bool", value = GetWidgetToggleValue("SSAO"), description = I18N('ui.settings.option.ssao_descr') },
		{ id = "ssao_strength", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.ssao_strength'), type = "slider", min = 5, max = 11, step = 1, value = 8, description = '',
		  onload = function(i)
		...
		{ id = "bloomdeferred", group = "gfx", category = types.basic, widget = "Bloom Shader Deferred", name = I18N('ui.settings.option.bloomdeferred'), type = "bool", value = GetWidgetToggleValue("Bloom Shader Deferred"), description = I18N('ui.settings.option.bloomdeferred_descr') },
		{ id = "bloomdeferredbrightness", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.bloomdeferredbrightness'), type = "slider", min = 0.4, max = 1.4, step = 0.05, value = 0.9, description = '',
		  onload = function(i)
		...
		{ id = "lighteffects", group = "gfx", category = types.basic, name = I18N('ui.settings.option.lighteffects'), type = "bool", value = GetWidgetToggleValue("Deferred rendering GL4"), description = I18N('ui.settings.option.lighteffects_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
			  if value then
		...
		{ id = "lighteffects_headlights", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.lighteffects_headlights'), type = "bool", value = Spring.GetConfigInt("headlights", 1) == 1, description = I18N('ui.settings.option.lighteffects_headlights_descr'),
		  onchange = function(i, value)
		...
		{ id = "lighteffects_buildlights", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.lighteffects_buildlights'), type = "bool", value = Spring.GetConfigInt("buildlights", 1) == 1, description = I18N('ui.settings.option.lighteffects_buildlights_descr'),
		  onchange = function(i, value)
		...
		{ id = "lighteffects_brightness", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.lighteffects_brightness'), min = 0.4, max = 1.5, step = 0.05, type = "slider", value = 1, description = I18N('ui.settings.option.lighteffects_brightness_descr'),
		  onload = function(i)
		...
		{ id = "lighteffects_radius", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.lighteffects_radius'), min = 0.4, max = 1.2, step = 0.05, type = "slider", value = 1, description = I18N('ui.settings.option.lighteffects_radius_descr'),
		  onload = function(i)
		...
		{ id = "lighteffects_screenspaceshadows", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.lighteffects_screenspaceshadows'), min = 0, max = 4, step = 1, type = "slider", value = 2, description = I18N('ui.settings.option.lighteffects_screenspaceshadows_descr'),
		  onload = function(i)
		...
		{ id = "distortioneffects", group = "gfx", category = types.basic, widget = "Distortion GL4", name = I18N('ui.settings.option.distortioneffects'), type = "bool", value = GetWidgetToggleValue("Distortion GL4"), description = I18N('ui.settings.option.distortioneffects_descr') },

		{ id = "darkenmap", group = "gfx", category = types.advanced, name = I18N('ui.settings.option.darkenmap'), min = 0, max = 0.33, step = 0.01, type = "slider", value = 0, description = I18N('ui.settings.option.darkenmap_descr'),
		  onload = function(i)
		...
		{ id = "label_gfx_environment", group = "gfx", name = I18N('ui.settings.option.label_environment'), category = types.basic },
		{ id = "label_gfx_environment_spacer", group = "gfx", category = types.basic },

		{ id = "featuredrawdist", group = "gfx", category = types.advanced, name = I18N('ui.settings.option.featuredrawdist'), type = "slider", min = 2500, max = 40000, step = 500, value = tonumber(Spring.GetConfigInt("FeatureDrawDistance", 10000)), description = I18N('ui.settings.option.featuredrawdist_descr'),
		  onchange = function(i, value)
		...
		{ id = "losopacity", group = "gfx", category = types.advanced, name = I18N('ui.settings.option.lineofsight')..widgetOptionColor .. "  " .. I18N('ui.settings.option.losopacity'), type = "slider", min = 0.01, max = 1, step = 0.01, value = (WG['los'] ~= nil and WG['los'].getOpacity ~= nil and WG['los'].getOpacity()) or 1, description = '',
		  onload = function(i)
		...
		{ id = "water", group = "gfx", category = types.basic, name = I18N('ui.settings.option.water'), type = "select", options = { I18N('ui.settings.option.select_low'), I18N('ui.settings.option.select_high') }, value = desiredWaterValue == 4 and 2 or 1,
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "mapedgeextension", group = "gfx", category = types.advanced, widget = "Map Edge Extension", name = I18N('ui.settings.option.mapedgeextension'), type = "bool", value = GetWidgetToggleValue("Map Edge Extension"), description = I18N('ui.settings.option.mapedgeextension_descr') },

		{ id = "mapedgeextension_brightness", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.mapedgeextension_brightness'), min = 0.2, max = 1, step = 0.01, type = "slider", value = 0.3, description = '',
		  onload = function(i)
		...
		{ id = "mapedgeextension_curvature", category = types.dev, group = "gfx", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.mapedgeextension_curvature'), type = "bool", value = true, description = I18N('ui.settings.option.mapedgeextension_curvature_descr'),
		  onload = function(i)
		...
		{ id = "decalsgl4", group = "gfx", category = types.basic, widget = "Decals GL4", name = I18N('ui.settings.option.decalsgl4'), type = "bool", value = GetWidgetToggleValue("Decals GL4") },
		{ id = "decalsgl4_lifetime", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.decalsgl4_lifetime'), min = 0.5, max = 8, step = 0.1, type = "slider", value = 1, description = I18N('ui.settings.option.decalsgl4_lifetime_descr'),
		  onload = function(i)
		...
		{ id = "decals", group = "gfx", category = types.basic, name = I18N('ui.settings.option.decals'), restart = true, min = 0, max = 3, step = 1, type = "slider", value = Spring.GetConfigInt("GroundDecals", 0), description = I18N('ui.settings.option.decals_descr'),
		  onchange = function(i, value)
		...
		{ id = "grass", group = "gfx", category = types.basic, widget = "Map Grass GL4", name = I18N('ui.settings.option.grass'), type = "bool", value = GetWidgetToggleValue("Map Grass GL4"), description = I18N('ui.settings.option.grass_desc') },
		{ id = "grassdistance", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.grassdistance'), type = "slider", min = 0.3, max = 1, step = 0.01, value = 1, description = I18N('ui.settings.option.grassdistance_desc'),
		  onload = function(i)
		...
		{ id = "treewind", group = "gfx", category = types.dev, name = I18N('ui.settings.option.treewind'), type = "bool", value = tonumber(Spring.GetConfigInt("TreeWind", 1) or 1) == 1, description = I18N('ui.settings.option.treewind_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "snow", group = "gfx", category = types.basic, widget = "Snow", name = I18N('ui.settings.option.snow'), type = "bool", value = GetWidgetToggleValue("Snow"), description = I18N('ui.settings.option.snow_descr') },
		{ id = "snowmap", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.snowmap'), type = "bool", value = true, description = I18N('ui.settings.option.snowmap_descr'),
		  onload = function(i)
		...
		{ id = "snowautoreduce", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.snowautoreduce'), type = "bool", value = true, description = I18N('ui.settings.option.snowautoreduce_descr'),
		  onload = function(i)
		...
		{ id = "snowamount", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.snowamount'), type = "slider", min = 0.2, max = 3, step = 0.2, value = 1, description = I18N('ui.settings.option.snowamount_descr'),
		  onload = function(i)
		...
		{ id = "clouds", group = "gfx", category = types.advanced, widget = "Volumetric Clouds", name = I18N('ui.settings.option.clouds'), type = "bool", value = GetWidgetToggleValue("Volumetric Clouds"), description = '' },
		{ id = "clouds_opacity", group = "gfx", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.clouds_opacity'), type = "slider", min = 0.2, max = 1.4, step = 0.05, value = 1, description = '',
		  onload = function(i)
		...
		{ id = "fogmult", group = "gfx", category = types.advanced, name = I18N('ui.settings.option.fog'), type = "slider", min = 0, max = 1, step = 0.01, value = Spring.GetConfigFloat("FogMult", 1), description = I18N('ui.settings.option.fogmult_descr'),
		  onload = function(i)
		...
		{ id = "label_gfx_effects", group = "gfx", name = I18N('ui.settings.option.label_effects'), category = types.basic },
		{ id = "label_gfx_effects_spacer", group = "gfx", category = types.basic },

		{ id = "particles", group = "gfx", category = types.basic, name = I18N('ui.settings.option.particles'), type = "slider", min = 10000, max = 40000, step = 1000, value = tonumber(Spring.GetConfigInt("MaxParticles", 1) or 15000), description = I18N('ui.settings.option.particles_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "dof", group = "gfx", category = types.advanced, widget = "Depth of Field", name = I18N('ui.settings.option.dof'), type = "bool", value = GetWidgetToggleValue("Depth of Field"), description = I18N('ui.settings.option.dof_descr') },
		{ id = "dof_autofocus", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.dof_autofocus'), type = "bool", value = true, description = I18N('ui.settings.option.dof_autofocus_descr'),
		  onload = function(i)
		...
		{ id = "dof_fstop", group = "gfx", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.dof_fstop'), type = "slider", min = 1, max = 6, step = 0.1, value = 2, description = I18N('ui.settings.option.dof_fstop_descr'),
		  onload = function(i)
		...
		{ id = "label_gfx_game", group = "gfx", name = I18N('ui.settings.option.label_game'), category = types.advanced },
		{ id = "label_gfx_game_spacer", group = "gfx", category = types.basic },
		{ id = "resurrectionhalos", group = "gfx", category = types.advanced, widget = "Resurrection Halos GL4", name = I18N('ui.settings.option.resurrectionhalos'), type = "bool", value = GetWidgetToggleValue("Resurrection Halos GL4"), description = I18N('ui.settings.option.resurrectionhalos_descr') },


		-- SOUND
		{ id = "snddevice", group = "sound", category = types.advanced, name = I18N('ui.settings.option.snddevice'), type = "select", restart = true, options = soundDevices, value = soundDevicesByName[Spring.GetConfigString("snd_device")], description = I18N('ui.settings.option.snddevice_descr'),
		  onchange = function(i, value)
		...
		{ id = "sndvolmaster", group = "sound", category = types.basic, name = I18N('ui.settings.option.volume') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.sndvolmaster'), type = "slider", min = 0, max = 80, step = 2, value = tonumber(Spring.GetConfigInt("snd_volmaster", 1) or 80),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "sndvolgeneral", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sndvolgeneral'), type = "slider", min = 0, max = 100, step = 2, value = tonumber(Spring.GetConfigInt("snd_volgeneral", 1) or 100),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "sndvolbattle", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sndvolbattle'), type = "slider", min = 0, max = 100, step = 2, value = tonumber(Spring.GetConfigInt("snd_volbattle_options", 100) or 100),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "sndvolui", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sndvolui'), type = "slider", min = 0, max = 100, step = 2, value = tonumber(Spring.GetConfigInt("snd_volui", 1) or 100),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		--  end,
		--},
		{ id = "console_chatvolume", group = "sound", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.console_chatvolume'), type = "slider", min = 0, max = 1, step = 0.01, value = (WG['chat'] ~= nil and WG['chat'].getChatVolume() or 0), description = I18N('ui.settings.option.console_chatvolume_descr'),
		  onload = function(i)
		...
		{ id = "mapmarkvolume", group = "sound", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.console_mapmarkvolume'), type = "slider", min = 0, max = 1, step = 0.01, value = (WG['mapmarkping'] ~= nil and WG['mapmarkping'].getMapmarkVolume() or 0.6), description = I18N('ui.settings.option.console_mapmarkvolume_descr'),
		  onload = function(i)
		...
		{ id = "sndvolmusic", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sndvolmusic'), type = "slider", min = 0, max = 99, step = 1, value = tonumber(Spring.GetConfigInt("snd_volmusic", 50) or 50),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "sndunitsound", group = "sound", category = types.advanced, name = I18N('ui.settings.option.sndunitsound'), type = "bool", value = (Spring.GetConfigInt("snd_unitsound", 1) == 1), description = I18N('ui.settings.option.sndunitsound_desc'),
		  onchange = function(i, value)
			  Spring.SetConfigInt("snd_unitsound", (value and 1 or 0))
		  end,
		},

		{ id = "sndairabsorption", group = "sound", category = types.advanced, name = I18N('ui.settings.option.sndairabsorption'), type = "slider", min = 0, max = 0.4, step = 0.01, value = tonumber(Spring.GetConfigFloat("snd_airAbsorption", .35) or .35), description = I18N('ui.settings.option.sndairabsorption_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "sndzoomvolume", group = "sound", category = types.advanced, name = I18N('ui.settings.option.sndzoomvolume'), type = "slider", min = 0, max = 3, step = 0.01, value = tonumber(Spring.GetConfigFloat("snd_zoomVolume", 1.00) or 1.00), description = I18N('ui.settings.option.sndzoomvolume_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "muteoffscreen", group = "sound", category = types.advanced, name = I18N('ui.settings.option.muteoffscreen'), type = "bool", value = (Spring.GetConfigInt("muteOffscreen", 0) == 1), description = I18N('ui.settings.option.muteoffscreen_descr'),
		  onchange = function(i, value)
		...
		{ id = "soundtrack", group = "sound", category = types.basic, name = I18N('ui.settings.option.label_soundtrack') },
		{ id = "soundtrack_spacer", group = "sound", category = types.basic },

		{ id = "soundtrackNew", group = "sound", category = types.basic, name = I18N('ui.settings.option.soundtracknew'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackNew', 1) == 1, description = I18N('ui.settings.option.soundtracknew_descr'),
			onchange = function(i, value)
		...
		{ id = "soundtrackRaptors", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.soundtrackraptors'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackRaptors', 0) == 1, description = I18N('ui.settings.option.soundtrackraptors_descr'),
		onchange = function(i, value)
		...
		{ id = "soundtrackScavengers", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.soundtrackscavengers'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackScavengers', 0) == 1, description = I18N('ui.settings.option.soundtrackscavengers_descr'),
		onchange = function(i, value)
		...
		{ id = "soundtrackAprilFools", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.soundtrackaprilfools'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackAprilFools', 1) == 1, description = I18N('ui.settings.option.soundtrackaprilfools_descr'),
			onchange = function(i, value)
		...
		{ id = "soundtrackAprilFoolsPostEvent", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.soundtrackaprilfoolspostevent'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackAprilFoolsPostEvent', 0) == 1, description = I18N('ui.settings.option.soundtrackaprilfoolspostevent_descr'),
		onchange = function(i, value)
		...
		{ id = "soundtrackHalloween", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.soundtrackhalloween'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackHalloween', 1) == 1, description = I18N('ui.settings.option.soundtrackhalloween_descr'),
			onchange = function(i, value)
		...
		{ id = "soundtrackHalloweenPostEvent", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.soundtrackhalloweenpostevent'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackHalloweenPostEvent', 0) == 1, description = I18N('ui.settings.option.soundtrackhalloweenpostevent_descr'),
		onchange = function(i, value)
		...
		{ id = "soundtrackXmas", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.soundtrackxmas'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackXmas', 1) == 1, description = I18N('ui.settings.option.soundtrackxmas_descr'),
			onchange = function(i, value)
		...
		{ id = "soundtrackXmasPostEvent", group = "sound", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.soundtrackxmaspostevent'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackXmasPostEvent', 0) == 1, description = I18N('ui.settings.option.soundtrackxmaspostevent_descr'),
		onchange = function(i, value)
		...
		{ id = "soundtrackCustom", group = "sound", category = types.advanced, name = I18N('ui.settings.option.soundtrackcustom'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackCustom', 1) == 1, description = I18N('ui.settings.option.soundtrackcustom_descr'),
			onchange = function(i, value)
		...
		{ id = "soundtrackInterruption", group = "sound", category = types.basic, name = I18N('ui.settings.option.soundtrackinterruption'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackInterruption', 1) == 1, description = I18N('ui.settings.option.soundtrackinterruption_descr'),
			onchange = function(i, value)
		...
		{ id = "soundtrackFades", group = "sound", category = types.basic, name = I18N('ui.settings.option.soundtrackfades'), type = "bool", value = Spring.GetConfigInt('UseSoundtrackFades', 1) == 1, description = I18N('ui.settings.option.soundtrackfades_descr'),
		  onchange = function(i, value)
		...
		{ id = "notifications_set", group = "notif", category = types.basic, name = I18N('ui.settings.option.notifications_set'), type = "select", options = {}, value = 1,
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		--  end,
		--},
		{ id = "notifications_messages", group = "notif", name = I18N('ui.settings.option.notifications_messages'), category = types.basic, type = "bool", value = (WG['notifications'] ~= nil and WG['notifications'].getMessages()), description = I18N('ui.settings.option.notifications_messages_descr'),
		  onload = function(i)
		...
		{ id = "notifications_spoken", group = "notif", name = I18N('ui.settings.option.notifications_spoken'), category = types.basic, type = "bool", value = (WG['notifications'] ~= nil and WG['notifications'].getSpoken()), description = I18N('ui.settings.option.notifications_spoken_descr'),
		  onload = function(i)
		...
		{ id = "notifications_volume", group = "notif", category = types.basic, name = I18N('ui.settings.option.notifications_volume'), type = "slider", min = 0.05, max = 1, step = 0.05, value = 0.7, description = I18N('ui.settings.option.notifications_volume_descr'),
		  onload = function(i)
		...
		{ id = "notifications_substitute", group = "notif", category = types.advanced, name = I18N('ui.settings.option.notifications_substitute'), type = "bool", value = Spring.GetConfigInt('NotificationsSubstitute', 0) == 1, description = I18N('ui.settings.option.notifications_substitute_descr'),
		  onchange = function(i, value)
		...
		{ id = "notifications_refresh", group = "notif", category = types.advanced, name = I18N('ui.settings.option.notifications_refresh'), type = "bool", value = false, description = I18N('ui.settings.option.notifications_refresh_descr'),
		  onchange = function(i, value)
				widgetHandler:DisableWidget("Notifications")
				widgetHandler:EnableWidget("Notifications")
				init()
		...
		{ id = "label_notif_messages", group = "notif", name = I18N('ui.settings.option.label_messages'), category = types.basic },
		{ id = "label_notif_messages_spacer", group = "notif", category = types.basic },

		-- CONTROL
		{ id = "label_ui_hotkeys", group = "control", name = I18N('ui.settings.option.label_hotkeys'), category = types.basic },
		{ id = "label_ui_hotkeys_spacer", group = "control", category = types.basic },

		{ id = "keylayout", group = "control", category = types.basic, name = I18N('ui.settings.option.keylayout'), type = "select", options = keyLayouts.layouts, value = 1, description = I18N('ui.settings.option.keylayout_descr'),
		  onload = function()
		...
		{ id = "keybindings", group = "control", category = types.basic, name = I18N('ui.settings.option.keybindings'), type = "select", options = keyLayouts.keybindingLayouts, value = 1, description = I18N('ui.settings.option.keybindings_descr'),
		  onload = function()
		...
		{ id = "gridmenu", group = "control", category = types.basic, name = I18N('ui.settings.option.gridmenu'), type = "bool", value = GetWidgetToggleValue("Grid menu"), description = I18N('ui.settings.option.gridmenu_descr'),
		  onchange = function(i, value)
			  if value then
				  widgetHandler:DisableWidget('Build menu')
				  widgetHandler:EnableWidget('Grid menu')
		...
		{ id = "gridmenu_alwaysreturn", group = "control", category = types.advanced, name = I18N('ui.settings.option.gridmenu_alwaysreturn'), type = "bool", value = (WG['gridmenu'] ~= nil and WG['gridmenu'].getAlwaysReturn ~= nil and WG['gridmenu'].getAlwaysReturn()), description = I18N('ui.settings.option.gridmenu_alwaysreturn_descr'),
		  onload = function()
		  end,
		  onchange = function(_, value)
		...
		{ id = "gridmenu_autoselectfirst", group = "control", category = types.advanced, name = I18N('ui.settings.option.gridmenu_autoselectfirst'), type = "bool", value = (WG['gridmenu'] ~= nil and WG['gridmenu'].getAutoSelectFirst ~= nil and WG['gridmenu'].getAutoSelectFirst()), description = I18N('ui.settings.option.gridmenu_autoselectfirst_descr'),
		  onload = function()
		  end,
		  onchange = function(_, value)
		...
		{ id = "gridmenu_labbuildmode", group = "control", category = types.advanced, name = I18N('ui.settings.option.gridmenu_labbuildmode'), type = "bool", value = (WG['gridmenu'] ~= nil and WG['gridmenu'].getUseLabBuildMode ~= nil and WG['gridmenu'].getUseLabBuildMode()), description = I18N('ui.settings.option.gridmenu_labbuildmode_descr'),
		  onload = function()
		  end,
		  onchange = function(_, value)
		...
		{ id = "label_ui_cursor", group = "control", name = I18N('ui.settings.option.label_cursor'), category = types.basic },
		{ id = "label_ui_cursor_spacer", group = "control", category = types.basic },

		{ id = "hwcursor", group = "control", category = types.basic, name = I18N('ui.settings.option.hwcursor'), type = "bool", value = tonumber(Spring.GetConfigInt("HardwareCursor", 0) or 0) == 1, description = I18N('ui.settings.option.hwcursor_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "setcamera_bugfix", group = "control", category = types.advanced, name = I18N('ui.settings.option.setcamera_bugfix'), type = "bool", value = true, description = I18N('ui.settings.option.setcamera_bugfix_descr'),
		  onload = function(i)
		...
		{ id = "cursorsize", group = "control", category = types.basic, name = I18N('ui.settings.option.cursorsize'), type = "slider", min = 0.3, max = 1.7, step = 0.1, value = 1, description = I18N('ui.settings.option.cursorsize_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
			  if WG['cursors'] then
		...
		{ id = "containmouse", group = "control", category = types.basic, name = I18N('ui.settings.option.containmouse'), type = "bool", value = Spring.GetConfigInt('grabinput', 1) == 1, description = I18N('ui.settings.option.containmouse_descr'),
          onload = function(i)
          end,
          onchange = function(i, value)
		...
		{ id = "doubleclicktime", group = "control", category = types.advanced, restart = true, name = I18N('ui.settings.option.doubleclicktime'), type = "slider", min = 150, max = 400, step = 10, value = Spring.GetConfigInt("DoubleClickTime", 200), description = I18N('ui.settings.option.doubleclicktime_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "dragthreshold", group = "control", category = types.advanced, restart = false, name = I18N('ui.settings.option.dragthreshold'), type = "slider", min = 4, max = 50, step = 1, value = Spring.GetConfigInt("MouseDragSelectionThreshold", 4), description = I18N('ui.settings.option.dragthreshold_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "label_ui_camera", group = "control", name = I18N('ui.settings.option.label_camera'), category = types.basic },
		{ id = "label_ui_camera_spacer", group = "control", category = types.basic },

		{ id = "middleclicktoggle", group = "control", category = types.basic, name = I18N('ui.settings.option.middleclicktoggle'), type = "bool", value = (Spring.GetConfigFloat("MouseDragScrollThreshold", 0.3) ~= 0), description = I18N('ui.settings.option.middleclicktoggle_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "screenedgemove", group = "control", category = types.basic, name = I18N('ui.settings.option.screenedgemove'), type = "bool", restart = true, value = tonumber(Spring.GetConfigInt("FullscreenEdgeMove", 1) or 1) == 1, description = I18N('ui.settings.option.screenedgemove_descr'),
		  onchange = function(i, value)
		...
		{ id = "screenedgemovewidth", group = "control", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.screenedgemovewidth'), type = "slider", min = 0, max = 0.1, step = 0.01, value = edgeMoveWidth, description = I18N('ui.settings.option.screenedgemovewidth_descr'),
		  onchange = function(i, value)
			  edgeMoveWidth = value
			  Spring.SetConfigFloat("EdgeMoveWidth", value)
		  end,
		},
		{ id = "screenedgemovedynamic", group = "control", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.screenedgemovedynamic'), type = "bool", restart = true, value = tonumber(Spring.GetConfigInt("EdgeMoveDynamic", 1) or 1) == 1, description = I18N('ui.settings.option.screenedgemovedynamic_descr'),
		  onchange = function(i, value)
		...
		{ id = "mincamheight", group = "control", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.mincamheight'), type = "slider", min = 0, max = 1500, step = 1, value = Spring.GetConfigInt("CamSpringMinZoomDistance", 0), description = I18N('ui.settings.option.mincamheight_descr'),
		  onchange = function(i, value)
			  Spring.SetConfigInt("CamSpringMinZoomDistance", value)
			  Spring.SetConfigInt("OverheadMinZoomDistance", value)
		  end,
		},
		{ id = "camerashake", group = "control", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.camerashake'), type = "slider", min = 0, max = 200, step = 10, value = 80, description = I18N('ui.settings.option.camerashake_descr'),
		  onload = function(i)
		...
		--  end,
		--},
		{ id = "smoothingmode", group = "control", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.smoothingmode'), type = "select", options = { I18N('ui.settings.option.smoothing_exponential'), I18N('ui.settings.option.smoothing_spring')}, value = (Spring.GetConfigInt("CamTransitionMode", 1) + 1),
		  onchange = function(i, value)
			  Spring.SetConfigInt("CamTransitionMode", (value - 1))
		  end,
		},
		{ id = "camerasmoothness", group = "control", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.camerasmoothness'), type = "slider", min = 0.04, max = 2, step = 0.01, value = cameraTransitionTime, description = I18N('ui.settings.option.camerasmoothness_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "camerapanspeed", group = "control", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.camerapanspeed'), type = "slider", min = -0.01, max = -0.00195, step = 0.0001, value = Spring.GetConfigFloat("MiddleClickScrollSpeed", 0.0035), description = I18N('ui.settings.option.camerapanspeed_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "cameramovespeed", group = "control", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.cameramovespeed'), type = "slider", min = 0, max = 100, step = 1, value = Spring.GetConfigInt("CamSpringScrollSpeed", 10), description = I18N('ui.settings.option.cameramovespeed_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "scrollspeed", group = "control", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.scrollspeed'), type = "slider", min = 1, max = 50, step = 1, value = math.abs(tonumber(Spring.GetConfigInt("ScrollWheelSpeed", 1) or 25)), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "scrollinverse", group = "control", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.scrollinverse'), type = "bool", value = (tonumber(Spring.GetConfigInt("ScrollWheelSpeed", 1) or 25) < 0), description = "",
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "invertmouse", group = "control", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.invertmouse'), type = "bool", value = tonumber(Spring.GetConfigInt("InvertMouse", 0)) == 1, description = "",
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "scrolltoggleoverview", group = "control", category = types.advanced, widget = "Scrolldown Toggleoverview", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.scrolltoggleoverview'), type = "bool", value = GetWidgetToggleValue("Scrolldown Toggleoverview"), description = I18N('ui.settings.option.scrolltoggleoverview_descr') },

		{ id = "camoverviewrestore", group = "control", category = types.advanced, widget = "Overview Camera Keep Position", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.camoverviewrestore'), type = "bool", value = GetWidgetToggleValue("Overview Camera Keep Position"), description = I18N('ui.settings.option.camoverviewrestore_descr') },
		...
		{ id = "allyselunits_select", group = "control", category = types.advanced, name = widgetOptionColor .. "   " ..I18N('ui.settings.option.allyselunits_select'), type = "bool", value = (WG['allyselectedunits'] ~= nil and WG['allyselectedunits'].getSelectPlayerUnits()), description = I18N('ui.settings.option.allyselunits_select_descr'),
		  onload = function(i)
		...
		{ id = "lockcamera_hideenemies", group = "control", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.lockcamera_hideenemies'), type = "bool", value = (WG.lockcamera and WG.lockcamera.GetHideEnemies()), description = I18N('ui.settings.option.lockcamera_hideenemies_descr'),
		  onload = function(i)
		...
		{ id = "lockcamera_los", group = "control", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.lockcamera_los'), type = "bool", value = (WG.lockcamera and WG.lockcamera.GetLos()), description = I18N('ui.settings.option.lockcamera_los_descr'),
		  onload = function(i)
		...
		{ id = "label_ui_command", group = "control", name = I18N('ui.settings.option.label_commands'), category = types.advanced },
		{ id = "label_ui_command_spacer", group = "control", category = types.basic },
		{ id = "drag_multicommand_shift", group = "control", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.drag_multicommand_shift'), type = "bool", value = (WG.customformations ~= nil and WG.customformations.getRepeatForSingleUnit()), description = I18N('ui.settings.option.drag_multicommand_shift_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		-- INTERFACE
		{ id = "label_ui_interface", group = "ui", name = I18N('ui.settings.option.label_interface'), category = types.basic },
		{ id = "label_ui_interface_spacer", group = "ui", category = types.basic },
		{ id = "language", group = "ui", category = types.basic, name = I18N('ui.settings.option.language'), type = "select", options = languageNames, value = languageCodes[I18N.getLocale()],
			onchange = function(i, value)
		...
		{ id = "language_english_unit_names", group = "ui", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.language_english_unit_names'), type = "bool", value = Spring.GetConfigInt("language_english_unit_names", 0) == 1,
			onchange = function(i, value)
				WG['language'].setEnglishUnitNames(value)
			end,
		},
		{ id = "uiscale", group = "ui", category = types.basic, name = I18N('ui.settings.option.interface') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.uiscale'), type = "slider", min = 0.8, max = 1.3, step = 0.01, value = Spring.GetConfigFloat("ui_scale", 1), description = '',
		  onload = function(i)
		  end,
		...
		{ id = "guiopacity", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.guiopacity'), type = "slider", min = 0.3, max = 1, step = 0.01, value = Spring.GetConfigFloat("ui_opacity", 0.7), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value, force)
		...
		{ id = "guitilescale", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.guitilescale'), type = "slider", min = 4, max = 40, step = 1, value = Spring.GetConfigFloat("ui_tilescale", 7), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value, force)
			  if force then
		...
		{ id = "guitileopacity", group = "ui", category = types.dev, name = widgetOptionColor .. "      " .. I18N('ui.settings.option.guitileopacity'), type = "slider", min = 0, max = 0.03, step = 0.001, value = Spring.GetConfigFloat("ui_tileopacity", 0.014), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value, force)
			  if force then
		...
		{ id = "guishader", group = "ui", category = types.basic, widget = "GUI Shader", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.guishader'), type = "bool", value = GetWidgetToggleValue("GUI Shader") },


		{ id = "minimap_maxheight", group = "ui", category = types.advanced, name = I18N('ui.settings.option.minimap') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.minimap_maxheight'), type = "slider", min = 0.2, max = 0.4, step = 0.01, value = Spring.GetConfigFloat("MinimapMaxHeight", 0.32), description = I18N('ui.settings.option.minimap_maxheight_descr'),
		  onchange = function(i, value)
			  Spring.SetConfigFloat("MinimapMaxHeight", value)
		  end,
		},
		{ id = "minimapleftclick", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.minimapleftclick'), type = "bool", value = Spring.GetConfigInt("MinimapLeftClickMove", 1) == 1, description = I18N('ui.settings.option.minimapleftclick_descr'),
		  onchange = function(i, value)
		...
		{ id = "minimapiconsize", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.minimapiconsize'), type = "slider", min = 2, max = 5, step = 0.25, value = tonumber(Spring.GetConfigFloat("MinimapIconScale", 3.5) or 1), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "minimap_minimized", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.minimapminimized'), type = "bool", value = Spring.GetConfigInt("MinimapMinimize", 0) == 1, description = I18N('ui.settings.option.minimapminimized_descr'),
		  onchange = function(i, value)
		...
		{ id = "minimappip", group = "ui", category = types.advanced, widget = "Picture-in-Picture Minimap", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.minimappip'), type = "bool", value = GetWidgetToggleValue("Picture-in-Picture Minimap"), description = I18N('ui.settings.option.minimappip_descr') },
		{ id = "pip_altkeyzoom", group = "ui", category = types.advanced, name = widgetOptionColor .. "      " .. I18N('ui.settings.option.pip_altkeyzoom'), type = "bool", value = Spring.GetConfigInt("PipAltKeyRequiredForZoom", 1) == 1, description = I18N('ui.settings.option.pip_altkeyzoom_descr'),
		  onchange = function(i, value)
		...
		{ id = "pip_commandfx", group = "ui", category = types.advanced, name = widgetOptionColor .. "      " .. I18N('ui.settings.option.pip_commandfx'), type = "bool", value = Spring.GetConfigInt("PipDrawCommandFX", 1) == 1, description = I18N('ui.settings.option.pip_commandfx_descr'),
		  onchange = function(i, value)
		...
		--   end,
		-- },
		{ id = "pip_engine_fallback_threshold", group = "ui", category = types.advanced, name = widgetOptionColor .. "      " .. I18N('ui.settings.option.pip_engine_fallback_threshold'), type = "slider", min = 1500, max = 5000, step = 100, value = 4000, description = I18N('ui.settings.option.pip_engine_fallback_threshold_descr'),
		  onload = function(i)
		...
		{ id = "pip", group = "ui", category = types.advanced, widget = "Picture-in-Picture", name = I18N('ui.settings.option.pip'), type = "bool", value = GetWidgetToggleValue("Picture-in-Picture"), description = I18N('ui.settings.option.pip_descr') },
		{ id = "pip2", group = "ui", category = types.advanced, widget = "Picture-in-Picture 2", name = I18N('ui.settings.option.pip2'), type = "bool", value = GetWidgetToggleValue("Picture-in-Picture 2"), description = I18N('ui.settings.option.pip2_descr') },
		...
		{ id = "buildmenu_maxposy", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.buildmenu_maxposy'), type = "slider", min = 0.66, max = 0.88, step = 0.01, value = 0.74, description = I18N('ui.settings.option.buildmenu_maxposy_descr'),
		  onload = function(i)
		...
		{ id = "buildmenu_alwaysshow", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.buildmenu_alwaysshow'), type = "bool", value = (WG['buildmenu'] ~= nil and WG['buildmenu'].getAlwaysShow ~= nil and WG['buildmenu'].getAlwaysShow()), description = I18N('ui.settings.option.buildmenu_alwaysshow_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "buildmenu_prices", group = "ui", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.buildmenu_prices'), type = "bool", value = (WG['buildmenu'] ~= nil and WG['buildmenu'].getShowPrice ~= nil and WG['buildmenu'].getShowPrice()), description = I18N('ui.settings.option.buildmenu_prices_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "buildmenu_groupicon", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.buildmenu_groupicon'), type = "bool", value = (WG['buildmenu'] ~= nil and WG['buildmenu'].getShowGroupIcon ~= nil and WG['buildmenu'].getShowGroupIcon()), description = I18N('ui.settings.option.buildmenu_groupicon_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "buildmenu_radaricon", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.buildmenu_radaricon'), type = "bool", value = (WG['buildmenu'] ~= nil and WG['buildmenu'].getShowRadarIcon ~= nil and WG['buildmenu'].getShowRadarIcon()), description = I18N('ui.settings.option.buildmenu_radaricon_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "ordermenu_colorize", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.ordermenu_colorize'), type = "slider", min = 0, max = 1, step = 0.1, value = 0.5, description = '',
		  onload = function(i)
		...
		{ id = "ordermenu_alwaysshow", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.ordermenu_alwaysshow'), type = "bool", value = (WG['ordermenu'] ~= nil and WG['ordermenu'].getAlwaysShow ~= nil and WG['ordermenu'].getAlwaysShow()), description = I18N('ui.settings.option.ordermenu_alwaysshow_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "ordermenu_hideset", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.ordermenu_hideset'), type = "bool", value = (WG['ordermenu'] ~= nil and WG['ordermenu'].getDisabledCmd ~= nil and WG['ordermenu'].getDisabledCmd('Move')), description = I18N('ui.settings.option.ordermenu_hideset_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "info_buildlist", group = "ui", category = types.advanced, name = I18N('ui.settings.option.info') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.info_buildlist'), type = "bool", value = (WG['info'] and WG['info'].getShowBuilderBuildlist ~= nil and WG['info'].getShowBuilderBuildlist()), description = I18N('ui.settings.option.info_buildlist_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "info_mappos", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.info_mappos'), type = "bool", value = (WG['info'] and WG['info'].getDisplayMapPosition ~= nil and WG['info'].getDisplayMapPosition()), description = I18N('ui.settings.option.info_mappos_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "info_alwaysshow", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.info_alwaysshow'), type = "bool", value = (WG['info'] ~= nil and WG['info'].getAlwaysShow ~= nil and WG['info'].getAlwaysShow()),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "advplayerlist_country", group = "ui", category = types.basic, name = I18N('ui.settings.option.advplayerlist') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.advplayerlist_country'), type = "bool", value = true, description = I18N('ui.settings.option.advplayerlist_country_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_scale", group = "ui", category = types.advanced, name =  widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_scale'), min = 0.85, max = 1.2, step = 0.01, type = "slider", value = 1, description = I18N('ui.settings.option.advplayerlist_scale_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_showallyid", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_showallyid'), type = "bool", value = false, description = I18N('ui.settings.option.advplayerlist_showallyid_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_showid", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_showid'), type = "bool", value = false, description = I18N('ui.settings.option.advplayerlist_showid_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_showplayerid", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_showplayerid'), type = "bool", value = false, description = I18N('ui.settings.option.advplayerlist_showplayerid_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_rank", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_rank'), type = "bool", value = true, description = I18N('ui.settings.option.advplayerlist_rank_descr'),
		  onload = function(i)
		...
		--  end,
		--},
		{ id = "advplayerlist_skill", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_skill'), type = "bool", value = true, description = I18N('ui.settings.option.advplayerlist_skill_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_cpuping", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_cpuping'), type = "bool", value = true, description = I18N('ui.settings.option.advplayerlist_cpuping_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_resources", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_resources'), type = "bool", value = true, description = I18N('ui.settings.option.advplayerlist_resources_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_income", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_income'), type = "bool", value = true, description = I18N('ui.settings.option.advplayerlist_income_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_absresbars", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_absresbars'), type = "bool", value = false, description = I18N('ui.settings.option.advplayerlist_absresbars_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_share", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_share'), type = "bool", value = true, description = I18N('ui.settings.option.advplayerlist_share_descr'),
		  onload = function(i)
		...
		{ id = "advplayerlist_hidespecs", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.advplayerlist_hidespecs'), type = "bool", value = true, description = I18N('ui.settings.option.advplayerlist_hidespecs_descr'),
		  onload = function(i)
		...
		{ id = "unittotals", group = "ui", category = types.advanced, widget = "AdvPlayersList Unit Totals", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.unittotals'), type = "bool", value = GetWidgetToggleValue("AdvPlayersList Unit Totals"), description = I18N('ui.settings.option.unittotals_descr') },
		{ id = "musicplayer", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. widgetOptionColor .. I18N('ui.settings.option.musicplayer'), type = "bool", value = (WG['music'] ~= nil and WG['music'].GetShowGui() or false), description = I18N('ui.settings.option.musicplayer_descr'),
		  onload = function(i)
		...
		{ id = "mascot", group = "ui", category = types.advanced, widget = "AdvPlayersList Mascot", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.mascot'), type = "bool", value = GetWidgetToggleValue("AdvPlayersList Mascot"), description = I18N('ui.settings.option.mascot_descr') },

		{ id = "displayselectedname", group = "ui", category = types.advanced, name = I18N('ui.settings.option.displayselectedname'), type = "bool", value = (WG['playertv'] ~= nil and WG['playertv'].GetAlwaysDisplayName() or false), description = I18N('ui.settings.option.displayselectedname_descr'),
		  onload = function(i)
		...
		{ id = "console_fontsize", group = "ui", category = types.basic, name = I18N('ui.settings.option.console') .. "   " .. widgetOptionColor .. I18N('ui.settings.option.console_fontsize'), type = "slider", min = 0.92, max = 1.12, step = 0.02, value = (WG['chat'] ~= nil and WG['chat'].getFontsize() or 1), description = '',
		  onload = function(i)
		...
		{ id = "console_backgroundopacity", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.console_backgroundopacity'), type = "slider", min = 0, max = 0.45, step = 0.01, value = (WG['chat'] ~= nil and WG['chat'].getBackgroundOpacity() or 0), description = I18N('ui.settings.option.console_backgroundopacity_descr'),
		  onload = function(i)
		...
		{ id = "console_hidespecchat", group = "ui", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.console_hidespecchat'), type = "bool", value = (Spring.GetConfigInt("HideSpecChat", 0) == 1), description = I18N('ui.settings.option.console_hidespecchat_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "console_hidespecchatplayer", group = "ui", category = types.basic, name = widgetOptionColor .. "      " .. widgetOptionColor .. I18N('ui.settings.option.console_hidespecchatplayer'), type = "bool", value = (Spring.GetConfigInt("HideSpecChatPlayer", 1) == 1), description = I18N('ui.settings.option.console_hidespecchatplayer_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "console_hide", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.console_hide'), type = "bool", value = (WG['chat'] ~= nil and WG['chat'].getHide() or false), description = I18N('ui.settings.option.console_hide_descr'),
		  onload = function(i)
		...
		{ id = "console_maxlines", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.console_maxlines'), type = "slider", min = 3, max = 7, step = 1, value = (WG['chat'] ~= nil and WG['chat'].getMaxLines() or 5), description = '',
		  onload = function(i)
		...
		{ id = "console_maxconsolelines", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.console_maxconsolelines'), type = "slider", min = 2, max = 12, step = 1, value = (WG['chat'] ~= nil and WG['chat'].getMaxConsoleLines() or 2), description = '',
		  onload = function(i)
		...
		--  end,
		--},
		{ id = "autoeraser", group = "ui", category = types.basic, widget = "Auto mapmark eraser", name = I18N('ui.settings.option.autoeraser'), type = "bool", value = GetWidgetToggleValue("Auto mapmark eraser"), description = I18N('ui.settings.option.autoeraser_descr') },
		{ id = "autoeraser_erasetime", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.autoeraser_erasetime'), type = "slider", min = 10, max = 200, step = 1, value = 60, description = I18N('ui.settings.option.autoeraser_erasetime_descr'),
		  onload = function(i)
		...
		{ id = "topbar_hidebuttons", group = "ui", category = types.advanced, name = I18N('ui.settings.option.topbar')..widgetOptionColor .. "  " .. I18N('ui.settings.option.topbar_hidebuttons'), type = "bool", value = (WG['topbar'] ~= nil and WG['topbar'].getAutoHideButtons() or 0),
		  onload = function(i)
		...
		{ id = "continuouslyclearmapmarks", group = "ui", category = types.dev, name = I18N('ui.settings.option.continuouslyclearmapmarks'), type = "bool", value = Spring.GetConfigInt("ContinuouslyClearMapmarks", 0) == 1, description = I18N('ui.settings.option.continuouslyclearmapmarks_descr'),
		  onchange = function(i, value)
		...
		{ id = "unitgroups", group = "ui", category = types.basic, widget = "Unit Groups", name = I18N('ui.settings.option.unitgroups'), type = "bool", value = GetWidgetToggleValue("Unit Groups"), description = I18N('ui.settings.option.unitgroups_descr') },
		{ id = "idlebuilders", group = "ui", category = types.basic, widget = "Idle Builders", name = I18N('ui.settings.option.idlebuilders'), type = "bool", value = GetWidgetToggleValue("Idle Builders"), description = I18N('ui.settings.option.idlebuilders_descr') },
		{ id = "buildbar", group = "ui", category = types.basic, widget = "BuildBar", name = I18N('ui.settings.option.buildbar'), type = "bool", value = GetWidgetToggleValue("BuildBar"), description = I18N('ui.settings.option.buildbar_descr') },

		{ id = "converterusage", group = "ui", category = types.advanced, widget = "Converter Usage", name = I18N('ui.settings.option.converterusage'), type = "bool", value = GetWidgetToggleValue("Converter Usage"), description = I18N('ui.settings.option.converterusage_descr') },

		{ id = "widgetselector", group = "ui", category = types.advanced, name = I18N('ui.settings.option.widgetselector'), type = "bool", value = Spring.GetConfigInt("widgetselector", 0) == 1, description = I18N('ui.settings.option.widgetselector_descr'),
		  onchange = function(i, value)
		...
		{ id = "label_ui_visuals", group = "ui", name = I18N('ui.settings.option.label_visuals'), category = types.basic },
		{ id = "label_ui_visuals_spacer", group = "ui", category = types.basic },

		{ id = "uniticon_scaleui", group = "ui", category = types.basic, name = I18N('ui.settings.option.uniticonscaleui'), type = "slider", min = 0.85, max = 3, step = 0.05, value = tonumber(Spring.GetConfigFloat("UnitIconScaleUI", 1) or 1), description = I18N('ui.settings.option.uniticonscaleui_descr'),
		  onchange = function(i, value)
			  Spring.SendCommands("iconscaleui " .. value)
			  Spring.SetConfigFloat("UnitIconScaleUI", value)
		  end,
		},
		{ id = "uniticon_distance", group = "ui", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.uniticondistance'), type = "slider", min = 1, max = 12000, step = 50, value = tonumber(Spring.GetConfigInt("UnitIconFadeVanish", 2700) or 1), description = I18N('ui.settings.option.uniticondistance_descr'),
		  onchange = function(i, value)
		...
		{ id = "uniticon_hidewithui", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.uniticonhidewithui'), type = "bool", value = (Spring.GetConfigInt("UnitIconsHideWithUI", 0) == 1), description = I18N('ui.settings.option.uniticonhidewithui_descr'),
		  onchange = function(i, value)
		...
		{ id = "teamplatter", group = "ui", category = types.basic, widget = "TeamPlatter", name = I18N('ui.settings.option.teamplatter'), type = "bool", value = GetWidgetToggleValue("TeamPlatter"), description = I18N('ui.settings.option.teamplatter_descr') },
		{ id = "teamplatter_opacity", category = types.advanced, group = "ui", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.teamplatter_opacity'), min = 0.05, max = 0.4, step = 0.01, type = "slider", value = 0.25, description = I18N('ui.settings.option.teamplatter_opacity_descr'),
		  onload = function(i)
		...
		{ id = "teamplatter_skipownteam", category = types.advanced, group = "ui", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.teamplatter_skipownteam'), type = "bool", value = false, description = I18N('ui.settings.option.teamplatter_skipownteam_descr'),
		  onload = function(i)
		...
		{ id = "enemyspotter", group = "ui", category = types.basic, widget = "EnemySpotter", name = I18N('ui.settings.option.enemyspotter'), type = "bool", value = GetWidgetToggleValue("EnemySpotter"), description = I18N('ui.settings.option.enemyspotter_descr') },
		{ id = "enemyspotter_opacity", category = types.advanced, group = "ui", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.enemyspotter_opacity'), min = 0.12, max = 0.4, step = 0.01, type = "slider", value = 0.15, description = I18N('ui.settings.option.enemyspotter_opacity_descr'),
		  onload = function(i)
		...
		{ id = "selectedunits_opacity", group = "ui", category = types.advanced, name = I18N('ui.settings.option.selectedunits')..widgetOptionColor .. "  " .. I18N('ui.settings.option.selectedunits_opacity'), min = 0, max = 0.5, step = 0.01, type = "slider", value = 0.19, description = I18N('ui.settings.option.selectedunits_opacity_descr'),
		  onload = function(i)
		...
		{ id = "selectedunits_teamcoloropacity", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.selectedunits_teamcoloropacity'), min = 0, max = 1, step = 0.01, type = "slider", value = 0.6, description = I18N('ui.settings.option.selectedunits_teamcoloropacity_descr'),
		  onload = function(i)
		...
		{ id = "highlightselunits", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.highlightselunits'),  type = "bool", value = true, description = I18N('ui.settings.option.selectedunits_teamcoloropacity_descr'),
		  onload = function(i)
		...
		{ id = "highlightunit", group = "ui", category = types.advanced, name = I18N('ui.settings.option.highlightunit'),  type = "bool", value = true, description = I18N('ui.settings.option.highlightunit_descr'),
		  onload = function(i)
		...
		{ id = "cursorlight", group = "ui", category = types.advanced, name = I18N('ui.settings.option.cursorlight'), type = "bool", value = false, description = I18N('ui.settings.option.cursorlight_descr'),
		  onload = function(i)
		...
		{ id = "cursorlight_lightradius", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.cursorlight_lightradius'), type = "slider", min = 0.3, max = 2, step = 0.05, value = 1, description = '',
		  onload = function(i)
		...
		{ id = "cursorlight_lightstrength", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.cursorlight_lightstrength'), type = "slider", min = 0.3, max = 2, step = 0.05, value = 1, description = '',
		  onload = function(i)
		...
		{ id = "label_ui_info", group = "ui", name = I18N('ui.settings.option.label_info'), category = types.basic },
		{ id = "label_ui_info_spacer", group = "ui", category = types.basic },

		{ id = "metalspots_values", group = "ui", category = types.advanced, name = I18N('ui.settings.option.metalspots')..widgetOptionColor .. "   " .. I18N('ui.settings.option.metalspots_values'), type = "bool", value = (WG['metalspots'] ~= nil and WG['metalspots'].getShowValue()), description = I18N('ui.settings.option.metalspots_values_descr'),
		  onload = function(i)
		...
		{ id = "metalspots_metalviewonly", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.metalspots_metalviewonly'), type = "bool", value = false, description = I18N('ui.settings.option.metalspots_metalviewonly_descr'),
		  onload = function(i)
		...
		{ id = "geospots", group = "ui", category = types.dev, widget = "Geothermalspots", name = I18N('ui.settings.option.geospots'), type = "bool", value = GetWidgetToggleValue("Metalspots"), description = I18N('ui.settings.option.geospots_descr') },

		{ id = "healthbarsscale", group = "ui", category = types.advanced, name = I18N('ui.settings.option.healthbars') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.healthbarsscale'), type = "slider", min = 0.6, max = 2.0, step = 0.1, value = 1, description = '',
		  onload = function(i)
		...
		{ id = "healthbarsheight", group = "ui", category = types.advanced, name = widgetOptionColor .. "  " .. I18N('ui.settings.option.healthbarsheight'), type = "slider", min = 0.7, max = 2, step = 0.1, value = (WG['healthbar'] ~= nil and WG['healthbar'].getHeight() or 0.9), description = '',
		  onload = function(i)
		...
		{ id = "healthbarsvariable", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.healthbarsvariable'), type = "bool", value = (WG['healthbar'] ~= nil and WG['healthbar'].getVariableSizes()), description = I18N('ui.settings.option.healthbarsvariable_descr'),
		  onload = function(i)
		...
		{ id = "healthbarswhenguihidden", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.healthbarswhenguihidden'), type = "bool", value = (WG['healthbar'] ~= nil and WG['healthbar'].getDrawWhenGuiHidden()), description = I18N('ui.settings.option.healthbarswhenguihidden_descr'),
		  onload = function(i)
		...
		{ id = "rankicons", group = "ui", category = types.advanced, widget = "Rank Icons GL4", name = I18N('ui.settings.option.rankicons'), type = "bool", value = GetWidgetToggleValue("Rank Icons GL4"), description = I18N('ui.settings.option.rankicons_descr') },
		{ id = "rankicons_distance", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.rankicons_distance'), type = "slider", min = 0.1, max = 1.5, step = 0.05, value = (WG['rankicons'] ~= nil and WG['rankicons'].getDrawDistance ~= nil and WG['rankicons'].getDrawDistance()), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "rankicons_scale", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.rankicons_scale'), type = "slider", min = 0.5, max = 2, step = 0.1, value = (WG['rankicons'] ~= nil and WG['rankicons'].getScale ~= nil and WG['rankicons'].getScale()), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "allycursors", group = "ui", category = types.basic, widget = "AllyCursors", name = I18N('ui.settings.option.allycursors'), type = "bool", value = GetWidgetToggleValue("AllyCursors"), description = I18N('ui.settings.option.allycursors_descr') },
		{ id = "allycursors_playername", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.allycursors_playername'), type = "bool", value = true, description = I18N('ui.settings.option.allycursors_playername_descr'),
		  onload = function(i)
		...
		{ id = "allycursors_showdot", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.allycursors_showdot'), type = "bool", value = true, description = I18N('ui.settings.option.allycursors_showdot_descr'),
		  onload = function(i)
		...
		{ id = "allycursors_spectatorname", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.allycursors_spectatorname'), type = "bool", value = true, description = I18N('ui.settings.option.allycursors_spectatorname_descr'),
		  onload = function(i)
		...
		{ id = "allycursors_lights", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.allycursors_lights'), type = "bool", value = true, description = I18N('ui.settings.option.allycursors_lights_descr'),
		  onload = function(i)
		...
		{ id = "allycursors_lightradius", group = "ui", category = types.dev, name = widgetOptionColor .. "      " .. I18N('ui.settings.option.allycursors_lightradius'), type = "slider", min = 0.15, max = 1, step = 0.05, value = 0.5, description = '',
		  onload = function(i)
		...
		{ id = "allycursors_lightstrength", group = "ui", category = types.dev , name = widgetOptionColor .. "      " .. I18N('ui.settings.option.allycursors_lightstrength'), type = "slider", min = 0.1, max = 1.2, step = 0.05, value = 0.85, description = '',
		  onload = function(i)
		...
		{ id = "allycursors_selfshadowing", group = "ui", category = types.dev , name = widgetOptionColor .. "      " .. I18N('ui.settings.option.allycursors_selfshadowing'), type = "bool", value = false, description = '',
		  onload = function(i)
		...
		{ id = "showbuilderqueue", group = "ui", category = types.advanced, widget = "Show Builder Queue", name = I18N('ui.settings.option.showbuilderqueue'), type = "bool", value = GetWidgetToggleValue("Show Builder Queue"), description = I18N('ui.settings.option.showbuilderqueue_descr') },

		{ id = "unitenergyicons", group = "ui", category = types.advanced, widget = "Unit Energy Icons", name = I18N('ui.settings.option.unitenergyicons'), type = "bool", value = GetWidgetToggleValue("Unit Energy Icons"), description = I18N('ui.settings.option.unitenergyicons_descr') },

		{ id = "unitidlebuildericons", group = "ui", category = types.advanced, widget = "Unit Idle Builder Icons", name = I18N('ui.settings.option.unitidlebuildericons'), type = "bool", value = GetWidgetToggleValue("Unit Idle Builder Icons"), description = I18N('ui.settings.option.unitidlebuildericons_descr') },

		{ id = "nametags_rank", group = "ui", category = types.advanced, name = I18N('ui.settings.option.nametags_rank'), type = "bool", value = true, description = I18N('ui.settings.option.nametags_rank_descr'),
		  onload = function(i)
		...
		{ id = "commandsfx", group = "ui", category = types.basic, widget = "Commands FX", name = I18N('ui.settings.option.commandsfx'), type = "bool", value = GetWidgetToggleValue("Commands FX"), description = I18N('ui.settings.option.commandsfx_descr') },

		{ id = "commandsfxopacity", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.commandsfxopacity'), type = "slider", min = 0.25, max = 1, step = 0.1, value = 1, description = '',
		  onload = function(i)
		...
		{ id = "commandsfxduration", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.commandsfxduration'), type = "slider", min = 0.5, max = 2, step = 0.01, value = 1, description = '',
		  onload = function(i)
		...
		{ id = "commandsfxfilterai", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.commandsfxfilterai'), type = "bool", value = true, description = I18N('ui.settings.option.commandsfxfilterai_descr'),
		  onload = function(i)
		...
		{ id = "commandsfxuseteamcolors", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.commandsfxuseteamcolors'), type = "bool", value = false, description = I18N('ui.settings.option.commandsfxuseteamcolors_descr'),
		  onload = function(i)
		...
		{ id = "commandsfxuseteamcolorswhenspec", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.commandsfxuseteamcolorswhenspec'), type = "bool", value = false, description = I18N('ui.settings.option.commandsfxuseteamcolorswhenspec_descr'),
		  onload = function(i)
		...
		{ id = "flankingicons", group = "ui", category = types.advanced, widget = "Flanking Icons GL4", name = I18N('ui.settings.option.flankingicons'), type = "bool", value = GetWidgetToggleValue("Flanking Icons GL4"), description = I18N('ui.settings.option.flankingicons_descr') },

		{ id = "displaydps", group = "ui", category = types.basic, name = I18N('ui.settings.option.displaydps'), type = "bool", value = tonumber(Spring.GetConfigInt("DisplayDPS", 0) or 0) == 1, description = I18N('ui.settings.option.displaydps_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "givenunits", group = "ui", category = types.advanced, widget = "Given Units", name = I18N('ui.settings.option.givenunits'), type = "bool", value = GetWidgetToggleValue("Given Units"), description = I18N('ui.settings.option.givenunits_descr') },

		{ id = "reclaimfieldhighlight", group = "ui", category = types.advanced, widget = "Reclaim Field Highlight", name = I18N('ui.settings.option.reclaimfieldhighlight'), type = "bool", description = I18N('ui.settings.option.reclaimfieldhighlight_descr') },

		{ id = "reclaimfieldhighlight_metal", group = "ui", category = types.advanced, widget = "Reclaim Field Highlight", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.reclaimfieldhighlight_metal'), type = "select", options = reclaimFieldHighlightOptions, value = 3, description = I18N('ui.settings.option.reclaimfieldhighlight_metal_descr'),
			onload = function(i)
		...
		{ id = "reclaimfieldhighlight_energy", group = "ui", category = types.advanced, widget = "Reclaim Field Highlight", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.reclaimfieldhighlight_energy'), type = "select", options = reclaimFieldHighlightOptions, value = 3, description = I18N('ui.settings.option.reclaimfieldhighlight_energy_descr'),
			onload = function(i)
		...
		{ id = "highlightcomwrecks", group = "ui", category = types.advanced, widget = "Highlight Commander Wrecks", name = I18N('ui.settings.option.highlightcomwrecks'), type = "bool", value = GetWidgetToggleValue("Highlight Commander Wrecks"), description = I18N('ui.settings.option.highlightcomwrecks_descr') },
		{ id = "highlightcomwrecks_teamcolor", group = "ui", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.highlightcomwrecks_teamcolor'), type = "bool", value = true, description = I18N('ui.settings.option.highlightcomwrecks_teamcolor_descr'),
		  onload = function(i)
		...
		{ id = "buildinggrid", group = "ui", category = types.basic, widget = "Building Grid GL4", name = I18N('ui.settings.option.buildinggrid'), type = "bool", value = GetWidgetToggleValue("Building Grid GL4"), description = I18N('ui.settings.option.buildinggrid_descr') },
		{ id = "buildinggridopacity", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.buildinggridopacity'), type = "slider", min = 0.3, max = 1, step = 0.05, value = (WG['buildinggrid'] ~= nil and WG['buildinggrid'].getOpacity ~= nil and WG['buildinggrid'].getOpacity()) or 1, description = '',
		  onload = function(i)
		...
		{ id = "startpositionsuggestions", group = "ui", category = types.basic, widget = "Start Position Suggestions", name = I18N('ui.settings.option.startpositionsuggestions'), type = "bool", value = GetWidgetToggleValue("Start Position Suggestions"), description = I18N('ui.settings.option.startpositionsuggestions_descr') },

		{ id = "label_ui_ranges", group = "ui", name = I18N('ui.settings.option.label_ranges'), category = types.basic },
		{ id = "label_ui_ranges_spacer", group = "ui", category = types.basic },


		-- Radar range rings:
		{ id = "radarrange", group = "ui", category = types.advanced, widget = "Sensor Ranges Radar", name = I18N('ui.settings.option.radarrange'), type = "bool", value = GetWidgetToggleValue("Sensor Ranges Radar"), description = I18N('ui.settings.option.radarrange_descr') },

		{ id = "radarrangeopacity", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.radarrangeopacity'), type = "slider", min = 0.01, max = 0.33, step = 0.01, value = (WG['radarrange'] ~= nil and WG['radarrange'].getOpacity ~= nil and WG['radarrange'].getOpacity()) or 0.08, description = '',
		  onload = function(i)
		...
		-- Sonar range
		{ id = "sonarrange", group = "ui", category = types.advanced, widget = "Sensor Ranges Sonar", name = I18N('ui.settings.option.sonarrange'), type = "bool", value = GetWidgetToggleValue("Sensor Ranges Sonar"), description = I18N('ui.settings.option.sonarrange_descr') },

		{ id = "sonarrangeopacity", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sonarrangeopacity'), type = "slider", min = 0.01, max = 0.33, step = 0.01, value = (WG['sonarrange'] ~= nil and WG['sonarrange'].getOpacity ~= nil and WG['sonarrange'].getOpacity()) or 0.08, description = '',
		  onload = function(i)
		...
		-- Jammer range
		{ id = "jammerrange", group = "ui", category = types.advanced, widget = "Sensor Ranges Jammer", name = I18N('ui.settings.option.jammerrange'), type = "bool", value = GetWidgetToggleValue("Sensor Ranges Jammer"), description = I18N('ui.settings.option.jammerrange_descr') },

		{ id = "jammerrangeopacity", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.jammerrangeopacity'), type = "slider", min = 0.01, max = 0.66, step = 0.01, value = (WG['jammerrange'] ~= nil and WG['jammerrange'].getOpacity ~= nil and WG['jammerrange'].getOpacity()) or 0.08, description = '',
		  onload = function(i)
		...
		-- LOS Range:
		{ id = "losrange", group = "ui", category = types.advanced, widget = "Sensor Ranges LOS", name = I18N('ui.settings.option.losrange'), type = "bool", value = GetWidgetToggleValue("Sensor Ranges LOS"), description = I18N('ui.settings.option.losrange_descr') },

		{ id = "losrangeopacity", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.losrangeopacity'), type = "slider", min = 0.01, max = 0.33, step = 0.01, value = (WG['losrange'] ~= nil and WG['losrange'].getOpacity ~= nil and WG['losrange'].getOpacity()) or 0.08, description = '',
		  onload = function(i)
		...
		{ id = "losrangeteamcolors", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.losrangeteamcolors'), type = "bool", value = (WG['losrange'] ~= nil and WG['losrange'].getUseTeamColors ~= nil and WG['losrange'].getUseTeamColors()), description = '',
		  onload = function(i)
		...
		{ id = "attackrange", group = "ui", category = types.basic, widget = "Attack Range GL4", name = I18N('ui.settings.option.attackrange'), type = "bool", value = GetWidgetToggleValue("Attack Range GL4"), description = I18N('ui.settings.option.attackrange_descr') },
		{ id = "attackrange_shiftonly", category = types.dev, group = "ui", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.attackrange_shiftonly'), type = "bool", value = (WG['attackrange'] ~= nil and WG['attackrange'].getShiftOnly ~= nil and WG['attackrange'].getShiftOnly()), description = I18N('ui.settings.option.attackrange_shiftonly_descr'),
		  onload = function(i)
		...
		{ id = "defrange", group = "ui", category = types.basic, widget = "Defense Range GL4", name = I18N('ui.settings.option.defrange'), type = "bool", value = GetWidgetToggleValue("Defense Range GL4"), description = I18N('ui.settings.option.defrange_descr') },

		{ id = "defrange_allyair", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.defrange_allyair'), type = "bool", value = (WG['defrange'] ~= nil and WG['defrange'].getAllyAir ~= nil and WG['defrange'].getAllyAir()), description = I18N('ui.settings.option.defrange_allyair_descr'),
		  onload = function(i)
		...
		{ id = "defrange_allyground", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.defrange_allyground'), type = "bool", value = (WG['defrange'] ~= nil and WG['defrange'].getAllyGround ~= nil and WG['defrange'].getAllyGround()), description = I18N('ui.settings.option.defrange_allyground_descr'),
		  onload = function(i)
		...
		{ id = "defrange_allynuke", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.defrange_allynuke'), type = "bool", value = (WG['defrange'] ~= nil and WG['defrange'].getAllyNuke ~= nil and WG['defrange'].getAllyNuke()), description = I18N('ui.settings.option.defrange_allynuke_descr'),
		  onload = function(i)
		...
		{ id = "defrange_allylrpc", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.defrange_allylrpc'), type = "bool", value = (WG['defrange'] ~= nil and WG['defrange'].getAllyLRPC ~= nil and WG['defrange'].getAllyLRPC()), description = I18N('ui.settings.option.defrange_allylrpc_descr'),
		  onload = function(i)
		...
		{ id = "defrange_enemyair", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.defrange_enemyair'), type = "bool", value = (WG['defrange'] ~= nil and WG['defrange'].getEnemyAir ~= nil and WG['defrange'].getEnemyAir()), description = I18N('ui.settings.option.defrange_enemyair_descr'),
		  onload = function(i)
		...
		{ id = "defrange_enemyground", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.defrange_enemyground'), type = "bool", value = (WG['defrange'] ~= nil and WG['defrange'].getEnemyGround ~= nil and WG['defrange'].getEnemyGround()), description = I18N('ui.settings.option.defrange_enemyground_descr'),
		  onload = function(i)
		...
		{ id = "defrange_enemynuke", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.defrange_enemynuke'), type = "bool", value = (WG['defrange'] ~= nil and WG['defrange'].getEnemyNuke ~= nil and WG['defrange'].getEnemyNuke()), description = I18N('ui.settings.option.defrange_enemynuke_descr'),
		  onload = function(i)
		...
		{ id = "defrange_enemylrpc", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.defrange_enemylrpc'), type = "bool", value = (WG['defrange'] ~= nil and WG['defrange'].getEnemyLRPC ~= nil and WG['defrange'].getEnemyLRPC()), description = I18N('ui.settings.option.defrange_enemylrpc_descr'),
		  onload = function(i)
		...
		{ id = "antiranges", group = "ui", category = types.advanced, widget = "Anti Ranges", name = I18N('ui.settings.option.antiranges'), type = "bool", value = GetWidgetToggleValue("Anti Ranges"), description = I18N('ui.settings.option.antiranges_descr') },

		{ id = "label_ui_spectator", group = "ui", name = I18N('ui.settings.option.label_spectator'), category = types.basic },
		{ id = "label_ui_spectator_spacer", group = "ui", category = types.basic },

		{ id = "spectator_hud", group = "ui", category = types.basic, widget = "Spectator HUD", name = I18N('ui.settings.option.spectator_hud'), type = "bool", value = GetWidgetToggleValue("Spectator HUD"), description = I18N('ui.settings.option.spectator_hud_descr') },
		{ id = "spectator_hud_size", group = "ui", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.spectator_hud_size'), type = "slider", min = 0.1, max = 2, step = 0.1, value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getWidgetSize ~= nil and WG['spectator_hud'].getWidgetSize()) or 0.8, description = '',
		  onload = function(i)
		...
		{ id = "spectator_hud_metric_metalIncome", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.metalIncome_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('metalIncome')) or 1, description = I18N('ui.spectator_hud.metalIncome_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_energyIncome", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.energyIncome_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('energyIncome')) or 1, description = I18N('ui.spectator_hud.energyIncome_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_buildPower", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.buildPower_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('buildPower')) or 1, description = I18N('ui.spectator_hud.buildPower_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_metalProduced", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.metalProduced_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('metalProduced')) or 1, description = I18N('ui.spectator_hud.metalProduced_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_energyProduced", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.energyProduced_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('energyProduced')) or 1, description = I18N('ui.spectator_hud.energyProduced_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_metalExcess", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.metalExcess_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('metalExcess')) or 1, description = I18N('ui.spectator_hud.metalExcess_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_energyExcess", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.energyExcess_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('energyExcess')) or 1, description = I18N('ui.spectator_hud.energyExcess_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_defenseValue", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.defenseValue_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('defenseValue')) or 1, description = I18N('ui.spectator_hud.defenseValue_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_economyValue", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.economyValue_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('economyValue')) or 1, description = I18N('ui.spectator_hud.economyValue_tooltip'),
			onload = function(i)
		...
		{ id = "spectator_hud_metric_damageDealt", group = "ui", category = types.advanced, name = widgetOptionColor .. "   " .. I18N('ui.spectator_hud.damageDealt_title'), type = "bool", value = (WG['spectator_hud'] ~= nil and WG['spectator_hud'].getMetricEnabled~= nil and WG['spectator_hud'].getMetricEnabled('damageDealt')) or 1, description = I18N('ui.spectator_hud.damageDealt_tooltip'),
			onload = function(i)
		...
		{ id = "label_ui_developer", group = "ui", name = I18N('ui.settings.option.label_developer'), category = types.advanced },
		{ id = "label_ui_developer_spacer", group = "ui", category = types.advanced },

		{ id = "devmode", group = "ui", category = types.advanced, name = I18N('ui.settings.option.devmode'), type = "bool", value = devUI, description = I18N('ui.settings.option.devmode_descr'),
		  onchange = function(i, value)
			  devUI = value
		...
		-- GAME
		{ id = "networksmoothing", restart = true, category = types.basic, group = "game", name = I18N('ui.settings.option.networksmoothing'), type = "bool", value = useNetworkSmoothing, description = I18N('ui.settings.option.networksmoothing_descr'),
		  onload = function(i)
			  options[i].onchange(i, options[i].value)
		  end,
		  onchange = function(i, value)
		...
		{ id = "autoquit", group = "game", category = types.basic, widget = "Autoquit", name = I18N('ui.settings.option.autoquit'), type = "bool", value = GetWidgetToggleValue("Autoquit"), description = I18N('ui.settings.option.autoquit_descr') },

		{ id = "singleplayerpause", group = "game", category = types.advanced, name = I18N('ui.settings.option.singleplayerpause'), type = "bool", value = pauseGameWhenSingleplayer, description = I18N('ui.settings.option.singleplayerpause_descr'),
		  onchange = function(i, value)
		...
		{ id = "catchupsmoothness", group = "game", category = types.dev, name = I18N('ui.settings.option.catchupsmoothness'), restart = true, type = "slider", min = 0.05, max = 0.3, step = 0.01, value = Spring.GetConfigFloat("MinSimDrawBalance", 0.15), description = I18N('ui.settings.option.catchupsmoothness_descr'),
		  onchange = function(i, value)
			  Spring.SetConfigFloat("MinSimDrawBalance", value)
		  end,
		},
		{ id = "catchupminfps", group = "game", category = types.dev, name = I18N('ui.settings.option.catchupminfps'), restart = true, type = "slider", min = 2, max = 15, step = 1, value = Spring.GetConfigInt("MinDrawFPS", 2), description = I18N('ui.settings.option.catchupminfps_descr'),
		  onchange = function(i, value)
			  Spring.SetConfigInt("MinDrawFPS", value)
		  end,
		},

		{ id = "label_ui_behavior", group = "game", name = I18N('ui.settings.option.label_behavior'), category = types.basic },
		{ id = "label_ui_behavior_spacer", group = "game", category = types.basic },


		{ id = "smartselect_includebuildings", group = "game", category = types.basic, name = I18N('ui.settings.option.smartselect_includebuildings'), type = "bool", value = false, description = I18N('ui.settings.option.smartselect_includebuildings_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "smartselect_includebuilders", group = "game", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.smartselect_includebuilders'), type = "bool", value = false, description = I18N('ui.settings.option.smartselect_includebuilders_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "smartselect_includeantinuke", group = "game", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.smartselect_includeantinuke'), type = "bool", value = false, description = I18N('ui.settings.option.smartselect_includeantinuke_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "smartselect_includeradar", group = "game", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.smartselect_includeradar'), type = "bool", value = false, description = I18N('ui.settings.option.smartselect_includeradar_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "smartselect_includejammer", group = "game", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.smartselect_includejammer'), type = "bool", value = false, description = I18N('ui.settings.option.smartselect_includejammer_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "prioconturrets", group = "game", category = types.basic, widget = "Priority Construction Turrets", name = I18N('ui.settings.option.prioconturrets'), type = "bool", value = GetWidgetToggleValue("Priority Construction Turrets"), description = I18N('ui.settings.option.prioconturrets_descr') },

		{
			...
			name = I18N('ui.settings.option.builderpriority'),
			type = "bool",
			value = GetWidgetToggleValue("Builder Priority"),
			description = I18N('ui.settings.option.builderpriority_descr'),
		},

		{
			...
			name = widgetOptionColor .. "   " .. I18N('ui.settings.option.builderpriority_nanos'),
			type = "bool",
			value = (
			...
			description = I18N('ui.settings.option.builderpriority_nanos_descr'),
			onload = function(i)
...

		{
			...
			group = "game",
			category = types.advanced,
			name = widgetOptionColor .. "   " .. I18N('ui.settings.option.builderpriority_cons'),
			type = "bool",
			value = (
					WG['builderpriority'] ~= nil
			...
			description = I18N('ui.settings.option.builderpriority_cons_descr'),
			onload = function(i)
...

		{
			...
			group = "game",
			category = types.advanced,
			name = widgetOptionColor .. "   " .. I18N('ui.settings.option.builderpriority_labs'),
			type = "bool",
			value = (
					WG['builderpriority'] ~= nil
			...
			description = I18N('ui.settings.option.builderpriority_labs_descr'),
			onload = function(i)
		...
		{ id = "factoryguard", group = "game", category = types.basic, widget = "Factory Guard Default On", name = I18N('ui.settings.option.factory') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.factoryguard'), type = "bool", value = GetWidgetToggleValue("Factory Guard Default On"), description = I18N('ui.settings.option.factoryguard_descr') },
		{ id = "factoryholdpos", group = "game", category = types.basic, widget = "Factory hold position", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.factoryholdpos'), type = "bool", value = GetWidgetToggleValue("Factory hold position"), description = I18N('ui.settings.option.factoryholdpos_descr') },
		{ id = "factoryrepeat", group = "game", category = types.basic, widget = "Factory Auto-Repeat", name = widgetOptionColor .. "   " .. I18N('ui.settings.option.factoryrepeat'), type = "bool", value = GetWidgetToggleValue("Factory Auto-Repeat"), description = I18N('ui.settings.option.factoryrepeat_descr') },
		...
		{ id = "onlyfighterspatrol", group = "game", category = types.basic, widget = "OnlyFightersPatrol", name = I18N('ui.settings.option.onlyfighterspatrol'), type = "bool", value = GetWidgetToggleValue("Autoquit"), description = I18N('ui.settings.option.onlyfighterspatrol_descr') },
		{ id = "fightersfly", group = "game", category = types.basic, widget = "Set fighters on Fly mode", name = I18N('ui.settings.option.fightersfly'), type = "bool", value = GetWidgetToggleValue("Set fighters on Fly mode"), description = I18N('ui.settings.option.fightersfly_descr') },

		{ id = "settargetdefault", group = "game", category = types.basic, widget = "Set target default", name = I18N('ui.settings.option.settargetdefault'), type = "bool", value = GetWidgetToggleValue("Set target default"), description = I18N('ui.settings.option.settargetdefault_descr') },
		{ id = "dgunnogroundenemies", group = "game", category = types.advanced, widget = "DGun no ground enemies", name = I18N('ui.settings.option.dgunnogroundenemies'), type = "bool", value = GetWidgetToggleValue("DGun no ground enemies"), description = I18N('ui.settings.option.dgunnogroundenemies_descr') },
		{ id = "dgunstallassist", group = "game", category = types.advanced, widget = "DGun Stall Assist", name = I18N('ui.settings.option.dgunstallassist'), type = "bool", value = GetWidgetToggleValue("DGun Stall Assist"), description = I18N('ui.settings.option.dgunstallassist_descr') },

		{ id = "unitreclaimer", group = "game", category = types.basic, widget = "Specific Unit Reclaimer", name = I18N('ui.settings.option.unitreclaimer'), type = "bool", value = GetWidgetToggleValue("Specific Unit Reclaimer"), description = I18N('ui.settings.option.unitreclaimer_descr') },

		{ id = "autogroup_immediate", group = "game", category = types.basic, name = I18N('ui.settings.option.autogroup_immediate'), type = "bool", value = (WG['autogroup'] ~= nil and WG['autogroup'].getImmediate ~= nil and WG['autogroup'].getImmediate()), description = I18N('ui.settings.option.autogroup_immediate_descr'),
		  onload = function(i)
		...
		{ id = "autogroup_persist", group = "game", category = types.basic, name = I18N('ui.settings.option.autogroup_persist'), type = "bool", value = (WG['autogroup'] ~= nil and WG['autogroup'].getPersist ~= nil and WG['autogroup'].getPersist()), description = I18N('ui.settings.option.autogroup_persist_descr'),
		  onload = function(i)
		...
		{ id = "label_ui_cloak", group = "game", name = I18N('ui.settings.option.label_cloak'), category = types.basic },
		{ id = "label_ui_cloak_spacer", group = "game", category = types.basic },

		{ id = "autocloak", group = "game", category = types.basic, widget = "Auto Cloak Units", name = I18N('ui.settings.option.autocloak'), type = "bool", value = GetWidgetToggleValue("Auto Cloak Units") },

		-- ACCESSIBILITY

		{ id = "label_teamcolors", group = "accessibility", name = I18N('ui.settings.option.label_teamcolors'), category = types.basic },
		{ id = "label_teamcolors_spacer", group = "accessibility", category = types.basic },

		{ id = "anonymous_r", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.anonymous_r'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("anonymousColorR", 255)), description = I18N('ui.settings.option.anonymous_descr'),
		  onchange = function(i, value, force)
			  if force then
		...
		{ id = "anonymous_g", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.anonymous_g'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("anonymousColorG", 0)), description = I18N('ui.settings.option.anonymous_descr'),
		  onchange = function(i, value, force)
			  if force then
		...
		{ id = "anonymous_b", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.anonymous_b'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("anonymousColorB", 0)), description = I18N('ui.settings.option.anonymous_descr'),
		  onchange = function(i, value, force)
			  if force then
		...
		{ id = "simpleteamcolors", group = "accessibility", category = types.basic, name = I18N('ui.settings.option.playercolors'), type = "bool", value = tonumber(Spring.GetConfigInt("SimpleTeamColors", 0) or 0) == 1, description = I18N('ui.settings.option.simpleteamcolors_descr'),
		  onchange = function(i, value)
		...
		{ id = "simpleteamcolors_reset", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " ..  I18N('ui.settings.option.simpleteamcolors_reset'), type = "bool", value = tonumber(Spring.GetConfigInt("SimpleTeamColors_Reset", 0) or 0) == 1,
		  onchange = function(i, value)
		...
		{ id = "simpleteamcolors_use_gradient", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_use_gradient'), type = "bool", value = tonumber(Spring.GetConfigInt("SimpleTeamColorsUseGradient", 0) or 0) == 1,
		  onchange = function(i, value)
		...
		{ id = "simpleteamcolors_player_r", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_player_r'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsPlayerR", 0)),
		  onchange = function(i, value)
			  Spring.SetConfigInt("SimpleTeamColorsPlayerR", value)
			  Spring.SetConfigInt("UpdateTeamColors", 1)
		  end,
		},
		{ id = "simpleteamcolors_player_g", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_player_g'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsPlayerG", 77)),
		  onchange = function(i, value)
			  Spring.SetConfigInt("SimpleTeamColorsPlayerG", value)
			  Spring.SetConfigInt("UpdateTeamColors", 1)
		  end,
		},
		{ id = "simpleteamcolors_player_b", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_player_b'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsPlayerB", 255)),
		  onchange = function(i, value)
		...
		{ id = "simpleteamcolors_ally_r", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_ally_r'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsAllyR", 0)),
		  onchange = function(i, value)
			  Spring.SetConfigInt("SimpleTeamColorsAllyR", value)
			  Spring.SetConfigInt("UpdateTeamColors", 1)
		  end,
		},
		{ id = "simpleteamcolors_ally_g", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_ally_g'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsAllyG", 255)),
		  onchange = function(i, value)
			  Spring.SetConfigInt("SimpleTeamColorsAllyG", value)
			  Spring.SetConfigInt("UpdateTeamColors", 1)
		  end,
		},
		{ id = "simpleteamcolors_ally_b", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_ally_b'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsAllyB", 0)),
		  onchange = function(i, value)
		...
		{ id = "simpleteamcolors_enemy_r", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_enemy_r'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsEnemyR", 255)),
		  onchange = function(i, value)
			  Spring.SetConfigInt("SimpleTeamColorsEnemyR", value)
			  Spring.SetConfigInt("UpdateTeamColors", 1)
		  end,
		},
		{ id = "simpleteamcolors_enemy_g", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_enemy_g'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsEnemyG", 16)),
		  onchange = function(i, value)
			  Spring.SetConfigInt("SimpleTeamColorsEnemyG", value)
			  Spring.SetConfigInt("UpdateTeamColors", 1)
		  end,
		},
		{ id = "simpleteamcolors_enemy_b", group = "accessibility", category = types.basic, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.simpleteamcolors_enemy_b'), type = "slider", min = 0, max = 255, step = 1, value = tonumber(Spring.GetConfigInt("SimpleTeamColorsEnemyB", 5)),
		  onchange = function(i, value)
		...
		-- DEV
		{ id = "customwidgets", group = "dev", category = types.dev, name = I18N('ui.settings.option.customwidgets'), type = "bool", value = widgetHandler.allowUserWidgets, description = I18N('ui.settings.option.customwidgets_descr'),
		  onchange = function(i, value)
		...
		{ id = "autocheat", group = "dev", category = types.dev, widget = "Dev Auto cheat", name = I18N('ui.settings.option.autocheat'), type = "bool", value = GetWidgetToggleValue("Dev Auto cheat"), description = I18N('ui.settings.option.autocheat_descr') },
		{ id = "restart", group = "dev", category = types.dev, name = I18N('ui.settings.option.restart'), type = "bool", value = false, description = I18N('ui.settings.option.restart_descr'),
		  onchange = function(i, value)
		...
		{ id = "label_dev_debug", group = "dev", name = I18N('ui.settings.option.label_debug'), category = types.dev },
		{ id = "label_dev_debug_spacer", group = "dev", category = types.dev },

		{ id = "profiler_widget", group = "dev", category = types.dev, widget = "Widget Profiler", name = I18N('ui.settings.option.profiler') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.profiler_widget'), type = "bool", value = GetWidgetToggleValue("Widget Profiler") },
		{ id = "profiler_gadget", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.profiler_gadget'), type = "bool", value = false,
		  onchange = function(i, value)
			  Spring.SendCommands("luarules profile")
		  end,
		},
		{ id = "profiler_sort_by_load", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.profiler_sort_by_load'), type = "bool", value = Spring.GetConfigInt("profiler_sort_by_load", 1), description = I18N('ui.settings.option.profiler_sort_by_load_descr'),
		onchange = function(i, value)
			Spring.SetConfigInt("profiler_sort_by_load", (value and '1' or '0'))
		end,
	  },
	  { id = "profiler_averagetime", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.profiler_averagetime'), type = "slider", min = 0.1, max = 10, step = 0.1, value = Spring.GetConfigFloat("profiler_averagetime", 2), description = I18N('ui.settings.option.profiler_averagetime_descr'),
		onchange = function(i, value)
			Spring.SetConfigFloat("profiler_averagetime", value)
		end,
	  },
		{ id = "framegrapher", group = "dev", category = types.dev, widget = "Frame Grapher", name = I18N('ui.settings.option.framegrapher'), type = "bool", value = GetWidgetToggleValue("Frame Grapher"), description = "" },

		{ id = "debugcolvol", group = "dev", category = types.dev, name = I18N('ui.settings.option.debugcolvol'), type = "bool", value = false, description = "",
		  onchange = function(i, value)
			  Spring.SendCommands("DebugColVol " .. (value and '1' or '0'))
		  end,
		},
		{ id = "echocamerastate", group = "dev", category = types.dev, name = I18N('ui.settings.option.echocamerastate'), type = "bool", value = false, description = I18N('ui.settings.option.echocamerastate_descr'),
		  onchange = function(i, value)
		...
		{ id = "label_dev_other", group = "dev", name = I18N('ui.settings.option.label_other'), category = types.dev },
		{ id = "label_dev_other_spacer", group = "dev", category = types.dev },

		{ id = "storedefaultsettings", group = "dev", category = types.dev, name = I18N('ui.settings.option.storedefaultsettings'), type = "bool", value = tonumber(Spring.GetConfigInt("StoreDefaultSettings", 0) or 0) == 1, description = I18N('ui.settings.option.storedefaultsettings_descr'),
		  onchange = function(i, value)
			  Spring.SetConfigInt("StoreDefaultSettings", (value and 1 or 0))
		  end,
		},

		{ id = "startboxeditor", group = "dev", category = types.dev, widget = "Startbox Editor", name = I18N('ui.settings.option.startboxeditor'), type = "bool", value = GetWidgetToggleValue("Startbox Editor"), description = I18N('ui.settings.option.startboxeditor_descr') },

		{ id = "language_dev", group = "dev", category = types.dev, name = I18N('ui.settings.option.language'), type = "select", options = devLanguageNames, value = devLanguageCodes[I18N.getLocale()],
			onchange = function(i, value)
				local devLanguage = devLanguageCodes[value]
				WG['language'].setLanguage(devLanguage)
			end
		},
		{ id = "font", group = "dev", category = types.dev, name = I18N('ui.settings.option.font'), type = "select", options = {}, value = 1, description = I18N('ui.settings.option.font_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "font2", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.font2'), type = "select", options = {}, value = 1, description = I18N('ui.settings.option.font2_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
			  if VFS.FileExists('fonts/' .. options[i].optionsFont[value]) then
		...
		{ id = "label_dev_map", group = "dev", name = I18N('ui.settings.option.label_map'), category = types.dev },
		{ id = "label_dev_map_spacer", group = "dev", category = types.dev },

		{ id = "sun_y", group = "dev", category = types.dev, name = I18N('ui.settings.option.sun') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.sun_y'), type = "slider", min = 0.05, max = 0.9999, step = 0.0001, value = select(2, gl.GetSun("pos")),
		  onchange = function(i, value)
		...
		{ id = "sun_x", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sun_x'), type = "slider", min = -0.9999, max = 0.9999, step = 0.0001, value = select(1, gl.GetSun("pos")),
		  onchange = function(i, value)
			  local sunX, sunY, sunZ = gl.GetSun("pos")
			  sunX = value
		...
		{ id = "sun_z", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sun_z'), type = "slider", min = -0.9999, max = 0.9999, step = 0.0001, value = select(3, gl.GetSun("pos")),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
			  local sunX, sunY, sunZ = gl.GetSun("pos")
		...
		{ id = "sun_reset", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.sun_reset'), type = "bool", value = false,
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "fog_start", group = "dev", category = types.dev, name = I18N('ui.settings.option.fog') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.fog_start'), type = "slider", min = 0, max = 1.99, step = 0.01, value = gl.GetAtmosphere("fogStart"),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "fog_end", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.fog_end'), type = "slider", min = 0.5, max = 2, step = 0.01, value = gl.GetAtmosphere("fogEnd"),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "fog_reset", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.fog_reset'), type = "bool", value = false, description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "fog_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.fog') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 1, step = 0.01, value = select(1, gl.GetAtmosphere("fogColor")), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "fog_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 1, step = 0.01, value = select(2, gl.GetAtmosphere("fogColor")), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
			  local fogColor = { gl.GetAtmosphere("fogColor") }
		...
		{ id = "fog_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 1, step = 0.01, value = select(3, gl.GetAtmosphere("fogColor")), description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
			  local fogColor = { gl.GetAtmosphere("fogColor") }
		...
		{ id = "fog_color_reset", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.fog_color_reset'), type = "bool", value = false, description = '',
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "map_voidwater", group = "dev", category = types.dev, name = I18N('ui.settings.option.map_voidwater'), type = "bool", value = false, description = "",
		  onload = function(i)
		...
		{ id = "map_voidground", group = "dev", category = types.dev, name = I18N('ui.settings.option.map_voidground'), type = "bool", value = false, description = "",
		  onload = function(i)
		...
		{ id = "map_splatdetailnormaldiffusealpha", group = "dev", category = types.dev, name = I18N('ui.settings.option.map_splatdetailnormaldiffusealpha'), type = "bool", value = false, description = "",
		  onload = function(i)
		...
		{ id = "map_splattexmults_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.map_splattexmults') .. widgetOptionColor .. "   0", type = "slider", min = 0, max = 1.5, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "map_splattexacales_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.map_splattexacales') .. widgetOptionColor .. "   0", type = "slider", min = 0, max = 0.02, step = 0.0001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "GroundShadowDensity", group = "dev", category = types.dev, name = I18N('ui.settings.option.GroundShadowDensity') .. widgetOptionColor .. "  ", type = "slider", min = 0, max = 1.5, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "UnitShadowDensity", group = "dev", category = types.dev, name = I18N('ui.settings.option.UnitShadowDensity') .. widgetOptionColor .. "  ", type = "slider", min = 0, max = 1.5, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "color_groundambient_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.color_groundambient') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "color_groundambient_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("ambient")
			  options[i].value = g
		  end,
		...
		{ id = "color_groundambient_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("ambient")
			  options[i].value = b
		  end,
		...
		{ id = "color_grounddiffuse_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.color_grounddiffuse') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "color_grounddiffuse_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("diffuse")
			  options[i].value = g
		  end,
		...
		{ id = "color_grounddiffuse_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("diffuse")
			  options[i].value = b
		  end,
		...
		{ id = "color_groundspecular_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.color_groundspecular') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "color_groundspecular_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 1, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("specular")
			  options[i].value = g
		  end,
		...
		{ id = "color_groundspecular_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 1, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("specular")
			  options[i].value = b
		  end,
		...
		{ id = "color_unitambient_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.color_unitambient') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "color_unitambient_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("ambient", "unit")
			  options[i].value = g
		  end,
		...
		{ id = "color_unitambient_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("ambient", "unit")
			  options[i].value = b
		  end,
		...
		{ id = "color_unitdiffuse_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.color_unitdiffuse') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "color_unitdiffuse_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("diffuse", "unit")
			  options[i].value = g
		  end,
		...
		{ id = "color_unitdiffuse_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("diffuse", "unit")
			  options[i].value = b
		  end,
		...
		{ id = "color_unitspecular_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.color_unitspecular') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "color_unitspecular_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("specular", "unit")
			  options[i].value = g
		  end,
		...
		{ id = "color_unitspecular_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 2, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetSun("specular", "unit")
			  options[i].value = b
		  end,
		...
		{ id = "suncolor_r", group = "dev", category = types.dev, name = "Sun" .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 1, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "suncolor_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 1, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetAtmosphere("sunColor")
			  options[i].value = g
		  end,
		...
		{ id = "suncolor_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 1, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetAtmosphere("sunColor")
			  options[i].value = b
		  end,
		...
		{ id = "skycolor_r", group = "dev", category = types.dev, name = I18N('ui.settings.option.skycolor') .. widgetOptionColor .. "  " .. I18N('ui.settings.option.red'), type = "slider", min = 0, max = 1, step = 0.001, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "skycolor_g", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.green'), type = "slider", min = 0, max = 1, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetAtmosphere("skyColor")
			  options[i].value = g
		  end,
		...
		{ id = "skycolor_b", group = "dev", category = types.dev, name = widgetOptionColor .. "   " .. I18N('ui.settings.option.blue'), type = "slider", min = 0, max = 1, step = 0.001, value = 0, description = "",
		  onload = function(i)
			  local r, g, b = gl.GetAtmosphere("skyColor")
			  options[i].value = b
		  end,
		...
		{ id = "sunlighting_reset", group = "dev", category = types.dev, name = I18N('ui.settings.option.sunlighting_reset'), type = "bool", value = false, description = I18N('ui.settings.option.sunlighting_reset_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "skyaxisangle_angle", group = "dev", category = types.dev, name = I18N('ui.settings.option.skybox') .. widgetOptionColor .. "  "..I18N('ui.settings.option.angle'), type = "slider", min = -3.14, max = 3.14, step = 0.01, value = 0, description = "",
		  onload = function(i)
		...
		{ id = "skyaxisangle_reset", group = "dev", category = types.dev, name = widgetOptionColor .. "   "..I18N('ui.settings.option.reset'), type = "bool", value = false, description = I18N('ui.settings.option.sunlighting_reset_descr'),
		  onload = function(i)
		  end,
		  onchange = function(i, value)
		...
		{ id = "label_dev_water", group = "dev", name = I18N('ui.settings.option.label_water'), category = types.dev },
	...
	if spectatorHUDConfigOptions[options[getOptionByID('spectator_hud_config')].value] ~= I18N('ui.settings.option.spectator_hud_config_custom') then
	...
	if not Utilities.Gametype.GetCurrentHolidays()["aprilfools"] then
	...
	if not Utilities.Gametype.GetCurrentHolidays()["halloween"] then
	...
	if not Utilities.Gametype.GetCurrentHolidays()["xmas"] then
	...
	if #displayNames <= 1 then
		...
		options[getOptionByID('resolution')].name = I18N('ui.settings.option.resolution')
	end
	...
	if type(trackList) == 'table' then
		...
		for i, option in pairs(options) do
			...
			if option.id == 'soundtrackFades' then
				count = count + 1
				newOptions[count] = { id = "label_sound_music", group = "sound", name = I18N('ui.settings.option.label_playlist'), category = types.basic }
				count = count + 1
			...
			if option.id == 'label_notif_messages_spacer' then
				for k, v in pairs(notificationList) do
					if type(v) == 'table' then
						...
						newOptions[count] = { id = "notifications_notif_" .. v[1], group = "notif", category = types.basic, name = color .. "   " .. I18N(v[3]), type = "bool", value = v[2], --description = v[3] and Spring.I18N(v[3]) or "",
											  onchange = function(i, value)
		...
		for i, option in pairs(options) do
			...
			if option.id == 'autocloak' then
				for k, v in pairs(unitdefConfig) do
					if UnitDefs[k] then
						local faction = I18N('units.factions.' .. string.sub(UnitDefs[k].name,1,3))
						if faction then
							count = count + 1
	...
	for name, data in pairs(widgetHandler.knownWidgets) do
		if not data.fromZip then
			if not userwidgetsDetected then
				...
				options[#options+1] = { id = "label_custom_widgets", group = "custom", name = I18N('ui.settings.option.label_widgets'), category = types.basic }
			...
			if data.author and data.author ~= '' then
				desc = desc .. (desc ~= '' and '\n' or '')..widgetOptionColor..I18N('ui.settings.option.author')..': '.. data.author
			end
	...
	if customOptionsCount > 0 then
		options[#options+1] = { id = "label_custom_options", group = "custom", name = I18N('ui.settings.option.label_options'), category = types.basic }
