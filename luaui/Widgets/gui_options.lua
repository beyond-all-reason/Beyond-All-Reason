...
function mouseEvent(mx, my, button, release)
	...
	if show then
		...
		elseif button == 1 then
			...
			elseif math_isInRect(mx, my, windowRect[1], windowRect[2], windowRect[3], windowRect[4]) then	-- on window
...
function init()
	...
	options = {
		...
		{ id = "sunlighting_reset", group = "dev", name = texts.option.sunlighting_reset, type = "bool", value = false, description = texts.option.sunlighting_reset_descr,
		  ...
		  { id = "water_shorewaves", group = "dev", name = "water (bump) "..widgetOptionColor .. "  shorewaves", type = "bool", value = Spring.GetConfigInt("BumpWaterShoreWaves", 1) == 1, description = "",
			onload = function(i)
			end,
			onchange = function(i, value)
