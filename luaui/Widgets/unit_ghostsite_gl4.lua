...
function widget:UnitEnteredLos(unitID, teamID)
	...
	if includedUnitDefIDs[unitDefID] and Engine.Shared.GetUnitIsBeingBuilt(unitID) then
...
local function updateGhostSites()
	for unitID, site in pairs(ghostSites) do
		if not unitshapes[unitID] then
			...
			if inLos and not Engine.Shared.GetUnitIsBeingBuilt(unitID) then
				removeUnitShape(unitID)
...
function widget:TeamDied(teamID)
	...
	if allyTeamDead then
		for unitID, site in pairs(ghostSites) do
			if select(6, Engine.Shared.GetTeamInfo(site.teamID)) == allyTeamID then
				removeUnitShape(unitID)
				ghostSites[unitID] = nil
			end
		end
...
function widget:SetConfigData(data)
	if Engine.Shared.GetGameFrame() > 0 and data.ghostSites ~= nil then
