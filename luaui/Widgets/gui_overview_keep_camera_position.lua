...
local function storeCamKeys()
	camKeys = {}
	local keyTable = Engine.Unsynced.GetActionHotKeys("toggleoverview")
...
local function isOverview()
	return Engine.Unsynced.GetCameraState().name == "ov"
end
...
function widget:KeyPress(key, modifier)
	...
	if isOverview() then
		...
		prevCamState = Engine.Unsynced.GetCameraState()
	end
end
