...
local function draw_buttons(b)
	...
	for i = 1, #b do
		UiButton(
			mathFloor((b[i].x * vsx) + 0.5),
			mathFloor((b[i].y * vsy) + 0.5),
			mathFloor(((b[i].x + bWidth) * vsx) + 0.5),
			mathFloor(((b[i].y + bHeight) * vsy) + 0.5),
			0,
			1,
			1,
			0,
			...
			nil,
			{ 0, 0, 0, ui_opacity },
			{ 0.2, 0.2, 0.2, ui_opacity },
			bgpadding * 0.5
		)
		font:Print(
			b[i].text,
			mathFloor((b[i].x * vsx) + 0.5),
			mathFloor(((b[i].y + bHeight / 2) * vsy) + 0.5),
			mathFloor((0.0115 * vsx) + 0.5),
			"vo"
		)
	end
	font:End()
...
function widget:DrawScreen()
	...
	if sceduleUpdate then
		...
		backgroundGuishader = gl.CreateList(function()
			RectRound(
				mathFloor((wPos.x * vsx) + 0.5),
				mathFloor((wPos.y * vsy) + 0.5),
				mathFloor(((wPos.x + bWidth) * vsx) + 0.5),
				mathFloor(((wPos.y + dy) * vsy) + 0.5),
				elementCorner,
				0,
				1,
				1,
	...
	if
		point_in_rect(
			b[#buttons].x,
			b[#buttons].y,
			b[topbutton].x + bWidth,
			b[topbutton].y + bHeight,
			mousex / vsx,
			mousey / vsy
		)
	then
		for i = 1, #b do
			if
				point_in_rect(b[i].x, b[i].y, b[i].x + bWidth, b[i].y + bHeight, mousex / vsx, mousey / vsy)
				or i == active_button
			then
				...
				RectRound(
					...
					1,
					0,
					{ 0.3, 0.3, 0.3, buttonstate and 0.25 or 0.15 },
					{ 1, 1, 1, buttonstate and 0.25 or 0.15 }
				)
				glBlending(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA)
