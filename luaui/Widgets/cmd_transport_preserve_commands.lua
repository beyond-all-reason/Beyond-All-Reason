...
function widget:UnitLoaded(unitID)
	orders[unitID] = Engine.Shared.GetUnitCommands(unitID, -1)
end
...
function widget:UnitUnloaded(unitID)
	if orders[unitID] and #orders[unitID] then
		...
		Engine.Shared.GiveOrderArrayToUnit(unitID, newOrders)
...
local function maybeRemoveSelf()
	if Engine.Unsynced.IsReplay() or Engine.Unsynced.GetSpectatingState() and (Engine.Shared.GetGameFrame() > 0) then
