...
local function initGL4()
	...
	for i, defRangeClass in ipairs(defenseRangeClasses) do
		...
		local newVAO = InstanceVBOTable.makeVAOandAttach(
			defenseRangeVAOs[defRangeClass].vertexVBO,
			defenseRangeVAOs[defRangeClass].instanceVBO
		)
...
local function UnitDetected(unitID, unitDefID, unitTeam, noUpload)
	...
	for i, weaponType in pairs(unitDefRings[unitDefID].weapons) do
		...
		if
			buttonConfig[allystring][weaponType] or (colorConfig.drawAllyCategoryBuildQueue and (allystring == "ally"))
		then
			--local weaponType = unitDefRings[unitDefID]['weapons'][weaponNum]
			...
			if ringParams then
				...
				local wpx, wpy = Spring.GetUnitWeaponVectors(unitID, weaponID)
				...
				if defenses[unitID] == nil then
					--lazy creation
					defenses[unitID] =
						{ posx = mpx, posy = mpy, posz = mpz, vaokeys = {}, allied = alliedUnit, unitDefID = unitDefID }
				end
...
function widget:FeatureCreated(featureID, allyTeam)
	...
	if featureDefID and featureDefIDtoUnitDefID[featureDefID] then
		...
		if unitID then
			if
				defenses[unitID]
				and defenses[unitID].allied == false
				and featureDefIDtoUnitDefID[featureDefID] == defenses[unitID].unitDefID
			then
...
function widget:PlayerChanged(playerID)
	...
	--
	local nowspec = spGetSpectatingState()
...
local function GetCameraHeightFactor() -- returns a smoothstepped value between 0 and 1 for height based rescaling of line width.
	...
	camheight = math.clamp(
		(camheight - colorConfig.distanceScaleStart) / (colorConfig.distanceScaleEnd - colorConfig.distanceScaleStart),
		0,
		1
	)
