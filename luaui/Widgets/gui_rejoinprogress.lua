...
function widget:Update(dt)
	...
	if not isReplay and serverFrame then
		...
		if t <= 0 then
			...
			if Engine.Shared.IsGameOver() then -- not sure if widget:GameOver() even works so I do this here as well
				widgetHandler:RemoveWidget()
				return
			end

			local speedFactor, _, isPaused = Engine.Unsynced.GetGameSpeed()
