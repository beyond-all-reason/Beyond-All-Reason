...
local function buildStaticRejoin()
	...
	dlistRejoinStatic = gl.CreateList(function()
		...
		RectRound(
			barArea[1] - addedSize - edgeWidth,
			barArea[2] - addedSize - edgeWidth,
			barArea[3] + addedSize + edgeWidth,
			barArea[4] + addedSize + edgeWidth,
			barHeight * 0.33,
			1,
			1,
			1,
		...
		RectRound(
			...
			barArea[2] - addedSize,
			barArea[3] + addedSize,
			barArea[2] + addedSize + addedSize + addedSize,
			barHeight * 0.2,
			0,
			0,
			1,
...
local function drawRejoinDynamic()
	...
	RectRound(
		barArea[1],
		barArea[4] - ((barArea[4] - barArea[2]) / 1.5),
		barArea[1] + valueWidth,
		barArea[4],
		barHeight * 0.2,
		1,
	...
	RectRound(
		...
		barArea[2],
		barArea[1] + valueWidth,
		barArea[2] + ((barArea[4] - barArea[2]) / 2),
		barHeight * 0.2,
		1,
		1,
		1,
	...
	DrawRect(
		(barArea[1] + valueWidth) + (glowSize * 2),
		barArea[2] - glowSize,
		barArea[1] + valueWidth,
		barArea[4] + glowSize,
		0.008
	...
	font2:Print(
		cachedTitleText,
		rejoinArea[1] + ((rejoinArea[3] - rejoinArea[1]) / 2),
		rejoinArea[2] + barHeight * 2 + (fontsize * 0.89),
		fontsize,
		"cor"
	)
	font2:End()
...
function widget:ViewResize()
	...
	rejoinArea = {
		mathFloor(0.5 * vsx) - mathFloor(width * 0.5),
		mathFloor(posY * vsy) - mathFloor(height * 0.5),
		mathFloor(0.5 * vsx) + mathFloor(width * 0.5),
		mathFloor(posY * vsy) + mathFloor(height * 0.5),
	}
	updateGeometry()
