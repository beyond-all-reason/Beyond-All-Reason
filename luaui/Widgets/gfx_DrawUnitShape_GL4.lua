...
local function DrawUnitGL4(
	...
	elseif armUnitDefIDs[unitDefID] then
		...
		spEcho(
			"DrawUnitGL4 : The given unitDefID",
			unitDefID,
...
local function DrawUnitShapeGL4(
	...
	if not DrawUnitShapeVBOTable then
		spEcho(
			"DrawUnitShapeGL4: The given unitDefID",
			unitDefID,
			UnitDefs[unitDefID].name,
...
local function StopDrawUnitShapeGL4(uniqueID)
	if uniqueIDtoUnitShapeVBOTable[uniqueID] then
		...
		spEcho(
			"DrawUnitShapeGL4: the given uniqueID",
			uniqueID,
			" is not present in the uniqueIDtoUnitShapeVBOTable, it might already have been removed?"
		)
	end
...
function widget:Initialize()
	...
	corDrawUnitVBOTable = InstanceVBOTable.makeInstanceVBOTable(
		VBOLayout,
		maxElements,
		"corDrawUnitVBOTable",
	...
	armDrawUnitVBOTable = InstanceVBOTable.makeInstanceVBOTable(
		VBOLayout,
		maxElements,
		"armDrawUnitVBOTable",
		unitIDAttributeIndex,
	...
	for tex1, _ in pairs(tex1ToVBO) do
		...
		local vboTable =
			InstanceVBOTable.makeInstanceVBOTable(VBOLayout, maxElements, vboname, unitIDAttributeIndex, "unitDefID")
		vboTable.VAO = InstanceVBOTable.makeVAOandAttach(vertexVBO, vboTable.instanceVBO, indexVBO)
		vboTable.indexVBO = indexVBO
		vboTable.vertexVBO = vertexVBO
