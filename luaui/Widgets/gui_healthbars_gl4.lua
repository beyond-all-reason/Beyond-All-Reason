...
for udefID, unitDef in pairs(UnitDefs) do
	...
	unitDefSizeMultipliers[udefID] = mathMin(
		1.45,
		math.max(0.85, (Spring.GetUnitDefDimensions(udefID).radius / 150) + mathMin(0.6, unitDef.power / 4000))
...
local function addBarsForUnit(unitID, unitDefID, unitTeam, unitAllyTeam, reason) -- TODO, actually, we need to check for all of these for stuff entering LOS
	...
	if
		...
		and (unitDefIgnore[unitDefID] == nil)
	then
...
function widget:GameFrame(n)
	...
	if (n + 1) % 3 == 0 then
		for unitID, oldempvalue in pairs(unitEmpDamagedWatch) do
			local health, maxHealth, newparalyzeDamage = spGetUnitHealth(unitID)
