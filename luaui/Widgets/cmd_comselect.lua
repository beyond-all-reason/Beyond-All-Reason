...
local function handleSelectComm(_, _, args)
	...
	if not includeSelected then
		...
		for unitDefID, selUnits in pairs(Engine.Unsynced.GetSelectedUnitsSorted()) do
	...
	local units = {}
	local teamUnits = Engine.Shared.GetTeamUnitsByDefs(myTeamID, commanderDefIDsList)
	...
	Engine.Unsynced.SelectUnit(unitID, appendSelection)
