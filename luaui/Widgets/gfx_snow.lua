...
local pausedTime = 0
local lastFrametime = Engine.Unsynced.GetTimer()
...
function widget:DrawWorld()
	...
	if isPaused then
		pausedTime = pausedTime + Engine.Unsynced.DiffTimers(Engine.Unsynced.GetTimer(), lastFrametime)
	end
	lastFrametime = Engine.Unsynced.GetTimer()
	if os.clock() - startOsClock > 0.5 then -- delay to prevent no textures being shown
		if shader ~= nil and particleLists[#particleTypes] ~= nil and particleLists[#particleTypes][particleStep] ~= nil then
			shader:Activate()
			camX, camY, camZ = Engine.Unsynced.GetCameraPosition()
			diffTime = Engine.Unsynced.DiffTimers(lastFrametime, startTimer) - pausedTime
