...
local function AddPrimitiveAtUnit(unitID, unitDefID)
	...
	local radius = Engine.Shared.GetUnitRadius(unitID) * 2.6 or 64
	local width = radius
	local length = radius
...
function widget:DrawWorldPreUnit()
	...
	if selectionVBO.usedElements > 0 then
		...
		local disticon = Engine.Unsynced.GetConfigInt("UnitIconDist", 200) -- iconLength = unitIconDist * unitIconDist * 750.0f;
		--gl.Culling(false)
