...
local metalSpotVBO = nil
local metalSpotInstanceVBO = nil
local metalSpotShader = nil
...
local function makeMetalSpotVBO()
	metalSpotVBO = gl.GetVBO(GL.ARRAY_BUFFER,false)
	if metalSpotVBO == nil then goodbye("Failed to create metalSpotVBO") end
	...
	local pieces = 8 --3 + math.ceil(innersize/11)
	local detail = 12 --math.ceil(innersize/pieces)
	...
	metalSpotVBO:Define(#VBOData/4, VBOLayout)
	metalSpotVBO:Upload(VBOData)
	return metalSpotVBO, #VBOData/4
end
...
local function initGL4()
  ...
  shaderCompiled = metalSpotShader:Initialize()
  if not shaderCompiled then goodbye("Failed to compile metalSpotShader GL4 ") end
  local metalSpotVBO,numVertices = makeMetalSpotVBO()
  local metalSpotInstanceVBOLayout = {
		  {id = 1, name = 'worldpos_radius', size = 4},
		  {id = 2, name = 'visibility', size = 4}, 
		}
  metalSpotInstanceVBO = makeInstanceVBOTable(metalSpotInstanceVBOLayout,8, "metalSpotInstanceVBO")
  metalSpotInstanceVBO.numVertices = numVertices
  metalSpotInstanceVBO.vertexVBO = metalSpotVBO
  metalSpotInstanceVBO.VAO = makeVAOandAttach(metalSpotInstanceVBO.vertexVBO, metalSpotInstanceVBO.instanceVBO)
  metalSpotInstanceVBO.primitiveType = GL.TRIANGLES
end

local function metalSpotKey(posx,posz)
...
function widget:Initialize()
	...
	for i = 1, #mSpots do
		...
		if tonumber(value) > 0.001 then
			...
			pushElementInstance(metalSpotInstanceVBO, {spot.x,  spGetGroundHeight(spot.x,spot.z), spot.z, scale, (occupied and 0) or 1, 0,0,0}, metalSpotKey(spot.x, spot.z))
...
function checkMetalspots()
	...
	for i=1, #metalSpots do
		...
		if occupied ~= prevOccupied then
			...
			local mspotkey = metalSpotKey(spot[1], spot[3])
			local oldinstance = getElementInstanceData(metalSpotInstanceVBO, mspotkey)
			oldinstance[5] = (occupied and 0) or 1
			oldinstance[6] = Spring.GetGameFrame()
			pushElementInstance(metalSpotInstanceVBO, oldinstance, mspotkey,true)
		end
	end
...
function widget:DrawWorldPreUnit()
	...
	metalSpotShader:Activate()
	drawInstanceVBO(metalSpotInstanceVBO)
	metalSpotShader:Deactivate()
