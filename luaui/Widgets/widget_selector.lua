...
if not Engine.Shared.GetModOptions().allowuserwidgets and not Engine.Unsynced.IsReplay() then
...
local function widgetselectorCmd(_, _, params)
	...
	if show then
		...
		Engine.Unsynced.SetConfigInt("widgetselector", 1)
	else
		Engine.Unsynced.SDLStopTextInput()
		widgetHandler.textOwner = nil --widgetHandler:DisownText()
	end
...
function widget:Initialize()
	...
	Engine.Unsynced.SendCommands("unbindkeyset f11")
...
function widget:MouseWheel(up, value)
	...
	local a, c, m, s = Engine.Unsynced.GetModKeyState()
	if a or m then
...
function widget:KeyPress(key, mods, isRepeat)
	if show and key == KEYSYMS.ESCAPE or (key == KEYSYMS.F11 and not isRepeat and not (mods.alt or mods.ctrl or mods.meta or mods.shift)) then
		if key == KEYSYMS.ESCAPE and inputText and inputText ~= "" then
			...
			show = newShow
			if show and not (Spring.Utilities.IsDevMode() or Spring.Utilities.ShowDevUI() or Engine.Unsynced.GetConfigInt("widgetselector", 0) == 1 or localWidgetCount > 0) then
				show = false
			end
			if show then
				widgetHandler.textOwner = self --widgetHandler:OwnText()
		...
		uiList2 = gl.CreateList(function()
			...
			if #widgetsList < #fullWidgetsList then
				...
				if activescrollbar then
					startEntry = mathMax(0, mathMin(floor(#fullWidgetsList * ((sby1 - sbsize) - (my - mathMin(scrollbargrabpos, sbsize))) / sbheight + 0.5), #fullWidgetsList - curMaxEntries)) + 1
				end
...
function widget:MousePress(x, y, button)
	if Engine.Unsynced.IsGUIHidden() or not show then
		return false
	end
...
function widget:MouseMove(x, y, dx, dy, button)
	if show and activescrollbar then
		startEntry = mathMax(0, mathMin(floor((#fullWidgetsList * ((sby1 - sbsize) - (y - mathMin(scrollbargrabpos, sbsize))) / sbheight) + 0.5), #fullWidgetsList - curMaxEntries)) + 1
		UpdateListScroll()
		return true
	end
	return false
...
function widget:MouseRelease(x, y, mb)
	...
	if mb == 1 then
		if maxx - 10 < x and x < maxx and maxy + bgPadding < y and y < maxy + buttonFontSize + 7 + bgPadding then
			...
			UpdateListScroll()
			UpdateGeometry()
			Engine.Unsynced.WarpMouse(x, y + 0.5 * (fontSize + fontSpace))
			return -1
		end
	...
	if showButtons and mb == 1 then
		...
		if buttonID == 4 then
			Engine.Unsynced.SendCommands("luaui reset")
			return -1
		end
		if buttonID == 5 then
			Engine.Unsynced.SendCommands("luaui factoryreset")
			return -1
		end
	end
...
function widget:Shutdown()
	Engine.Unsynced.SendCommands("bind f11 luaui selector") -- if this one is removed or crashes, then have the backup one take over.
	cancelChatInput()
	if WG["guishader"] then
