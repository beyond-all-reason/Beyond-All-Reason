...
local EMOTE_NAMES = {
	-- Positive
	"laugh",     "happy",    "smile",    "love",      "wink",
	"excited",   "star",     "cheer",    "blush",     "smug",   "cool",
	-- Negative
	"cry",       "sad",      "angry",    "rage",      "pain",
	"scared",    "nervous",  "dead",     "tears",
	-- Neutral / physical
	"surprised", "shock",    "think",    "confused",  "sleep",
	"yawn",      "sweat",    "dizzy",    "wave",
}
...
local function RectQuad(px, py, sx, sy)
	...
	gl.TexCoord(o,   1-o)  gl.Vertex(px, py, 0)
	gl.TexCoord(1-o, 1-o)  gl.Vertex(sx, py, 0)
	gl.TexCoord(1-o, o)    gl.Vertex(sx, sy, 0)
...
local function createList(size)
	...
	if drawlist[1] then glDeleteList(drawlist[1]) end
	...
	if drawlist[2] then glDeleteList(drawlist[2]) end
	...
	if drawlist[3] then glDeleteList(drawlist[3]) end
	...
	if drawlist[4] then glDeleteList(drawlist[4]) ; drawlist[4] = nil end
end
...
local function createEmoteList(size, emotePath)
	if drawlist[4] then glDeleteList(drawlist[4]) end
	drawlist[4] = nil
...
local function triggerRandomEmote()
	...
	if not emoteList or #emoteList == 0 then return end
...
function widget:Initialize()
	...
	if not OPTIONS[currentOption] then currentOption = 1 end
	if OPTIONS[currentOption] then loadOption() end
...
function widget:DrawScreen()
	if not drawlist[1] then return end
	glPushMatrix()
