...
for udid, udef in pairs(UnitDefs) do
	...
	local builder = (udef.buildOptions and udef.buildOptions[1])
		or ((udef.canRepair and udef.repairSpeed > 0) or (udef.canReclaim and udef.reclaimSpeed > 0))
...
function widget:Update(dt)
	...
	if
		(prevSelRect[1] and prevSelRect[1] ~= x1)
		or (not prevSelRect[1] and x1)
		or (prevSelRect[2] and prevSelRect[2] ~= y1)
		or (not prevSelRect[2] and y1)
		or (prevSelRect[3] and prevSelRect[3] ~= x2)
		or (not prevSelRect[3] and x2)
		or (prevSelRect[4] and prevSelRect[4] ~= y2)
		or (not prevSelRect[4] and y2)
	then
		selRectChanged = true
	end
	...
	WG.SmartSelect_SelectUnits = function(units)
		...
		for i = 1, #mouseSelection do
			...
			if
				...
				and (
					isGodMode
					or ((not spec or spGetUnitTeam(uid) ~= GaiaTeamID) and not ignoreUnits[spGetUnitDefID(uid)])
				)
			then
				included[#included + 1] = uid
			end
