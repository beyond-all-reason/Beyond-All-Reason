...
function widget:MousePress(mx, my, button)
	if enabled and (not Engine.Unsynced.IsAboveMiniMap(mx, my)) then
		...
		if legalPos(pos) then
			...
			Engine.Unsynced.MarkerAddPoint(pos[1], 0, pos[3], mexIndex)
			mexIndex = mexIndex + 1
			--else
...
function widget:Shutdown()
	for _, i in pairs(markers) do
		Engine.Unsynced.MarkerErasePosition(i[1], i[2], i[3])
	end
	if handle ~= nil then
		io.close(handle)
