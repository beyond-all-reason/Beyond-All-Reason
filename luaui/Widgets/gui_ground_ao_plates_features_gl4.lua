...
function widget:Initialize()
	...
	for id, featureDefID in pairs(FeatureDefs) do
		...
		if
			FD.modelname
			and (string.find(FD.modelname:lower(), "_dead", nil, true) or string.find(FD.name, "_heap", nil, true))
		then -- todo TREES!
				...
				for x = coords[1], coords[1] + size * maxx, size do
					if i <= #matches then
						commandqueue[#commandqueue + 1] =
							string.format("give %s @%d,%d,%d", matches[i], x, Spring.GetGroundHeight(x, z), z)
						i = i + 1
					end
				end
			end
