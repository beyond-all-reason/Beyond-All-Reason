...
local function AddPrimitiveAtUnit(featureID, unitDefID, noUpload)
	local gf = Spring.GetGameFrame()
	unitDefID = unitDefID or Spring.GetFeatureDefID(featureID)
  ...
  --if true then return end
	for id , featureDefID in pairs(FeatureDefs) do
      ...
      if atlassedImages[modelnamenos3o] then
        ...
        local sizestr = string.sub(atlasname,string.find(atlasname, "_dead", nil, true) + 6, string.find(atlasname, "_aoplane.dds", nil, true)-1)
