...
local function createAward(pic, award, note, noteColour, winnersTable, offset)
	...
	thisAward = gl.CreateList(function()
		...
		if award ~= 2 then -- award
			...
			gl.Texture(pic)
			gl.TexRect(
				widgetX + mathFloor(12 * widgetScale),
				widgetY + widgetHeightScaled - offset - mathFloor(70 * widgetScale),
				widgetX + mathFloor(108 * widgetScale),
				widgetY + widgetHeightScaled - offset + mathFloor(25 * widgetScale)
			)
			gl.Texture(false)
			...
			font2:Begin()
			font2:Print(
				colourNames(winnerTeamID) .. winnerName,
				widgetX + mathFloor(120 * widgetScale),
				widgetY + widgetHeightScaled - offset - mathFloor(15 * widgetScale),
				25 * widgetScale,
				"o"
			)
			font2:End()
			...
			font:Print(
				noteColour .. note,
				widgetX + mathFloor(130 * widgetScale),
				widgetY + widgetHeightScaled - offset - mathFloor(40 * widgetScale),
				15 * widgetScale,
				"o"
			)
		else -- others
		...
		if award == 0 then
			...
			if winnerTeamID >= 0 then
				...
				font:Print(
					colourNames(winnerTeamID) .. winnerScore,
			...
			if secondScore > 0 then
				...
				font2:Begin()
				font2:Print(
					colourNames(secondTeamID) .. secondName,
					widgetX + mathFloor(520 * widgetScale),
					widgetY + widgetHeightScaled - offset - mathFloor(27 * widgetScale),
					16 * widgetScale,
					"o"
				)
				font2:End()
				font:Begin()
				font:Print(
					colourNames(secondTeamID) .. secondScore,
					widgetX + widgetWidthScaled / 2 + mathFloor(275 * widgetScale),
					widgetY + widgetHeightScaled - offset - mathFloor(27 * widgetScale),
					14 * widgetScale,
					"o"
			...
			if thirdScore > 0 then
				...
				font2:Begin()
				font2:Print(
					colourNames(thirdTeamID) .. thirdName,
					widgetX + mathFloor(520 * widgetScale),
					widgetY + widgetHeightScaled - offset - mathFloor(49 * widgetScale),
					16 * widgetScale,
					"o"
				)
				font2:End()
				font:Begin()
				font:Print(
					colourNames(thirdTeamID) .. thirdScore,
					widgetX + widgetWidthScaled / 2 + mathFloor(275 * widgetScale),
					widgetY + widgetHeightScaled - offset - mathFloor(49 * widgetScale),
					14 * widgetScale,
					"o"
...
local function ProcessAwards(awards)
	...
	if awards.ecoKill[1].teamID >= 0 then
		FirstAward =
			createAward("fuscup", 0, BAR.I18N("ui.awards.resourcesDestroyed"), white, awards.ecoKill, offset)
		offset = offset + offsetAdd
	end
	if awards.fightKill[1].teamID >= 0 then
		SecondAward =
			createAward("bullcup", 0, BAR.I18N("ui.awards.enemiesDestroyed"), white, awards.fightKill, offset)
		offset = offset + offsetAdd
	end
...
function widget:MousePress(x, y, button)
	if drawAwards then
		...
		-- Close button
		if
			(x > widgetX + widgetWidthScaled - closeRightX - mathFloor(5 * widgetScale))
			and (x < widgetX + widgetWidthScaled - closeRightX + mathFloor(20 * widgetScale) * font:GetTextWidth("X") + mathFloor(
				5 * widgetScale
			))
			and (
				y > widgetY + widgetHeightScaled - mathFloor((10 + 17 + 5) * widgetScale)
...
function widget:DrawScreen()
	...
	-- Close button
	if
		(x > widgetX + widgetWidthScaled - closeRightX - (5 * widgetScale))
		and (x < widgetX + widgetWidthScaled - closeRightX + mathFloor(20 * widgetScale) * font2:GetTextWidth("X") + mathFloor(
			5 * widgetScale
		))
	...
	font2:Print(
		graphColour .. "X",
		widgetX + widgetWidthScaled - closeRightX,
		widgetY + widgetHeightScaled - mathFloor((10 + 17) * widgetScale),
		20 * widgetScale,
		"o"
	)
	font2:End()
