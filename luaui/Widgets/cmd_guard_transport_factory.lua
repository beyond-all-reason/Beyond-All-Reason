...
local function getValidRallyCommandDestination(unitID)
	local cmdID, options, tag, targetX, targetY, targetZ = Engine.Shared.GetUnitCurrentCommand(unitID, 2)
...
local function isWaiting(unitID)
	local cmdID = Engine.Shared.GetUnitCurrentCommand(unitID, 1)
	return cmdID and cmdID == CMD.WAIT
end

local function tryDeactivateWait(unitID)
	if isWaiting(unitID) then
		Engine.Shared.GiveOrderToUnit(unitID, CMD.WAIT, {}, CMD.OPT_ALT)
	end
end
...
local function IsUnitAlive(unitID)
	return Engine.Shared.ValidUnitID(unitID) and not Engine.Shared.GetUnitIsDead(unitID)
end
...
local function activateTransportGuard(transportID, factoryID)
	...
	local carriedUnits = Engine.Shared.GetUnitIsTransporting(transportID)
	if carriedUnits and #carriedUnits > 0 then
		local x, _, z = Engine.Shared.GetUnitPosition(transportID)
		transportState[transportID] = transport_states.picking_up
		activeTransportToUnit[transportID] = carriedUnits[1]
		unitToDestination[carriedUnits[1]] = { x, Engine.Shared.GetGroundHeight(x, z), z }
	else
...
function widget:Initialize()
	if Engine.Unsynced.GetSpectatingState() or Engine.Unsynced.IsReplay() then
	...
	for _, unitID in ipairs(Engine.Shared.GetTeamUnits(Engine.Unsynced.GetLocalTeamID())) do
		local cmdID, _, _, targetUnitID = Engine.Shared.GetUnitCurrentCommand(unitID, 1)
...
local function isTransportingUnit(transportID, unitID)
	local transported = Engine.Shared.GetUnitIsTransporting(transportID) or {}
...
local function handleTransport(transportID, target)
	...
	if not IsUnitAlive(target) then
		...
		Engine.Shared.GiveOrderToUnit(transportID, CMD.GUARD, transportToFactory[transportID], CMD.OPT_SHIFT) -- go back to base
		return
	else
		-- Order the built unit to stop if it's out of the factory
		if transportState[transportID] == transport_states.picking_up then
			local factoryLocation = { Engine.Shared.GetUnitPosition(transportToFactory[transportID]) }
			local unitLocation = { Engine.Shared.GetUnitPosition(target) }
			...
			if isTransportingUnit(transportID, target) then
				...
				Engine.Shared.GiveOrderToUnit(transportID, CMD.UNLOAD_UNIT, unitToDestination[target], CMD.OPT_RIGHT)
				return
			end
		...
		if transportState[transportID] == transport_states.approaching then
			if isWaiting(target) then
				transportState[transportID] = transport_states.picking_up
				Engine.Shared.GiveOrderToUnit(transportID, CMD.LOAD_UNITS, target, CMD.OPT_RIGHT) --Load Unit
			end
...
local function canTransport(transportID, unitID)
	local udef = Engine.Shared.GetUnitDefID(unitID)
	local tdef = Engine.Shared.GetUnitDefID(transportID)
	...
	local trans = Engine.Shared.GetUnitIsTransporting(transportID) -- capacity check
	...
	for _, a in ipairs(trans) do
		local aDefID = Engine.Shared.GetUnitDefID(a)
		if aDefID then
...
function widget:UnitFromFactory(unitID, unitDefID, unitTeam, factID, factDefID, userOrders)
	...
	if Engine.Shared.AreTeamsAllied(unitTeam, Engine.Unsynced.GetLocalTeamID()) then
		if isTransport(createdUnitID) then
			-- Handle case where transport is rallied to another lab
			local cmdID, _, _, targetUnitID = Engine.Shared.GetUnitCurrentCommand(createdUnitID, 1)
		...
		elseif factoryToGuardingTransports[factID] and next(factoryToGuardingTransports[factID]) then
			...
			local unitDefID_created = Engine.Shared.GetUnitDefID(createdUnitID)
			...
			for transportID, _ in pairs(factoryToGuardingTransports[factID]) do
				if transportState[transportID] == transport_states.idle and canTransport(transportID, createdUnitID) then
					local unitLocation = { Engine.Shared.GetUnitPosition(unitID) }
					local transportLocation = { Engine.Shared.GetUnitPosition(transportID) }

					local tDefID = Engine.Shared.GetUnitDefID(transportID)
			...
			if bestTransportID > -1 then
				...
				local unitWaitDestination = { Engine.Shared.GetUnitPosition(createdUnitID) }
				Engine.Shared.GiveOrderToUnit(bestTransportID, CMD.MOVE, unitWaitDestination, CMD.OPT_RIGHT)
				Engine.Shared.GiveOrderToUnit(bestTransportID, CMD.GUARD, factID, CMD.OPT_SHIFT)
