...
local allyTeam = SpringUnsynced.GetLocalAllyTeamID()

-- Speedups

local spGiveOrderToUnitArray = SpringShared.GiveOrderToUnitArray
...
function widget:CommandNotify(id, params, options)
	...
	for i = 1, #areaUnits do
		...
		local enemyUnit = not SpringShared.AreTeamsAllied(SpringShared.GetUnitTeam(unitID), SpringUnsynced.GetLocalTeamID())
		if enemyUnit then
	...
	if #newCmds > 0 then
		SpringShared.GiveOrderArrayToUnitArray(selectedUnits, newCmds)
		return true
	end
end
