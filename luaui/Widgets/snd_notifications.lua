...
local useDefaultVoiceFallback = Engine.Unsynced.GetConfigInt("NotificationsSubstitute", 0) == 1 --false    -- when a voiceset has missing file, try to load the default voiceset file instead
local playWelcome = Engine.Unsynced.GetConfigInt("WelcomeMessagePlayed", 0) == 0
...
if not (spGetSpectatingState() or Engine.Unsynced.IsReplay()) then
...
local voiceSet = Engine.Unsynced.GetConfigString("voiceset", defaultVoiceSet)
...
local myRank = select(9, Engine.Shared.GetPlayerInfo(myPlayerID))
...
local function updateCommanders()
	local units = Engine.Shared.GetTeamUnits(myTeamID)
	for i = 1, #units do
...
local function queueNotification(event, forceplay)
	if Engine.Shared.GetGameFrame() > 20 or forceplay then
...
function widget:Initialize()
		...
		if notification[event] then
			if notification[event].voiceFiles and #notification[event].voiceFiles > 0 then
				...
				if math.random() < 0.05 and notification[event].voiceFilesRare[mRare] then
					Engine.Unsynced.PlaySoundFile(notification[event].voiceFilesRare[mRare], globalVolume, "ui")
				elseif notification[event].voiceFiles[m] then
					Engine.Unsynced.PlaySoundFile(notification[event].voiceFiles[m], globalVolume, "ui")
				else
					spEcho('notification "' .. event .. '" missing sound file: #' .. m)
				end
			end
			if notification[event].soundEffect then
				Engine.Unsynced.PlaySoundFile(soundEffectsFolder .. notification[event].soundEffect .. ".wav", globalVolume, "ui")
			end
	...
	if not WG.NotificationSoundDefsLoaded then -- To prevent reloads and warning spam when changing/refreshing announcers
		Engine.Unsynced.LoadSoundDef("gamedata/soundsVoice.lua")
		WG.NotificationSoundDefsLoaded = true
		Engine.Shared.Echo("Notification Sound Items Loaded")
	end
...
function widget:GameFrame(gf)
	...
	if gameframe % 30 == 15 then
		...
		for unitID, frame in pairs(idleBuilder) do
			if Engine.Shared.GetUnitTeam(unitID) == myTeamID then
		...
		-- max units check
		local maxUnits, currentUnits = Engine.Shared.GetTeamMaxUnits(myTeamID)
...
function widget:UnitFinished(unitID, unitDefID, unitTeam)
	...
	if #Engine.Shared.GetTeamList(myAllyTeamID) > 1 then
		if isT2mobile[unitDefID] then
...
function widget:UnitEnteredLos(unitID, unitTeam)
	...
	if isMine[udefID] then
		...
		if #Engine.Shared.GetUnitsInCylinder(x, z, 1700, myTeamID) > 0 then
...
function widget:UnitDamaged(unitID, unitDefID, unitTeam, damage, paralyzer)
	...
	if unitTeam == myTeamID then
		...
		if commanders[unitID] then
			...
			local camX, camY, camZ = Engine.Unsynced.GetCameraPosition()
	...
	if #soundQueue > 0 then
		...
		if not muteWhenIdle or not isIdle or notification[event].tutorial then
			if spoken and #notification[event].voiceFiles > 0 then
				if Engine.Shared.GetGameFrame() < 30 then
...
function widget:Update(dt)
	...
	if passedTime > 0.2 then
		...
		if playWelcome then
			Engine.Unsynced.SetConfigInt("WelcomeMessagePlayed", 1)
