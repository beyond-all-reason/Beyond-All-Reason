...
local function processNotificationDefs()
	...
	for notifID, notifDef in pairs(notificationTable) do
		...
		if notifDef.notifText then
			...
			notifTexts[currentEntry] = "tips.notifications."
				.. string.sub(notifID, 1, 1):lower()
				.. string.sub(notifID, 2)
		end
...
local e_currentLevel, e_storage, e_pull, e_income, e_expense, e_share, e_sent, e_received =
	spGetTeamResources(myTeamID, "energy")
local m_currentLevel, m_storage, m_pull, m_income, m_expense, m_share, m_sent, m_received =
...
if UnitDefNames.armcom_scav then -- quick check if scav units exist
	...
	for i = 1, #unitIsReadyTab do
		if UnitDefNames[UnitDefs[unitIsReadyTab[1][1]].name .. "_scav"].id then
			unitIsReadyScavAppend[#unitIsReadyScavAppend + 1] =
				{ UnitDefNames[UnitDefs[unitIsReadyTab[i][1]].name .. "_scav"].id, unitIsReadyTab[i][2] }
		end
	end
...
for udefID, def in ipairs(UnitDefs) do
	if
		not string.find(def.name, "critter")
		and not string.find(def.name, "raptor")
		and (not def.modCategories or not def.modCategories.object)
	then
		...
		if def.customParams.techlevel then
			if
				...
				and not (def.customParams.iscommander or def.customParams.isscavcommander)
			then
				isT2[udefID] = true
			end
			if
				def.customParams.techlevel == "2"
				and not (def.customParams.iscommander or def.customParams.isscavcommander or def.isBuilding)
			then
...
local function gadgetNotificationEvent(msg)
	...
	if not isSpec or (isSpec and lockPlayerID ~= nil) or forceplay then
		...
		if
			forceplay
			or (tonumber(player) and (tonumber(player) == Spring.GetLocalPlayerID()))
			or (isSpec and tonumber(player) == lockPlayerID)
		then
	...
	WG.notifications.getNotificationList = function()
		...
		for i, event in pairs(notificationOrder) do
			soundInfo[#soundInfo + 1] =
				{ event, notificationList[event], notification[event].textID, #notification[event].voiceFiles }
		end
	...
	WG.notifications.playNotification = function(event)
		if notification[event] then
			if notification[event].voiceFiles and #notification[event].voiceFiles > 0 then
				...
				local mRare = #notification[event].voiceFilesRare > 1
						and mathRandom(1, #notification[event].voiceFilesRare)
					or 1
			...
			if notification[event].soundEffect then
				Spring.PlaySoundFile(
					soundEffectsFolder .. notification[event].soundEffect .. ".wav",
					globalVolume,
					"ui"
				)
			end
...
function widget:GameFrame(gf)
	...
	if gameframe % 30 == 15 then
		e_currentLevel, e_storage, e_pull, e_income, e_expense, e_share, e_sent, e_received =
			spGetTeamResources(myTeamID, "energy")
		m_currentLevel, m_storage, m_pull, m_income, m_expense, m_share, m_sent, m_received =
			spGetTeamResources(myTeamID, "metal")
		...
		if
			m_currentLevel
			and (m_currentLevel / m_storage) < 0.025
			and (m_income * 2 < m_pull or m_income + 100 < m_pull)
			and m_currentLevel < 1000
		then
