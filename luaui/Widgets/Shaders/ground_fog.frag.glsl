...
vec4 raymarch(vec3 startpoint, vec3 endpoint, float steps, vec4 sphereposrad, vec2 screenuv, float noisethreshold, inout vec4 dbgdata){
	...
	vec3 noiseoffset;
	noiseoffset.y = -0.3 * currenttime * windZ; 
	noiseoffset.xz = -0.0001 * currenttime * (windInfo.xz*windInfo.w) * windX;
...
void main(void)
{
	...
	if (mapWorldPos.y < 0){
		...
		float underwaterfraction = (- mapWorldPos.y) / abs(mapToCam.y);
		mapWorldPos.xyz += mapToCam * underwaterfraction;
	}
	vec3 fragWorldPos = v_worldPos.xyz;
	if (fragWorldPos.y < 0){
		vec3 mapToCam = camPos - fragWorldPos.xyz;
		float underwaterfraction = (- fragWorldPos.y) / abs(mapToCam.y);
		fragWorldPos.xyz += mapToCam * underwaterfraction;
	}
	
	
	fragColor.rgb = fract(mapWorldPos.xyz * 0.001);
	fragColor.rgb = fract(fragWorldPos * 0.001);
	fragColor.a = 1.0;
	
	fragColor.rgb = vec3(1.0);
	
	float distthroughfog = min(length(mapWorldPos.xyz - fragWorldPos),512);
	fragColor.a = clamp(length(mapWorldPos.xyz - fragWorldPos)/500 , 0.0 ,1.0);
	fragColor.a = clamp(length(mapWorldPos.xyz - fragWorldPos)/500 , 0.0 ,1.0);
	float fogamount = smoothstep(0, 1, 1.0 - exp(-0.005 * distthroughfog));
	...
	vec4 noisesample = texture(noise64cube, fragWorldPos.xyz * noiseScale  + noiseOffset);
	fragColor.rgb = vec3(1.0, 1.0, 1.0);
	fragColor.a = noisesample.r  *fogamount;
	
	int steps = 32;
	vec3 rayStart = fragWorldPos.xyz;
	vec3 planetoworld =  mapWorldPos.xyz - fragWorldPos.xyz;
	vec3 rayStep = planetoworld / steps;
	float collectedNoise = 0.0;
	float collectedShadow = 0.0;
	vec4 simpleDither = texture(simpledither, gl_FragCoord.xy/ 256 + (timeInfo.x + timeInfo.w)*0.123456789);
	
	for (int i = 0; i < steps; i++){
		vec3 marchPos = rayStart + rayStep* (float(i) + simpleDither.r);// * sign((i%2)-0.5);
		
		float localShadow= shadowAtWorldPos(marchPos);
		float rightShadow = localShadow + dFdx(localShadow);
		float upShadow = localShadow + dFdy(localShadow);
		collectedShadow += dot(vec3(0.33), abs(vec3(localShadow, rightShadow,upShadow))); 
		
		
		vec4 localNoise =  texture(noise64cube, marchPos.xyz * noiseScale  + noiseOffset);
	...
	collectedNoise = collectedNoise/steps;// * (areaNoiseWorld.r + areaNoiseFrag.r);
	//collectedNoise = 1.0;
	fragColor.a = collectedNoise  *fogamount;
	//fragColor.a = simpleDither.g;
	//fragColor.a = 1.0;
	/*
