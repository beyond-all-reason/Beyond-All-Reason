	...
	if not _penDiagPrinted then
		...
		Echo(
			string.format(
				...
				pressureMapped,
				sensitivity,
				tostring(tbState.penPressureModulateSize),
				tostring(tbState.penPressureModulateIntensity)
			)
		)
	end
...
local function allocateSquare(s)
	...
	if not ok then
		Echo(
			"[Diffuse Painter] GetMapSquareTexture failed for "
				.. s.sx
				.. ","
				.. s.sy
				.. " (seeding disabled for this square)"
		)
					...
					if not layer._pbrLogged then
						...
						Echo(
							string.format(
								...
								tostring(useTex),
								tostring(useNormal),
								tostring(layer.normalPath or "-"),
								tostring(useRough),
								tostring(layer.roughPath or "-")
							)
						)
					end
...
local function projectLoadTick()
	...
	while req.i <= #req.squares and done < PROJECT_LOAD_SQUARES_PER_TICK do
		...
		if s and gl.Texture(texName) then
			...
			s.compositeTex = s.seedTex
					and glCreateTexture(TILE_PX, TILE_PX, {
						border = false,
						min_filter = GL.LINEAR,
						mag_filter = GL.LINEAR,
						wrap_s = GL.CLAMP_TO_EDGE,
