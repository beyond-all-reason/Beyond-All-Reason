...
function widget:ViewResize()
	...
	if buildmenuBottomPosition then
		...
		if WG.ordermenu then
			local oposX, oposY = WG.ordermenu.getPosition()
			if oposY > 0.5 then
				posY = 0.16
			end
		end
...
function widget:Update(dt)
	...
	if messageLines[currentTypewriterLine] ~= nil then
		...
		if messageLines[currentTypewriterLine].charstyped <= messageLines[currentTypewriterLine].textlen then
			...
			messageLines[currentTypewriterLine].charstyped =
				math.ceil(messageLines[currentTypewriterLine].timepassed / charDelay)
...
local function processLine(i)
	if
		messageLines[i].displaylist == nil
		or messageLines[i].charstyped ~= messageLines[i].charsindisplaylist
		or messageLines[i].pos ~= (currentLine + 1) - i
	then
