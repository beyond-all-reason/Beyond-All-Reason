...
function widget:MouseMove(mx, my, dx, dy)
	...
	if #fNodes == 2 then
		...
		if pathCandidate then
			local _, ctrl, meta, shift = GetModKeys()
		...
		if draggingPath then
			local distx, distz = pos[1] - lastPathPos[1], pos[3] - lastPathPos[3]
			if (distx * distx + distz * distz) > minPathSpacingSq then
				local _, ctrl, meta = GetModKeys()
			...
			if #mUnits > 0 then
				...
				if meta then
					...
					for i = 1, #orders do
						...
						GiveNotifyingOrderToUnit(unitArr, orderArr, orderPair[1], CMD_INSERT,
							{ 0, usingCmd, cmdOpts.coded, orderPos[1], orderPos[2], orderPos[3] }, altOpts)
					...
					for i = 1, #orders do
						...
						Spring.GiveOrderToUnit(orderPair[1], usingCmd, orderPair[2], cmdOpts.coded + 8)
						--if (i == #orders and #unitArr > 0) or #unitArr >= 100 then
						--	Spring.Echo(os.clock(), 'order', i)
						--	Spring.Debug.TableEcho(orderArr)
						--	Spring.GiveOrderArrayToUnitArray(unitArr, orderArr, true)
						--	unitArr = {}
						--	orderArr = {}
						--end
...
function widget:ViewResize(viewSizeX, viewSizeY)
	Xs, Ys = viewSizeX * 0.5, viewSizeY * 0.5
end
...
local function doPrime(array, colcover, rowcover, n, starscol, r, c, rmax, primescol)
	primescol[r] = c
...
local function stepPrimeZeroes(array, colcover, rowcover, n, starscol, primescol)
	-- Infinite loop
	while true do
...
local function stepFiveStar(colcover, rowcover, row, col, n, starscol, primescol)
