...
local function DrawText(t1, t2)
	...
	maxWidth = max(
		maxWidth,
		(font:GetTextWidth(t1) * fontSize) + (bgpadding * 10),
		(font:GetTextWidth(t2) * fontSize) + (fontSize * 6.5) + (bgpadding * 10)
	)
end
...
local function renderPanelToTexture(uDefID, uID)
	...
	gl.R2tHelper.RenderInRect(panelTex, panelLeft, panelBottom, panelRight, panelTop, function()
		...
		if uID then
			...
			glColor(1, 1, 1, 1)
			UiUnit(
				tLeft + bgpadding + iconPadding,
				tBottom + iconPadding,
				tLeft + (tTop - tBottom) - iconPadding,
				tTop - bgpadding - iconPadding,
				nil,
				1,
				1,
				1,
...
local function GetTeamName(teamID)
	...
	leaderName = ((WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(teamLeader))
		or leaderName
...
local function computeContent(uDefID, uID, shiftBool)
	...
	if isBuilding then
		...
		if mEta >= 0 then
			DrawText(
				texts.metal .. ":",
				format("%d / %d (" .. yellow .. "%d" .. white .. ", %ds)", mTotal * buildProg, mTotal, mRem, mEta)
			)
		else
			DrawText(
				texts.metal .. ":",
				format("%d / %d (" .. yellow .. "%d" .. white .. ")", mTotal * buildProg, mTotal, mRem)
			)
		end
		...
		if eEta >= 0 then
			DrawText(
				texts.energy .. ":",
				format("%d / %d (" .. yellow .. "%d" .. white .. ", %ds)", eTotal * buildProg, eTotal, eRem, eEta)
			)
		else
			DrawText(
				texts.energy .. ":",
				format("%d / %d (" .. yellow .. "%d" .. white .. ")", eTotal * buildProg, eTotal, eRem)
			)
		end
	...
	DrawText(
		texts.cost .. ":",
		format(
			metalColor .. "%d" .. white .. " / " .. energyColor .. "%d" .. white .. " / " .. buildColor .. "%d",
			uDef.metalCost,
			uDef.energyCost,
			uDef.buildTime
		)
	...
	if not (uDef.isBuilding or uDef.isFactory) then
		if not uID or not Spring.GetUnitMoveTypeData(uID) then
			DrawText(
				texts.move .. ":",
				format(
					"%.1f / %.1f / %.0f (" .. texts.speedaccelturn .. ")",
					uDef.speed,
					900 * uDef.maxAcc,
					simSpeed * uDef.turnRate * (180 / 32767)
				)
			)
		else
	...
	DrawText(
		texts.los .. ":",
		losRadius .. (airLosRadius > losRadius and format(" (" .. texts.airlos .. ": %d)", airLosRadius) or "")
	)
	...
	if maxHP then
		...
		if armoredMultiple and armoredMultiple ~= 1 then
			local message =
				format("%s: %d (+%d%%)", texts.maxhp, maxHP / armoredMultiple, 100 * (1 / armoredMultiple - 1))
			if uDef.customParams.reactive_armor_health then
				message = message
					.. (", %d to break, %d%s to restore"):format(
	...
	for i = 1, #wepsCompact do
		...
		if range > 0 and uWep.customParams.bogus ~= "1" then
			...
			if string.find(uWep.name, "disintegrator") then
				infoText = format("%.2f", (useExp and reload or uWep.reload))
					.. "s "
					.. texts.reload
					.. ", "
					.. format("%d " .. texts.range, useExp and range or uWep.range)
			elseif uWep.interceptor ~= 0 and uWep.coverageRange > 0 then
				...
				infoText = format(
					"%.2f%s %s (%d%s %s), %d %s",
					useExp and reload or uWep.reload,
					texts.s,
					texts.reload,
					stockpile,
					texts.s,
					texts.stockpile:lower(),
					coverage,
					texts.coverage
				)
			else
				if wpnName == texts.deathexplosion or wpnName == texts.selfdestruct then
					...
					infoText = format("%.2f", (useExp and reload or uWep.reload))
						...
						.. ", "
						.. format(
							"%d " .. texts.range .. ", %d " .. texts.aoe .. ", %d%% " .. texts.edge,
							useExp and range or uWep.range,
							uWep.damageAreaOfEffect,
							100 * uWep.edgeEffectiveness
						)
				end
			...
			if groupLast ~= groupNext and not (groupLast == 0 and groupNext == 1) then
				...
				if totaldps > 0 then
					DrawText(
						...
						texts.dps
							.. " = "
							.. (format(yellow .. "%d", totaldps))
							.. white
							.. "; "
							.. texts.burst
							.. " = "
							.. (format(yellow .. "%d", totalbDamages))
							.. white
							.. "."
					)
				end
	...
	if
		damageStats
		and damageStats[gameName]
		and damageStats[gameName].team
		and damageStats[gameName].team[uDef.name]
		and damageStats[gameName].team[uDef.name].cost
		and damageStats[gameName].team[uDef.name].killed_cost
	then
		effectivenessRate = "   "
			.. damageStats[gameName].team[uDef.name].killed_cost / damageStats[gameName].team[uDef.name].cost
	end
	cachedTitleText = "\255\190\255\190" .. UnitDefs[uDefID].translatedHumanName
	if uID then
		cachedTitleText = cachedTitleText
			.. "   "
			.. grey
			.. uDef.name
			.. "   #"
...
local function drawStats(uDefID, uID)
	...
	gl.R2tHelper.BlendTexRect(
		panelTex,
		screenX + panelOffsets[1],
		screenY + panelOffsets[2],
		screenX + panelOffsets[3],
		screenY + panelOffsets[4],
		true
	)
...
function widget:DrawScreen()
	...
	local uDefID = (uID and spGetUnitDefID(uID))
		or (useHoverID and WG.buildmenu and WG.buildmenu.hoverID)
