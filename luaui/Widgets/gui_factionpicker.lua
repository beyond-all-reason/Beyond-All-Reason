...
do
	local validStartUnits = string.split(
		Spring.GetTeamRulesParam(myTeamID, "validStartUnits") or Spring.GetGameRulesParam("validStartUnits"),
		"|"
	)
...
local function drawFactionpicker()
	...
	for i, faction in pairs(factions) do
		...
		UiUnit(
			factionRect[i][1] + bgpadding,
			factionRect[i][2] + bgpadding,
			factionRect[i][3],
			factionRect[i][4],
			nil,
			1,
...
function widget:DrawScreen()
	...
	if not factionpickerTex then
		factionpickerTex = gl.CreateTexture(
			mathFloor(width * vsx) * (vsy < 1400 and 2 or 1),
			mathFloor(height * vsy) * (vsy < 1400 and 2 or 1),
			{
				target = GL.TEXTURE_2D,
				format = GL.ALPHA,
				fbo = true,
		...
		for i, faction in pairs(factions) do
			if math_isInRect(x, y, factionRect[i][1], factionRect[i][2], factionRect[i][3], factionRect[i][4]) then
				...
				RectRound(
					...
					1,
					1,
					{ 0.3, 0.3, 0.3, (b and 0.5 or 0.25) },
					{ 1, 1, 1, (b and 0.3 or 0.15) }
				)
