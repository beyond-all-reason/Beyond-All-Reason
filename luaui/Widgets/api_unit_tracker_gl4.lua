...
local function Scream(reason, unitID) -- This will pause the game and play some sound to alert anyone in debug mode of issue
	...
	if lastknownunitpos[unitID] then
		Spring.MarkerAddPoint(
			lastknownunitpos[unitID][1],
			lastknownunitpos[unitID][2],
			lastknownunitpos[unitID][3],
			lastknownunitpos[unitID][4],
			true
		)
	end
		...
		then
			if debuglevel >= (verbose or 0) then
				...
				spEcho(
					...
					"GetUnitIsDead",
					spGetUnitIsDead(unitID),
					"Ignore",
					unitDefIgnore[unitDefID],
					"LOSstate",
					spGetUnitLosState(unitID, myAllyTeamID, true)
				)
			end
			return false
		end
...
function widget:UnitCreated(unitID, unitDefID, unitTeam, builderID, reason, silent) -- this was visible at the time
	...
	if health == maxhealth and buildProgress == 0 then
		if debuglevel >= 3 then
			spEcho(
				"Skipping visibleUnitsAdd for CreateUnit'ed unit",
...
function widget:GameFrame()
	...
	if debuglevel >= 1 then -- here we will scan all units and ensure that they match what we expect
		...
		if cntvisibleunits ~= numVisibleUnits then
			Scream(
				"cntvisibleunits ~= numVisibleUnits "
					.. tostring(cntvisibleunits)
					.. " vs "
					.. tostring(numVisibleUnits)
			)
		end
		...
		if cntalliedunits ~= numAlliedUnits then
			Scream(
				"cntalliedunits ~= numAlliedUnits " .. tostring(cntalliedunits) .. " vs " .. tostring(numAlliedUnits)
			)
		end
...
function widget:AddConsoleLine(lines, priority)
	...
	if username and frameNumber and gotChecksum and correctChecksum then
		...
		if myPlayerName == username then
			...
			local complex_match_event =
				string.format("complex-match-event:%s", string.base64Encode(Json.encode(jsondict)))
