...
local function CreatePanelDisplayList()
	...
	if currentTime > gameInfo.scavGracePeriod then
		if gameInfo.scavBossAnger < 100 then
			...
			if Spring.GetGameRulesParam("ScavBossAngerGain_Base") then
				font:Print(textColor .. BAR.I18N("ui.scavs.bossAngerBase", { value = math.round(Spring.GetGameRulesParam("ScavBossAngerGain_Base"), 3) }), panelMarginX + 5, PanelRow(3), panelFontSize, "")
				font:Print(textColor .. BAR.I18N("ui.scavs.bossAngerAggression", { value = math.round(Spring.GetGameRulesParam("ScavBossAngerGain_Aggression"), 3) }), panelMarginX + 5, PanelRow(4), panelFontSize, "")
			...
			font:Print(textColor .. BAR.I18N("ui.scavs.bossAngerWithTech", { anger = mathFloor(0.5 + gameInfo.scavBossAnger), techAnger = gameInfo.scavTechAnger }), panelMarginX, PanelRow(1), panelFontSize, "")
			...
			if totalSeconds < 1800 or revealedBossEta then
				...
				font:Print(textColor .. BAR.I18N("ui.scavs.bossETA", { count = nBosses, time = time }), panelMarginX + 5, PanelRow(2), panelFontSize, "")
			end
			...
			font:Print(textColor .. BAR.I18N("ui.scavs.bossHealth", { count = nBosses, health = gameInfo.scavBossHealth }), panelMarginX, PanelRow(1), panelFontSize, "")

			if Spring.GetGameRulesParam("scavBossStaggerActive") == false then
				font:Print(textColor .. BAR.I18N("ui.scavs.bossStaggerPercentage", { count = nBosses, value = 100 - Spring.GetGameRulesParam("scavBossStaggerPercentage") }), panelMarginX, PanelRow(2), panelFontSize, "")
			else
				font:Print("\255\255\255\0" .. BAR.I18N("ui.scavs.bossStaggerActive", { count = nBosses }), panelMarginX, PanelRow(2), panelFontSize, "")
				font:Print("\255\255\255\0" .. BAR.I18N("ui.scavs.bossStaggerPercentage", { count = nBosses, value = 100 - Spring.GetGameRulesParam("scavBossStaggerPercentage") }), panelMarginX, PanelRow(3), panelFontSize, "")
			end
			if nBosses > 1 then
				font:Print(textColor .. BAR.I18N("ui.scavs.bossesKilled", { nKilled = gameInfo.scavBossesKilled, nTotal = nBosses }), panelMarginX, PanelRow(4), panelFontSize, "")
			end
			for i = 1, #currentlyResistantToNames do
				if i == 1 then
					font:Print(textColor .. BAR.I18N("ui.scavs.bossResistantToList", { count = nBosses }), panelMarginX, PanelRow(12), panelFontSize, "")
				end
				font:Print(textColor .. currentlyResistantToNames[i], panelMarginX + 20, PanelRow(12 + i), panelFontSize, "")
			end
		end
	else
		font:Print(textColor .. BAR.I18N("ui.scavs.gracePeriod", { time = string.formatTime(mathCeil(((currentTime - gameInfo.scavGracePeriod) * -1) - 0.5)) }), panelMarginX, PanelRow(1), panelFontSize, "")
	end
	...
	if Spring.GetModOptions().scav_endless then
		endless = " (" .. BAR.I18N("ui.scavs.difficulty.endless") .. ")"
	end
	local difficultyCaption = BAR.I18N("ui.scavs.difficulty." .. difficultyOption)
	font:Print(textColor .. BAR.I18N("ui.scavs.mode", { mode = difficultyCaption }) .. endless, panelMarginX, h - 195, panelFontSize, "")
	font:End()
...
local function getMarqueeMessage(scavEventArgs)
	...
	if scavEventArgs.type == "firstWave" then
		messages[1] = textColor .. BAR.I18N("ui.scavs.firstWave1")
		messages[2] = textColor .. BAR.I18N("ui.scavs.firstWave2")
	elseif scavEventArgs.type == "boss" then
		messages[1] = textColor .. BAR.I18N("ui.scavs.bossIsAngry1", { count = nBosses })
		messages[2] = textColor .. BAR.I18N("ui.scavs.bossIsAngry2")
	elseif scavEventArgs.type == "airWave" then
		messages[1] = textColor .. BAR.I18N("ui.scavs.wave1", { waveNumber = scavEventArgs.waveCount })
		messages[2] = textColor .. BAR.I18N("ui.scavs.airWave1")
		messages[3] = textColor .. BAR.I18N("ui.scavs.airWave2", { unitCount = scavEventArgs.number })
	elseif scavEventArgs.type == "wave" then
		messages[1] = textColor .. BAR.I18N("ui.scavs.wave1", { waveNumber = scavEventArgs.waveCount })
		messages[2] = textColor .. BAR.I18N("ui.scavs.wave2", { unitCount = scavEventArgs.number })
	end
...
local function getResistancesMessage()
	local messages = {}
	messages[1] = textColor .. (BAR.I18N("ui.scavs.resistanceUnits", { count = nBosses }))
