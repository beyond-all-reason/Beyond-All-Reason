...
local chunk = loadfile("LuaUI/config/AttackRangeConfig2.lua")
if chunk then
	local tmp = {}
	setfenv(chunk, tmp)
...
local function AddSelectedUnit(unitID, mouseover, newRange)
	...
	local x, y, z, mpx, mpy, mpz = spGetUnitPosition(unitID, true, true)
	...
	for j, weaponType in pairs(weaponTypes) do
		...
		if drawIt and ringParams[1] > 0 then
			...
			local wpx, wpy = spGetUnitWeaponVectors(unitID, weaponID)
...
function widget:Update(dt)
	...
	if gameFrame % 3 == 2 then
		local cmdIndex, cmdID = GetActiveCommand()
		if shift_only then
			if shifted then
