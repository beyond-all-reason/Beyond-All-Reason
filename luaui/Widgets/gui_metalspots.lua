...
if Engine.Shared.GetModOptions().unit_restrictions_noextractorDefs then
	return
end
...
local incomeMultiplier = select(7, Engine.Shared.GetTeamInfo(spGetMyTeamID(), false))
...
local function InitializeAtlas(mSpots)
	...
	for i, teamID in ipairs(Engine.Shared.GetTeamList()) do
		local incomeMultiplier = select(7, Engine.Shared.GetTeamInfo(teamID, false))
	...
	for i = 1, #mSpots do
		...
		if tonumber(value) > 0.001 and tonumber(value) < maxValue then
			...
			if scale < maxScale then
				...
				if uvcoords then
					local gh = Engine.Shared.GetGroundHeight(spot.x, spot.z)
					pushElementInstance(
						spotInstanceVBO, -- vbo
	...
	for i = 1, #mySpots do
		...
		if spot.scale < maxScale and valueNumber > 0.001 and valueNumber < maxValue then
			...
			if uvcoords then
				pushElementInstance(
					spotInstanceVBO, -- vbo
					{ spot.x, spot.y, spot.z, spot.scale, (occupied and 0) or 1, -1000, uvcoords.w, uvcoords.h, uvcoords.x, uvcoords.X, uvcoords.y, uvcoords.Y }, -- instanceData
...
function widget:PlayerChanged(playerID)
	...
	incomeMultiplier = select(7, Engine.Shared.GetTeamInfo(spGetMyTeamID(), false))
...
local function getWaterLevel()
	...
	local level = Engine.Shared.GetGameRulesParam("lavaLevel")
