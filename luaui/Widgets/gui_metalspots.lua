...
local function initGL4()
	...
	local spotVBO,numVertices = makeMetalSpotVBO()
...
local function checkMetalspots()
	...
	for i=1, #spots do
		...
		if occupied ~= prevOccupied then
			...
			local oldinstance = getElementInstanceData(spotInstanceVBO, mspotkey)
			oldinstance[5] = (occupied and 0) or 1
			oldinstance[6] = Spring.GetGameFrame()
			pushElementInstance(spotInstanceVBO, oldinstance, mspotkey, true)
		end
	end
...
function widget:Initialize()
	...
	for i = 1, #mSpots do
		...
		if tonumber(value) > 0.001 then
			...
			pushElementInstance(spotInstanceVBO, {spot.x,  spGetGroundHeight(spot.x,spot.z), spot.z, scale, (occupied and 0) or 1, 0,0,0}, metalSpotKey(spot.x, spot.z))
