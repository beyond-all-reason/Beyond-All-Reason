...
function widget:GameFrame(n)
	...
	if select(2, Engine.Unsynced.GetActiveCommand()) ~= CMD_LOAD_UNITS then
	...
	for i = 1, #selectedUnits do
		...
		if validTrans[transDefID] then
			local transportedUnits = Engine.Shared.GetUnitIsTransporting(transID)
	...
	local visibleUnits = Engine.Unsynced.GetVisibleUnits()
	...
	for _, unitID in ipairs(visibleUnits) do
		...
		if not cantBeTransported[passengerDefID] and not Engine.Unsynced.IsUnitIcon(unitID) then
			...
			if canBePickedUp then
				local x, y, z = Engine.Shared.GetUnitBasePosition(unitID)
				if x then
