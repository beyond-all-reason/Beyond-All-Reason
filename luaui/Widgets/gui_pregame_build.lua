...
local startDefID = Engine.Shared.GetTeamRulesParam(myTeamID, "startUnit")
...
local function buildFacingHandler(command, line, args)
	...
	local facing = Engine.Unsynced.GetBuildFacing()
	local action = args and args[1]
	if action == "inc" then
		facing = (facing + 1) % 4
		Engine.Unsynced.SetBuildFacing(facing)
		return true
	...
	elseif action and facingMap[action] then
		Engine.Unsynced.SetBuildFacing(facingMap[action])
		return true
	end
end
...
local function setPreGamestartDefID(uDefID)
	...
	if isMex then
		if Engine.Unsynced.GetMapDrawMode() ~= "metal" then
			Engine.Unsynced.SendCommands("ShowMetalMap")
		end
	elseif Engine.Unsynced.GetMapDrawMode() == "metal" then
		Engine.Unsynced.SendCommands("ShowStandard")
	end
...
local function convertBuildQueueFaction(previousFactionSide, currentFactionSide)
	Engine.Shared.Log("gui_pregame_build", LOG.DEBUG, string.format("Calling SubLogic.processBuildQueueSubstitution (in-place) from %s to %s for %d queue items.", previousFactionSide, currentFactionSide, #buildQueue))
...
local function handleSelectedBuildingConversion(currentSelDefID, prevFactionSide, currentFactionSide, currentSelBuildData)
	if not currentSelDefID then
		Engine.Shared.Log("gui_pregame_build", LOG.WARNING, "handleSelectedBuildingConversion: Called with nil currentSelDefID.")
	...
	if newSelDefID ~= currentSelDefID then
		...
		if prevFactionSide ~= currentFactionSide then
			...
			Engine.Shared.Log("gui_pregame_build", LOG.INFO, string.format("Selected item '%s' remains unchanged for %s faction (or was already target faction).", originalUnitName, currentFactionSide))
		else
			Engine.Shared.Log("gui_pregame_build", LOG.DEBUG, string.format("selBuildQueueDefID %s remained unchanged (sides were the same: %s).", tostring(currentSelDefID), currentFactionSide))
		end
	end
	return newSelDefID
end
...
function widget:Initialize()
	...
	Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, "Pregame Queue Initializing. Local SubLogic is assumed available.")

	-- Get our starting unit
	if preGamestartPlayer then
		if not startDefID or startDefID ~= Engine.Shared.GetTeamRulesParam(myTeamID, "startUnit") then
	...
	WG["pregame-build"].setPreGamestartDefID = function(value)
		...
		if startDefID and UnitDefs[startDefID] and UnitDefs[startDefID].buildOptions then
			...
			Engine.Shared.Log(widget:GetInfo().name, LOG.WARNING, "setPreGamestartDefID: startDefID is nil or invalid, cannot determine build options.")
		end
...
local function getMouseWorldPosition(unitDefID, x, y)
	...
	if pos then
		local buildFacing = Engine.Unsynced.GetBuildFacing()
		local posX = pos.x or pos[1]
		local posY = pos.y or pos[2]
		local posZ = pos.z or pos[3]
		local bx, by, bz = Engine.Shared.Pos2BuildPos(unitDefID, posX, posY, posZ, buildFacing)
...
function widget:Update(dt)
	...
	local _, _, _, shift = Engine.Unsynced.GetModKeyState()
	...
	if not leftButton then
		if buildModeState.startPosition and #buildModeState.buildPositions > 0 and selBuildQueueDefID then
			...
			for _, buildPos in ipairs(buildModeState.buildPositions) do
				...
				local posX, posY, posZ = Engine.Shared.Pos2BuildPos(selBuildQueueDefID, buildPosX, buildPosY, buildPosZ, buildPosFacing or Engine.Unsynced.GetBuildFacing())
				local buildFacingPos = buildPosFacing or Engine.Unsynced.GetBuildFacing()
				local buildDataPos = { selBuildQueueDefID, posX, posY, posZ, buildFacingPos }

				local hasConflicts = false

				local cx, cy, cz = Engine.Shared.GetTeamStartPosition(myTeamID)
				if cx ~= -100 then
					local cbx, cby, cbz = Engine.Shared.Pos2BuildPos(startDefID, cx, cy, cz)
				...
				if not hasConflicts and Engine.Shared.TestBuildOrder(selBuildQueueDefID, posX, posY, posZ, buildFacingPos) == 0 then
					hasConflicts = true
				end
				if not hasConflicts then
					local isMex = UnitDefs[selBuildQueueDefID] and UnitDefs[selBuildQueueDefID].extractsMetal > 0
...
function widget:MousePress(mx, my, button)
	...
	local _, _, meta, shift = Engine.Unsynced.GetModKeyState()
	...
	local alt, ctrl = Engine.Unsynced.GetModKeyState()
		...
		if buildAroundTarget then
			...
			if #aroundPositions > 0 then
				...
				for _, buildPos in ipairs(aroundPositions) do
					...
					local buildPosZ = buildPos.z
					local buildPosFacing = buildPos.facing
					local posX, posY, posZ = Engine.Shared.Pos2BuildPos(selBuildQueueDefID, buildPosX, buildPosY, buildPosZ, buildPosFacing or buildFacing)
	...
	if button == 1 then
		...
		local posY = pos.y or pos[2]
		local posZ = pos.z or pos[3]
		local bx, by, bz = Engine.Shared.Pos2BuildPos(selBuildQueueDefID, posX, posY, posZ, buildFacing)
		...
		if Engine.Shared.TestBuildOrder(selBuildQueueDefID, bx, by, bz, buildFacing) ~= 0 then
			local hasConflicts = false
	...
	if button == 1 and #buildQueue > 0 and buildQueue[1][1] > 0 then
		...
		local cbx, cby, cbz = Engine.Shared.Pos2BuildPos(startDefID, pos[1], pos[2], pos[3])
...
function widget:DrawWorld()
	...
	if selBuildQueueDefID then
		...
		if pos then
			local buildFacing = Engine.Unsynced.GetBuildFacing()
			local bx, by, bz = Engine.Shared.Pos2BuildPos(selBuildQueueDefID, pos[1], pos[2], pos[3], buildFacing)
			selBuildData = { selBuildQueueDefID, bx, by, bz, buildFacing }
		end
	end

	if startDefID ~= Engine.Shared.GetTeamRulesParam(myTeamID, "startUnit") then
		startDefID = Engine.Shared.GetTeamRulesParam(myTeamID, "startUnit")
	end

	local sx, sy, sz = Engine.Shared.GetTeamStartPosition(myTeamID) -- Returns 0, 0, 0 when none chosen (was -100, -100, -100 previously)
	...
	if prevStartDefID ~= startDefID then
		...
		elseif previousFactionSide and currentFactionSide and previousFactionSide == currentFactionSide then
			Engine.Shared.Log(widget:GetInfo().name, LOG.DEBUG, string.format("Sides determined but are the same (%s), no conversion needed.", currentFactionSide))
		else
			Engine.Shared.Log(widget:GetInfo().name, LOG.WARNING, string.format("Could not determine sides for conversion: prevDefID=%s (name: %s), currentDefID=%s (name: %s). Names might be unhandled by SubLogic.getSideFromUnitName, or SubLogic itself might be incomplete from a non-critical load error.", tostring(prevStartDefID), tostring(prevDefName), tostring(startDefID), tostring(currentDefName)))
		end
	...
	local function convertBuildPosToPreviewData(buildPos, buildFacing)
		local posX, posY, posZ = Engine.Shared.Pos2BuildPos(selBuildQueueDefID, buildPos.x, buildPos.y, buildPos.z, buildPos.facing or buildFacing)
	...
	if showPreview then
		...
		if getBuildQueueSpawnStatus then
			...
			for _, previewBuildData in ipairs(validPreviewPositions) do
				...
				local isValid = Engine.Shared.TestBuildOrder(selBuildQueueDefID, posX, posY, posZ, buildFacingPos) ~= 0
...
function widget:GameFrame(n)
	...
	if startDefID then
		...
		Engine.Unsynced.SendCommands("luarules initialQueueTime " .. buildTime)
	end

	local tasker
	-- Search for our starting unit
	local units = Engine.Shared.GetTeamUnits(spGetMyTeamID())
	for u = 1, #units do
		local uID = units[u]
		if GetUnitCanCompleteQueue(uID) then
			tasker = uID
			if Engine.Shared.GetUnitRulesParam(uID, "startingOwner") == Spring.GetMyPlayerID() then
	...
	if tasker then
		local quickStartOption = Engine.Shared.GetModOptions().quick_start
		...
		if quickStartEnabled and #buildQueue > 0 then
			...
			Engine.Shared.Echo(string.format("=== Build Queue for Commander (unitID: %d) ===", tasker))
			for b = 1, #buildQueue do
				local buildData = buildQueue[b]
				...
				Engine.Shared.Echo(string.format("  [%d] %s at (%.1f, %.1f, %.1f) facing: %d", b, unitDefName, x, y, z, facing))
			end
			Engine.Shared.Echo(string.format("=== Total queue items: %d ===", #buildQueue))
		end

		for b = 1, #buildQueue do
			local buildData = buildQueue[b]
			Engine.Shared.GiveOrderToUnit(tasker, -buildData[1], { buildData[2], buildData[3], buildData[4], buildData[5] }, { "shift" })
		end
		buildQueue = {}
	end
end
...
function widget:GameStart()
	...
	local currentStartDefID_GS = Engine.Shared.GetTeamRulesParam(myTeamID, "startUnit")
	if startDefID ~= currentStartDefID_GS then
		Engine.Shared.Log("gui_pregame_build", LOG.DEBUG, string.format("GameStart: startDefID (%s) differs from current rules param (%s). Updating.", tostring(startDefID), tostring(currentStartDefID_GS)))
	...
	if prevStartDefID ~= startDefID then
		...
		elseif previousFactionSide and currentFactionSide and previousFactionSide == currentFactionSide then
			...
			Engine.Shared.Log("gui_pregame_build", LOG.WARNING, string.format("Could not determine sides for conversion in GameStart: prevDefID=%s, currentDefID=%s", tostring(prevStartDefID), tostring(startDefID)))
		end
		prevStartDefID = startDefID
	end
...
function widget:SetConfigData(data)
	if data.buildQueue and spGetGameFrame() == 0 and data.gameID and data.gameID == (Game.gameID and Game.gameID or Engine.Shared.GetGameRulesParam("GameID")) then
