...
function widget:DrawWorld()
	...
	if prevStartDefID ~= startDefID then
        ...
        elseif previousFactionSide and currentFactionSide and previousFactionSide == currentFactionSide then
            SpringShared.Log(widget:GetInfo().name, LOG.DEBUG, string.format(
                "Sides determined but are the same (%s), no conversion needed.", currentFactionSide))
        else
            SpringShared.Log(widget:GetInfo().name, LOG.WARNING, string.format(
...
function widget:SetConfigData(data)
	if
		...
		and data.gameID
		and data.gameID == (Game.gameID and Game.gameID or SpringShared.GetGameRulesParam("GameID"))
	then
