	...
	function ConfigFile:getLoadCoroutine()
		...
		return coroutine.create(function()
			for _, startPoint in ipairs(self.startPoints) do
				Spring.SendCommands(
					string.format("give %s @%s,%s,%s", startPointUnitName, startPoint.x, 0, startPoint.z)
				)
				coroutine.yield()
			end
...
local function tryLoadWIPConfig(currentMapName)
	local availableWIPConfigs =
		VFS.DirList(WIPConfigsDir, string.format("%s%s*.lua", WIPConfigFilePrefix, currentMapName))
	if #availableWIPConfigs > 0 then
		...
		Spring.Log(
			...
			LOG.INFO,
			string.format(
				"found WIP config file %s previously written by editor for current map %s",
				lastWIPConfig,
				currentMapName
			)
		)
