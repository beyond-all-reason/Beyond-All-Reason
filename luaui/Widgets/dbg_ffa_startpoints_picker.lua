...
function widget:GetInfo()
	return {
		...
		desc = "Companion widget for generating FFA start points configs as expected by the **FFA start setup** gadget (`game_ffa_start_setup.lua`)",
		author = "nbusseneau",
		date = "2023-08-20",
	...
	function ConfigFile:getLoadCoroutine()
		...
		return coroutine.create(function()
			for _, startPoint in ipairs(self.startPoints) do
				Engine.Unsynced.SendCommands(string.format("give %s @%s,%s,%s", startPointUnitName, startPoint.x, 0, startPoint.z))
				coroutine.yield()
			end
...
local function tryLoadWIPConfig(currentMapName)
	...
	if #availableWIPConfigs > 0 then
		...
		Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, string.format("found WIP config file %s previously written by editor for current map %s", lastWIPConfig, currentMapName))
...
local function tryLoadConfigFromBAR(currentMapName)
	...
	for _, configFile in pairs(availableConfigFiles) do
		...
		if string.find(sanitizedCurrentMapName, basename) then
			Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, string.format("found config %s provided by BAR for current map %s", configFile, currentMapName))
...
local function doLoad(config)
	...
	Engine.Unsynced.SelectUnitArray(existingUnits)
	Engine.Unsynced.SendCommands("remove")
end
...
local function giveStartPoint()
	Engine.Unsynced.SendCommands("give " .. startPointUnitName)
end
...
local function addLayout()
	...
	if #selectedUnits >= 3 then
		...
		if not byAllyTeamCount:addLayout(layout) then
			Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, "layout was already registered")
		end
	else
		Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "must select at least 3 start points to add a layout")
	end
end
...
local function removeLayout()
	...
	if #selectedUnits >= 3 then
		...
		if not byAllyTeamCount:removeLayout(layout) then
			Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, "this layout is not registered")
		end
	end
end
...
local function copyToClipboard()
	...
	Engine.Unsynced.SetClipboard(output)
	Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, "copied to clipboard and stored ephemeral config")
end
...
local function resetLastCopy()
	if WG[ephemeralConfig] then
		...
		Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, "no ephemeral config found to reset")
	end
end
...
local function saveWIPConfig()
	...
	if f then
		f:write(output)
		f:close()
		Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, string.format("saved to WIP config file %s", filepath))
	elseif err then
		Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, err)
	end
end
...
local function loadWIPOrBARConfig()
	...
	if config then
		...
		Engine.Shared.Log(widget:GetInfo().name, LOG.INFO, "no config found in WIP configs nor BAR configs for this map")
	end
end
...
local function registerKeybinds(widget)
	for name, action in pairs(actions) do
		...
		Engine.Unsynced.SendCommands(string.format("unbindkeyset %s", action.keybind))
		Engine.Unsynced.SendCommands(string.format("bind %s %s", action.keybind, name))
	end
end
...
local function unregisterKeybinds(widget)
	...
	Engine.Unsynced.SendCommands("keyreload")
end
...
function widget:DrawScreenEffects()
	if IsGUIHidden() or not isUIInitialized then
		return
	end
	drawStartPoints()
	drawByAllyTeamCount()
...
local function isSoloDevMode()
	...
	local gaiaAllyTeamID = select(6, Engine.Shared.GetTeamInfo(Engine.Shared.GetGaiaTeamID(), false))
	...
	local teamList = Engine.Shared.GetTeamList(Spring.GetMyAllyTeamID())
	...
	if not isSolo or not Spring.Utilities.IsDevMode() or Engine.Unsynced.IsReplay() then
		return false
	end
	return true
end
...
function widget:Initialize()
	...
	initialFrame = Engine.Shared.GetGameFrame()
...
function widget:GameFrame(f)
	if f > initialFrame + 10 then
		...
		if not Engine.Shared.IsCheatingEnabled() then
			Engine.Shared.Log(widget:GetInfo().name, LOG.ERROR, "this widget needs cheating enabled")
			widgetHandler:RemoveWidget(self)
			return
		end
