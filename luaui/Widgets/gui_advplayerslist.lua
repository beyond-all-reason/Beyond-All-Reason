		...
		if cpuLvl ~= nil then
			...
			if tipY then
				PingCpuTip(
					...
					ping,
					cpu,
					last.fpsData[playerID],
					last.gpuMemData[playerID],
					last.luaMemData[playerID],
					last.systemData[playerID],
					name,
					team,
					spec,
					last.apmData[team]
...
do -- keep the palette out of the main chunk's local scope (200 locals limit) while creating it only once
	...
	ConvertRGBToClosestName = function(r, g, b)
		...
		return BAR.I18N("ui.playersList.color." .. bestKey)
	end
end
...
function NameTip(mouseX, playerID, accountID, nameIsAlias, spec)
	...
	if
		mouseX >= widgetPosX + (m_name.posX + (1 * playerScale)) * widgetScale
