...
function widget:GetInfo()
    return {
        name      = "CustomFormations2 with BAP",
        desc      = "Allows you to draw your own formation line. Uses Bottleneck assignment instead of Min Sum",
        ...
        if fDists[#fNodes] < adjustedMinFormationLength or (usingCmd == CMD.UNLOAD_UNIT and fDists[#fNodes] < 64*(selectedUnitsCount - 1)) then
            ...
            if #mUnits > 0 then
                ...
                if (#mUnits <= maxHungarianUnits) then
                    orders = GetOrdersThreshold(interpNodes, mUnits, #mUnits, shift and not meta)
                else
...
function GetOrdersThreshold(nodes, units, unitCount, shifted)
    -------------------------------------------------------------------------------------
    -------------------------------------------------------------------------------------
    -- (the following code is written by gunblob) and edited by Blopson
    --   this code finds the optimal solution (slow, but effective!)
    --   it used the hungarian algorithm from http://www.public.iastate.edu/~ddoty/HungarianAlgorithm.html
    ...
    for i = 1, unitCount do
		...
		if ux then
			...
			for j = 1, unitCount do
                ...
                -- Added * 10 for increased granularity after division
                local speed = unitDef.speed
                if speed < 1 then
                    speed = 10
                end
                dists[j] = floor((sqrt(dx*dx + dz*dz)*10 + 0.5)/speed)
    ...
    local result = thresholdBottleneckAssignment(distances, unitCount)
    --------------------------------------------------------------------------------------------
    ...
    if (delay > maxHngTime) and (maxHungarianUnits > minHungarianUnits) then
        ...
        if (#units > maxHungarianUnits*unitIncreaseThresh) then
            ...
            -- Changed from Hungarian algorithm which is O(n3)
            -- Uses threshold bottleneck algorithm and is O(n1.5)
...
function thresholdBottleneckAssignment(distances, unitCount)
    local function canAssign(threshold)
        local assigned = {} -- Keeps track of assigned tasks
        local visited = {} -- Visited units during DFS
        local bestTask = {} -- Store best possible task for each unit

        -- Try to find an augmenting path using DFS
        local function dfs(unit)
            if visited[unit] then return false end
            visited[unit] = true

            for task = 1, unitCount do
                if distances[unit][task] <= threshold then
                    if not assigned[task] or dfs(assigned[task]) then
                        assigned[task] = unit
                        bestTask[unit] = task
                        return true
                    end
        ...
        -- Try to assign each unit using DFS
        for unit = 1, unitCount do
            visited = {} -- Reset visited array for each unit
            if not dfs(unit) then
                return false -- If any unit can't be assigned, the threshold is too low
            end
        end
        return assigned, bestTask
    end
    ...
    for i = 1, unitCount do
        for j = 1, unitCount do
            minVal = math.min(minVal, distances[i][j])
            maxVal = math.max(maxVal, distances[i][j])
        end
    end

    local bestAssignment = {}
    local bestTasks = {}
    while minVal <= maxVal do
        local mid = math.floor((minVal + maxVal) / 2)
        local assignment, bestTask = canAssign(mid)
        if assignment then
            bestAssignment = assignment
            bestTasks = bestTask
            maxVal = mid - 1 -- Try to lower the threshold further
        else
            minVal = mid + 1
        end
    end

    -- Convert assignment result into expected output format
    local result = {}
    for unit, task in pairs(bestTasks) do
        table.insert(result, {unit, task})
    end
    return result
end
