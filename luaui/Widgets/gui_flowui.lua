...
local function DrawButtonResolved(px, py, sx, sy, data)
	...
	WG.FlowUI.Draw.RectRound(px, py, sx, sy, cs, data.tl, data.tr, data.br, data.bl, { data.c1r, data.c1g, data.c1b, data.c1a }, { data.c2r, data.c2g, data.c2b, data.c2a })
	...
	WG.FlowUI.Draw.RectRound(px + data.pxPad, sy - data.syPad - glossHeight, sx - data.sxPad, sy - data.syPad, data.bgpadding, data.tl, data.tr, 0, 0, { 1, 1, 1, 0 }, { 1, 1, 1, topGlossAlpha })
	...
	WG.FlowUI.Draw.RectRound(px + data.pxPad, py + data.pyPad, sx - data.sxPad, py + data.pyPad + glossHeight, data.bgpadding, 0, 0, data.br, data.bl, { 1, 1, 1, bottomGlossAlpha }, { 1, 1, 1, 0 })
	...
	WG.FlowUI.Draw.RectRoundOutline(px + data.pxPad, py + data.pyPad, sx - data.sxPad, sy - data.syPad, cs, outlineWidth, data.tl, data.tr, data.br, data.bl, { 1, 1, 1, outlineAlpha }, { 1, 1, 1, 0 })
end
...
local function ButtonRecordsEqual(a, b)
	return a.width == b.width and a.height == b.height and a.cornerMask == b.cornerMask and a.pxPad == b.pxPad and a.pyPad == b.pyPad and a.sxPad == b.sxPad and a.syPad == b.syPad and a.bgpadding == b.bgpadding and a.glossMult == b.glossMult and a.c1r == b.c1r and a.c1g == b.c1g and a.c1b == b.c1b and a.c1a == b.c1a and a.c2r == b.c2r and a.c2g == b.c2g and a.c2b == b.c2b and a.c2a == b.c2a
end
...
WG.FlowUI.Draw.Button = function(px, py, sx, sy, tl, tr, br, bl, ptl, ptr, pbr, pbl, opacity, color1, color2, bgpadding, glossMult)
	...
	local valid = width > 0 and height > 0 and width == width and height == height and data.pxPad == data.pxPad and data.pyPad == data.pyPad and data.sxPad == data.sxPad and data.syPad == data.syPad and resolvedBgpadding == resolvedBgpadding and resolvedGlossMult == resolvedGlossMult and data.c1r == data.c1r and data.c1g == data.c1g and data.c1b == data.c1b and data.c1a == data.c1a and data.c2r == data.c2r and data.c2g == data.c2g and data.c2b == data.c2b and data.c2a == data.c2a
	if not valid then
...
WG.FlowUI.Draw.Unit = function(px, py, sx, sy, cs, tl, tr, br, bl, zoom, borderSize, borderOpacity, texture, radarTexture, groupTexture, price, queueCount)
	...
	if width <= 0 or height <= 0 or width ~= width or height ~= height or cs ~= cs or offset ~= offset or borderSize ~= borderSize or borderOpacity ~= borderOpacity then
