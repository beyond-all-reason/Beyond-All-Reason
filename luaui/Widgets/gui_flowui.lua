...
WG.FlowUI.Draw.Element = function(px, py, sx, sy, tl, tr, br, bl, ptl, ptr, pbr, pbl, opacity, color1, color2, bgpadding, opaque)
	...
	WG.FlowUI.Draw.RectRound(px + pxPad, py + pyPad, sx - sxPad, sy - syPad, cs, tl, tr, br, bl, { color2[1] * 0.33, color2[2] * 0.33, color2[3] * 0.33, elemAlpha }, { color2[1], color2[2], color2[3], elemAlpha })
	...
	WG.FlowUI.Draw.RectRound(px + pxPad + innerPad, py + pyPad + innerPad, sx - sxPad - innerPad, sy - syPad - innerPad, cs * 0.5, tl, tr, br, bl, { color1[1] + (innerBrightness * 0.7), color1[2] + (innerBrightness * 0.7), color1[3] + (innerBrightness * 0.7), innerAlpha }, { color1[1] + innerBrightness, color1[2] + innerBrightness, color1[3] + innerBrightness, innerAlpha })
	...
	if doBottomFx then
		...
		WG.FlowUI.Draw.RectRound(px + pxPad + 2, py + 2, sx - sxPad - 2, py + ((sy - py) * 0.75), cs * 1.66, 0, 0, br, bl, { c, c, c, opaque and 1 or 0.05 * glossMult }, { c2, c2, c2, opaque and 1 or 0 })
	end
	...
	WG.FlowUI.Draw.RectRound(px + pxPad + 1, sy - syPad - 1 - glossHeight, sx - sxPad - 1, sy - syPad - 1, cs * 0.5, tl, tr, 0, 0, { 0.12, 0.12, 0.12, opaque and 1 or 0 }, { glossTopC, glossTopC, glossTopC, glossTopAlpha })
	...
	if doBottomFx then
		...
		WG.FlowUI.Draw.RectRound(px + pxPad + 1, py + pyPad + 1, sx - sxPad - 1, py + pyPad + glossHeight, cs, 0, 0, br, bl, { glossBotC, glossBotC, glossBotC, glossBotAlpha }, { 0.06, 0.06, 0.06, opaque and 1 or 0 })
	end
	...
	if syPad > 0 then
		...
		WG.FlowUI.Draw.RectRound(px + pxPad + 1, sy - syPad - (cs * 2.5), sx - sxPad - 1, sy - syPad - 1, cs, tl, tr, 0, 0, { 0.24, 0.24, 0.24, opaque and 1 or 0 }, { edgeTopC, edgeTopC, edgeTopC, edgeTopAlpha })
	end
	...
	if pyPad > 0 then
		...
		WG.FlowUI.Draw.RectRound(px + pxPad + 1, py + pyPad + 1, sx - sxPad - 1, py + pyPad + (cs * 2), cs, 0, 0, br, bl, { edgeBotC, edgeBotC, edgeBotC, edgeBotAlpha }, { 0.13, 0.13, 0.13, opaque and 1 or 0 })
	end
