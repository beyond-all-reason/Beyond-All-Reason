...
local function refreshScreenGeometries(numDisplays)
	for displayIndex = 1, numDisplays do
		local sx, sy, px, py = Engine.Unsynced.GetScreenGeometry(displayIndex - 1)
...
local function getMaximalWindowGeometry(minI, maxI)
	...
	for displayNum = 2, Engine.Unsynced.GetNumDisplays() do
...
local function insertMultiMonitorModes(modes)
	local numDisplays = Engine.Unsynced.GetNumDisplays()
...
local function refreshScreenModes()
	...
	for _, videoMode in ipairs(Platform.availableVideoModes) do
		...
		if display ~= videoMode.display then
			...
			local w, h, x, y = Engine.Unsynced.GetScreenGeometry(display - 1)
			displays[display] = {
		...
		for display = 1, numDisplays do
			for display2 = 1, numDisplays do
				if display ~= display2 then
					local w, h, x, y = Engine.Unsynced.GetScreenGeometry(display - 1)
					local w2, h2, x2, y2 = Engine.Unsynced.GetScreenGeometry(display2 - 1)
...
local function changeScreenMode(index)
	...
	if screenMode.type == windowType.fullscreen then
		Engine.Unsynced.SetWindowGeometry(screenMode.display, 0, 0, screenMode.width, screenMode.height, true, false)
	elseif screenMode.type == windowType.borderless then
		Engine.Unsynced.SetWindowGeometry(screenMode.display, screenMode.x or 0, screenMode.y or 0, screenMode.width, screenMode.height, true, true)
	elseif screenMode.type == windowType.multimonitor then
		Engine.Unsynced.SetWindowGeometry(screenMode.actualDisplay, screenMode.x or 0, screenMode.y or 0, screenMode.width, screenMode.height, false, true)
	elseif screenMode.type == windowType.windowed then
		...
		local _, _, _, _, borderTop, borderLeft, borderBottom, borderRight = Engine.Unsynced.GetWindowGeometry()
		local width = screenMode.width - borderLeft - borderRight
		local height = screenMode.height - borderTop - borderBottom
		Engine.Unsynced.SetWindowGeometry(screenMode.display, borderLeft, borderTop, width, height, false, false)

		if firstPassDrawFrame then
			firstPassDrawFrame = nil
		else
			firstPassDrawFrame = Engine.Unsynced.GetDrawFrame()
		end
	end
end
...
function widget:Update()
	...
	if Engine.Unsynced.GetDrawFrame() - firstPassDrawFrame <= 2 then
		return
