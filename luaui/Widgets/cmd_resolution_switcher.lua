...
local function refreshScreenModes()
	...
	for _, videoMode in ipairs(Platform.availableVideoModes) do
		...
		if display ~= videoMode.display then
			...
			local fullscreen = {
				...
				name = BAR.I18N("ui.resolutionswitcher.fullscreen"),
			...
			local borderless = {
				display = display,
				name = BAR.I18N("ui.resolutionswitcher.borderless"),
				displayName = videoMode.displayName,
		...
		if videoMode.w >= 800 and videoMode.h > 600 then
			local windowed = {
				display = display,
				displayName = videoMode.displayName,
				name = BAR.I18N("ui.resolutionswitcher.window") .. " " .. videoMode.w .. " × " .. videoMode.h .. "  (" .. videoMode.hz .. "hz)",
	...
	if numDisplays > 1 then
						...
						if x + w == x2 or x2 + w2 == x or x2 - w == x or x - w2 == x2 then -- make sure they are next to eachother
							if not addedDisplayCombo[display] or addedDisplayCombo[display] ~= display2 then
								...
								tableInsert(screenModes, {
									...
									name = BAR.I18N("ui.resolutionswitcher.displays") .. " " .. display .. " + " .. display2 .. " (" .. w + w2 .. " x " .. mathMin(h, h2) .. ")",
									displayName = "",
									type = windowType.multimonitor,
									x = mathMin(x, x2),
									y = mathMax(y, y2),
								...
								if h ~= h2 then
									tableInsert(screenModes, {
										display = #displays + 1, -- not actual display number
										actualDisplay = (x < x2 and display or display2),
										name = BAR.I18N("ui.resolutionswitcher.displays") .. " " .. display .. " + " .. display2 .. " (" .. w + w2 .. " x " .. mathMax(h, h2) .. ")",
										displayName = "",
										type = windowType.multimonitor,
										x = mathMin(x, x2),
										y = mathMin(y, y2),
		...
		for k, v in pairs(addedDisplayCombo) do
			displays[#displays + 1] = {
				name = BAR.I18N("ui.resolutionswitcher.multidisplay"),
				width = 0,
				height = 0,
				hz = 0,
				x = 0,
