...
local function updateUI()
    ...
    local layoutChanged = layoutDirty
        or widgetScale ~= nextWidgetScale
        or area[1] ~= nextArea1 or area[2] ~= nextArea2 or area[3] ~= nextArea3 or area[4] ~= nextArea4
        or currentUseSkew ~= useSkew or currentSkewTan ~= localSkewTan
    ...
    if not layoutChanged
        and dlistCU ~= nil
        and converterUse == displayedConverterUse
        and eConverted == displayedEConverted
        and mConverted == displayedMConverted
    then
        return
    end
