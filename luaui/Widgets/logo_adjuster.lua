...
local faction = "_a"
if UnitDefs[Engine.Shared.GetTeamRulesParam(spGetMyTeamID(), "startUnit")].name == "corcom" then
	faction = "_c"
end
...
local function SetIcon(path)
	currentIcon = path
	Engine.Unsynced.SetWMIcon(path, true)
end

function widget:Initialize()
	if Platform.osName == "Windows 7" then
...
function widget:Update(dt)
	...
	if sec > 1.25 then
		...
		if gameFrame > 0 then
			local _, gameSpeed, isPaused = Engine.Unsynced.GetGameSpeed()
		...
		if doNotify then
			...
			if currentIcon then
				Engine.Unsynced.SetWMIcon(currentIcon, true)
			end
		end
	end
end
