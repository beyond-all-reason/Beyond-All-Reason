...
local function getBlueprintCenter()
	...
	for i = 1, #selectedunits do
		...
		centerposx[unit], centerposy[unit], centerposz[unit] = Engine.Shared.GetUnitPosition(unit)
	...
	blueprintCenterY = Engine.Shared.GetGroundHeight(blueprintCenterX, blueprintCenterZ)
end
...
local function generateCode(type)
	...
	for _, unitID in ipairs(selectedUnits) do
		local unitDirection = Engine.Shared.GetUnitBuildFacing(unitID)
	...
	Engine.Shared.Echo(blueprintName .. " written to " .. outputFile)
end
