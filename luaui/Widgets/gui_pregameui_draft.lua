...
local function DrawState(playerID, posX, posY)
	...
	local _, _, _, _, _, _, _, _, ai = Spring.GetPlayerInfo(playerID, false)
	if ai then
...
local function findPlayerName(playerID)
	...
	tname = ((WG.playernames and WG.playernames.getPlayername) and WG.playernames.getPlayername(playerID))
...
local function DrawTeamPlacement()
	...
	TeamPlacementUI = glCreateList(function()
		...
		max_width = mathMax(
			(max_width * player_name_font_size * uiScale) + padding_left + player_column_offset + padding_left,
			button_width
		)
...
local function checkButtonStateChanged(color, cantPlaceNow, blinkButton, currentButtonW, currentButtonH)
	return buttonStateChanged
		or lastButtonText ~= buttonText
		or lastShowLockButton ~= showLockButton
		or lastButtonColor[1] ~= color[1]
		or lastButtonColor[2] ~= color[2]
		or lastButtonColor[3] ~= color[3]
		or lastButtonW ~= currentButtonW
		or lastButtonH ~= currentButtonH
		or lastBlinkButton ~= blinkButton
		or lastCantPlaceNow ~= cantPlaceNow
end
...
local function drawButton()
	...
	if checkButtonStateChanged(color, cantPlaceNow, blinkButton, currentButtonW, currentButtonH) then
		...
		buttonRect =
			{ buttonX - (buttonW / 2), buttonY - (buttonH / 2), buttonX + (buttonW / 2), buttonY + (buttonH / 2) }
...
function widget:Initialize()
	...
	if xn and (xn ~= 0 or zn ~= 0 or xp ~= mapSizeX or zp ~= mapSizeZ) then
		hasStartbox = true
	end
...
function widget:DrawScreen()
	...
	if
		Spring.GetGameRulesParam("all_players_joined") == 1
		and not gameStarting
		and auto_ready
		and not auto_ready_disable
	then
...
function widget:RecvLuaMsg(msg, playerID)
		...
		if myTeamPlayersOrder == nil then
			...
			if devUItestMode then
				...
				for i = 1, fakePlayers do
					tableInsert(
						myTeamPlayersOrder,
						{ id = 30 + i, name = "Player" .. tostring((i + 9 + mathRandom(1000000))) }
					) -- debug
				end
			end
			for i = 3, #words do
	...
	elseif words[1] == "DraftOrderPlayerTurn" then
		...
		if not devUItestMode then -- production: trust the gadget
			...
			if myTeamPlayersOrder and oldIndex ~= currentPlayerIndex then
				...
				if
					myTeamPlayersOrder[currentPlayerIndex + 1]
					and myTeamPlayersOrder[currentPlayerIndex + 1].id ~= nil
				then
					next_playerID = myTeamPlayersOrder[currentPlayerIndex + 1].id
				else
					next_playerID = -1
	...
	elseif words[1] == "DraftOrderShowCountdown" then
		allyTeamID_about = tonumber(words[2] or -1)
		if
			(allyTeamID_about == myAllyTeamID)
			and (myAllyTeamJoined ~= true)
			and (connectionTimeoutHappened == false)
		then
