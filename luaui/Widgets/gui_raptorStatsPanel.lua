...
if not SpringShared.GetGameRulesParam("raptorDifficulty") then
	return false
end
...
local difficultyOption = SpringShared.GetModOptions().raptor_difficulty
local nBosses = SpringShared.GetModOptions().raptor_queen_count
...
local function CreatePanelDisplayList()
	...
	if currentTime > gameInfo.raptorGracePeriod then
		if gameInfo.raptorQueenAnger < 100 then
			local gain = 0
			if SpringShared.GetGameRulesParam("RaptorQueenAngerGain_Base") then
				font:Print(textColor .. I18N("ui.raptors.queenAngerBase", { value = math.round(SpringShared.GetGameRulesParam("RaptorQueenAngerGain_Base"), 3) }), panelMarginX + 5, PanelRow(3), panelFontSize, "")
				font:Print(textColor .. I18N("ui.raptors.queenAngerAggression", { value = math.round(SpringShared.GetGameRulesParam("RaptorQueenAngerGain_Aggression"), 3) }), panelMarginX + 5, PanelRow(4), panelFontSize, "")
				...
				gain = math.round(SpringShared.GetGameRulesParam("RaptorQueenAngerGain_Base"), 3) + math.round(SpringShared.GetGameRulesParam("RaptorQueenAngerGain_Aggression"), 3) + math.round(SpringShared.GetGameRulesParam("RaptorQueenAngerGain_Eco"), 3)
			end
			...
			font:Print(textColor .. I18N("ui.raptors.queenAngerWithTech", { anger = mathFloor(0.5 + gameInfo.raptorQueenAnger), techAnger = gameInfo.raptorTechAnger }), panelMarginX, PanelRow(1), panelFontSize, "")
			...
			if totalSeconds < 1800 or revealedQueenEta then
				...
				font:Print(textColor .. I18N("ui.raptors.queenETA", { count = nBosses, time = time }), panelMarginX + 5, PanelRow(2), panelFontSize, "")
			end
			...
			font:Print(textColor .. I18N("ui.raptors.queenHealth", { count = nBosses, health = gameInfo.raptorQueenHealth }), panelMarginX, PanelRow(1), panelFontSize, "")
			if nBosses > 1 then
				font:Print(textColor .. I18N("ui.raptors.queensKilled", { nKilled = gameInfo.raptorQueensKilled, nTotal = nBosses }), panelMarginX, PanelRow(2), panelFontSize, "")
			end
			for i = 1, #currentlyResistantToNames do
				if i == 1 then
					font:Print(textColor .. I18N("ui.raptors.queenResistantToList", { count = nBosses }), panelMarginX, PanelRow(11), panelFontSize, "")
				end
				font:Print(textColor .. currentlyResistantToNames[i], panelMarginX + 20, PanelRow(11 + i), panelFontSize, "")
			end
		end
	else
		font:Print(textColor .. I18N("ui.raptors.gracePeriod", { time = string.formatTime(mathCeil(((currentTime - gameInfo.raptorGracePeriod) * -1) - 0.5)) }), panelMarginX, PanelRow(1), panelFontSize, "")
	end

	font:Print(textColor .. I18N("ui.raptors.raptorKillCount", { count = gameInfo.raptorKills }), panelMarginX, PanelRow(6), panelFontSize, "")
	local endless = ""
	if SpringShared.GetModOptions().raptor_endless then
		endless = " (" .. I18N("ui.raptors.difficulty.endless") .. ")"
	end
	local difficultyCaption = I18N("ui.raptors.difficulty." .. difficultyOption)
	font:Print(textColor .. I18N("ui.raptors.mode", { mode = difficultyCaption }) .. endless, 80, h - 170, panelFontSize, "")
	font:End()
...
local function getMarqueeMessage(raptorEventArgs)
	...
	elseif raptorEventArgs.type == "queen" then
		messages[1] = textColor .. I18N("ui.raptors.queenIsAngry1", { count = nBosses })
...
local function getResistancesMessage()
	local messages = {}
	messages[1] = textColor .. I18N("ui.raptors.resistanceUnits", { count = nBosses })
...
function RaptorEvent(raptorEventArgs)
	if raptorEventArgs.type == "firstWave" or (raptorEventArgs.type == "queen" and SpringUnsynced.DiffTimers(SpringUnsynced.GetTimer(), bossToastTimer) > 10) then
		showMarqueeMessage = true
...
function widget:Shutdown()
	if hasRaptorEvent then
		SpringUnsynced.SendCommands({ "luarules HasRaptorEvent 0" })
	end
...
function widget:GameFrame(n)
	if not hasRaptorEvent and n > 1 then
		SpringUnsynced.SendCommands({ "luarules HasRaptorEvent 1" })
		hasRaptorEvent = true
	end
	if n % 30 < 1 then
		UpdateRules()
