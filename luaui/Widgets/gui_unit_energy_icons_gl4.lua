...
local function UpdateTeamEnergy()
	for i, teamID in pairs(teamList) do --onlyShowOwnTeam and myTeamID or nil
		if (fullview and not onlyShowOwnTeam) -- this is pretty fucking stupid, but false
			or not onlyShowOwnTeam -- true, however this garbage doesnt take into account ally teams
			or teamID == myTeamID then
			teamEnergy[teamID] = select(1, spGetTeamResources(teamID, 'energy'))
			--Spring.Echo("UpdateTeamEnergy", teamID, teamEnergy[teamID])
		end
	end
end
