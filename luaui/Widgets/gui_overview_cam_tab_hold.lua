...
local function setActionHotkeys(action)
	camKeys = {}
	local keyTable = Engine.Unsynced.GetActionHotKeys(action)
...
local function isCamKey(keyNum)
	local pressedSymbol = Engine.Unsynced.GetKeySymbol(keyNum):upper()
...
function widget:KeyRelease(key, modifier)
	...
	if prevCamState ~= nil and isOverview then
		...
		Engine.Unsynced.SetCameraState(prevCamState, 1)
		return true
	end
