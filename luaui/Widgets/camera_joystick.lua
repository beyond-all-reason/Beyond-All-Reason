...
local buttonCommands = { -- key is button number, value is command like you would type into console without the beginning /
	...
	[Bbutton[2]] = function()
		Engine.Unsynced.SendCommands("hideinterface")
	end,
	[Xbutton[2]] = function()
		Engine.Unsynced.SendCommands("togglelos")
	end,
	[LShoulderbutton[2]] = function()
		Engine.Unsynced.SendCommands("slowdown")
	end,
	[RShoulderbutton[2]] = function()
		Engine.Unsynced.SendCommands("speedup")
	end,
...
local function dumpConfig()
	...
	for _, conf in ipairs({ "TCPAllowConnect", "TCPAllowListen", "UDPAllowConnect", "UDPAllowListen" }) do
		spEcho(conf .. " = " .. Engine.Unsynced.GetConfigString(conf, ""))
	end
end
...
function widget:Initialize()
	Engine.Unsynced.SendCommands({ "set SmoothTimeOffset 2" })
	...
	if connected then
		Engine.Unsynced.SetConfigInt("RotOverheadClampMap", 0)
	else
...
function widget:Update(dt) -- dt in seconds
	...
	if cs.name == "rot" and joystate.axes then
		...
		if debugMode and Engine.Shared.GetGameFrame() % 60 == 0 then
		...
		local gh = Engine.Shared.GetGroundHeight(cs.px, cs.pz)
