...
local buttonCommands =
	{ -- key is button number, value is command like you would type into console without the beginning /
		[Abutton[2]] = function()
...
function widget:Update(dt) -- dt in seconds
	...
	if cs.name == "rot" and joystate.axes then
		...
		if joystate[RightXAxis[1]][RightXAxis[2]] then
			...
			local rotYx, rotYy, rotYz = rotateVector(
				{ cs.dx, cs.dy, cs.dz },
				{ 0, 1, 0 },
				-1.0 * lrturn * rotmult * frameSpeed
			)
			cs.dx = rotYx
			cs.dy = rotYy
			cs.dz = rotYz
		...
		if joystate[RightYAxis[1]][RightYAxis[2]] then
			...
			if not ((cs.dy < -0.98 and turnupdown >= 0) or (cs.dy > 0.98 and turnupdown <= 0)) then -- gimbal lock prevention
				local rotUpx, rotUpy, rotUpz = rotateVector(
					{ cs.dx, cs.dy, cs.dz },
					{ ndz, 0, -ndx },
					turnupdown * rotmult * frameSpeed
				)
				cs.dx = rotUpx
				cs.dy = rotUpy
				cs.dz = rotUpz
