...
function widget:GetInfo()
   return {
      name      = "Seismic Ping",
      desc      = "Draws Seismic Pings at discovered stealth unit locations",
      author    = "Floris",
      date      = "2026",
...
local innerThicknessRatio = baseThickness * 0.6 / baseRadius
...
local function DrawPing(ping, currentTime)
	...
	-- Inner outlines
	glColor(0.08, 0, 0, innerAlpha * 0.25)
	for i = 0, 1 do
		glPushMatrix()
	...
	local centerScale = baseRadius * 0.85 * (1 - centerProgress) * thicknessScale
	if centerScale > 0.1 then
		local centerAlphaMultiplier
		if centerProgress < 0.15 then
			centerAlphaMultiplier = centerProgress / 0.15
		else
			centerAlphaMultiplier = (1 - centerProgress) / 0.85
		end
		local centerAlpha = max(0, centerAlphaMultiplier * 0.55)
		glColor(1, 0.4, 0.4, centerAlpha)
		glPushMatrix()
