...
local function drawIcon(unitDefID, rect, lightness, zoom, texSize, highlightOpacity)
	...
	UiUnit(
		...
		zoom,
		nil,
		mathMax(0.1, highlightOpacity or 0.1),
		"#" .. unitDefID,
		nil,
		nil,
		nil,
	...
	if highlightOpacity then
		...
		RectRound(
			...
			rect[3],
			rect[4],
			min(max(1, floor((rect[3] - rect[1]) * 0.024)), floor((vsy * 0.0015) + 0.5))
		)
	...
	if numGroups > 0 then
		...
		for group = 0, 9 do
			if existingGroups[group] then
				...
				font2:Begin(true)
				font2:Print(
					"\255\200\255\200" .. group,
					groupRect[1] + ((groupRect[3] - groupRect[1]) / 2),
					groupRect[2] + iconMargin + (fontSize * 0.28),
					fontSize,
					"co"
				)
				font2:End()
...
function widget:Update(dt)
	...
	if WG.buildmenu then
		...
		if buildmenuBottomPosition and not buildmenuAlwaysShow and WG.info then
			if
				(not selectedUnits[1] or not WG.buildmenu.getIsShowing())
				and (posX > 0 or not WG.info.getIsShowing())
			then
				if posY ~= 0 then
					posY = 0
					doUpdate = true
