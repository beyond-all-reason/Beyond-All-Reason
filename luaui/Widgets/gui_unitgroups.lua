...
local spGetGroupList = Engine.Unsynced.GetGroupList
local spGetGroupUnitsCounts = Engine.Unsynced.GetGroupUnitsCounts
local spGetGroupUnitsCount = Engine.Unsynced.GetGroupUnitsCount
...
local hovered = false
local numGroups = 0
local selectedUnits = Engine.Unsynced.GetSelectedUnits() or {}
...
function widget:PlayerChanged(playerID)
	...
	if not showWhenSpec and Engine.Shared.GetGameFrame() > 1 and spec then
	...
	if numGroups > 0 then
		...
		for group = 0, 9 do
			if existingGroups[group] then
				...
				if showStack then
					if udefID_5 then
						drawIcon(udefID_5, { groupRect[1] + iconMargin + (offset * 4), groupRect[4] - iconMargin - (offset * 4) - iconSize, groupRect[1] + iconMargin + (offset * 4) + iconSize, groupRect[4] - iconMargin - (offset * 4) }, 0.33, zoom, texSize, highlightOpacity)
					end
					if udefID_4 then
						drawIcon(udefID_4, { groupRect[1] + iconMargin + (offset * 3), groupRect[4] - iconMargin - (offset * 3) - iconSize, groupRect[1] + iconMargin + (offset * 3) + iconSize, groupRect[4] - iconMargin - (offset * 3) }, 0.45, zoom, texSize, highlightOpacity)
					end
					if udefID_3 then
						drawIcon(udefID_3, { groupRect[1] + iconMargin + (offset * 2), groupRect[4] - iconMargin - (offset * 2) - iconSize, groupRect[1] + iconMargin + (offset * 2) + iconSize, groupRect[4] - iconMargin - (offset * 2) }, 0.55, zoom, texSize, highlightOpacity)
					end
					if udefID_2 then
						drawIcon(udefID_2, { groupRect[1] + iconMargin + offset, groupRect[4] - iconMargin - offset - iconSize, groupRect[1] + iconMargin + offset + iconSize, groupRect[4] - iconMargin - offset }, 0.7, zoom, texSize, highlightOpacity)
					end
				end
				drawIcon(udefID_1, { groupRect[1] + iconMargin, groupRect[4] - iconMargin - iconSize, groupRect[1] + iconMargin + iconSize, groupRect[4] - iconMargin }, 1, zoom, texSize, highlightOpacity)
	...
	if sec > 0.5 then
		...
		if highlightSelectedGroups and selectionHasChanged then
			...
			for group, _ in pairs(existingGroups) do
				...
				local groupUnits = Engine.Unsynced.GetGroupUnits(group)
			...
			for i, v in pairs(groupButtons) do
				if math_isInRect(x, y, groupButtons[i][1], groupButtons[i][2], groupButtons[i][3], groupButtons[i][4]) then
					if shift then
						...
						local groupUnits = Engine.Unsynced.GetGroupUnits(groupButtons[i][5])
						for i = 1, #groupUnits do
							units[#units + 1] = groupUnits[i]
						end
						selectedUnits = units
						selectionHasChanged = true
						Engine.Unsynced.SelectUnitArray(units)
					elseif ctrl then
						local units = selectedUnits
						local groupUnits = Engine.Unsynced.GetGroupUnits(groupButtons[i][5])
						local keyGroupUnits = {}
						...
						selectionHasChanged = true
						Engine.Unsynced.SelectUnitArray(selectedUnits)
					else
						selectedUnits = Engine.Unsynced.GetGroupUnits(groupButtons[i][5])
						selectionHasChanged = true
						Engine.Unsynced.SelectUnitArray(selectedUnits)
					end
					if button == 3 then
					...
					if playSounds then
						Engine.Unsynced.PlaySoundFile((button == 3 and rightclick or leftclick), soundVolume, "ui")
					end
					return true
				end
			end
