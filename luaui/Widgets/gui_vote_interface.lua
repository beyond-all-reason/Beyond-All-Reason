...
local myPlayerName, _, mySpec, myTeamID, myAllyTeamID = Engine.Shared.GetPlayerInfo(myPlayerID, false)
...
local function isTeamPlayer(playerName)
	...
	for _, pID in ipairs(players) do
		local name, _, _, _, allyTeamID = Engine.Shared.GetPlayerInfo(pID, false)
...
function widget:PlayerChanged(playerID)
	...
	myPlayerName, _, mySpec, myTeamID, myAllyTeamID = Engine.Shared.GetPlayerInfo(myPlayerID, false)
end
			...
			if ssub(line, 1, 1) == ">" and ssub(line, 3, 3) ~= "<" then
				...
				if sfind(line, " called a vote ", nil, true) then
					...
					for _, playerID in ipairs(players) do
						local playerName, _, spec, teamID, allyTeamID = Engine.Shared.GetPlayerInfo(playerID, false)
					...
					if isResignVote or isResignVoteMyTeam then
						local players = Engine.Shared.GetPlayerList()
						for _, pID in ipairs(players) do
							local name, _, spec, teamID, allyTeamID = Engine.Shared.GetPlayerInfo(pID, false)
		...
		if not WG["topbar"] or not WG["topbar"].showingQuit() then
			if eligibleToVote then
				...
				elseif windowArea and math_isInRect(x, y, windowArea[1], windowArea[2], windowArea[3], windowArea[4]) then
					if not weAreVoteOwner and math_isInRect(x, y, yesButtonArea[1], yesButtonArea[2], yesButtonArea[3], yesButtonArea[4]) or math_isInRect(x, y, noButtonArea[1], noButtonArea[2], noButtonArea[3], noButtonArea[4]) or math_isInRect(x, y, closeButtonArea[1], closeButtonArea[2], closeButtonArea[3], closeButtonArea[4]) then
						StartVote() -- refresh
					end
				end
			end
