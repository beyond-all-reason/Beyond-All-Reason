...
local function StartVote(name) -- when called without params its just to refresh (when hovering over buttons)
	...
	voteDlist = gl.CreateList(function()
		...
		closeButtonArea = {
			...
			ypos + mathFloor(height / 6),
			xpos + (width / 2),
			ypos + (height / 2),
		}
		yesButtonArea = {
			xpos - (width / 2) + buttonMargin,
			ypos - (height / 2) + buttonMargin + progressbarHeight,
			xpos - (buttonMargin / 2),
		...
		if votesEligible then
			if votesRequired then
				...
				color2 = { 0, 1, 0, 0.1 }
				RectRound(
					windowArea[1] + bgpadding,
					windowArea[2] + bgpadding,
					windowArea[1] + bgpadding + w,
					windowArea[2] + bgpadding + progressbarHeight,
					elementCorner * 0.6,
					0,
					0,
					0,
			...
			if votesCountYes > 0 then
				...
				RectRound(
					windowArea[1] + bgpadding,
					windowArea[2] + bgpadding + (progressbarHeight / 2),
					windowArea[1] + bgpadding + w,
					windowArea[2] + bgpadding + progressbarHeight,
					0,
					0,
		...
		font:Print(
			...
			windowArea[4] - bgpadding - bgpadding - bgpadding - fontSize,
			fontSize,
			"con"
		)
		font:End()
		...
		if eligibleToVote and not minimized and not voteEndText then
			...
			RectRound(
				closeButtonArea[1] + bgpadding,
				closeButtonArea[2] + bgpadding,
				closeButtonArea[3] - bgpadding,
				closeButtonArea[4] - bgpadding,
				elementCorner * 0.66,
				0,
				1,
				0,
...
function widget:Update(dt)
	if debug then
		...
		if debugSec > 2 and debugStep < 2 then
			...
			widget:AddConsoleLine(
				'> [teh]cluster2[00] * Vote in progress: "resign PyroTech TEAM" [y:1/1(4), n:0/1(3), votes:1/3] (40s remaining)',
				false
			)
		end
		if debugSec > 2.75 and debugStep < 3 then
			debugStep = 3
			widget:AddConsoleLine(
				'> [teh]cluster1[00] * Vote in progress: "stop" [y:2/4, n:1/3] (42s remaining)',
				false
			)
		end
		if debugSec > 3.3 and debugStep < 4 then
			debugStep = 4
			widget:AddConsoleLine(
				'> [teh]cluster1[00] * Vote in progress: "stop" [y:3/4, n:1/3] (41s remaining)',
				false
			)
		end
		if debugSec > 4.2 and debugStep < 5 then
			debugStep = 5
			widget:AddConsoleLine(
				'> [teh]cluster1[00] * Vote in progress: "stop" [y:3/4, n:2/3] (41s remaining)',
				false
			)
		end
		...
		for line in lines:gmatch("[^\n]+") do
			...
			if ssub(line, 1, 1) == ">" and ssub(line, 3, 3) ~= "<" then
				...
				if sfind(line, " called a vote ", nil, true) then
					...
					local isResignVoteMyTeam = mySpec
						or (isIndividualResignVote and isTeamPlayer(individualResignTarget))
					...
					if isResignVote or isResignVoteMyTeam then
						...
						for _, pID in ipairs(players) do
							...
							if pos then
								title = ssub(title, 1, pos - 1)
									.. colourNames(teamID)
									.. " "
									.. name
									.. " "
									.. titlecolor
									.. ssub(title, pos + string.len(" " .. name .. " "))
								break
							end
						end
					end
				...
				elseif voteDlist and not voteEndTime then
					...
					if
						sfind(line, "* Vote cancelled", nil, true)
						or sfind(line, "* Game starting, cancelling ", nil, true)
				...
				if voteDlist and sfind(slower(line), " users allowed to vote.", nil, true) then
					local text = ssub(
						line,
						sfind(slower(line), "* ", nil, true) + 2,
						sfind(slower(line), " users allowed to vote.", nil, true) - 1
					)
					if text then
...
function widget:MousePress(x, y, button)
	if voteDlist and eligibleToVote and not voteEndText and button == 1 then
		if math_isInRect(x, y, windowArea[1], windowArea[2], windowArea[3], windowArea[4]) then
			if
				not weAreVoteOwner
				and math_isInRect(x, y, yesButtonArea[1], yesButtonArea[2], yesButtonArea[3], yesButtonArea[4])
			then
		...
		if not WG.topbar or not WG.topbar.showingQuit() then
			if eligibleToVote then
				...
				if hovered then
					StartVote() -- refresh
				elseif
					windowArea and math_isInRect(x, y, windowArea[1], windowArea[2], windowArea[3], windowArea[4])
				then
					if
						not weAreVoteOwner
							and math_isInRect(
						...
						or math_isInRect(x, y, noButtonArea[1], noButtonArea[2], noButtonArea[3], noButtonArea[4])
						or math_isInRect(
							x,
							y,
							closeButtonArea[1],
