...
local function initGL4(DPATname)
	...
	decalLargeVBO.VAO =
		InstanceVBOTable.makeVAOandAttach(decalLargeVBO.vertexVBO, decalLargeVBO.instanceVBO, decalLargeVBO.indexVBO)
		...
		if vbo and vbo.usedElements > 0 then
			...
			for instanceID, instanceIndex in pairs(vbo.instanceIDtoIndex) do
				...
				activeDecalData[instanceID] = {
					...
					alphadecay,
					spawnframe,
					footprintDecalSet[instanceID] or false,
					width,
					length,
					rotation,
					p,
...
local function AddDecal(
	...
	if lifetime then
		...
		if deathtime ~= deathtime then -- NaN check
			spEcho(
				"gfx_decals_gl4: NaN deathtime for decal index",
				decalIndex,
				"spawnframe:",
				spawnframe,
				"lifetime:",
...
local function decalsgl4statsCmd(_, line)
	local tricount = 4 * 4 * 2 * decalVBO.usedElements
		+ resolution * resolution * 2 * decalLargeVBO.usedElements
		+ 4 * 4 * resolution * resolution * 2 * decalExtraLargeVBO.usedElements
	spEcho(
		string.format(
			"Small decal = %d, Medium decal = %d, Large decal = %d, tris = %d",
	...
	if n % 67 == 0 then -- About every 2 seconds
		...
		if autoupdate and removed > 0 then
			spEcho(
				"Removed",
				removed,
				"decals from decal instance tables: s=",
				decalVBO.usedElements,
				" l=",
				decalLargeVBO.usedElements,
				"xl=",
	...
	if (not nodecal) and (not string.find(weaponDef.cegTag, "aa")) then
		...
		elseif weaponDef.type == "Cannon" then
			...
			elseif string.find(weaponDef.name, "tremor") then
				textures = {
					"t_groundcrack_17_a.tga",
					"t_groundcrack_21_a.tga",
					"t_groundcrack_10_a.tga",
					"t_groundcrack_09_a.tga",
				}
				radius = radius * 0.96
		...
		elseif string.find(weaponDef.name, "flamebug") then
			textures = {
				"t_groundcrack_23_a.tga",
				"t_groundcrack_24_a.tga",
				"t_groundcrack_25_a.tga",
				"t_groundcrack_27_a.tga",
			}
		...
		elseif string.find(weaponDef.name, "corkorg_laser") then
			...
			bwfactor = 0.1
		elseif
			string.find(weaponDef.name, "pineappleofdoom")
			or string.find(weaponDef.name, "heatraylarge")
			or string.find(weaponDef.name, "skybeam")
			or string.find(weaponDef.name, "heat_ray")
		then --legbastion leginc legphoenix legaheattank
...
local function UnitScriptDecal(unitID, unitDefID, whichDecal, posx, posz, heading)
	...
	if
		...
		and UnitScriptDecals[unitDefID]
		and UnitScriptDecals[unitDefID][whichDecal]
	then
