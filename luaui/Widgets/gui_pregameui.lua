...
local myPlayerName, _, mySpec, myTeamID = Engine.Shared.GetPlayerInfo(myPlayerID, false)
...
function widget:GameSetup(state, ready, playerStates)
	local spec, fullview = Engine.Unsynced.GetSpectatingState()
	-- sends a "I arrived" message
	-- NOTE: Spring.GetGameRulesParam("player_" .. Spring.GetMyPlayerID() .. "_joined") seems to be always nil!
	if not spec and not ihavejoined and Engine.Shared.GetGameRulesParam("player_" .. Spring.GetMyPlayerID() .. "_joined") == nil then
		Engine.Unsynced.SendLuaRulesMsg("joined_game")
		ihavejoined = true
	end

	-- check when the 3.2.1 countdown starts
	if not gameStarting and ((Engine.Unsynced.GetPlayerTraffic(SYSTEM_ID, NETMSG_STARTPLAYING) or 0) > 0) then
	...
	if Engine.Shared.GetGameRulesParam("all_players_joined") == 1 and not gameStarting and auto_ready then
		auto_ready_timer = auto_ready_timer - Engine.Unsynced.GetLastUpdateSeconds()
	end
	...
	for _, playerID in pairs(playerList) do
		local _, _, spectator_flag = Engine.Shared.GetPlayerInfo(playerID, false)
			...
			if sx > buttonRect[1] and sx < buttonRect[3] and sy > buttonRect[2] and sy < buttonRect[4] then
				...
				if not readied then
					if not mySpec then
						if not readied then
							...
							elseif startPointChosen then
								pressedReady = true
								readied = true
								Engine.Unsynced.SendLuaRulesMsg("ready_to_start_game")
								-- also default lock player in place
								locked = true
								Engine.Unsynced.SendLuaRulesMsg("locking_in_place")
							else
					...
					elseif eligibleAsSub then
						...
						Engine.Unsynced.SendLuaRulesMsg(offeredAsSub and "\144" or "\145")
					end
				-- lock position text showing
				else
					if locked then
						locked = false
						Engine.Unsynced.SendLuaRulesMsg("unlocking_in_place")
					else
						locked = true
						Engine.Unsynced.SendLuaRulesMsg("locking_in_place")
					end
...
local function checkStartPointChosen()
	if not mySpec then
		local x, y, z = Engine.Shared.GetTeamStartPosition(myTeamID)
...
function widget:Initialize()
	...
	if Engine.Shared.GetGameFrame() > 0 or isReplay then
		widgetHandler:RemoveWidget()
		return
	end
...
function widget:DrawScreen()
	...
	if gameStarting then
		timer = timer + Engine.Unsynced.GetLastUpdateSeconds()
	...
	elseif showbutton == true then
		...
		if numPlayers > 3 then
			for _, playerID in pairs(playerList) do
				local readystate = Engine.Shared.GetGameRulesParam("player_" .. tostring(playerID) .. "_readyState")
		...
		if x > buttonRect[1] and x < buttonRect[3] and y > buttonRect[2] and y < buttonRect[4] then
			...
			timer2 = timer2 + Engine.Unsynced.GetLastUpdateSeconds()
			if mySpec then
...
function widget:DrawWorld()
	...
	for i = 1, #teamList do
		...
		if WG["map_startbox"] and WG["map_startbox"].GetEffectiveStartPosition then
			...
			tsx, tsy, tsz = Engine.Shared.GetTeamStartPosition(teamID)
		end
		if tsx and tsx > 0 then
			local startUnitDefID = Engine.Shared.GetTeamRulesParam(teamID, "startUnit")
