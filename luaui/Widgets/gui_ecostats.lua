				...
				if tID ~= gaiaID then
					...
					if GetGameSeconds() > 0 then
						if not tData.isDead then
							...
							DrawSideImage(sideImg, posx, posy + floor(tH * 0.125), r, g, b, alpha, not hasCom, Button["player"][tID]["mouse"], t, false, tID)
						else
							alpha = 0.8
							DrawSideImage(images["dead"], posx, posy + floor(tH * 0.125), r, g, b, alpha, true, Button["player"][tID]["mouse"], t, true, tID) --dead, big icon
						end
					else
...
function widget:DrawScreen()
	...
	if Spring.IsGUIHidden() or not inSpecMode then
		return
	end
