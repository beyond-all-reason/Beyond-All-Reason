	...
	for expression in string.gmatch(inputCegString, "([^,]+)") do
		...
		while i <= #expression do
			...
			elseif tonumber(char) or char == "-" or char == "." then
				...
				while
					i <= #expression
					and (tonumber(expression:sub(i, i)) or expression:sub(i, i) == "." or expression:sub(i, i) == "-")
				do
...
local function isValidTexture(value)
	if
		(type(value) == "string" and (projectileTexures[value] or projectileTexures[string.lower(value)]))
		or value == "none"
		or value == "nil"
		or value == "null"
	then
		...
		if
			(type(value) == "string")
			and (projectileTexures[value .. "-anim"] or projectileTexures[string.lower(value) .. "-anim"])
		then
			return true
		end
								...
								if cegDefTemplate[k2].validator then
									...
									if not res then
										...
										local msg = string.format(
											...
											tostring(spawnername),
											tostring(k2),
											((type(v2) == "string") and '"' or "")
												.. tostring(v2)
												.. ((type(2) == "string") and '"' or ""),
											tostring(err)
										)
