...
local function spotHasExtractor(spot)
	local units = SpringShared.GetUnitsInCylinder(spot.x, spot.z, Game_extractorRadius)
...
local function spotHasExtractorQueued(spot, builders)
	...
	if isPregame then
		...
		for i = 1, #builders do
			local hasOrder = checkQueue(SpringShared.GetUnitCommands(builders[i], 100))
			if hasOrder then
				return true
			end
		end
...
local function extractorCanBeUpgraded(currentExtractorUuid, newExtractorId)
	local isAllied = SpringShared.AreTeamsAllied(spGetMyTeamID(), spGetUnitTeam(currentExtractorUuid))
...
local function sortBuilders(units, constructorIds, buildingId, shift)
	...
	local function hasExistingGuardOrder(uid)
		local queue = SpringShared.GetUnitCommands(uid, 10)
		for i = 1, #queue do
			local cmd = queue[i]
...
local function ApplyPreviewCmds(cmds, constructorIds, shift)
	...
	local _, _, meta, _ = SpringUnsynced.GetModKeyState()
	...
	for i = 1, #cmds do
		...
		if meta then -- put at front of queue
			...
			SpringSynced.GiveOrderToUnitArray(unitArray, CMD.INSERT, { i - 1, -buildingId, 0, unpack(orderParams) }, { "alt" })
		else
			...
			SpringSynced.GiveOrderToUnitArray(unitArray, -buildingId, orderParams, opt)
		end
	end
end
