...
local spGetCurrentTooltip = Engine.Unsynced.GetCurrentTooltip
local spGetSelectedUnits = Engine.Unsynced.GetSelectedUnits
local spGetSelectedUnitsCounts = Engine.Unsynced.GetSelectedUnitsCounts
...
local SelectedUnitsCount = Engine.Unsynced.GetSelectedUnitsCount()
...
local spGetTeamUnitsSorted = Engine.Shared.GetTeamUnitsSorted
local spSelectUnitMap = Engine.Unsynced.SelectUnitMap
local spGetUnitHealth = Engine.Shared.GetUnitHealth
local spGetUnitResources = Engine.Shared.GetUnitResources
local spGetUnitExperience = Engine.Shared.GetUnitExperience
...
local mySpec = Engine.Unsynced.GetSpectatingState()
...
local _, _, mapMinWater, _ = Engine.Shared.GetGroundExtremes()
...
local function checkGeothermalFeatures()
	...
	for i = 1, #features do
		if geoThermalFeatures[Engine.Shared.GetFeatureDefID(features[i])] then
...
function widget:PlayerChanged(playerID)
	...
	mySpec = Engine.Unsynced.GetSpectatingState()
end
...
function widget:Initialize()
	isPregame = Engine.Shared.GetGameFrame() < 1
	...
	Engine.Unsynced.SetDrawSelectionInfo(false) -- disables springs default display of selected units count
	Engine.Unsynced.SendCommands("tooltip 0")
...
function widget:Shutdown()
	Engine.Unsynced.SetDrawSelectionInfo(true) --disables springs default display of selected units count
	Engine.Unsynced.SendCommands("tooltip 1")
	if infoBgTex then
		gl.DeleteTexture(infoBgTex)
	end
	if infoTex then
...
function widget:Update(dt)
	...
	if displayUnitID and not Engine.Shared.ValidUnitID(displayUnitID) then
		displayMode = "text"
		displayUnitID = nil
...
local function drawUnitInfo()
	...
	if displayUnitID then
		...
		local teamID = Engine.Shared.GetUnitTeam(displayUnitID)
		if mySpec or (myTeamID ~= teamID) then
			local _, playerID, _, isAiTeam = Engine.Shared.GetTeamInfo(teamID, false)
			local name = Engine.Shared.GetPlayerInfo(playerID, false)
			...
			if not mySpec and Engine.Shared.GetModOptions().teamcolors_anonymous_mode ~= "disabled" then
				name = anonymousName
			end
			if name then
				...
				--else
				name = spColorString(Engine.Unsynced.GetTeamColor(teamID)) .. name
				--end
	...
	elseif displayMode == "unit" and unitDefInfo[displayUnitDefID].transport and (Engine.Shared.GetUnitIsTransporting(displayUnitID) and #Engine.Shared.GetUnitIsTransporting(displayUnitID) or 0) > 0 then
		local units = Engine.Shared.GetUnitIsTransporting(displayUnitID)
		if #units > 0 then
			gridHeight = math_ceil(height * 0.975)
			local rows = 2
		...
		if displayMode == "unit" then
			...
			if unitDefInfo[displayUnitDefID].mainWeapon ~= nil then
				maxRange = Engine.Shared.GetUnitWeaponState(displayUnitID, unitDefInfo[displayUnitDefID].mainWeapon, "range")
			else
				maxRange = range
			end
			if not exp then
	...
	if not cameraPanMode and not mouseOffScreen then
		...
		if showEngineTooltip then
			...
			if hoverType == "ground" then
				...
				local groundType1, groundType2, metal, hardness, tankSpeed, botSpeed, hoverSpeed, shipSpeed, receiveTracks = Engine.Shared.GetGroundInfo(coords[1], coords[3])
				local text = ""
				local height = 0
				font:Begin(true)
				font:SetTextColor(1, 1, 1, 1)
				...
				if displayMapPosition then
					...
					font:Print(tooltipLabelTextColor .. Spring.I18N("ui.info.elevation") .. "  " .. tooltipValueColor .. math.floor(Engine.Shared.GetGroundHeight(coords[1], coords[3])), backgroundRect[1] + contentPadding + (fontSize * 6.6), backgroundRect[4] - contentPadding - (fontSize * 0.8) - height, fontSize, "o")
			...
			elseif hoverType == "feature" then
				local featureDefID = Engine.Shared.GetFeatureDefID(hoverData)
				...
				font:SetOutlineColor(0.1, 0.1, 0.1, 1)
				text = ""
				local metal, _, energy, _ = Engine.Shared.GetFeatureResources(hoverData)
				if energy > 0 then
					height = height + heightStep
...
local function LeftMouseButton(unitDefID, unitTable)
	...
	if acted then
		Engine.Unsynced.PlaySoundFile(sound_button, 0.5, "ui")
	end
end
...
local function MiddleMouseButton(unitDefID, unitTable)
	...
	if ctrl then
		...
		Engine.Unsynced.SendCommands(viewSelectionCmd)
	else
...
local function RightMouseButton(unitDefID, unitTable)
	...
	selectedUnits = spGetSelectedUnits()
	SelectedUnitsCount = spGetSelectedUnitsCount()
	Engine.Unsynced.PlaySoundFile(sound_button2, 0.5, "ui")
end
...
local function unloadTransport(transportID, unitID, x, z, shift, depth)
	...
	local areaUnits = Engine.Shared.GetUnitsInSphere(x, y, z, unitSphereRadius)
	if #areaUnits == 0 then -- unblocked spot!
		unloadParams[1], unloadParams[2], unloadParams[3], unloadParams[4] = x, y, z, unitID
		Engine.Shared.GiveOrderToUnit(transportID, CMD.UNLOAD_UNIT, unloadParams, shift and shiftTable or emptyTable)
	else
		...
		for i = 1, samples + 1 do
			...
			if x > 0 and z > 0 and x < mapSizeX and z < mapSizeZ then
				...
				areaUnits = Engine.Shared.GetUnitsInSphere(x, y, z, unitSphereRadius)
				if #areaUnits == 0 then -- unblocked spot!
					local areaFeatures = Engine.Shared.GetFeaturesInSphere(x, y, z, unitSphereRadius)
		...
		if displayMode == "unit" and button == 1 then
			...
			if units and #units > 0 then
				for cellID, unitID in pairs(units) do
					if cellRect[cellID] and math_isInRect(x, y, cellRect[cellID][1], cellRect[cellID][2], cellRect[cellID][3], cellRect[cellID][4]) then
						local x, y, z = Engine.Shared.GetUnitPosition(displayUnitID)
						local alt, ctrl, meta, shift = spGetModKeyState()
						if shift then
							local cmdQueue = Engine.Shared.GetUnitCommands(displayUnitID, 35) or {}
							if cmdQueue[1] then
								if cmdQueue[#cmdQueue] and cmdQueue[#cmdQueue].id == CMD.MOVE and cmdQueue[#cmdQueue].params[3] then
									...
									Engine.Shared.GiveOrderToUnit(displayUnitID, CMD.STOP, emptyTable, 0)
									for c = 1, #cmdQueue do
										if c < #cmdQueue then
											Engine.Shared.GiveOrderToUnit(displayUnitID, cmdQueue[c].id, cmdQueue[c].params, shiftTable)
										end
									end
								end
							end
...
function checkChanges()
	...
	if isPregame and not mySpec then
		...
		activeCmdID = select(2, Engine.Unsynced.GetActiveCommand())
	end
	...
	elseif cfgDisplayUnitID and Engine.Shared.ValidUnitID(cfgDisplayUnitID) then
		displayMode = "unit"
	...
	elseif not cameraPanMode and (customHoverType or not math_isInRect(x, y, backgroundRect[1], backgroundRect[2], backgroundRect[3], backgroundRect[4])) and hoverType and hoverType == "feature" then
		...
		if featureDef.reclaimable then
			local metal, _, energy, _ = Engine.Shared.GetFeatureResources(featureID)
	...
	if displayMode == "text" and isPregame then
		if not mySpec then
			displayMode = "unitdef"
			displayUnitDefID = Engine.Shared.GetTeamRulesParam(myTeamID, "startUnit")
			hideBuildlist = true
		else
			emptyInfo = true
		end
