	...
	elseif displayMode == "unit" and unitDefInfo[displayUnitDefID].transport and (Spring.GetUnitIsTransporting(displayUnitID) and #Spring.GetUnitIsTransporting(displayUnitID) or 0) > 0 then
		...
		if unitDefInfo[displayUnitDefID].weapons then
			...
			if exp and exp > 0.009 then
				...
				addTextInfo(BAR.I18N("ui.info.maxhealth"), "+" .. round((maxHealth / unitDefInfo[displayUnitDefID].health - 1) * 100, 0) .. "%")
		...
		if unitDefInfo[displayUnitDefID].cloakCost then
			if unitDefInfo[displayUnitDefID].cloakCostMoving then
				addTextInfo(BAR.I18N("ui.info.cloakcost"), unitDefInfo[displayUnitDefID].cloakCost .. "/" .. unitDefInfo[displayUnitDefID].cloakCostMoving)
			else
		...
		if unitDefInfo[displayUnitDefID].metalmaker then
			...
			addTextInfo(BAR.I18N("ui.info.convertedm"), round(unitDefInfo[displayUnitDefID].metalmaker[1] / (1 / unitDefInfo[displayUnitDefID].metalmaker[2]), 1))
		end
		...
		if showEngineTooltip then
			...
			if hoverType == "ground" then
				...
				if displayMapPosition then
					...
					font:Print(tooltipLabelTextColor .. BAR.I18N("ui.info.elevation") .. "  " .. tooltipValueColor .. math.floor(Spring.GetGroundHeight(coords[1], coords[3])), backgroundRect[1] + contentPadding + (fontSize * 6.6), backgroundRect[4] - contentPadding - (fontSize * 0.8) - height, fontSize, "o")
					height = height + heightStep
				end
				if tankSpeed ~= 1 or botSpeed ~= 1 or hoverSpeed ~= 1 or (shipSpeed ~= 1 and coords[2] <= 0) then
					text = ""
					if tankSpeed ~= 1 then
						text = text .. (text ~= "" and "   " or "") .. tooltipLabelTextColor .. BAR.I18N("ui.info.tank") .. " " .. tooltipValueColor .. math.floor(tankSpeed * 100) .. "%"
					end
					if botSpeed ~= 1 then
						text = text .. (text ~= "" and "   " or "") .. tooltipLabelTextColor .. BAR.I18N("ui.info.bot") .. " " .. tooltipValueColor .. math.floor(botSpeed * 100) .. "%"
					end
					if hoverSpeed ~= 1 then
						text = text .. (text ~= "" and "   " or "") .. tooltipLabelTextColor .. BAR.I18N("ui.info.hover") .. " " .. tooltipValueColor .. math.floor(hoverSpeed * 100) .. "%"
					end
					if shipSpeed ~= 1 and coords[2] <= 0 then
						text = text .. (text ~= "" and "   " or "") .. tooltipLabelTextColor .. BAR.I18N("ui.info.ship") .. " " .. tooltipValueColor .. math.floor(shipSpeed * 100) .. "%"
					end
					...
					font:Print(tooltipDarkTextColor .. BAR.I18N("ui.info.speedmultipliers") .. "   " .. text, backgroundRect[1] + contentPadding, backgroundRect[4] - contentPadding - (fontSize * 0.8) - height, fontSize, "o")
			...
			elseif hoverType == "feature" then
				...
				if energy > 0 then
					height = height + heightStep
					text = tooltipLabelTextColor .. BAR.I18N("ui.info.energy") .. "  \255\255\255\000" .. string.formatSI(energy)
					font:Print(text, backgroundRect[1] + contentPadding, backgroundRect[4] - contentPadding - (fontSize * 0.8) - height, fontSize, "o")
				end
				if metal > 0 then
					height = height + heightStep
					text = tooltipLabelTextColor .. BAR.I18N("ui.info.metal") .. "  " .. tooltipValueColor .. string.formatSI(metal)
					font:Print(text, backgroundRect[1] + contentPadding, backgroundRect[4] - contentPadding - (fontSize * 0.8) - height, fontSize, "o")
				end
				font:End()
			else
...
function checkChanges()
	...
	elseif not cameraPanMode and (customHoverType or not math_isInRect(x, y, backgroundRect[1], backgroundRect[2], backgroundRect[3], backgroundRect[4])) and hoverType and hoverType == "feature" then
		...
		if featureDef.reclaimable then
			...
			local reclaimText = BAR.I18N("ui.reclaimInfo.metal", { metal = string.formatSI(metal) }) .. "\255\255\255\128" .. " " .. BAR.I18N("ui.reclaimInfo.energy", { energy = string.formatSI(energy) })
