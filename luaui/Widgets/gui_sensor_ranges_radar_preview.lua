...
function widget:DrawWorld()
	...
	if Engine.Unsynced.IsGUIHidden() or (WG["topbar"] and WG["topbar"].showingQuit()) then
		return
	end
	...
	if selectedRadarUnitID then
		mousepos = { Engine.Shared.GetUnitPosition(selectedRadarUnitID) }
	else
		local mx, my, lp, mp, rp, offscreen = Engine.Unsynced.GetMouseState()
