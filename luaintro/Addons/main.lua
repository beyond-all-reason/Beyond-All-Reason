...
function addon.DrawLoadScreen()
	...
	-- background
	local scale = 1
	local ssx = Spring.GetScreenGeometry()
	...
	local wrappedTipText = font2:WrapText(randomTip, vsx * 1.35)
	local tipLines = lines(wrappedTipText)
	local tipPosYtop = posY
		+ (height / vsy)
		+ (borderSize / vsy)
		+ (posY * 0.9)
		+ ((tipTextLineHeight * #tipLines) / vsy)
	...
	if guishader then
		if not blurShader then
			CreateShaders()
			guishaderRects.loadprocess1 = {
				(posX * vsx) - borderSize,
				(posY * vsy) - borderSize,
				(vsx - (posX * vsx)) + borderSize,
				((posY * vsy) + height + borderSize),
			}
			if showTips and showTipAboveBar and showTipBackground then
				guishaderRects.loadprocess2 = {
					(posX * vsx) - borderSize,
					((posY * vsy) + height + borderSize),
					(vsx - (posX * vsx)) + borderSize,
					tipPosYtop * vsy,
	...
	if showTips then
		...
		if showTipBackground and showTipAboveBar then
			...
			gl.Rect(
				posX - (borderSize / vsx),
				posY + (height / vsy) + (borderSize / vsy),
				1 - posX + (borderSize / vsx),
				tipPosYtop
			)
	...
	if hasLowRam then
		...
		font2:SetOutlineColor(0, 0, 0, 0.8)
		font2:Print(
			BAR.I18N("ui.loadScreen.lowRamWarning", { textColor = "\255\200\200\200", warnColor = "\255\255\255\255" }),
			0,
			0,
			height * 0.66,
			"oac"
