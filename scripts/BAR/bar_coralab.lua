...
function script.Killed(recentDamage, maxHealth)
	...
	if (severity <= 50) then
		corpsetype = 2;
		UnitScript.Explode(base, SFX.NONE);
		UnitScript.Explode(head1,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(head2,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(side1,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(side2,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(nano1,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(nano2,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(nano3,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(nano4,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(center1,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(center2,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		return corpsetype;
	end
	if (severity <= 99) then
		corpsetype = 3;
		UnitScript.Explode(base,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(head1,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(head2,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(side1,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
		UnitScript.Explode(side2,  SFX.SMOKE + SFX.FIRE + SFX.EXPLODE);
