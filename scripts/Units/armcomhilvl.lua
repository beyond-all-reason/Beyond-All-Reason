...
local spSetUnitArmored = Engine.Synced.SetUnitArmored
...
local function BelowWater(piecename)
	local _, y, _ = Engine.Shared.GetUnitPiecePosition(unitID, piecename)
	-- this returns unit space, so why does it work for corcom?
	local _, py, _ = Engine.Shared.GetUnitPosition(unitID)
...
function walk()
	if bMoving then --Frame:4
		if leftArm then
			turn(biggun, 1, -48.215180, 113.735764 / animSpeed) --delta=-3.79
			...
			if leftArm then
				turn(luparm, 1, 9.919362, 279.367450 / animSpeed)
			end --delta=9.31
			if leftArm then
				turn(luparm, 3, -12.399998, 114.000041 / animSpeed)
			end --delta=-3.80
			if leftArm then
				turn(luparm, 2, 0.965138, 187.380006 / animSpeed)
			end --delta=-6.25
			...
			if rightArm then
				turn(rloarm, 1, -56.129627, 87.888962 / animSpeed)
			end --delta=2.93
			...
			if rightArm then
				turn(ruparm, 1, 21.424041, 98.073542 / animSpeed)
			end --delta=3.27
			if rightArm then
				turn(ruparm, 2, 1.762963, 52.888889 / animSpeed)
			end --delta=1.76
		...
		while bMoving do
			if bMoving then --Frame:8
				if leftArm then
					turn(biggun, 1, -46.675856, 46.179694 / animSpeed)
				end --delta=-1.54
				...
				if leftArm then
					turn(luparm, 1, 17.175860, 217.694940 / animSpeed)
				end --delta=-7.26
				if leftArm then
					turn(luparm, 2, 2.994882, 60.892341 / animSpeed)
				end --delta=2.03
				...
				if rightArm then
					turn(rloarm, 1, -61.570378, 163.222533 / animSpeed)
				end --delta=5.44
				...
				if rightArm then
					turn(ruparm, 1, 16.696394, 141.829407 / animSpeed)
				end --delta=4.73
				if rightArm then
					turn(ruparm, 2, 5.908277, 124.359412 / animSpeed)
				end --delta=4.15
			...
			if bMoving then --Frame:12
				if leftArm then
					turn(biggun, 1, -46.110552, 16.959121 / animSpeed)
				end --delta=-0.57
				...
				if leftArm then
					turn(luparm, 1, 20.474264, 98.952127 / animSpeed)
				end --delta=-3.30
				if leftArm then
					turn(luparm, 2, 5.388938, 71.821661 / animSpeed)
				end --delta=2.39
				...
				if rightArm then
					turn(rloarm, 1, -64.499997, 87.888552 / animSpeed)
				end --delta=2.93
				...
				if rightArm then
					turn(ruparm, 1, 12.622570, 122.214735 / animSpeed)
				end --delta=4.07
				if rightArm then
					turn(ruparm, 2, 6.799999, 26.751678 / animSpeed)
				end --delta=0.89
			...
			if bMoving then --Frame:16
				if leftArm then
					turn(biggun, 1, -47.068049, 28.724907 / animSpeed)
				end --delta=0.96
				...
				if leftArm then
					turn(luparm, 1, 19.047441, 42.804693 / animSpeed)
				end --delta=1.43
				...
				if rightArm then
					turn(rloarm, 1, -62.031478, 74.055575 / animSpeed)
				end --delta=-2.47
				...
				if rightArm then
					turn(ruparm, 1, 12.782766, 4.805883 / animSpeed)
				end --delta=-0.16
				if rightArm then
					turn(ruparm, 2, 6.141609, 19.751703 / animSpeed)
				end --delta=-0.66
			...
			if bMoving then --Frame:20
				if not Engine.Shared.GetUnitIsCloaked(unitID) then
					UnitScript.EmitSfx(lfootstep, 1024 + 2)
				end
				if leftArm then
					turn(biggun, 1, -49.335727, 68.030323 / animSpeed)
				end --delta=2.27
				...
				if leftArm then
					turn(luparm, 1, 15.615877, 102.946916 / animSpeed)
				end --delta=3.43
				if leftArm then
					turn(luparm, 2, 5.076670, 9.368019 / animSpeed)
				end --delta=-0.31
				...
				if rightArm then
					turn(rloarm, 1, -56.185181, 175.388910 / animSpeed)
				end --delta=-5.85
				...
				if rightArm then
					turn(ruparm, 1, 13.904138, 33.641183 / animSpeed)
				end --delta=-1.12
				if rightArm then
					turn(ruparm, 2, 2.429630, 111.359389 / animSpeed)
				end --delta=-3.71
			...
			if bMoving then --Frame:24
				if leftArm then
					turn(biggun, 1, -52.006372, 80.119348 / animSpeed)
				end --delta=2.67
				...
				if leftArm then
					turn(luparm, 1, 11.452920, 124.888703 / animSpeed)
				end --delta=4.16
				if leftArm then
					turn(luparm, 2, 3.308728, 53.038265 / animSpeed)
				end --delta=-1.77
				...
				if rightArm then
					turn(rloarm, 1, -49.299998, 206.555473 / animSpeed)
				end --delta=-6.89
				...
				if rightArm then
					turn(ruparm, 1, 16.947865, 91.311783 / animSpeed)
				end --delta=-3.04
				if rightArm then
					turn(ruparm, 2, 0.900000, 45.888888 / animSpeed)
				end --delta=-1.53
			...
			if bMoving then --Frame:28
				if leftArm then
					turn(biggun, 1, -53.142724, 34.090567 / animSpeed)
				end --delta=1.14
				...
				if leftArm then
					turn(luparm, 1, 8.807109, 79.374337 / animSpeed)
				end --delta=2.65
				if leftArm then
					turn(luparm, 2, -0.396598, 111.159777 / animSpeed)
				end --delta=-3.71
				...
				if rightArm then
					turn(rloarm, 1, -43.425926, 176.222159 / animSpeed)
				end --delta=-5.87
				...
				if rightArm then
					turn(ruparm, 1, 22.234335, 158.594124 / animSpeed)
				end --delta=-5.29
				if rightArm then
					turn(ruparm, 2, -0.812436, 51.373077 / animSpeed)
				end --delta=-1.71
			...
			if bMoving then --Frame:32
				if leftArm then
					turn(biggun, 1, -55.253092, 63.311038 / animSpeed)
				end --delta=2.11
				...
				if leftArm then
					turn(luparm, 1, 2.317686, 194.682700 / animSpeed)
				end --delta=6.49
				if leftArm then
					turn(luparm, 2, -5.045461, 139.465893 / animSpeed)
				end --delta=-4.65
				...
				if rightArm then
					turn(rloarm, 1, -39.457405, 119.055627 / animSpeed)
				end --delta=-3.97
				...
				if rightArm then
					turn(ruparm, 1, 27.520808, 158.594175 / animSpeed)
				end --delta=-5.29
				if rightArm then
					turn(ruparm, 2, -1.893975, 32.446164 / animSpeed)
				end --delta=-1.08
				turn(torso, 1, -3.674753, 18.402335 / animSpeed) --delta=-0.61
			...
			if bMoving then --Frame:36
				if leftArm then
					turn(biggun, 1, -56.389444, 34.090567 / animSpeed)
				end --delta=1.14
				...
				if leftArm then
					turn(luparm, 1, -3.274263, 167.758466 / animSpeed)
				end --delta=5.59
				if leftArm then
					turn(luparm, 2, -6.556594, 45.333998 / animSpeed)
				end --delta=-1.51
				...
				if rightArm then
					turn(rloarm, 1, -38.000000, 43.722158 / animSpeed)
				end --delta=-1.46
				...
				if rightArm then
					turn(ruparm, 1, 29.923749, 72.088224 / animSpeed)
				end --delta=-2.40
				if rightArm then
					turn(ruparm, 2, -2.615000, 21.630765 / animSpeed)
				end --delta=-0.72
				turn(torso, 1, -2.141225, 46.005832 / animSpeed) --delta=-1.53
			...
			if bMoving then --Frame:40
				if leftArm then
					turn(biggun, 1, -55.431951, 28.724805 / animSpeed)
				end --delta=-0.96
				turn(head, 1, -0.505722, 15.861819 / animSpeed) --delta=0.53
				turn(head, 2, 6.127755, 45.958102 / animSpeed) --delta=-1.53
				...
				if leftArm then
					turn(luparm, 1, -3.054371, 6.596772 / animSpeed)
				end --delta=-0.22
				if leftArm then
					turn(luparm, 2, -6.061839, 14.842650 / animSpeed)
				end --delta=0.49
				...
				if rightArm then
					turn(rloarm, 1, -40.468516, 74.055472 / animSpeed)
				end --delta=2.47
				...
				if rightArm then
					turn(ruparm, 1, 28.884286, 31.183878 / animSpeed)
				end --delta=1.04
				if rightArm then
					turn(ruparm, 2, -1.954060, 19.828204 / animSpeed)
				end --delta=0.66
				turn(torso, 1, -3.061342, 27.603503 / animSpeed) --delta=0.92
			...
			if bMoving then --Frame:44
				...
				if leftArm then
					turn(biggun, 1, -53.164273, 68.030323 / animSpeed)
				end --delta=-2.27
				turn(head, 1, -2.091907, 47.585562 / animSpeed) --delta=1.59
				turn(head, 2, 3.829846, 68.937274 / animSpeed) --delta=-2.30
				...
				if leftArm then
					turn(luparm, 1, -1.515114, 46.177713 / animSpeed)
				end --delta=-1.54
				if leftArm then
					turn(luparm, 2, -4.161596, 57.007282 / animSpeed)
				end --delta=1.90
				...
				if rightArm then
					turn(rloarm, 1, -46.314813, 175.388910 / animSpeed)
				end --delta=5.85
				...
				if rightArm then
					turn(ruparm, 1, 26.384334, 74.998551 / animSpeed)
				end --delta=2.50
				if rightArm then
					turn(ruparm, 2, -0.722308, 36.952569 / animSpeed)
				end --delta=1.23
				turn(torso, 1, -3.981459, 27.603503 / animSpeed) --delta=0.92
			...
			if bMoving then --Frame:48
				if leftArm then
					turn(biggun, 1, -50.493628, 80.119348 / animSpeed)
				end --delta=-2.67
				turn(head, 1, -3.149363, 31.723676 / animSpeed) --delta=1.06
				turn(head, 2, 0.000000, 114.895376 / animSpeed) --delta=-3.83
				...
				if leftArm then
					turn(luparm, 1, 2.662867, 125.339441 / animSpeed)
				end --delta=-4.18
				if leftArm then
					turn(luparm, 2, -1.793232, 71.050921 / animSpeed)
				end --delta=2.37
				move(pelvis, 2, -2.000000, 25.555551 / animSpeed) --delta=0.85
				...
				if rightArm then
					turn(rloarm, 1, -53.199999, 206.555576 / animSpeed)
				end --delta=6.89
				...
				if rightArm then
					turn(ruparm, 1, 23.351554, 90.983396 / animSpeed)
				end --delta=3.03
				if rightArm then
					turn(ruparm, 2, 0.479402, 36.051284 / animSpeed)
				end --delta=1.20
				turn(torso, 1, -4.594870, 18.402335 / animSpeed) --delta=0.61
			...
			if bMoving then --Frame:52
				if leftArm then
					turn(biggun, 1, -48.215180, 68.353460 / animSpeed)
				end --delta=-2.28
				turn(head, 1, -2.620635, 15.861851 / animSpeed) --delta=-0.53
				turn(head, 2, -3.829846, 114.895376 / animSpeed) --delta=-3.83
				...
				if leftArm then
					turn(luparm, 1, 9.919362, 217.694824 / animSpeed)
				end --delta=-7.26
				if leftArm then
					turn(luparm, 2, 0.965138, 82.751096 / animSpeed)
				end --delta=2.76
				...
				if rightArm then
					turn(rloarm, 1, -56.129627, 87.888859 / animSpeed)
				end --delta=2.93
				...
				if rightArm then
					turn(ruparm, 1, 21.424041, 57.825406 / animSpeed)
				end --delta=1.93
				if rightArm then
					turn(ruparm, 2, 1.762963, 38.506840 / animSpeed)
				end --delta=1.28
				turn(torso, 1, -4.288164, 9.201174 / animSpeed) --delta=-0.31
				turn(torso, 3, 1.713372, 51.401158 / animSpeed) --delta=-1.71
				turn(torso, 2, 7.879434, 236.383029 / animSpeed) --delta=7.88
...
function StopWalking()
	...
	if leftArm then
		turn(biggun, 1, -52.006372, 28.433941)
	end
	...
	if leftArm then
		turn(luparm, 1, 19.231610, 69.841862)
	end
	if leftArm then
		turn(luparm, 2, 7.211138, 46.845002)
	end
	if leftArm then
		turn(luparm, 3, -16.200000, 28.500010)
	end
	...
	if rightArm then
		turn(rloarm, 1, -53.199995, 51.638894)
	end
	turn(rthigh, 1, -21.950628, 255.149679)
	turn(rthigh, 2, -11.905752, 66.515336)
	turn(rthigh, 3, 9.396449, 151.125663)
	if rightArm then
		turn(ruparm, 1, 24.693159, 39.648544)
	end
	if rightArm then
		turn(ruparm, 2, 0.000000, 31.089853)
	end
	if rightArm then
		turn(ruparm, 3, 12.399998, -7.153845)
	end
...
function UnitSpeed()
	maxSpeed = UnitDefs[Engine.Shared.GetUnitDefID(unitID)].speed
	animFramesPerKeyframe = 4 --we need to calc the frames per keyframe value, from the known animtime
	maxSpeed = maxSpeed + (maxSpeed / (2 * animFramesPerKeyframe)) -- add fudge
	while true do
		vx, vy, vz, Speed = Engine.Shared.GetUnitVelocity(unitID)
...
function script.AimWeapon(weapon, heading, pitch)
	...
	local reloadingFrameTach = Engine.Shared.GetUnitWeaponState(unitID, 4, "reloadFrame")
	if weapons[weapon] == "laser" then
...
function script.StartBuilding(heading, pitch)
	...
	Engine.Synced.UnitScript.SetUnitValue(COB.INBUILDSTANCE, true)
...
function script.StopBuilding()
	...
	leftArm = true
	isBuilding = false
	Engine.Synced.UnitScript.SetUnitValue(COB.INBUILDSTANCE, false)
	Signal(SIG_AIM)
	SetSignalMask(SIG_AIM)
	StartThread(Restore)
	Hide(armhexl_emit)
