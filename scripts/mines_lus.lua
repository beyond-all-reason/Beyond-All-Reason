...
function GetClosestEnemyDistance()
	targetID = Engine.Shared.GetUnitNearestEnemy(unitID, triggerRange)
	if targetID then
		local tx, ty, tz = Engine.Shared.GetUnitPosition(targetID)
...
function script.Create()
	ux, uy, uz = Engine.Shared.GetUnitPosition(unitID)
...
function EnemyDetect()
	while true do
		...
		local firestate = Engine.Shared.GetUnitStates(unitID, false)
		local stunned = Engine.Shared.GetUnitIsStunned(unitID)
...
function Detonate()
	Sleep(500)
	Engine.Synced.DestroyUnit(unitID, false, false)
end
