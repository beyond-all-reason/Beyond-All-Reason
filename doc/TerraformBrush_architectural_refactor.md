...
| Phase 2 step 4 — `{{interpolation}}` for labels | Medium | **PROMOTED pre-1.0** (PR #7527). ~40 `inner_rml = tostring(v)` sites in tf-brush. **Landed Apr 2026: tf_startpos (6), tf_splat (11), tf_metal (13), tf_grass (14), tf_features (11), tf_lights (6 of 14 — 8 IDs not in RML), tf_clone (3), tf_environment dim panel (7). Landed May 2026: gui_terraform_brush ring-width + restore-strength (6 sites — dm fields existed but RML `{{…}}` + Lua wiring was absent; wired now). tf_weather had zero. Remaining: tf_decals/tf_noise generic helpers (justified imperative — bulk setLbl by id), keybind editor + history list builders (Phase 3 / data-for territory), syncTBMirrorControls snap/step/count/angle labels (5 prefixes × 4 labels = 20 fields, possibly justified).** |
