...
- The Tileset window gained a DEPOSIT section (requested by PtaQ and MrBob, for Teizer's craters): a SURFACE variant slot the shader may claim on its own where wind-blown sand would gather, on the lee side of slopes relative to a wind direction and in the pockets the intermediary already reads. It only fills ground no stroke claimed, so paint wins; cliffs stay clean; and it is off until a slot is chosen, since whether a top may ever be placed automatically is MrBob's call. Crater floors are flat rather than concave at the shader's curvature radius, so they are painted: CRATER shape for the geometry, then a WASH stroke of the sand variant with an INFLUENCE altitude band, and the deposit slot dresses the sheltered slopes.
