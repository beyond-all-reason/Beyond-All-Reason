...
describe("the declared result", function()
	...
	it("product: factors from every module multiply into one answer", function()
		...
		-- no factor at all is a broken owner, never a silent nil
		assert.has_error(function()
			run(pipeline, {})
		end)
	end)
...
describe("the fold result", function()
	...
	it("hands one context through every Answer, owner's first, and returns it", function()
		local stages = fold(
			PolicyBuilder.Pipeline()
				.Answer("Base", function(ctx)
	...
	it("every stage is a Answer: a fold has nothing to refuse", function()
		local stages = fold(
			PolicyBuilder.Pipeline()
		...
		end, "defs.unit_def: a fold pipeline runs every Answer over the context; Never is a guard")
	end)
end)
