...
describe("MissionApiBuilder", function()
	...
	describe("Build", function()
		...
		it("exposes all module stubs", function()
			...
			assert.is_function(api.Modules.Stages.ChangeStage)
			assert.is_function(api.Modules.Objectives.Update)
		end)
	...
	describe("mission data", function()
		...
		it("seeds action and condition definitions", function()
			local api = Builders.MissionApi
				.new()
				:WithActionDefinitions({ Victory = {} })
				:WithConditionDefinitions({ TimeElapsed = {} })
				:Build()

			assert.is_table(api.ActionDefinitions.Victory)
			assert.is_table(api.ConditionDefinitions.TimeElapsed)
		end)
	end)

	describe("Modules.Tracking", function()
		it("seeds tracked units bidirectionally", function()
			local api = Builders.MissionApi.new():WithTrackedUnit("bots", 101):WithTrackedUnit("bots", 102):Build()
		...
		it("seeds tracked features bidirectionally", function()
			local api = Builders.MissionApi.new():WithTrackedFeature("wreck", 7):Build()
		...
		it("untracks a unit name and cleans up empty id entries", function()
			local api = Builders.MissionApi.new():WithTrackedUnit("pair", 1):WithTrackedUnit("pair", 2):Build()
		...
		it("keeps other names when untracking an id with multiple names", function()
			local api = Builders.MissionApi.new():WithTrackedUnit("alpha", 1):WithTrackedUnit("beta", 1):Build()
	...
	describe("module call tracking", function()
		it("records Loadout calls and passes orders through unchanged", function()
			...
			assert.are.equal(1, #api.calls.convertOrdersTargetingNames)
			assert.are.equal(1, #api.calls.spawnUnitLoadout)
			assert.are.same({ "a" }, api.calls.spawnUnitLoadout[1].loadout)
			assert.are.equal(1, #api.calls.spawnFeatureLoadout)
			assert.are.same({ "b" }, api.calls.spawnFeatureLoadout[1].loadout)
		end)
		...
		it("records Sounds calls", function()
			...
			assert.are.equal(1, #api.calls.playSound)
			assert.are.equal("a.wav", api.calls.playSound[1].soundfile)
			assert.are.equal(0.5, api.calls.playSound[1].volume)
			assert.are.same(position, api.calls.playSound[1].position)
			assert.are.equal(1, #api.calls.enqueueSound)
			assert.are.equal("b.wav", api.calls.enqueueSound[1].soundfile)
			assert.are.equal(1, #api.calls.processSoundQueue)
			assert.are.equal(30, api.calls.processSoundQueue[1].frame)
		end)

		it("records Stages and Objectives calls", function()
			local api = Builders.MissionApi.new():Build()
			local objective = { textKey = "kill" }

			api.Modules.Stages.ChangeStage("stage2")
			api.Modules.Objectives.TryAdvanceStage(objective)
			api.Modules.Objectives.Complete("obj1")
			api.Modules.Objectives.Update("obj1", true, "ui.done")
			api.Modules.Objectives.ReportProgress("obj1", 2)
			...
			assert.are.equal("obj1", api.calls.completeObjective[1].objectiveID)

			local update = api.calls.updateObjective[1]
			assert.are.equal("obj1", update.objectiveID)
			assert.is_true(update.completed)
			assert.are.equal("ui.done", update.textKey)

			local progress = api.calls.reportProgress[1]
			assert.are.equal("obj1", progress.objectiveID)
			assert.are.equal(2, progress.progress)

			assert.are.equal("obj1", api.calls.echoObjectiveUpdate[1].objectiveID)
		end)
		...
		it("clears every tracked call list via clearCalls", function()
			...
			api.Modules.Stages.ChangeStage("s")
			api.Modules.Stages.Announce("s")
			api.Modules.Objectives.TryAdvanceStage({})
			api.Modules.Objectives.Complete("o")
			api.Modules.Objectives.Update("o")
			api.Modules.Objectives.ReportProgress("o", 1)
			api.Modules.Objectives.EchoObjectiveUpdate("o", {})

			api.clearCalls()

			assert.are.equal(0, #api.calls.convertOrdersTargetingNames)
			assert.are.equal(0, #api.calls.spawnUnitLoadout)
			assert.are.equal(0, #api.calls.spawnFeatureLoadout)
			assert.are.equal(0, #api.calls.playSound)
			...
			assert.are.equal(0, #api.calls.announceStage)
			assert.are.equal(0, #api.calls.tryAdvanceStage)
			assert.are.equal(0, #api.calls.completeObjective)
			assert.are.equal(0, #api.calls.updateObjective)
			assert.are.equal(0, #api.calls.reportProgress)
	...
	describe("Install", function()
		...
		it("resets recorded calls on reinstall", function()
			local api = Builders.MissionApi.new():Install()
			api.Modules.Stages.ChangeStage("s1")
			assert.are.equal(1, #api.calls.changeStage)
