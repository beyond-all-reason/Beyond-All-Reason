...
describe("EngineUnsyncedBuilder", function()
	...
	it("exposes UnitDefs registered via WithUnitDef", function()
		local widget = Builders.EngineUnsynced.new():WithUnitDef(7, { name = "armcom", buildSpeed = 100 }):LoadWidget(TMP_WIDGET_PATH)
	...
	it("maps unit IDs to def IDs by numeric defID", function()
		local widget = Builders.EngineUnsynced.new():WithUnitDef(7, { name = "armcom", buildSpeed = 100 }):WithUnit(42, 7):LoadWidget(TMP_WIDGET_PATH)
	...
	it("WithUnit by unknown numeric defID errors", function()
		assert.has_error(function()
			Builders.EngineUnsynced.new():WithUnit(1, 999)
		end)
	end)

	it("resolves WithUnit by name when the def has been registered", function()
		local widget = Builders.EngineUnsynced.new():WithUnitDef(7, { name = "armcom", buildSpeed = 100 }):WithUnit(42, "armcom"):LoadWidget(TMP_WIDGET_PATH)
	...
	it("WithUnit by unknown name errors with a helpful message", function()
		assert.has_error(function()
			Builders.EngineUnsynced.new():WithUnit(1, "nonesuch")
		end)
	end)

	it("WithUnitDef accepts a UnitDefBuilder", function()
		local widget = Builders.EngineUnsynced.new():WithUnitDef(Builders.UnitDef.new("armcon"):WithDefID(100):WithSpeed(100):Builds(10, 11)):LoadWidget(TMP_WIDGET_PATH)
	...
	it("WithSpringFn overrides individual Spring functions", function()
		local widget = Builders.EngineUnsynced
			.new()
	...
	it("WithVFSInclude overrides VFS.Include for a specific path", function()
		local widget = Builders.EngineUnsynced.new():WithVFSInclude("any/fake/path.lua", { sentinel = true }):LoadWidget(TMP_WIDGET_PATH)
	...
	it("each LoadWidget call yields an isolated env", function()
		local a = Builders.EngineUnsynced.new():LoadWidget(TMP_WIDGET_PATH)
		local b = Builders.EngineUnsynced.new():LoadWidget(TMP_WIDGET_PATH)
