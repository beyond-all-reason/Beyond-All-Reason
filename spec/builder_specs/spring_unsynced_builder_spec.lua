...
describe("SpringUnsyncedBuilder", function()
	...
	it("exposes UnitDefs registered via WithUnitDef", function()
		local widget = Builders.SpringUnsynced.new():WithUnitDef(7, { name = "armcom", buildSpeed = 100 }):LoadWidget(TMP_WIDGET_PATH)
	...
	it("maps unit IDs to def IDs by numeric defID", function()
		local widget = Builders.SpringUnsynced.new():WithUnitDef(7, { name = "armcom", buildSpeed = 100 }):WithUnit(42, 7):LoadWidget(TMP_WIDGET_PATH)
	...
	it("resolves WithUnit by name when the def has been registered", function()
		local widget = Builders.SpringUnsynced.new():WithUnitDef(7, { name = "armcom", buildSpeed = 100 }):WithUnit(42, "armcom"):LoadWidget(TMP_WIDGET_PATH)
	...
	it("WithUnitDef accepts a UnitDefBuilder", function()
		local widget = Builders.SpringUnsynced.new():WithUnitDef(Builders.UnitDef.new("armcon"):WithDefID(100):WithSpeed(100):Builds(10, 11)):LoadWidget(TMP_WIDGET_PATH)
	...
	it("WithVFSInclude overrides VFS.Include for a specific path", function()
		local widget = Builders.SpringUnsynced.new():WithVFSInclude("any/fake/path.lua", { sentinel = true }):LoadWidget(TMP_WIDGET_PATH)
