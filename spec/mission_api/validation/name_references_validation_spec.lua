---
--- The unit, feature and marker names created and referenced across a mission.
---
...
describe("mission_api.validation.name_references_validation", function()
	...
	it("passes when every name is both created and referenced", function()
		V.assertValid(V.validate(V.mission()
			...
			:WithAction('add', { type = V.actionTypes.AddMarker, parameters = { position = { x = 0, z = 0 }, name = 'flag' } })
	...
	it("counts mission loadout entries as creating names", function()
		V.assertValid(V.validate(V.mission()
			...
			:WithAction('destroyFeature', { type = V.actionTypes.DestroyFeatures, parameters = { featureName = 'loadoutRock' } })
	...
	it("warns about names that are only created, or only referenced", function()
		local result = V.validate(V.mission()
			...
			:WithAction('deleteUnknown', { type = V.actionTypes.DestroyFeatures, parameters = { featureName = 'unknownRock' } })
			:WithAction('addUnused', { type = V.actionTypes.AddMarker, parameters = { position = { x = 0, z = 0 }, name = 'unusedFlag' } })
			:WithAction('eraseUnknown', { type = V.actionTypes.EraseMarker, parameters = { name = 'unknownFlag' } }))

		V.assertMessage(result, "Unit name is created, but never referenced. Unit name: unusedUnit, Created in: action spawnUnused (unitLoadout[1])")
		V.assertMessage(result, "Unit name is referenced, but never created. Unit name: unknownUnit, Referenced in: action useUnknown")
		V.assertMessage(result, "Feature name is created, but never referenced. Feature name: unusedRock, Created in: action createUnused (featureLoadout[1])")
		V.assertMessage(result, "Feature name is referenced, but never created. Feature name: unknownRock, Referenced in: action deleteUnknown")
		V.assertMessage(result, "Marker name is created, but never referenced. Marker name: unusedFlag, Created in: action addUnused")
		V.assertMessage(result, "Marker name is referenced, but never created. Marker name: unknownFlag, Referenced in: action eraseUnknown")
	end)
	...
	it("names every source of a name separately, in a stable order", function()
		local result = V.validate(V.mission()
			:WithAction('spawnUnused', spawnUnits('unusedUnit'))
			:WithAction('nameUnused', { type = V.actionTypes.NameUnits, parameters = { unitName = 'unusedUnit', teamID = 0 } }))

		V.assertMessage(result, "Unit name is created, but never referenced. Unit name: unusedUnit, " ..
