...
describe("mission_api.triggers.construction_progress", function()
	...
	it("filters by unitDefName", function()
		...
		step(trigger({ teamName = "thePlayerTeam", progress = 0.5, unitDefName = "armsolar" }), context, 100)
	...
	it("filters by unitName", function()
		...
		building(100, 0.6)
		step(trigger({ teamName = "thePlayerTeam", progress = 0.5, unitName = "target" }), context, 100)
		assert.are.equal(0, fired())
	end)
	...
	it("fires when a unit under construction reaches the threshold", function()
		local context, fired = newContext()
		building(100, 0.6)
		local t = trigger({ teamName = "thePlayerTeam", progress = 0.5, unitDefName = "armsolar" })
		step(t, context, 100)
		step(t, context, 100)
		assert.are.equal(1, fired())
	end)
	...
	it("fires at full progress as a tracked unit finishes (progress = 1.0)", function()
		local context, fired = newContext()
		local t = trigger({ teamName = "thePlayerTeam", progress = 1.0, unitDefName = "armsolar" })
		building(100, 0.9)
		step(t, context, 100)
		assert.are.equal(0, fired())
		finished(100)
	...
	it("latches a unit that crosses unnamed, so that naming it later cannot fire", function()
		...
		local t = trigger({ teamName = "thePlayerTeam", progress = 0.5, unitName = "target" })
		building(100, 0.6)
		step(t, context, 100)
		assert.are.equal(0, fired())
	...
	it("ignores a frame whose steps net a loss", function()
		local context, fired = newContext()
		building(100, 0.6)
		step(trigger({ teamName = "thePlayerTeam", progress = 0.5, unitDefName = "armsolar" }), context, 100, -0.1)
		assert.are.equal(0, fired())
	end)
	...
	it("ignores a frame whose steps net zero", function()
		local context, fired = newContext()
		building(100, 0.6)
		step(trigger({ teamName = "thePlayerTeam", progress = 0.5, unitDefName = "armsolar" }), context, 100, 0)
		assert.are.equal(0, fired())
	end)
