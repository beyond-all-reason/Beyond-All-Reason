...
describe("mission_api.triggers.unit_ordered", function()
	...
	it("filters by teamName", function()
		local context, fired = newContext()
		order(trigger({ command = CMD.MOVE, unitDefName = "armpw", teamName = "theEnemyTeam" }), context, CMD.MOVE, { 0, 0, 0 }) -- unitTeam 0
		assert.are.equal(0, fired())
	end)
	...
	it("filters by command through a queue insertion", function()
		local context, fired = newContext()
		order(
			trigger({ command = CMD.MOVE, unitDefName = "armpw" }),
			context,
			CMD.INSERT,
			{ 0, CMD.RECLAIM, 0, 1, 2, 3, 4 }
		)
		assert.are.equal(0, fired())
	end)
