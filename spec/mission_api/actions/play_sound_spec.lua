...
describe("mission_api.actions.play_sound", function()
    ...
    describe("actionFunction", function()
        it("calls Sounds.PlaySound when enqueue is nil", function()
            ...
            assert.are.equal(1, #missionApi._playSoundCalls)
            assert.are.equal(0, #missionApi._enqueueSoundCalls)
            assert.are.equal('sounds/bang.wav', missionApi._playSoundCalls[1].soundfile)
        end)
        ...
        it("calls Sounds.EnqueueSound when enqueue is true", function()
            ...
            assert.are.equal(0, #missionApi._playSoundCalls)
            assert.are.equal(1, #missionApi._enqueueSoundCalls)
            assert.are.equal('sounds/bang.wav', missionApi._enqueueSoundCalls[1].soundfile)
        end)

        it("passes volume and position to PlaySound", function()
            local pos = { x = 1, y = 2, z = 3 }
            action.actionFunction('sounds/x.wav', 0.5, pos, false)
            assert.are.equal(0.5, missionApi._playSoundCalls[1].volume)
            assert.are.same(pos, missionApi._playSoundCalls[1].position)
        end)
    end)
