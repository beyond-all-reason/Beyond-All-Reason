...
local function buildResourceResult(spring, taxRate, metalThreshold, energyThreshold, sender, receiver, resourceType)
  ...
  springApi.GetModOptions = function()
    return {
      ...
      player_metal_send_threshold = tostring(metalThreshold),
      player_energy_send_threshold = tostring(energyThreshold),
    }
  end
...
describe(ModeEnums.ModOptions.TaxResourceSharingAmount .. " #policy", function()
  ...
  describe("simple taxation", function()
    ...
    before_each(function()
      ...
      metalResult = buildResourceResult(spring, taxRate, 0, 0, sender, receiver, TransferEnums.ResourceType.METAL)
      energyResult = buildResourceResult(spring, taxRate, 0, 0, sender, receiver, TransferEnums.ResourceType.ENERGY)
    end)
  ...
  describe("when a receiver has more metal capacity than the threshold", function()
    it("should have an untaxed portion that is the threshold", function()
      ...
      local metalResult = buildResourceResult(spring, taxRate, 0, 0, sender, receiver, TransferEnums.ResourceType.METAL)
  ...
  describe("when a sender has less metal than the receiver has capacity", function()
    ...
    before_each(function()
      ...
      metalResult = buildResourceResult(spring, taxRate, metalThreshold, 0, sender, receiver, TransferEnums.ResourceType.METAL)
    end)
  ...
  describe("rate = 0.7, receiver capacity 300, sender 1000", function()
    ...
    before_each(function()
      ...
      energyResult = buildResourceResult(spring, testTaxRate, 0, 0, sender, receiver, TransferEnums.ResourceType.ENERGY)
    end)
  ...
  describe("when taxation is disabled", function()
    ...
    it("should not tax metal transfers", function()
      ...
      metalResult = buildResourceResult(spring, 0, 0, 0, sender, receiver, TransferEnums.ResourceType.METAL)
      assert.equal(1000, metalResult.amountReceivable)
    ...
    it("should not tax energy transfers", function()
      ...
      energyResult = buildResourceResult(spring, 0, 0, 0, sender, receiver, TransferEnums.ResourceType.ENERGY)
      assert.equal(1000, energyResult.amountReceivable)
  ...
  describe("basic resource transfer", function()
    it("should transfer metal without tax when untaxed portion covers full amount", function()
      ...
      local ctx = {
        ...
        transferCategory = TransferEnums.TransferCategory.MetalTransfer,
...
describe("Resource comms #comms", function()
  describe("DecideCommunicationCase", function()
    it("should return OnSelf when sender equals receiver", function()
      local policy = { senderTeamId = 1, receiverTeamId = 1, canShare = true, taxRate = 0.3, resourceShareThreshold = 0 }
    ...
    it("should return OnTaxFree when tax rate is zero", function()
      local policy = { senderTeamId = 1, receiverTeamId = 2, canShare = true, taxRate = 0, resourceShareThreshold = 0 }
      assert.equal(TransferEnums.ResourceCommunicationCase.OnTaxFree, ResourceShared.DecideCommunicationCase(policy))
    end)

    it("should return OnTaxed when taxed without threshold", function()
      local policy = { senderTeamId = 1, receiverTeamId = 2, canShare = true, taxRate = 0.3, resourceShareThreshold = 0 }
      assert.equal(TransferEnums.ResourceCommunicationCase.OnTaxed, ResourceShared.DecideCommunicationCase(policy))
    end)

    it("should return OnTaxedThreshold when threshold is set", function()
      local policy = { senderTeamId = 1, receiverTeamId = 2, canShare = true, taxRate = 0.3, resourceShareThreshold = 500 }
      assert.equal(TransferEnums.ResourceCommunicationCase.OnTaxedThreshold, ResourceShared.DecideCommunicationCase(policy))
    end)

    it("should return nil (OnDisabled) when canShare is false", function()
      local policy = { senderTeamId = 1, receiverTeamId = 2, canShare = false, taxRate = 0, resourceShareThreshold = 0 }
      local result = ResourceShared.DecideCommunicationCase(policy)
      -- OnDisabled is not in the enum, so DecideCommunicationCase returns nil
