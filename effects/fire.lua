  ...
  ["fire-flames-repeat"] = {
    ...
    flame1 = {
      ...
      properties = {
        airdrag             = 0.88,
        colormap            =
        [[0 0 0 0.01   0.9 0.87 0.87 0.4  0.65 0.65 0.68 0.2   0.1 0.1 0.1 0.28   0.08 0.07 0.06 0.12   0 0 0 0.01]],
        directional         = false,
        emitrot             = 40,
        emitrotspread       = 30,
        emitvector          = [[0.2, -0.4, 0.2]],
    ...
    blacksmoke = {
      ...
      properties = {
        ...
        --colormap           = [[0.01 0.01 0.01 0.01   0.02 0.02 0.01 0.2   0.15 0.14 0.12 0.68   0.13 0.12 0.10 0.75   0.11 0.10 0.09 0.85  0.09 0.08 0.08 0.7    0.075 0.07 0.07 0.6   0.05 0.05 0.05 0.4   0.01 0.01 0.01 0.01]],
        colormap            =
        [[0.01 0.01 0.01 0.01   0.02 0.02 0.01 0.2   0.15 0.14 0.12 0.68   0.11 0.10 0.09 0.85    0.01 0.01 0.01 0.01]],
        directional         = false,
        emitrot             = 90,
        emitrotspread       = 70,
        emitvector          = [[0.3, 1, 0.3]],
        ...
        particlelife        = 88, -- this is a long animated effect; we can't just decrease its ttl
        particlelifespread  = 44, -- but this change tries to do so anyway
        particlesize        = 45,
        particlesizespread  = 60,
        particlespeed       = 3,
        particlespeedspread = 2,
  ...
  ["fire-burnground-repeat"] = {
    ...
    flamemattalways = {
      ...
      properties = {
        airdrag             = 0.92,
        colormap            =
        [[0.25 0.22 0.18 0.75   0.75 0.77 0.71 1   0.72 0.51 0.39 1    0.67 0.47 0.34 0.99   0.63 0.41 0.27 0.98   0.58 0.37 0.29 0.97   0.48 0.31 0.22 0.91    0.11 0.11 0.12 0.50   0.016 0.011 0.07 0.45   0 0 0 0.01]],
        directional         = false,
        emitrot             = 90,
        emitrotspread       = 5,
        emitvector          = [[0.32, 0.7, 0.32]],
...
-- Check your work (wolframalpha input):
-- irwin hall distribution, n = 2, min = 0, max = 4
-- probability that x>7, given that x~irwin hall distribution, n = 2, min = 0, max = 4
local function cegop_normaldist(mean, variance, n)
	n = n or 2 -- n=2 is a triangular distribution
	local rangeRate = math.sqrt(variance * (12 / n))
	return string.rep("r1 ", n) ..
         string.format("%.1f y0 %f x0 %f", -n / 2, rangeRate, mean + 0.49999)
end
...
for ii = 1, #areaCegSizes do
  ...
  local name = 'fire-area-' .. math.floor(areaCegSizes[ii]) .. '-repeat'
  definitions[name] = table.copy(definitions['fire-area-repeat'])

  local count = math.max(1, math.min(255, math.floor(((size + 30) / 60)^2)))
  local delay, pos
  if count > 16 then
    count = math.floor(count / 2)
    definitions[name].fireflamearea.properties.explosiongenerator = [[custom:fire-flames-large-repeat]]
    definitions[name].fireflameground.properties.explosiongenerator = [[custom:fire-burnground-large-repeat]]
  end
  if count == 1 then
    delay = "r9"
    pos = string.format([[%f r%f, r%f, %f r%f]], -size / 4, size / 2, 20, -size / 4, size / 2)
  elseif count < 16 then
    -- spread out the delay a little bit; stops flames looking like they pop out all at once
    delay = cegop_normaldist(3+count/2, 10+count, 2) -- https://ideone.com/65EG7z
            .. " i"..math.floor(math.sqrt(count))
    pos = string.format(
      [[%f r%f]]..[[ r%f, ]]..[[%f r%f]],
      -size / 2 - 60, size + 120,
      32,
  ...
  definitions[name].fireflamearea.count = count
  definitions[name].fireflameground.count = count
  definitions[name].fireflamearea.properties.pos = pos
  definitions[name].fireflameground.properties.pos = pos
  definitions[name].fireflamearea.properties.delay = delay
  definitions[name].fireflameground.properties.delay = delay

  if debugEffects == true then
    definitions[name].debugcircle = {
      class      = [[CBitmapMuzzleFlame]],
      count      = 1,
      air        = true,
      ground     = true,
      ...
      water      = true,
      properties = {
        colormap     = [[.1 0 0 0.05   .1 0 0 0.01   0 0 0 0]],
        dir          = [[0, 1, 0]],
        drawOrder    = 2,
        frontoffset  = 0,
